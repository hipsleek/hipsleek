...
void swap(ref node x, ref node y)
  ...
  ensures x'::dll<_,m,S3>*y'::dll<_,n,S4>; 
{
  node tmp = x;
  x = y;
  y = tmp;
...
void push_front(ref node x, int v)
  ...
  ensures x'::node<v,_,q>*q::dll<_,n,S1> & PUF(S1,S);
{
  if (x==null) {
...
node pop_front(ref node x)
  ...
  ensures x'::dll<_,m-1,S2> & PF(S1,S2); 
{
  node tmp = x;
  if (x.next == null) {
    x = x.next;
...
void append2(node x, node y)
  infer [APP2]
	...
	ensures x::dll<q, t, S> & t=m+n & APP2(S,S1,S2);
{
	node tmp;
	if (x.next == null) {
		x.next = y;
...
node get_next(ref node x)
  ...
  ensures x'::dll<_,1,S1> * res::dll<_,n-1,S2> & GN(S,S1,S2); 
{
  node tmp = x.next;
  x.next = null;
  x.prev = null;
...
void set_next(ref node x, node y)
  ...
  ensures x::node<v,_,y>*y::dll<_,j,S2> & SN(S,S2); 
{
  if (y==null) 
    x.next = y;
  else {
...
node get_next_next(node x)
  ...
  ensures res::dll<_,n-2,S2> & GNN(S,S2); 
{
  return x.next.next;
}
...
void insert(node x, int a)
  ...
  ensures x::dll<p, m, S1> & m=n+1 & INSERT(S,S1,a); 
{
  if (x.next == null)
...
node delete2(ref node x, int a)
  requires x::dll<_,n,S>
  ensures res::dll<_,m,S1> & m<=n & (a notin S & S = S1 | S=union(S1, {a}));
{
	if (x == null)
		return x;
	else 
...
void reverse(ref node xs, ref node ys)
  ...
  ensures ys'::dll<_,n+m,S> & xs' = null & S=union(S1,S2); 
{
  if (xs != null) {
    node tmp;
    tmp = xs.next;
...
node split1(ref node x, int a)
  ...
  ensures x'::dll<p,n1,S1>*res::dll<_,n2, S2> & n=n1+n2 & n1>0 & n2>0 & n1=a & SPLIT(S,S1,S2);
{
	node tmp;
	if (a == 1)
	{
...
void list_traverse(node x)
  ...
  ensures x::dll<p,n,S2> & TRAV(S1,S2);
{
  node t;
  if(x != null) {
    t = x;
...
node list_copy(node x)
  ...
  ensures x::dll<p,n,S> * res::dll<_,n,S2> & S=S2;
{
  node tmp;
  if (x != null) {
...
node list_remove2(node x, int v)
  requires x::dll<p,n,S> & n >= 0
  ensures res::dll<p,m,S2> & m <= n & S2 subset S;
{
  node tmp;
  if(x != null) {
    if(x.val == v) {
...
node list_filter2(ref node x, int v)
  ...
  ensures res::dll<_,m,S2> & m <= n & S2 subset S; 
{
  node tmp;
  if(x != null) {
    if(x.val == v){
...
node find_ge(node x, int v)
  infer[FGE]
  requires x::dll<_,n,S> & n >= 0
  ensures res = null or res::node<m,_,_> & m > v & FGE(S,m);
{
  if(x == null)
    return null;
  else {
...
void splice (ref node x, node y)
  requires x::dll<_,n,S1> * y::dll<_,m,S2>
  ensures x'::dll<_,m+n,S> & S=union(S1,S2); 
{
  if(x == null)
    x = y;
  else {
