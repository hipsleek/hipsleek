...
node insert(node t, int v)
  ...
  ensures res::pq1<n+1, ma,S2> & (v>=mx & ma = v | ma = mx) & {v} subset S2 & INS(S1,S2);
{
...
int deleteoneel(ref node t)
  requires t::pq1<n, mx,S1> & n > 0
  ensures t'::pq1<n-1, mx2,S2> & mx2 <= mx & 0 <= res <= mx ;
{
	int v;
...
int deletemax(ref node t)
  requires t::pq1<n, mx,S1> & n > 0
  ensures t'::pq1<n-1, mx2,S2> & mx2 <= res <= mx;
{
	int v, tmp;
