 ...
 case{
  prio = 0 -> case {
    curJob = null -> case{
      ...
      pq3'::ll1<n6,S6>*new_process::node<_,v2,null>*curJob'::node<_,_,null> & n4+n5+n6=n1+n2+n3-1 & v4>=1 & v4<=3 &res = 0;//'add bag constraint here
      n1+n2+n3<=0 ->
...
int finish1(ref node curJob, ref node pq1, ref node pq2, ref node pq3)
  ...
  curJob !=null -> requires curJob::node<_,_,null> case {
    ...
    //& S7=union(S1,S2,S3) & S8=union(S4,S5,S6) & S7=union(S8,{v});//'
...
int flush1(ref node curJob, ref node pq1, ref node pq2, ref node pq3)
 ...
 case {
  ...
  curJob != null | n1!=0 | n2!=0 | n3!=0 -> requires curJob::node<_,_,null>
    ensures pq1'::ll1<n4,S4>*pq2'::ll1<n5,S5>*pq3'::ll1<n6,S6> &
      n4+n5+n6<=n1+n2+n3 & res=0;
// & S7=union(S1,S2,S3) & S8=union(S4,S5,S6) & S8 subset S7;//'
}
{
    //while(!finish());
...
node del_ele1(ref node x, node ele)
  requires (exists j,q: x::lseg1<ele,j,S1> * ele::node<v1,v2,q> * q::ll1<m,S2>) //&  v2 >=0 & v2 <= 3
