...
avl2<m, n, bal> == self = null & m = 0 & n = 0 & bal=1
  ...
  inv m >= 0 & n >= 0 & 0<=bal<=2;
        /*
avl3<m, n, S> == self = null & m = 0 & n = 0 & S={}
  or self::node<v, n, p, q> * p::avl3<m1, n1, S1> * q::avl3<m2, n2, S2>
		& m = 1+m1+m2 & n=1+max(n1, n2)
  & -1 <= n1-n2 <=1 &  S=union(S1,S2,{v})
  inv m >= 0 & n >= 0;
        */
...
case {
   ...
   x!=null ->
     ...
     (tn=resn | resn=tn+1 & resb!=1);//resb!=1
  }
...
case {
    x=null ->
      ensures res::avl2<1,1,b> & INS1(b);//1=b
   x!=null ->
      requires x::avl2<tm, tn, b>
      ensures res::avl2<tm+1, resn, resb> & tm>0 &tn>0 &
     (tn=resn | resn=tn+1 & INS(resb));//resb!=1
  }
/*
b=0 or b=2
 */
...
relation INSI( int c).
//relation INSI1( int c).
node insert_inline(node x, int a)
  infer[INSI]
case {
    x=null ->
      ensures res::avl2<1,1,b> & INSI(b);//1=b
   x!=null ->
      requires x::avl2<tm, tn, b>
      ensures res::avl2<tm+1, resn, resb> & tm>0 &tn>0 &
...
/*
  old spec:
  requires x::avl2<m, n,_>
  ensures res::avl2<m + 1, n1,_> & n <= n1 <= n+1;
*/
{
...
case {
  t1=null -> ensures res::avl2<s2,h2,b3> & MRG1(b2,b3);//b2>=0 & 2>=b2 & b2=b3
