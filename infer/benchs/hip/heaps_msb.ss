...
pq1<n,S> == self = null & n = 0 & S={}
  or (exists m3: self::node<d, m1, m2, l, r> * l::pq1<m1,S1> * r::pq1<m2,S2>
	& n = 1 + m1 + m2 & d >= 0 & m3 = m1-m2 & m3 >= 0 & m3 <= 1 & S=union(S1,S2,{d})) 
	inv n >= 0;
...
node insert(node t, int v)
  ...
  ensures res::pq1<n+1,S1> & INS(S1,S,v);//S1=union(S,{v});//INS(S1,S,v);
{
...
int deleteone1(ref int m1, ref int  m2, ref node l, ref node r)
  ...
  ensures l'::pq1<m1',S3> * r'::pq1<m2',S4> & res>=0 & m1' + m2' + 1 = m1 + m2 & 0 <= m1' - m2'<= 1;//'
