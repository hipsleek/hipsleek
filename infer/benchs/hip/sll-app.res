...
!!! Inferred constraints:[ xn!=0, xn!=0]
Inferred Heap:[]
         ...
         EBase true & xl<=ys & xs<=yl & 1<=xn & 0<=yn & MayLoop &
               {FLOW,(1,23)=__flow}
                 EAssume 1::
                   ...
                   yn>=0 & xn>=1 & xn+yn=m & 0<=xn & xs<=xl & 0<=yn & 
                   ys<=yl & {FLOW,(20,21)=__norm}
NEW RELS: [ ( xn=1 & yn=m - 1 & xs<=yl & r<=yl & yl<=t & 1<=m) -->  D(r,xs,t,yl,m,xn,yn), ( yl_595=yl & m=m_657+1 & yn_593=yn & xn_590=xn - 1 & r<=xs_591 & 
xs<=xs_591 & xs_591<=r_655 & r_655<=t_656 & t_656<=t & 1<=m_657 & 0<=yn & 
1<=xn & D(r_655,xs_591,t_656,yl_595,m_657,xn_590,yn_593)) -->  D(r,xs,t,yl,m,xn,yn)]
