...
node rotate_case_3(node a, node b, node c)
  infer [ROT3]
	...
	ensures res::rb1<0, bha + 1, S4> & ROT3(S4,S1,S2,S3);//& S4 = union(S1, S2, S3, {0}, {0});
...
node case_2(node a, node b, node c, node d)
  infer [ROT2]
	...
	ensures res::rb1<0, bha + 1, S5> & ROT2(S5,S1,S2,S3,S4);//S5 = union(S1, S2, S3, S4, {0}, {0}, {0});
...
node rotate_case_3r(node a, node b, node c)
  infer [ROT3R]
	...
	ensures res::rb1<0, bha + 1, S4> & ROT3R(S4,S1,S2,S3);//S4 = union(S1, S2, S3, {0}, {0});
...
node case_2r(node a, node b, node c, node d)
  infer [ROT2R]
	requires a::rb1<0, bha, S1> * b::rb1<0, bha, S2> * c::rb1<0, bha, S3> * d::rb1<0, bha, S4>
	ensures res::rb1<0, bha + 1, S5> & ROT2R(S5,S1,S2,S3,S4);//S5 = union(S1, S2, S3, S4, {0}, {0}, {0});
...
bool is_red(node x)
	...
	ensures x::rb1<cl, bh, S1> & cl = 1 & res & RED1(S1,S)
		or x::rb1<cl, bh, S2> & cl = 0 & !res & RED2(S2,S);
