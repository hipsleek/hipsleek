...
/*	ensures res::rb1<na+nb+nc+2, 0, h + 2, S4> & DEL61(S1,S2,S3,S4) & color = 0 or */
/*		res::rb1<na+nb+nc+2, 1, h + 1, S5> & DEL62(S1,S2,S3,S5) & color = 1;*/
/* */
/*{*/
/*	node tmp; */
...
node del_4(node a, node b, node c)
  infer [DEL4]
	...
	ensures res::rb1<na+nb+nc+2, 0, ha + 1, S4> & DEL4(S1,S2,S3,S4);//S4 = union(S1, S2, S3, {0}, {0});
...
node del_4r(node a, node b, node c)
  infer [DEL4R]
	requires a::rb1<na, 0, ha, S1> * b::rb1<nb, 0, ha, S2> * c::rb1<nc, 0, ha, S3> 
	ensures res::rb1<na+nb+nc+2, 0, ha + 1, S4> & DEL4R(S1,S2,S3,S4);//& S4 = union(S1, S2, S3, {0}, {0});
...
node del_3(node a, node b, node c)
  infer [DEL3]
	requires a::rb1<na, 0, ha, S1> * b::rb1<nb, 0, ha, S2> * c::rb1<nc, 0, ha, S3> 
	ensures res::rb1<na+nb+nc+2, 0, ha + 1, S4> & DEL3(S1,S2,S3,S4);//& S4 = union(S1, S2, S3, {0}, {0});
{
	node tmp;
...
node del_3r(node a, node b, node c)
  infer [DEL3R]
	requires a::rb1<na, 0, ha, S1> * b::rb1<nb, 0, ha, S2> * c::rb1<nc, 0, ha, S3> 
	ensures res::rb1<na+nb+nc+2, 0, ha + 1, S4> & DEL3R(S1,S2,S3,S4);//& S4 = union(S1, S2, S3, {0}, {0});
...
node del_2_1(node a, node b, node c)
	...
	ensures res::rb1<na+nb+nc+2, 0, h + 2, S4> & S4 = union(S1, S2, S3, {0}, {0});
...
node insert_1(node x, int v)
  infer [INS]
	...
	ensures res::rb1<n+1, _, bh1, S1> & res != null & bh <= bh1 <= bh & INS(S1,S,v);//S1 = union(S, {v});
