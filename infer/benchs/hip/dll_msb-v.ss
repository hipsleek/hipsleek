...
//infer[PUF]
  ...
  ensures x'::node<v,_,q>*q::dll<_,n,S1> & S1=union(S,{v});//& PUF(S1,S);//' S1=S
{
  if (x==null) {
...
//  infer[PF]
  ...
  ensures x'::dll<_,m-1,S2> & S2 subset S1; //'S2<subset> S1 PF(S1,S2)
{
  node tmp = x;
  if (x.next == null) {
    x = x.next;
...
// infer [APP2]
  ...
  ensures x::dll<q, t, S> & t=m+n & S=union(S1,S2);//APP2(S,S1,S2); //S=Union(S1,S2)
{
	node tmp;
	if (x.next == null) {
		x.next = y;
...
//infer[GN]
  ...
  ensures x'::dll<_,1,S1> * res::dll<_,n-1,S2> & S = union(S1, S2);//GN(S,S1,S2); //'S = union(S1, S2)
{
  node tmp = x.next;
  x.next = null;
  x.prev = null;
...
//infer[SN]
  ...
  ensures x::node<v,_,y>*y::dll<_,j,S2> & S2=S;//SN(S,S2); // S2=S
{
  if (y==null) 
    x.next = y;
  else {
...
node get_next_next(node x)
  ...
  ensures res::dll<_,n-2,S2> & S2 subset S;//GNN(S,S2); //S2 subset S
{
  return x.next.next;
}
...
void insert(node x, int a)
  ...
  ensures x::dll<p, m, S1> & m=n+1 & S1=union(S,{a});//INSERT(S,S1,a); //& S1=S+{a}
{
  if (x.next == null)
...
//infer [TRAV]
  ...
  ensures x::dll<p,n,S2> & S1=S2;//TRAV(S1,S2);//S1=S2
{
  node t;
  if(x != null) {
    t = x;
...
// infer[FGE]
  requires x::dll<_,n,S> & n >= 0
  ensures res = null or res::node<m,_,_> & m > v & m in S;//FGE(S,m);//{m} <subset> S;
{
  if(x == null)
    return null;
  else {
