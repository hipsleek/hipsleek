...
//dll
int front(node x)
  requires x::node<v,p>*p::sll2<m,_, sm, lg>
  ensures res=v;
{
  return x.val;
}
...
void push_front(ref node x, int v)
  infer[PUF]
  requires x::sll2<n,S, sm, lg>
  ensures x'::sll2<n+1,S1, v, lg1>  & PUF(v,sm);//'v<=sm
{
