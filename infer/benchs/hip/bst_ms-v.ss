...
// infer[APP]
  ...
  ensures res::dll<r, k> & m>=0 & k>=m & k=n+m;
{
	node2 z;
...
//  infer[CNT]
  requires z::bst0<n>
  ensures  z::bst0<n1> & res = n & n>=0 & n=n1;
{
	int cleft, cright;
...
//  infer[INS]
  requires x::bst0<m>
  ensures res::bst0<m1> & res != null & m>=0 & m+1=m1;
{
	node2 tmp;
        node2 tmp_null = null;
...
bool search(node2 x, int a)
  requires x::bst0<n>
  ensures x::bst0<n> & (res | !res) ;//'n>=0 & h>=0 & n=n1 & h=h1
{
	int tmp;
