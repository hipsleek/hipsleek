...
*/
  /* infer [ann1, ann2] */
  /* requires (x::bigint<v1>@ann1 & y::bigint<v2>@ann2) & 0 <= c <= 1 */
...
node add(node x, node y)
  /* infer [ann1, ann2] */
  /* requires (x::bigint<v1>@ann1 & y::bigint<v2>@ann2) & true */
...
node sub_one_digit(node x, int c)
  infer [ann]
  requires x::bigint<v>@ann & 0 <= c <= 9 & c <= v
  ensures res::bigint<v-c>;
{
...
*/
  /* infer [ann1, ann2] */
  /* requires (y::bigint<v2>@ann1 & x::bigint<v1>@ann2) & 0 <= c <= 1 & v1 >= v2+c */
...
node sub(node x, node y)
  /* infer [ann1, ann2] */
  /* requires (x::bigint<v1>@ann1 & y::bigint<v2>@ann2) & v1 >= v2 */
...
node mult_c(node x, int d, int c)
  infer [ann]
  requires x::bigint<v>@ann & 0 <= c <= 9 & 0 <= d <= 9 
...
node mult2(node x, node y)
  /* infer [ann1, ann2] */
  /* requires (x::bigint<v1>@ann1 & y::bigint<v2>@ann2) */
