...
let gen_lemma prog formula_rev_fnc manage_unsafe_lemmas_fnc es lem_type
  ...
  try
    ...
    (*NORM lhs-rhs*)
    let ( _,mix_lf,_,_,_,_,_,_) = Cformula.split_components (Cformula.Base lhs_b0) in
    let l_emap0 = get_eqset (Mcpure.pure_of_mix mix_lf) in
    let (_,mix_rf0,_,_,_,_,_,_) = Cformula.split_components (Cformula.Base rhs_b0) in
    ...
    | chp::rest ->
      ...
      with Not_found ->
        let n_ihp = {
          ...
          (* Iast.hp_view = None; *)
          Iast.hp_formula = Iformula.mkBase Iformula.HEmp (Ipure.mkTrue no_pos) IvpermUtils.empty_vperm_sets top_flow [] [] Iformula.Sec_LO no_pos;
          (* ADI TODO: to check *)
        }
        in
...
let gen_lemma_infer_x (prog) ass_stk hpdef_stk
  ...
  in
  (*for subst*)
  let ( _,mf,_,_,_,_,_,_) = Cformula.split_components es.Cformula.es_formula in
