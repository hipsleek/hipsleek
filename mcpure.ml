...
let empty_var_aset = Gen.is_empty_a_set_eq eq_spec_var 
let empty_var_aset = Gen.is_empty_a_set_eq eq_spec_var 
...
let print_alias_set aset = Gen.EqMap(Slices.SV).string_of !print_sv_f aset
let print_alias_set aset = Gen.EqMap(Slices.SV).string_of !print_sv_f aset
    
let rec mfv (m: memo_pure) : spec_var list = Gen.BList.remove_dups_eq (=)_f (List.concat (List.map (fun c-> c.memo_group_fv) m)) eq_spec_var
let rec mfv (m: memo_pure) : spec_var list = Gen.BList.remove_dups_eq (=)_f (List.concat (List.map (fun c-> c.memo_group_fv) m)) eq_spec_var
  ...
  match (List.length f) with
    ...
    | 1 ->
        ...
        (match ((List.length r.memo_group_cons),(List.length r.memo_group_slice)) with
            | 0,1 -> isConstTrue (List.hd r.memo_group_slice) && (Gen.EqMap(Slices.SV).is_empty r.memo_group_aset)
            | 0,1 -> isConstTrue (List.hd r.memo_group_slice) && (Gen.EqMap(Slices.SV).is_empty r.memo_group_aset)
            | 1,0 -> isConstMConsTrue (List.hd r.memo_group_cons) && (Gen.EqMap(Slices.SV).is_empty r.memo_group_aset)
            | 1,0 -> isConstMConsTrue (List.hd r.memo_group_cons) && (Gen.EqMap(Slices.SV).is_empty r.memo_group_aset)
            | _ -> false)
    | _ -> false
      
and isConstGroupTrue (f:memoised_group) : bool = match f.memo_group_slice with
  | [] -> f.memo_group_cons == [] && (Gen.EqMap(Slices.SV).is_empty f.memo_group_aset) 
  | [] -> f.memo_group_cons == [] && (Gen.EqMap(Slices.SV).is_empty f.memo_group_aset) 
  | x::[] -> f.memo_group_cons == [] && (Gen.EqMap(Slices.SV).is_empty f.memo_group_aset) && (isConstTrue x)
  | x::[] -> f.memo_group_cons == [] && (Gen.EqMap(Slices.SV).is_empty f.memo_group_aset) && (isConstTrue x)
  | _ -> false
...
and isConstMFalse f = 
  match (List.length f) with
    | 1 ->
        let r = List.hd f in
        (match ((List.length r.memo_group_cons),(List.length r.memo_group_slice)) with
            | 0,1 -> isConstFalse (List.hd r.memo_group_slice)&& (Gen.EqMap(Slices.SV).is_empty r.memo_group_aset)
            | 0,1 -> isConstFalse (List.hd r.memo_group_slice)&& (Gen.EqMap(Slices.SV).is_empty r.memo_group_aset)
            | 1,0 -> isConstMConsFalse (List.hd r.memo_group_cons) && (Gen.EqMap(Slices.SV).is_empty r.memo_group_aset)
            | 1,0 -> isConstMConsFalse (List.hd r.memo_group_cons) && (Gen.EqMap(Slices.SV).is_empty r.memo_group_aset)
            | _ -> false)
    | _ -> false
...
and group_mem_by_fv (lst: memo_pure):memo_pure = 
  ...
  let r = List.fold_left (fun acc (v_list, mem) -> 
			    ...
			    let l_v,l_m = List.fold_left (fun (a1,a2)(c1,c2)-> (a1@c1,c2@a2)) (v_list,mem) l_merged in
			      ((Gen.BList.remove_dups_eq (=)_f l_v eq_spec_var),l_m)::l_unmerged ) [] n_l in
			      ((Gen.BList.remove_dups_eq (=)_f l_v eq_spec_var),l_m)::l_unmerged ) [] n_l in
    ...
    List.map (fun (v_l,m_l)-> 
		let mc_l, f_l,a_l = List.fold_left (fun (a1,a2,a3) c -> match c with
						      ...
						      | None, None, Some f -> (a1,a2,Gen.EqMap(Slices.SV).merge_eset(*_debug !print_sv_f*) f a3)
						      | None, None, Some f -> (a1,a2,Gen.EqMap(Slices.SV).merge_eset(*_debug !print_sv_f*) f a3)
  ...
  let rec helper c = match c with
	...
	let h_m = List.fold_left (fun a c-> 
				    { memo_group_fv = a.memo_group_fv @c.memo_group_fv;
				      ...
				      memo_group_changed = true;
				      memo_group_aset = Gen.EqMap(Slices.SV).merge_eset(*_debug !print_sv_f*) a.memo_group_aset c.memo_group_aset;}) h h_merged in
				      memo_group_aset = Gen.EqMap(Slices.SV).merge_eset(*_debug !print_sv_f*) a.memo_group_aset c.memo_group_aset;}) h h_merged in
	let r_h = {h_m with memo_group_fv = Gen.BList.remove_dups_eq (=)_f h_m.memo_group_fv eq_spec_var;} in      
	let r_h = {h_m with memo_group_fv = Gen.BList.remove_dups_eq (=)_f h_m.memo_group_fv eq_spec_var;} in      
...
and subst_avoid_capture_memo (fr : spec_var list) (t : spec_var list) (f_l : memo_pure) : memo_pure=
  ...
  let helper  (s:(spec_var*spec_var) list) f  = 
    let r = Gen.EqMap(Slices.SV).rename_eset_allow_clash(*_debug*) !print_sv_f (subs_one s) f.memo_group_aset in
    let r = Gen.EqMap(Slices.SV).rename_eset_allow_clash(*_debug*) !print_sv_f (subs_one s) f.memo_group_aset in
...
and m_apply_one (s:spec_var * spec_var) f = 
  let r1 = List.map (fun c -> 
		       let r = Gen.EqMap(Slices.SV).subs_eset(*_debug !print_sv_f*) s c.memo_group_aset in
		       let r = Gen.EqMap(Slices.SV).subs_eset(*_debug !print_sv_f*) s c.memo_group_aset in
...
(*and alias_of_ptr_eqs s = List.fold_left (fun a (c1,c2) -> Gen.EqMap(Slices.SV).add_equiv a c1 c2) Gen.is_empty_aset s*)
(*and alias_of_ptr_eqs s = List.fold_left (fun a (c1,c2) -> Gen.EqMap(Slices.SV).add_equiv a c1 c2) Gen.is_empty_aset s*)
(*and alias_of_ptr_eqs s = List.fold_left (fun a (c1,c2) -> Gen.EqMap(Slices.SV).add_equiv a c1 c2) Gen.is_empty_aset s*)
(*and alias_of_ptr_eqs s = List.fold_left (fun a (c1,c2) -> Gen.EqMap(Slices.SV).add_equiv a c1 c2) Gen.is_empty_aset s*)
...
and get_subst_equation_memo_formula (f0 : memo_pure) (v : spec_var) only_vars: ((spec_var * exp) list * memo_pure) = 
  let r = List.fold_left (fun (a1,a2) c ->
			    if not(a1=[]) then (a1,c::a2)
			      ...
			      else match Gen.EqMap(Slices.SV).find_equiv_elim v c.memo_group_aset with
			      else match Gen.EqMap(Slices.SV).find_equiv_elim v c.memo_group_aset with
  ...
  List.exists (fun c-> 
		 let r = (List.exists (is_member_pure p) c.memo_group_slice)||
		   ...
		   if r then true
		     ...
		     | BForm (Eq(Var(v1,_),Var(v2,_),_), _) -> Gen.EqMap(Slices.SV).is_equiv c.memo_group_aset v1 v2
		     | BForm (Eq(Var(v1,_),Var(v2,_),_), _) -> Gen.EqMap(Slices.SV).is_equiv c.memo_group_aset v1 v2
		     | _ -> false ) mm
  ...
  let r=  if (isConstMFalse l1)||(isConstMTrue l2) then l1
   ...
   if !f_1_slice then 
      (if (List.length l1)>1 || (List.length l2)>1  then (print_string "multi slice problem " ;failwith "multi slice problem" );      
        ...
        let na =Gen.EqMap(Slices.SV).merge_eset h1.memo_group_aset h2.memo_group_aset in
        let na =Gen.EqMap(Slices.SV).merge_eset h1.memo_group_aset h2.memo_group_aset in
    ...
    List.fold_left (fun a c-> 
			...
			(fun (a1,a2,a3,a4) d-> 
			   let r = (Gen.EqMap(Slices.SV).merge_eset(*_debug !print_sv_f*) a4 d.memo_group_aset) in
			   let r = (Gen.EqMap(Slices.SV).merge_eset(*_debug !print_sv_f*) a4 d.memo_group_aset) in
			     (d.memo_group_fv@a1,
			...
			let n_slc = if (not slice_check_dups) then n3 
			  ...
			  (Gen.Profiling.push_time "merge_mems_r_dups";
			   let r = Gen.BList.remove_dups_eq (=)_f n3 eq_pure_formula in
			   let r = Gen.BList.remove_dups_eq (=)_f n3 eq_pure_formula in
...
(*add cm to l_init and depending on the fnf flag
  ...
  d.memo_group_cons::a2,
  d.memo_group_slice::a3, 
  Gen.EqMap(Slices.SV).merge_eset(*_debug !print_sv_f*) a4 d.memo_group_aset))
  Gen.EqMap(Slices.SV).merge_eset(*_debug !print_sv_f*) a4 d.memo_group_aset))
...
and memo_pure_push_exists_eq_x (qv:spec_var list) (f0:memo_pure) pos : (memo_pure* spec_var list)  = 
     ...
     let split_eqs eq_list qv  = 
        let aliases  = List.map (fun c-> (c,Gen.EqMap(Slices.SV).find_equiv_all c eq_list)) qv in
        let aliases  = List.map (fun c-> (c,Gen.EqMap(Slices.SV).find_equiv_all c eq_list)) qv in
  ...
  let ante_memo0 = 
    if !f_2_slice then match ante_memo0 with
       ...
       | h::t -> [List.fold_left (fun a c-> 
          let na =Gen.EqMap(Slices.SV).merge_eset a.memo_group_aset c.memo_group_aset in
          let na =Gen.EqMap(Slices.SV).merge_eset a.memo_group_aset c.memo_group_aset in
