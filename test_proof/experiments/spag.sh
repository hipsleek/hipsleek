...
do
	...
	(time (./timeout -TERM $TIMEOUT $SLEEK -tp $2 $SPAG/spaguetti-$i.slk --eps --enable-slicing)) >$LOG/spaguetti.eps.aus.$2.$1 2>&1
	(time (./timeout -TERM $TIMEOUT $SLEEK -tp $2 $SPAG/spaguetti-$i.slk --eps --efp --enable-slicing)) >$LOG/spaguetti.efp.aus.$2.$1 2>&1
	(time (./timeout -TERM $TIMEOUT $SLEEK -tp $2 $SPAG/spaguetti-$i.slk --eps --enable-slicing --slc-ann-ineq)) >$LOG/spaguetti.eps.ans.$2.$1 2>&1
	(time (./timeout -TERM $TIMEOUT $SLEEK -tp $2 $SPAG/spaguetti-$i.slk --eps --efp --enable-slicing --slc-ann-ineq)) >$LOG/spaguetti.efp.ans.$2.$1 2>&1
