...
let simplify_post_helper heap pure post_fml post_vars prog subst_fml pre_vars inf_post ref_vars =
  ...
  let p,pre,bag_vars = begin
    ...
    | Some triples (*(rel, post, pre)*) ->
      ...
      if inf_post then
        ...
        let rels = CP.get_RelForm p in
        let pres, posts = List.split (List.concat (List.map (fun (a1,a2,a3) -> 
            if (is_empty rels) || (Gen.BList.mem_eq CP.equalFormula a1 rels)
            then [(a3,a2)] else []) triples)) in
...
let simplify_post_helper heap pure post_fml post_vars prog subst_fml pre_vars inf_post ref_vars =
  ...
  Debug.no_3 "simplify_post_helper" pr1 (add_str "post_vars" pr_svl) 
    (add_str "subst_fml" pr_3) pr_r 
    (fun _ _ _ -> simplify_post_helper heap pure post_fml post_vars prog subst_fml pre_vars inf_post ref_vars)
    post_fml post_vars subst_fml

let rec simplify_post post_fml post_vars prog subst_fml pre_vars inf_post evars ref_vars = 
  match post_fml with
  | CF.Or _ ->
  ...
  | CF.Exists e ->
    let h,p,pre,bag_vars = x_add simplify_post_helper e.CF.formula_exists_heap e.CF.formula_exists_pure post_fml post_vars prog subst_fml pre_vars inf_post ref_vars in
  ...
  | CF.Base b ->
    let h,p,pre,bag_vars = x_add simplify_post_helper b.CF.formula_base_heap b.CF.formula_base_pure post_fml post_vars prog subst_fml pre_vars inf_post ref_vars in
    (*print_endline ("VARS: " ^ Cprinter.string_of_spec_var_list pre_vars);*)
...
let rec simplify_pre pre_fml lst_assume = 
  match pre_fml with
  | CF.Or _ ->
  ...
  | _ ->
    ...
    let p = 
      if !do_infer_inc then TP.pairwisecheck_raw (Infer.simplify_helper (CP.conj_of_list p2 no_pos))
      else CP.mkAnd (TP.pairwisecheck_raw (Infer.simplify_helper (CP.conj_of_list p2 no_pos))) (CP.conj_of_list p1 no_pos) no_pos
    in
    let p = 
      if lst_assume = [] then x_add_1 CP.drop_rel_formula p (* need to recheck *)
      else
        let rels = CP.get_RelForm p in
        let p = x_add_1 CP.drop_rel_formula p in
        let ps = List.filter (fun x -> not (CP.isConstTrue x)) (CP.list_of_conjs p) in
        let pres = List.concat (List.map (fun (a1, a2, a3) ->
            if (Gen.BList.mem_eq CP.equalFormula a2 rels) &&
               not (TP.imply_raw p a3) then [a3] else []) lst_assume) in
        let lexvar_lst, pfl = List.partition CP.is_lexvar (ps@pres) in
        let () = y_tinfo_hp (add_str "pfl" (pr_list !CP.print_formula)) pfl in
        ...
        let pre = CP.conj_of_list (pre::lexvar_lst) no_pos in
        pre
    in
...
let simplify_pre pre_fml lst_assume =
  ...
  Debug.no_2 "simplify_pre" pr pr1 pr
      (fun _ _ -> simplify_pre pre_fml lst_assume)
      pre_fml lst_assume
...
let rec simplify_relation (sp:CF.struc_formula) subst_fml pre_vars post_vars prog inf_post evars lst_assume
  ...
  | CF.EBase b ->
    ...
    in
    let base =
      if pres = [] then x_add simplify_pre b.CF.formula_struc_base lst_assume
      else
        let pre = CP.conj_of_list pres no_pos in
...
let simplify_relation sp subst_fml pre_vars post_vars prog inf_post evars lst_assume =
  ...
  Debug.no_4 "simplify_relation" 
    (add_str "spec" pr) (add_str "subst_fml" pr1) (add_str "lst_assume" pr2) 
    (add_str "inf_post" string_of_bool) (pr_pair pr (pr_list pr_f))
    (fun _ _ _ _ -> simplify_relation sp subst_fml pre_vars post_vars prog inf_post evars lst_assume) sp subst_fml lst_assume inf_post
...
let pre_calculate fp_func input_fml pre_vars proc_spec
  ...
  let () = x_binfo_hp (add_str "top_down_fp" (pr_list (pr_pair pr pr))) top_down_fp no_pos in
  ...
  | [(_,rec_inv)] ->
    ...
    let pre_rec = x_add_1 TP.simplify_raw fml in
    ...
    let final_pre1 = x_add_1 TP.simplify_raw final_pre0 in
  ...
  | [] -> let exist_post = List.exists (fun (_,post) -> not (CP.isConstTrue post)) rel_posts in
    if rel_posts != [] && exist_post then
      ...
      [(constTrue,constTrue, pre, pure_oblg_to_check)]
...
(*
only apply for obl of pre
 rel /\ \pi ==> true
 --> \pi ==> rel
 *)
let strengthen_preoblg_x reloblgs=
  let is_rel_form p=
    match p with
      | CP.BForm ((CP.RelForm _ ,_) ,_) -> true
      | _ -> false
  in
  let trans_obj (acc_oblgs) ((rel_cat, lhs, rhs) as oblg)=
    match rel_cat with
      | CP.RelAssume rels -> begin
           match rels with
             | [rel] ->
                   if CP.isConstTrue rhs then
                     let ps = CP.list_of_conjs lhs in
                     let rel_ps, rems = List.partition is_rel_form ps in
                     let new_rhs = CP.conj_of_list rems no_pos in
                     let new_lhs = CP.conj_of_list rel_ps no_pos in
                      (acc_oblgs@[(rel_cat,new_lhs ,new_rhs )])
                   else
                     (acc_oblgs@[oblg])
             | _ -> (acc_oblgs@[oblg])
        end
  ...
  List.fold_left trans_obj ([]) reloblgs

let strengthen_preoblg reloblgs=
  let pr1 = Cprinter.string_of_pure_formula in
  let pr2 = pr_pair pr1 pr1 in
  let pr3 = CP.print_rel_cat in
  let pr4 = (pr_list (pr_triple pr3 pr1 pr1)) in
  (* let pr5 = pr_list pr2 in *)
  Debug.no_1 "strengthen_preoblg" pr4 pr4
      (fun _ -> strengthen_preoblg_x reloblgs) reloblgs
...
let pre_rel_fixpoint pre_rel pre_fmls pre_invs fp_func reloblgs pre_vars proc_spec pre_rel_df=
  ...
  if checkpoint1 (* && false *) then [(constTrue,constTrue,pre_rel,constTrue)]
    ...
    x_add calculate_gfp fp_func input_fml pre_vars proc_spec
      pre_inv (* constTrue *) pure_oblg_to_check ([constTrue,constTrue],pre_rel) pre_fmls pre_rel_vars pre_rel_df (* reloblgs *)
...
let update_with_td_fp bottom_up_fp pre_rel_fmls pre_fmls pre_invs fp_func
  ...
  | _,_ ->
    try
      let () = Debug.info_hprint (add_str "pr_rel_fmls" (pr_list Cprinter.string_of_pure_formula)) pre_rel_fmls no_pos in
