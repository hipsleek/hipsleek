...
let rev_imply_formula f1 f2 = match (f1,f2) with
  | (CF.Base _, CF.Base _) | (CF.Exists _, CF.Exists _) -> 
    let h1,p1,vp1,fl1,b1,t1,sec1 = CF.split_components f1 in
    let h2,p2,vp2,fl2,b2,t2,sec2 = CF.split_components f2 in
    (* ADI TODO: use sec1 & sec2 *)
...
let helper heap pure post_fml post_vars prog subst_fml pre_vars inf_post ref_vars =
  let h, p, _, _, _, _, _ = CF.split_components post_fml in
...
let rec simplify_pre pre_fml lst_assume = match pre_fml with
  ...
  | _ ->
    let h, p, vp, fl, t, a, sec = CF.split_components pre_fml in
    (* ADI TODO: use sec *)
