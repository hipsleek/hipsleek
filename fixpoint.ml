...
let rev_imply_formula f1 f2 = match (f1,f2) with
  | (CF.Base _, CF.Base _) | (CF.Exists _, CF.Exists _) -> 
    let h1,p1,vp1,fl1,b1,t1,sec1,ctx1 = CF.split_components f1 in
    let h2,p2,vp2,fl2,b2,t2,sec2,ctx2 = CF.split_components f2 in
    (* ADI TODO: use sec1 & sec2 & ctx1 & ctx2 *)
...
let helper heap pure post_fml post_vars prog subst_fml pre_vars inf_post ref_vars =
  let h, p, _, _, _, _, _, _ = CF.split_components post_fml in
...
let rec simplify_post post_fml post_vars prog subst_fml pre_vars inf_post evars ref_vars = match post_fml with
  ...
  | CF.Base b ->
    ...
    let fml = CF.mkExists (CP.remove_dups_svl (evars @ bag_vars @ exists_h_vars))
                h (MCP.mix_of_pure p) b.CF.formula_base_vperm b.CF.formula_base_type b.CF.formula_base_flow
                b.CF.formula_base_and b.CF.formula_base_sec b.CF.formula_base_sec_ctx b.CF.formula_base_pos
    in (fml,pre)
...
let rec simplify_pre pre_fml lst_assume = match pre_fml with
  ...
  | _ ->
    let h, p, vp, fl, t, a, sec, ctx = CF.split_components pre_fml in
    (* ADI TODO: use sec & ctx *)
    ...
    let p = if lst_assume = [] then x_add_1 CP.drop_rel_formula p (* need to recheck *)
        ...
        pre
    in
    CF.mkBase h (MCP.mix_of_pure p) vp t fl a sec ctx no_pos
