...
let helper heap pure post_fml post_vars prog subst_fml pre_vars inf_post ref_vars =
  ...
  let p,pre,bag_vars = begin
    ...
    | None ->
      ...
      (p,[],bag_vars)
    | Some triples (*(rel_post, post, rel_pre pre)*) ->
      let process_tables results =
        let sd = Fixcalc.get_reorder () in
        List.map (fun (r,post,_,pre) -> match r with
      ...
      if inf_post then
        ...
        let () = x_tinfo_hp (add_str "triples" (pr_list (pr_quad pr pr pr pr)) ) triples no_pos in
        ...
        let post = p in
        let pre = CP.conj_of_list (List.map (fun (_,_,_,pre) -> pre) triples) no_pos in
...
let helper heap pure post_fml post_vars prog subst_fml pre_vars inf_post ref_vars =
  ...
  let pr_3 = pr_option (pr_list (pr_quad pr_pf pr_pf pr_pf pr_pf)) in
...
let pre_rel_fixpoint pre_rel pre_fmls pre_invs fp_func reloblgs pre_vars proc_spec pre_rel_df=
  ...
  let () = Debug.info_hprint (add_str "oblg to check" !CP.print_formula) pure_oblg_to_check no_pos in
  ...
  if checkpoint1 (* && false *) then [(constTrue,constTrue,pre_rel,constTrue)]
    ...
    let pr = Cprinter.string_of_pure_formula in
    let () = Debug.dinfo_hprint (add_str "input_fml" (pr_list (pr_pair pr pr))) input_fml no_pos in
