...
/*node c1(int val, node next) 
static 
requires next::node<_, _> or next = null
ensures res::node<val, next>;
{
...
class List {
			...
			ensures x::ll<n+1>;
	{
		...
		if (x.next == null)
			x.next = new node(a, tmp); //c1(a);
		else 
			this.insert(x.next, a);
	}
}
