...
/*
  Add r to the free list. The new value of the free list pointer is r.
 */
static void
freeroute(Route *r)
{
...
/*
  Allocate new routetree node. The node will first be taken from the
  freelist. If the freelist is empty, then the node will be freshly
  allocated.
 */
static Route*
allocroute(int type)
...
/*
  What would be a description of this queue data structure?
  mid pointer makes the list
  left pointer points to elements
...
/*
  This function should be inlined for verification.
 */
static int
...
/*
  Calls addnode(f, root, p) (p.left and p.right set to null)
  Recursively walks down the left and right subtrees of p.

  So the net result would be destructing the tree pointed to by p,
  adding all nodes to tree root by using addnode.
 */
static void
