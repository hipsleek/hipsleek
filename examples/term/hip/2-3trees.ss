	...
	ensures x'::tree2_3<n+1> & x = x' & res = null;
{
	...
	if (x == null) {					// x is empty
		...
		if (x.left == null) {	// x is just one leaf node
			if (x.maxl <= a) {
				node3 aux = new node3(x.maxl, a, x, leaf, null);
				x = null;	
				return aux;		
			...
			if(a < x.maxl) {			// we have to insert in the left substree 
				...
				/*if( tmp1.left.left == null) {	
					...
					node3 aux = tmp1.left;
					/*if(a < tmp1.maxl)*/ tmp2 = insert(aux/*tmp1.left*/, a);
					assert aux'::tree2_3<0> * tmp2'::tree2_3<n1> & n1 > 0 & aux' = null
					or aux'::tree2_3<n2> * tmp2'::tree2_3<0> & n2 > 0 & tmp2' = null;
					/*else {
						if(a < tmp1.maxm || tmp1.right == null) tmp2 = insert(tmp1.middle, a);
						else tmp2 = insert(tmp1.right, a);
					}*/
					if (tmp2 != null) {
						node3 aux1 = tmp1.right;
						//assume aux1'::tree2_3<nn> & nn>0;
						tmp1.left = tmp1.middle;
						tmp1.middle = tmp1.right;
						tmp1.right = null;
						assert tmp1'::tree2_3<_>;
						if(x.right == null) {	// x has 2 children		
							x.right = x.middle;
							x.middle = x.left;
							x.left = tmp2;
							...
							node3 newnode = new node3(0, 0, tmp2, x.left, null);
							x.left = x.middle;
							x.middle = x.right;
							x.right = null;
							node3 aux = new node3(0, 0, newnode, x, null);
							assert aux'::tree2_3<_>;
							x = null;
							return aux;
						}
					}
