...
rb<"n":n, cl, "b":bh> == 
	self = null & cl = 0 & ["n":n=0; "b":bh = 1] or 
	self::node<v, 1, l, r> * l::rb<nl, 0, bhl> * r::rb<nr, 0, bhr> & cl = 1 & ["n":n = 1 + nl + nr; "b":bhl = bh & bhr = bh] or
	self::node<v, 0, l, r> * l::rb<nl, _, bhl> * r::rb<nr, _, bhr> & cl = 0 & ["n":n = 1 + nl + nr; "b":bhl = bhr & bh = 1 + bhl]
	inv 0 <= cl <= 1 & ["n":n >= 0; "b":bh >= 1];
...
node rotate_case_3(node a, node b, node c)
	...
	ensures res::rb<nr, 0, br> & ["n":nr=na + nb + nc + 2; "b":br=bha + 1];
...
node case_2(node a, node b, node c, node d)
	...
	ensures res::rb<nr, 0, br> & ["n":nr=na + nb + nc + nd + 3; "b":br=bha + 1];
...
bool is_red(node x)
	...
	ensures x::rb<n, cl, bh> & cl = 1 & res or 
		x::rb<n, cl, bh> & cl = 0 & !res;
...
node del_6r(node a, node b, node c, int color)
	...
	ensures res::rb<nr, 0, hr> & color = 0 & ["n":nr = na + nb + nc + 2; "b":hr = ha + 2] or 
		res::rb<nr, 1, hr> & color = 1 & ["n":nr = na + nb + nc + 2; "b":hr = ha + 1];
...
node del_5(node a, node b, node c, node d, int color)
	...
	ensures res::rb<nr, 0, hr> & color = 0 & ["n":nr = na + nb + nc + nd + 3; "b":hr = h + 2] or 
		res::rb<nr, 1, hr> & color = 1 & ["n":nr = na + nb + nc + nd + 3; "b":hr = h + 1];
...
node del_4(node a, node b, node c)
	...
	ensures res::rb<nr, 0, hr> & ["n":nr=na + nb + nc + 2; "b":hr=ha + 1];
...
node del_2(node a, node b, node c)
	...
	ensures res::rb<nr, 0, hr> & ["n":nr=na + nb + nc + 2; "b":hr=ha + 2];
...
int remove_min(ref node x)
	...
	ensures x'::rb<nr, cl2, br> & cl = 1 & 0 <= cl2 <= 1 & ["n":nr=n-1; "b":br=bh] or
		x'::rb<nr, 0, bh2> & cl = 0 & ["n":nr=n-1; "b":bh-1 <= bh2 <= bh];
...
void del(ref node  x, int a)
	...
	ensures  x'::rb<nr, cl2, bh> & cl = 1 & 0 <= cl2 <= 1 & ["n":nr=n-1] or
		 x'::rb<nr, 0, bh2> & cl = 0 & ["n":nr=n-1; "b":bh-1 <= bh2 <= h] or
		 x'::rb<n, cl, bh>;
...
node insert(node x, int v)
	...
	ensures res::rb<nr, _, bh1> & res != null & ["n":nr=n+1; "b":bh<=bh1<=bh];
