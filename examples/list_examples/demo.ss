...

{
	if (a == 0) {
		...
		a = a - 1;
		return new node (v, create_list(a, v));
	}
}
...
/* function to delete the node with value a in a singly linked list */
node delete_val_new(node x, int a)
	requires x::ll<L> & a notinlist L1 & L = app(L1, a:::L2)
	ensures res::ll<L0> & L0 = app(L1, L2);
{
	if (x.val == a) {
		return x.next;
	} else {
		return new node(x.val, delete_val_new(x.next, a));
	}
}