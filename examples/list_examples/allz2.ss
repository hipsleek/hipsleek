...
coercion self::lz<L> <-> self::ll<L> & alln(0, L);
...
node get_next(node x)
	...
	requires x::lz<L> & len(L) > 0
	ensures x::ll<L1> * res::ll<L2> & len(L1) = 1 & len(L2) = len(L) - 1 & L1 = [| 0 |] & alln(0, L2);
...
/* node delete_val(node x, int a)
	...
	requires x::lz<L1> & x != null
	ensures res::ll<L2> & (a = 0 & len(L2) = len(L1) - 1 & alln(0, L2) | a != 0 & L2 = L1);
