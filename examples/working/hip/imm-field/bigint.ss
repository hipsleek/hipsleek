...
bigint<v,a1,a2> == self = null & v = 0 or
  self::node<p@a1, q@a2> * q::bigint<v1,a1,a2> & 0 <= p <= 9 & v = 10*v1 + p  
             inv v >= 0;

node clone(node x)
  requires x::bigint<v,@L,@L>
  ensures  res::bigint<v,@M,@M>;
{
...
node add_one_digit(node x, int c)
  requires x::bigint<v,@L,@L> & 0 <= c <= 9
  ensures res::bigint<v+c,@M,@M>;
{
  if (x == null) return new node(c, clone(x));
  int t = x.val + c;
...
node add_c(node x, node y, int c)
  requires x::bigint<v1,@L,@L> * y::bigint<v2,@L,@L> & 0 <= c <= 1
  ensures res::bigint<v1+v2+c,@M,@M>;
{
  if (x == null) {
    if (y == null) {
      if (c == 0) return null;
...
node add(node x, node y)
  requires x::bigint<v1,@L,@L> * y::bigint<v2,@L,@L>
  ensures res::bigint<v1+v2,@M,@M>;
{
  return add_c(x, y, 0);
}
...
node sub_one_digit(node x, int c)
  requires x::bigint<v,@L,@L> & 0 <= c <= 9 & c <= v
  ensures res::bigint<v-c,@M,@M> ;
{
  if (x == null) return null;
...
node sub_c(node x, node y, int c)
  requires x::bigint<v1,@L,@L> * y::bigint<v2,@L,@L> & 0 <= c <= 1 & v1 >= v2+c
  ensures res::bigint<v1-v2-c,@M,@M> ;
{
  if (x == null) return null;
...
node sub(node x, node y)
  requires x::bigint<v1,@L,@L> * y::bigint<v2,@L,@L> & v1 >= v2
  ensures res::bigint<v1-v2,@M,@M> ;
{
  return sub_c(x, y, 0);
}
...
node mult_c(node x, int d, int c)
  requires x::bigint<v,@L,@L> & 0 <= c <= 9 & 0 <= d <= 9 
  ensures res::bigint<v*d+c,@M,@M> ;
{
  if (x == null) {
...
node shift_left(node x)
  requires x::bigint<v,@L,@L>
  ensures res::bigint<v*10,@M,@M>;
{
  if (x == null) return null;
  else {
    node tmp = new node(0, clone(x));
    return tmp;
  }
}
...
int compare2(node x, node y)
  requires x::bigint<v1,@L,@L> * y::bigint<v2,@L,@L>
  ensures  (res = 0 & v1 = v2 | res = 1 & v1 > v2 | res = -1 & v1 < v2);
{
  if (x == null) {
