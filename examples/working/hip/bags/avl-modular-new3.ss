...
/*function to verificate  a is element of avl tree */

bool is_mem(node x, int a)
requires x:: avl<m,n,S> & x != null
ensures x:: avl<m,n,S> & (a notin S | res) & (a in S | !res);
{
    if(x.val == a )  
      return true;   
	if (x.val < a)
	{	if(x.right == null) return false;
		return is_mem(x.right,a);
	}
	else
	{	if(x.left == null) return false;
		return is_mem(x.left,a);
	}
...
node insert(node x, int a) throws dup_exc
  requires x::avl<m, n, S> & x != null //& (a notin S)
  ensures res::avl<nm, nn, Sn> & ["m":nm = m + 1; "n": n <= nn <= n+1; "S" : Sn = union(S, {a})]
  or res::dup_exc<> & flow dup_exc;
{
  node tmpn, tmp_null = null;
  int hr, hl;
  myint ret = new myint(0);
  
  if (is_mem(x, a)) raise (new dup_exc());
...
node remove_min_add(node x, ref myint a)
  requires x::avl<m,n,S> * a::myint<vv> & x != null & vv notin S  //& exists (yy : (yy notin S | yy > vv))
  ensures res::avl<m,n,Sn> * a'::myint<r> /*'*/ & 
    ["S" : union(S, {vv}) = union(Sn, {r}) &  forall (xx : (xx notin Sn | r < xx) & r <= vv)]; 
{
  int ti;
  node tn;
  node tr;
...
node remove_max_add(node x, ref myint a)
  requires x::avl<m,n,S> * a::myint<vv> & x != null  & vv notin S & exists (yy : (yy notin S | yy > vv))
  ensures res::avl<m,n,Sn> * a'::myint<r> & [ "S" : union(S, {vv}) = union(Sn, {r}) & r notin Sn &  forall (xx : (xx notin Sn | r > xx) & r >= vv)];
...
node remove_min(node x, ref myint a)
    ...
    ensures res::avl<nm, nn, Sn> * a'::myint<r> & 
      ["m" : nm = m - 1; 
	   "n" : n-1 <= nn <= n; 
	   "S" : union(Sn, {r}) = S & r notin Sn & forall (xx : (xx notin S | r <= xx))];
