...
//infer @post []
  requires x::ll2<m,S3> & n>=0
   case {
  n = 0 -> ensures x'=null;
n > 0 -> ensures x'::ll2<n,S4> & forall (_x: _x notin S4 | _x = v); //' forall _x: _x notin S4 | _x = v;
 n < 0 -> ensures true;
}
{
  x = create_list1(n, v);
...
//infer[PUF]
  requires x::ll2<n,S>
  ensures x'::ll2<n+1,S1> & S1=union(S,{v});//' S1=S+{v}
{
...
// infer[PF]
  ...
  ensures x'::ll2<m-1,S2> & S2 subset S1; //'S2<subset> S1
{
  node tmp = x;
  x = x.next;
  tmp.next=null;
...
//  infer [APP]
  ...
  ensures x::ll2<m,S> & m=n1+n2 & S=union(S1,S2);//APP(S,S1,S2); //S=union(S1,S2)
{
  if (x.next == null)
    x.next = y;
  else
...
//  infer[GN]
  ...
  ensures x'::ll2<1,S1> * res::ll2<n-1,S2> & S = union(S1, S2);//GN(S,S1,S2); //'S = union(S1, S2)
{
  node tmp = x.next;
  x.next = null;
  return tmp;
...
// infer[SN]
  ...
  ensures x::ll2<k,S2> & k>=1 & k=j+1 & S2=union(S2,{v});
{
	x.next = y;
}
...
//  infer [INS]
  ...
  ensures x::ll2<n+1,S1> & S1=union(S,{a});//INS(S,S1,a);//S1=union(S,{a})
{
  node tmp = null;
  if (x.next == null)
...
// infer [DEL]
  ...
  ensures x::ll2<m,S1> & S1 subset S;//DEL(S,S1);//S1 subset S
{
  if (a == 1){
    x.next = x.next.next;
  }
...
// infer [DEL2]
  requires x::ll2<n,S>
  ensures res::ll2<m,S1> & m<=n & (a notin S & S = S1 | S=union(S1, {a}));//& (a notin S & S = S1 | S=union(S1, {a}));
{
	if (x == null)
		return x;
	else {
...
//  infer [REV]
  ...
  ensures ys'::ll2<n+m,S> & xs' = null & S=union(S1,S2);//REV(S,S1,S2); //S=union(S1,S2)
{
  if (xs != null) {
    node tmp;
    tmp = xs.next;
...
//  infer[SPLIT]
  ...
  ensures x'::ll2<n1, S1>*res::ll2<n2, S2> & n=n1+n2 & n1>0 & n2>0 & n1=a & S = union(S1, S2);
...
//  infer [TRAV]
  requires x::ll2<n,S1>
  ensures x::ll2<n,S2> & S1=S2;//TRAV(S1,S2);//S1=S2
{
  node t;
  if(x != null) {
    t = x;
...
// infer [CPY]
  requires x::ll2<n,S>
  ensures x::ll2<n,S> * res::ll2<n,S>;// & CPY(S,S2); //S2=S
{
  node tmp;
  if (x != null) {
...
//infer [FIL]
  requires x::ll2<n,S> & n >= 0
  ensures res::ll2<m,S2> & m <= n & S2 subset S;//FIL(S,S2);//S2 subset S
{
  node tmp;
  if(x != null) {
    if(x.val == v){
...
// infer[SPI]
  requires x::ll2<n,S1> * y::ll2<m,S2>
  ensures x'::ll2<m+n,S> & S=union(S1,S2);//'SPI(S,S1,S2); //S=union(S1,S2)
{
  if(x == null)
    x = y;
  else {
