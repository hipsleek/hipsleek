...
// infer[PF]
  ...
  ensures x'::sll1<m-1, S2> & S2 subset S1;//PF(S1,S2);//'S2 subset S1
{
  node tmp = x;
  x = x.next;
  tmp.next=null;
...
//  infer[GN]
  ...
  ensures x::node<v,null> * res::sll1<n-1,S2> & S = union(S2, {v});//GN(S,S2,v); //S = union(S2, {v})
{
  node tmp = x.next;
  x.next = null;
  return tmp;
...
//  infer[SN]
  ...
  ensures x'::sll1<k,S2> & k=j+1 & S2=union(S,{v});
//SN(S,S2,v);//'S2=union(S,{v});//SN(S,S2,v);
{
  node tmp = x;
  tmp.next = null;
  x = insert2(y, tmp);
...
//  infer[GT]
  requires x::sll1<n, S> & x != null
  ensures res::sll1<n-1, S1> & S1 subset S;//GT(S,S1);//S1 subset S;
{
	return x.next;
}
...
//  infer[GNN]
  ...
  ensures res::sll1<n-2,S2> & S2 subset S;//GNN(S, S2);//S2 subset S;
{
  return x.next.next;
}
...
//  infer[DEL2]
  requires x::sll1<n, S>
  ensures res::sll1<m, S1> & n-1 <= m <= n & (a notin S & S = S1 | S=union(S1, {a}));
 // (v notin S & S = S1 | S=union(S1, {v}));
{
	node tmp;
...
//  infer [CPY]
  requires x::sll1<n,S>
  ensures x::sll1<n,S> * res::sll1<n,S> ;//& CPY(S,S1);//
{
  node tmp;
  if (x != null) {
...
//  infer [FIL]
  requires x::sll1<n, S>
  ensures res::sll1<m, S2> &  m <= n & S2 subset S;//FIL(S,S2);
{
  node tmp;
  if(x != null) {
    if(x.val == v){
...
//  infer[FGE]
  ...
  ensures res = null or res::node<m,_> & m >= v & m in S;
//FGE(S,m);//& FGE(S,m);//{m}<subset> S
{
  if(x == null)
    return null;
  else {
