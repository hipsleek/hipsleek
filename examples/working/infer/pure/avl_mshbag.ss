...
relation HEIGHT(bag a, bag b).
int height1(node x)
  infer [HEIGHT]
  requires x::avl<m, n, S>
  ensures x::avl<m, n, S1> & res = n & HEIGHT(S1,S); //S=S1
{
  int tmpi;
  if (x == null)
    return 0;
...
relation ROTL(bag a, bag b, bag c, bag d, int e, int f).
node rotate_left(node l, node rl, node rr, int v, int vr)
  ...
  ensures res::avl<lm+rlm+rrm+2, 2+rln, S> & ROTL(S,Sl,Srl,Srr,v,vr);//S = union(Sl, Srl, Srr, {v,vr});//ROTL(S,Sl,Srl,Srr,v,vr);//S = union(Sl, Srl, Srr, {v,vr});
{
  node tmp, tmp2;
  int h;
  h = height(rl) + 1;
...
relation ROTR(bag a, bag b, bag c, bag d, int e, int f).
node rotate_right(node ll, node lr, node r, int vl, int v)
  ...
  ensures res::avl<2 + llm + lrm + rm, 2 + lrn, S> & ROTR(S,Sll,Slr,Sr,v,vl);//S = union(Sll,Slr,Sr,{vl,v});
{
	node tmp;
	int h;
	h = height(lr) + 1;
...
relation ROTDL(bag a, bag b, bag c, bag d, bag e, int m, int n, int p).
node rotate_double_left(node a, node b, node c, node d, int v, int vrl, int vr)
  ...
  ensures res::avl<3 + am + bm + cm + dm, an + 2, S> & ROTDL(S,Sl,Srll,Srlr,Srr,v,vrl,vr);//S = union(Sl,Srll,Srlr,Srr,{v,vrl,vr});
{
	node tmp1, tmp2;
	int h;
...
relation ROTDR(bag a, bag b, bag c, bag d, bag e, int m, int n, int p).
node rotate_double_right(node a, node b, node c, node d, int vl, int vlr, int v)
  ...
  ensures res::avl<3 + am + bm + cm + dm, 2 + an, S> & ROTDR(S,Sll,Slrl,Slrr,Sr,v,vl,vlr);//S = union(Sll,Slrl,Slrr,Sr,{vl,vlr,v});
{
	node tmp1, tmp2;
	int h;
