%{
/* $Id: parser.y,v 1.2 2009-02-17 08:55:21 chinwn Exp $
 * -------------------------------------------------------
...
%token CONNECTCMD UMODES
%token AWAYNICKNAME AWAYREASON
...
keywords:	NICKNAME '=' STRING {
	...
	|	ALTNICKNAME '=' STRING {
		...
		|	AWAYNICKNAME '=' STRING {
                	if( strlen( $3 ) > NICKSIZE ) report( "WARNING: overlength awaynickname given\n" );
                	ASSIGN_PARAM( cfg.awaynickname, $3 );
		}        		
        |	AWAYREASON '=' STRING		{ ASSIGN_PARAM( cfg.awayreason, $3 ); }
	...
	|	CONNECTCMD '=' STRING			{ ASSIGN_PARAM( cfg.connectcmd, $3 ); }
	|	UMODES '=' STRING			{ ASSIGN_PARAM( cfg.umodes, $3 ); }
	;
