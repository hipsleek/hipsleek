  ...
  and unify k1 k2 tl =
    match k1,k2 with
      ...
      | Named n1, Named n2 when (String.compare n2 "memLoc" = 0) || n2=""  ->   (* k2 is primitive memory predicate *)
        (tl, Some (Named n1))
      | NUM, Named n1 -> (tl, Some (Named n1))
      (* | Named n1, NUM -> (tl, Some (Named n1)) *)
      (* | Int, Named n1 -> (tl, Some (Named n1)) *)
      | Named n1, Int -> (tl, Some (Named n1))
      | t1, t2  -> (
...
and gather_type_info_heap_x prog (h0 : IF.h_formula) tlist =
  ...
  | IF.Phase { IF.h_formula_phase_rd = h1;
    ...
    n_tl
  | IF.HeapNode2 h2 ->
    let () = y_tinfo_hp (add_str "tlist(HeapNode2)" string_of_tlist) tlist in
  ...
  | IF.HeapNode { IF.h_formula_heap_node = (v, p); (* ident, primed *)
    ...
    let () = y_tinfo_hp (add_str "tlist(HeapNode)" string_of_tlist) tlist in
    let ft = cperm_typ () in
    let gather_type_info_ho_args hoa tlist =
      let () = y_tinfo_hp (add_str "tlist(ho_args)" string_of_tlist) tlist in
      let rs = List.fold_left (fun tl a ->
          x_add gather_type_info_formula prog a.IF.rflow_base tl false) tlist hoa in
      let () = y_tinfo_hp (add_str "tlist(ho_args-end)" string_of_tlist) tlist in
      rs
    in
    let gather_type_info_ann c tlist = 
      (
      let () = y_tinfo_hp (add_str "tlist(info_anns)" string_of_tlist) tlist in
      match c with
      | IP.NoAnn -> tlist
      | IP.ConstAnn _ -> tlist
    ...
    let rec gather_type_info_param_ann lst tl = (
      let () = y_tinfo_hp (add_str "tlist(info_param_anns)" string_of_tlist) tl in
      match lst with
      | [] -> tl
      | (Some h)::t ->
    ...
    let () = y_tinfo_hp (add_str "tlist(0)" string_of_tlist) tlist in
    let n_tl = x_add gather_type_info_perm perm tlist in
    let () = y_tinfo_hp (add_str "n_tl(1)" string_of_tlist) n_tl in
    let n_tl = x_add gather_type_info_ann ann n_tl in
    let () = y_tinfo_hp (add_str "n_tl(2)" string_of_tlist) n_tl in
    let n_tl = (* if (!Globals.allow_field_ann) then *) x_add gather_type_info_param_ann ann_param n_tl (* else n_tl *) in
    let () = y_tinfo_hp (add_str "n_tl(3)" string_of_tlist) n_tl in
    let n_tl = x_add gather_type_info_ho_args hoa n_tl in
    let () = y_tinfo_hp (add_str "n_tl(4)" string_of_tlist) n_tl in
    ...
    if (v_name = Parser.generic_pointer_type_name) then
      ...
      *)
      ...
      if type_found then
        ...
        let () = y_tinfo_hp (add_str "n_tl(after exp)" string_of_tlist) n_tl in
        n_tl
      else n_tl
