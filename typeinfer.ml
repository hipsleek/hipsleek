...
let string_of_var_type_list (tlist:spec_var_type_list) = 	
	begin 
  	let rec aux t_l = 		
  		match t_l with
  		| [] -> ""
...
let string_of_var_type_list_type (tl,t)= 
	(string_of_var_type_list tl)^", "^(string_of_typ t)

let string_of_var_type_list_type_option (tl,t) = 
	(string_of_var_type_list tl)^", "^(pr_option string_of_typ t)
...
let gen_primitives (prog : I.prog_decl) : (I.proc_decl list) * (I.rel_decl list) = (* AN HOA : modify return types *)
  ...
  in
    ...
    in
		(*print out the primitive relations parsed -- Problem : no relation parsed! *)
		(* let _ = print_endline "Primitive relations : " in *)
		(* let _ = List.map (fun x -> print_endline x.I.rel_name) prog.I.prog_rel_decls in *)
		(*THIS IS NOT THE PLACE THE PRIMITIVES IN prelude.ss IS ADDED! *)

	 	(*modify to return the list of primitive relations *)
...
and convert_struc2_x prog (f0:IF.struc_formula):IF.struc_formula = 
	match f0 with
...
let order_views (view_decls0 : I.view_decl list) : I.view_decl list =
  ...
  let rec gen_name_pairs_struc vname (f:IF.struc_formula): (ident * ident) list = 
		match f with
  ...
  let rec reorder_views (view_decls : I.view_decl list) (ordered_names : ident list) =
...
let rec seq_elim (e:C.exp):C.exp = 
	match e with
  ...
  | C.Cond b -> 
			C.Cond {b with C.exp_cond_then_arm  = (seq_elim b.C.exp_cond_then_arm);
  ...
  | C.Seq b -> 
    	  ...
    	  C.Try (
    		{	
    			C.exp_try_type = b.C.exp_seq_type;
				...
				C.Seq 
				{b with C.exp_seq_exp1 = seq_elim b.C.exp_seq_exp1 ;
  ...
  let need_break_continue_x lb ne non_generated_label :bool = 
      ...
      I.fold_exp ne (fun c-> 
			 ...
			 I.fold_exp ne  (fun c-> 
				...
				| I.While {
						I.exp_while_jump_label=b; 
						I.exp_while_pos = pos}-> 
							(match b with
							| I.NoJumpLabel -> None 
							| I.JumpLabel l -> 
									if (String.compare l lb) ==0 then 
										Gen.report_error pos("label"^l^" is duplicated")
									else None)			 
				| I.Break {I.exp_break_jump_label=b}
...
and while_return e ret_type = I.map_exp e (fun c-> 
		match c with 
		| I.While b -> 
				let needs_ret = needs_ret b.I.exp_while_body in 
				if needs_ret then
					let new_body = I.map_exp b.I.exp_while_body (fun c -> 
						match c with 
						| I.Return b-> 
...
and substitute_seq (fct: C.proc_decl): C.proc_decl = 
	match fct.C.proc_body with
	| None -> fct
	| Some e-> {fct with C.proc_body = Some (seq_elim e)}  

and trans_logical_vars lvars =
        ...
        if not (Gen.is_empty dups) then
          ...
          let _ = prog.I.prog_hp_ids <- List.map (fun rd -> (HpT,rd.I.hp_name)) prog.I.prog_hp_decls in
  	      ...
  	      let cprog = {
	          ...
	          C.prog_rel_decls = crels; 
            C.prog_hp_decls = chps;
	          C.prog_axiom_decls = caxms;
          ...
          let cprog3 = 
						if (!Globals.enable_case_inference || (not !Globals.dis_ps) (* or !Globals.allow_pred_spec *)) 
          	then pred_prune_inference cprog2 else cprog2 
					in
  ...
  let sat_warnings_op () =
    ...
    let n_pred_list = 
        ...
        if ((List.length unsat_list)> 0) then warn c.Cast.view_name unsat_list else ();            
  	      ...
  	      | _ -> [f] in
        let ncf = 
					match c.Cast.view_formula with 
  	      | CF.EList b-> CF.EList (List.concat (List.map (fun (l,c)-> List.map (fun c-> (l,c)) (test c)) b))
  	      | _ -> 
						(match test c.Cast.view_formula with 
	          | x::[] -> x
	          | _ as l-> CF.EList (List.map (fun c-> (empty_spec_label_def,c)) l)) 
				in
...
and trans_data (prog : I.prog_decl) (ddef : I.data_decl) : C.data_decl =
  ...
  (** 
      translate field with inline consideration.
  **)
  ...
  (* let compute_view_x_formula_x_op ()= *)
    (if (n > 0 (* && not(vdef.C.view_is_prim) *)) then
      (		
    	...
    	if !Globals.print_x_inv && (n = 0) then
...
and add_param_ann_constraints_to_pure (h_f: CF.h_formula) (p_f: MCP.mix_formula option): MCP.mix_formula = 
  let mix_f = 
    ...
    | CF.DataNode h -> 
  			let data_ann = h.CF.h_formula_data_imm in
...
and trans_view_x (prog : I.prog_decl) (vdef : I.view_decl) : C.view_decl =  
  ...
  let (n_tl,mem_form) = (match vdef.I.view_mem with
...
and trans_hp (prog : I.prog_decl) (hpdef : I.hp_decl) : C.hp_decl =
  ...
  let n_tl = List.map (fun (var_type, var_name) -> (var_name,{ sv_info_kind = (trans_type prog var_type pos);id = fresh_int () })) hpdef.I.hp_typed_vars in
  (* Need to collect the type information before translating the formula *)
...
and find_materialized_prop params (f0 : CF.formula) : C.mater_property list = 
  ...
  let rec merge_mater_lists l1 l2 = 
		match l1,l2 with 
    | [], _ -> to_partial l2
    | _ , [] -> to_partial l1 
    | x::t1,y::t2 -> 
...
and all_paths_return (e0 : I.exp) : bool =
  ...
  | I.Raise _ -> true
  | I.Try e -> 
			(all_paths_return e.I.exp_try_block) || (List.fold_left (fun a b-> a && (all_paths_return b)) true e.I.exp_catch_clauses)
...
and check_return (proc : I.proc_decl) : bool =
  ...
  | None -> true
  | Some e ->
      if(not (I.are_same_type I.void_type proc.I.proc_return)) &&
  ...
  let trans_proc_x_op () =
    ...
    if not (Gen.is_empty dup_names) then
      (let p = List.hd dup_names in
        ...
        (** compute the important variables **)
...
(**	collect important variables in the specification
...
and collect_important_vars_in_spec (spec : CF.struc_formula) : (CP.spec_var list) =
  (**Internal function to collect important variables in the an ext_formula **)	
  let rec helper f = 
		match f with
...
and trans_one_coercion_x (prog : I.prog_decl) (coer : I.coercion_decl) :
    ...
    | _ -> 
        ...
        if ( (List.length hs) > 0) then
          ...
          | CF.StarMinus {
        	...
        	| _ -> let _ = print_string "[astsimp] Warning: lhs head node of a coercion is neither a view node nor a data node\n" in 
                    ""
        else
  ...
  if lhs_name = "" then
    ...
    (  
      ...
      let c_coer ={ 
				C.coercion_type = coer.I.coercion_type;
        ...
        C.coercion_case = (Cast.case_of_coercion c_lhs c_rhs)} 
			in
      let change_univ c = 
				(match c.C.coercion_univ_vars with
  ...
  let rec helper ie =
    ...
    | I.Unfold { I.exp_unfold_var = (v, p); I.exp_unfold_pos = pos } ->
      ((C.Unfold {
        ...
        let (n_tl,assert_cf_o) =
          (match assert_f_o with
          | Some f -> 
							let (n_tl,cf) = trans_I2C_struc_formula 6 prog false free_vars (fst f) n_tl false true (*check_pre*) (*(Cpure.Void) [])*) in  
							(n_tl,Some cf)
          | None -> (n_tl,None)) in
        let (n_tl,assume_cf_o) =
        ...
        let rec produce_member_exps base fseqs = 
					match base with
          | I.Member{	
	            ...
	            List.map (fun x -> 	I.Member 
							{	I.exp_member_base = base_e;
                I.exp_member_fields = List.append fs [x];
                I.exp_member_path_id = pid;
                I.exp_member_pos = pos
							}) fseqs
          | I.Var _ -> List.map (fun x -> I.Member 
  					{ I.exp_member_base = base;
        ...
        let expand_field_list = List.map I.get_field_name expand_field_list in
	        if (expand_field_list != []) then (* inline type --> expand into a sequence *)
	          ...
	          helper expanded_exp
	        else (* end of additional pre-processing, continue as usual *)
  			  	(match aop with
  			    | I.OpAssign ->
			      	(match lhs with
                     	...
                     	let _ = proc.I.proc_important_vars <- proc.I.proc_important_vars @ [vsv] in*)
                      	(assign_e, C.void_type)
                       	...
                       	(let seq_e = C.Seq{
				                    ...
				                    (* MARKED : THE CASE LHS IS AN ARRAY ACCESS IS SIMILAR TO VARIABLE & BINARY - WE NEED TO CONVERT THIS INTO A FUNCTION *)
				                    (* (Iast) a[i] = v    ===>     (Iast) a = update___(a,i,v);   ===>   (Cast) Scall {a = update___(a,i,v)} *)
	            | I.ArrayAt { 
									I.exp_arrayat_array_base = a; 
									I.exp_arrayat_index = index; 
                  ...
                  (* fix this case with inline field access *)
                  ...
                  if ( != ) (List.length vs) (List.length ddef.I.data_fields) then
                    ...
                    (let _ = List.map2 
                      (fun vi ti -> 
                            ...
                            E.var_type = ti;})) 
                      	vs
	                  (* Convert hard code of data fields typ extraction into *)
                      	(List.map I.get_field_typ ddef.I.data_fields) 
										in
      ...
      (* let _ = print_string "trans_exp :: case CallNRecv\n" in*)
        ...
        if this_recv then 
              ...
              let method_name = 
								match method_name_exp with
                | I.Var { I.exp_var_name = v; I.exp_var_pos = pos } -> v
                | _ -> Err.report_error { Err.error_loc = pos; Err.error_text = ("fork:" ^ "expecting the first argument as a var: a method name"); }
              in
                ...
                if ( != ) (List.length method_args) (List.length pdef.I.proc_args) then
                  ...
                  if List.exists2 (fun t1 t2 -> not (sub_type t1 t2)) cts parg_types then
                    Err.report_error { Err.error_loc = pos; Err.error_text = "argument types do not match"; }
										...
										if Inliner.is_inlined mn then 
											(let inlined_exp = Inliner.inline prog pdef ie in helper inlined_exp)
                  	else 
              ...
              (if (List.length args <1) then
                ...
                if ( != ) (List.length args) (List.length pdef.I.proc_args) then
                  Err.report_error { Err.error_loc = pos; Err.error_text = "number of arguments does not match"; }
									...
									if (mn=Globals.join_name) &&  ((List.length args) != 1) then
        ...
        if not (exlist # sub_type_obj cvt c_flow) then 
					Err.report_error { 
  					Err.error_loc = pos; 
            Err.error_text = "can not catch a not raisable object" }
	          ...
	          match cv with
		        | Some x ->
	              if (String.compare cvt c_flow)=0 then  
									begin
		            		E.push_scope();
                    ...
                    (*TO CHECK: diffrent between then and else ??? *)
    		            ...
    		            C.exp_catch_pos = pos;},ct2) 
									...
									begin
		            		let cvt_rev = 
											match alt_cvt with | None -> Named cvt | Some t -> t in
		            		E.push_scope();
		            		let alpha = E.alpha_name x in
		        ...
		        | None ->  
	              ...
	              E.pop_scope();
	              (C.Catch{	
									C.exp_catch_flow_type = exlist # get_hash cvt;
  	              C.exp_catch_flow_var = cfv;
    ...
    | I.Java { I.exp_java_code = jcode; I.exp_java_pos = pos } ->
        ...
        (* compact the field access sequence *)
        ...
        if not (rem = "") then
          ...
          r
		(**	Translate the new int[x] into core language.
    ...
    | I.ArrayAlloc {
        ...
        let field_types = 
          if !Globals.allow_locklevel  && c=lock_name then [level_data_typ] 
					else
        ...
        (*=========processing waitlevel===============*)
          if (!Globals.allow_locklevel && c=lock_name && nargs>1) then
            Err.report_error{ Err.error_loc = pos; Err.error_text = "number of arguments does not match: " ^ lock_name ^ " should have at most 1 arguments";}
						...
						if (!Globals.allow_locklevel && c=lock_name && nargs=0) then
              ...
              if (!= ) nargs (List.length field_types) 
                ...
                let parg_types = List.map (fun ft -> trans_type prog ft pos) field_types in
                  	...
                  	Err.report_error { Err.error_loc = pos; Err.error_text = "argument types do not match";}
										...
										(let positions = Gen.repeat pos nargs in
                		let (local_vars, init_seq, arg_vars) = trans_args (Gen.combine3 cargs cts positions) in
                		let new_e = 
											C.New {
                        C.exp_new_class_name = c;
    ...
    | I.Return {
        ...
        I.exp_return_pos = pos
          ...
          let cret_type = trans_type prog proc.I.proc_return proc.I.proc_loc in
          let _=
						if(!Globals.proof_logging_txt) then (*proof logging*)   
            Globals.return_exp_pid := !Globals.return_exp_pid @ [pi] 
					in
          	match oe with
            | None -> 
                if CP.are_same_types cret_type C.void_type then
                  (C.Sharp ({ 
                		...
                		C.exp_sharp_flow_type = 
											C.Sharp_ct {CF.formula_flow_interval = !ret_flow_int ; CF.formula_flow_link = None};
    ...
    | Iast.Continue _ -> failwith (Iprinter.string_of_exp ie)
              ...
              if ff then 
                ...
                C.exp_sharp_flow_type = (
                  ...
                  | I.Const_flow c -> 
											(C.Sharp_ct 
                      {
												CF.formula_flow_interval = (exlist # get_hash c); 
                  ...
                  let shar = 
                      ...
                      C.exp_sharp_flow_type = 
                            ...
                            | _ -> Error.report_error {Error.error_loc = pos; Error.error_text = ("expecting constant flow")}
        	                  ...
        	                  | _ -> Error.report_error {
        														Error.error_loc = pos; 
																		Error.error_text = ("malfunction, primitive thrown type ")} );
                      C.exp_sharp_unpack = false;
                      C.exp_sharp_val = Cast.Sharp_var (ct,fn);
                      C.exp_sharp_path_id = pi;
                      C.exp_sharp_pos = pos }) 
        ...
        if ((List.length fl_list)>0) then
          ...
          | _ -> 
                            ...
                            C.Try({
                              ...
                              C.exp_catch_clause = 
                                  ...
                                  C.exp_catch_body = 
                                    	...
                                    	C.exp_sharp_flow_type = 
  																			C.Sharp_ct {CF.formula_flow_interval = !spec_flow_int; CF.formula_flow_link = Some fl_var};
                                      C.exp_sharp_val = Cast.Sharp_no_val;
                                      C.exp_sharp_unpack = false;
                                      C.exp_sharp_pos = pos;
		          ...
		          let r = 
								C.Try({
									C.exp_try_type = (match r1 with | C.Try t -> t.C.exp_try_type | _ -> Err.report_error { Err.error_loc = pos; Err.error_text = "translation failed, compacting case's failed" });
		          		C.exp_try_body = r1;
    		          C.exp_try_path_id = pid;
    		          C.exp_catch_clause = 
										C.Catch{
...
(**
   * compact field access by combining inline fields. For example, given
...
and compute_ann_list all_fields (diff_fields : ident list) (default_ann : CF.ann) : CF.ann list =
  let pr1 ls = 
    let helper i = 
      ...
      | ((_,h), _, _) -> h
    in
    List.fold_left (fun res id -> res ^ ", " ^ (helper id)) "" ls 
	in
...
and insert_dummy_vars (ce : C.exp) (pos : loc) : C.exp =
  ...
  | _ ->
    ...
    | None -> ce
    | Some t -> 
			if CP.are_same_types t C.void_type then ce
      else
...
and case_coverage_x (instant:Cpure.spec_var list)(f:CF.struc_formula): bool =
  ...
  let rec struc_case_coverage (instant:Cpure.spec_var list) ctx (f1:CF.struc_formula):bool = match f1 with
    ...
    | CF.ECase b -> 
        ...
        let _ = 
					if not(Gen.BList.subset_eq (=) (Cpure.fv all) instant) then 
          	let _ = print_string (
        ...
        let _ = 
          let coverage_error = 
      			...
      			(*not (Tpdispatcher.simpl_imply_raw ctx all)*) 
					in
          if coverage_error then 
            ...
            Err.error_text = "the guards don't cover the whole domain for : "^s^"\n";} 	
				in        
        let rec p_check (p:Cpure.formula list):bool = 
					match p with
          | [] -> false 
          | p1i::p2 -> 
        		...
        		let _ = List.map (fun (c1,c2)->struc_case_coverage instant (CP.mkAnd c1 ctx no_pos) c2) b.CF.formula_case_branches in true
                      ...
                      | CF.EList b -> List.for_all (fun c-> struc_case_coverage instant ctx (snd c)) b 
						in
						struc_case_coverage instant (CP. mkTrue no_pos) f
						
and trans_var p (tlist:spec_var_type_list) pos =
  let pr = pr_var_prime in
  Debug.no_2 "trans_var" pr string_of_var_type_list Cprinter.string_of_spec_var  
		(fun _ _ -> trans_var_x p tlist pos)  p tlist
		
(* TODO WN : need to test how type checking handle # vars *)
and trans_var_x (ve, pe) (tlist:spec_var_type_list) pos =
  (* [23/08/2011] Variables with "#" should not be considered.*)
  (* if (ve.[0] = '#') then  *)
  ...
  if (is_dont_care_var ve) then 
    ...
    try
      ...
      | UNK ->
        ...
        {
          Err.error_loc = pos;
          Err.error_text = "couldn't infer type for " ^ ve^(match pe with |Unprimed->""|Primed -> "'")^" in "^(string_of_var_type_list tlist)^"\n";
        }
    ...
    with Not_found ->   
      ...
      {
        Err.error_loc = pos;
        Err.error_text = "type list does not contain an entry for " ^ ve^(match pe with |Unprimed->""|Primed -> "'")^" in "^(string_of_var_type_list tlist)^"\n, could it be an unused var?\n";
      }			

and trans_var_safe (ve, pe) et tlist pos =
  (* Variables with "#" should not be considered.*)
...
and add_pre_x (prog :C.prog_decl) (f:CF.struc_formula):CF.struc_formula = 
  let rec helper (pf:Cpure.formula) (f:CF.struc_formula): CF.struc_formula = 
		match f with
...
and trans_I2C_struc_formula i (prog : I.prog_decl) (quantify : bool) (fvars : ident list) (f0 : IF.struc_formula) 
    (tlist:spec_var_type_list) (check_self_sp:bool) (*disallow self in sp*) (check_pre:bool) : (spec_var_type_list*CF.struc_formula) = 
  		let prb = string_of_bool in
			let prrs = fun (tl,cf) -> (string_of_var_type_list tl)^": "^(Cprinter.string_of_struc_formula cf) in			
  		(* Debug.no_5_loop    *)
  		Debug.no_eff_5_num  i
    		"trans_I2C_struc_formula" [true] string_of_var_type_list prb prb Cprinter.str_ident_list 
    		(add_str "Input Struc:" Iprinter.string_of_struc_formula) 
    		(add_str "Output Struc:" prrs)
    		(fun _ _ _ _ _ -> trans_I2C_struc_formula_x prog quantify fvars f0 tlist check_self_sp (check_pre:bool)) 
    		tlist (* type list *) quantify (* quantified flag *) check_self_sp
    		fvars (* free vars *) f0 (*struc formula *)

and trans_I2C_struc_formula_x (prog : I.prog_decl) (quantify : bool) (fvars : ident list)
    (f0 : IF.struc_formula) tlist (check_self_sp:bool) (check_pre:bool): (spec_var_type_list*CF.struc_formula) = 
	let rec trans_struc_formula (fvars : ident list) tl (f0 : IF.struc_formula) :(spec_var_type_list*CF.struc_formula) = 
		match f0 with
  	| IF.EAssume (b,y,t)->	(*add res, self*)
				let (n_tl,cf) = trans_formula prog true (self::res_name::eres_name::fvars) false b tl true in
        (n_tl,CF.EAssume ([], cf, y, t))
		| IF.ECase b-> 
			(*Need to consider*)
				(let rec aux tlist clist = 
						...
						| (c1,c2)::tl -> 								
								...
								(n_tlist,(f1,f2)::n_cl))
				in	
				let (n_tl,n_cl) = aux tl b.IF.formula_case_branches
				in 
        let cf = 
					CF.ECase {
		...
		| IF.EBase b-> 		
			let (n_tl,nc) = 
				...
				| None -> (tl,None)
				| Some x -> 
						let (n_tl,cf) = trans_struc_formula (fvars @ (fst (List.split(IF.heap_fv b.IF.formula_struc_base)))) tl x in 
						(n_tl,Some cf) 
			in									
      ...
      (* TODO : check iv - fvars = {} *)
		...
		| IF.EList b ->
			...
			(n_tl,CF.mkEList n_cl)
			 
  	| IF.EOr b -> 
				let (n_tl,cf1) = trans_struc_formula fvars tl b.IF.formula_struc_or_f1 in
				let (n_tl,cf2) = trans_struc_formula fvars n_tl b.IF.formula_struc_or_f2 in
				let cf = 
  				CF.EOr {
  	      CF.formula_struc_or_f1 = cf1;
  	      CF.formula_struc_or_f2 = cf2;
  	      CF.formula_struc_or_pos = b.IF.formula_struc_or_pos;}
				in (n_tl,cf)			
	in 
  ...
  let n_tl = type_store_clean_up r n_tl in
...
and trans_formula (prog : I.prog_decl) (quantify : bool) (fvars : ident list) sep_collect
  ...
  let prb = string_of_bool in
	let prrs = fun (tl,rs) -> (string_of_var_type_list tl)^": "^(Cprinter.string_of_formula rs) in
  Debug.no_eff_5 "trans_formula" [true] string_of_var_type_list
    (add_str "quantify" prb) 
    (add_str "cleanres" prb) Cprinter.str_ident_list Iprinter.string_of_formula prrs
...
and trans_formula_x (prog : I.prog_decl) (quantify : bool) (fvars : ident list) sep_collect (f0 : IF.formula) (tlist:spec_var_type_list) (clean_res:bool) : (spec_var_type_list*CF.formula) =
 	    ...
 	    (*let _ = print_string("Formula: "^(Iprinter.string_of_formula f0) ^"\n") in*)
          ...
          let n_tl = 
						if sep_collect then
  						...
  						let n_tl = gather_type_info_heap prog h n_tl in
  					 	n_tl
						else n_tl in 		    
					(*let _ = List.map helper_one_formula a tl in*)          
          let (n_tl,ch) = linearize_formula prog f0 n_tl in					
          (*let ch1 = linearize_formula prog false [] f0 tlist in*)
          let n_tlist = 
            if sep_collect then               
								if quantify then
					...
					in 
					let n_tl = res_replace n_tlist rl clean_res fl in
					(n_tl,ch)        
				)  
			| IF.Exists	{
          ...
          let n_tl = 
						if sep_collect then 
							(let n_tl = gather_type_info_pure prog p n_tl in
							let n_tl =  gather_type_info_heap prog h n_tl
							in n_tl) 
						else n_tl 
					in 
          ...
          let qsvars = List.map (fun qv -> 											
						trans_var qv n_tl pos) qvars in					
...
and linearize_formula (prog : I.prog_decl)  (f0 : IF.formula)(tlist : spec_var_type_list):(spec_var_type_list*CF.formula) =
  let pr1 prog = "prog" in
	let prrs = fun (tl,rs) -> (string_of_var_type_list tl)^": "^(Cprinter.string_of_formula rs) in
  Debug.no_3 "linearize_formula" pr1 Iprinter.string_of_formula string_of_var_type_list prrs
	linearize_formula_x prog f0 tlist

and linearize_formula_x (prog : I.prog_decl)  (f0 : IF.formula)(tlist : spec_var_type_list):(spec_var_type_list*CF.formula) =
  let rec match_exp (hargs : (IP.exp * Label_only.spec_label) list) pos tl : CP.spec_var list=
    (match hargs with
    | (e, _) :: rest ->
      let e_hvars = 
				match e with
        | IP.Var ((ve, pe), pos_e) -> 
						let cp = trans_var_safe (ve, pe) UNK tl pos_e in
						cp
        | _ -> report_error (IF.pos_of_formula f0)("malfunction with float out exp: "^(Iprinter.string_of_formula f0))
			in
    	let rest_hvars = match_exp rest pos tl in
	...
	let rec linearize_heap (f : IF.h_formula) pos (tl:spec_var_type_list): ( CF.h_formula * CF.t_formula) = 
        ...
        | IF.HeapNode{
          ...
          if (c = Parser.generic_pointer_type_name || String.contains v '.') then
  	        ...
  	        let rootptr_type_name = 
							match rootptr_type with 
							| Named c -> c 
							| _ -> failwith ("[linearize_heap] " ^ rootptr ^ " must be a pointer.") in
  	        let rootptr, p = 
  						let rl = String.length rootptr in
    	        	if rootptr.[rl-1] = '\'' 
    						then (String.sub rootptr 0 (rl - 1), Primed)
    	        	else (rootptr, Unprimed) 
  					in 
  	        ...
  	        let hvars = match_exp (List.combine exps labels) pos tl in
          ...
          (* [Internal] Extends hvars with holes and collect the list of holes! *)
  	        let rec extend_and_collect_holes vs offset num_ptrs =
            	...
            	let holes = 
  							List.fold_left (fun l i -> let d = i - offset in
  	          										if (d < 0 || d >= numargs) then List.append l [i] else l) [] temp	
  						in
            	let newvs = 
  							List.map (fun i -> if (List.mem i holes) then 
  	            						CP.SpecVar (UNK,"#",Unprimed) 
  	          							else List.nth vs (i - offset)) temp 
  						in (newvs,holes) 
  					in
            ...
            let permvar = (match perm with
              ...
              | Some f -> 
                    ...
                    let permvars = match_exp (List.combine perms permlabels) pos tl in
...
and check_dfrac_wf e1 e2 pos = 
	if (CP.has_e_tscons e1)||(CP.has_e_tscons e2) then
    match e1,e2 with 
    | CP.Var _ , CP.Var _ 
...
and trans_pure_b_formula (b0 : IP.b_formula) (tlist:spec_var_type_list) : CP.b_formula =
  Debug.no_2 "trans_pure_b_formula" (Iprinter.string_of_b_formula) string_of_var_type_list (Cprinter.string_of_b_formula) 
		(fun b _-> trans_pure_b_formula_x b tlist) b0 tlist			
...
and trans_pure_b_formula_x (b0 : IP.b_formula) (tlist:spec_var_type_list) : CP.b_formula =
  ...
  let npf =  
    ...
    | IP.VarPerm (ct,ls,pos) ->
        ...
        (* let ls1 = List.map (fun (v,p) -> trans_var (v,p) tlist pos) ls in *)
        (* let ls1 = List.map (fun v -> trans_var (v,Unprimed) tlist pos) ls in *)
...
and trans_pure_exp (e0 : IP.exp) (tlist:spec_var_type_list) : CP.exp =
  Debug.no_2 "trans_pure_exp" 
      (Iprinter.string_of_formula_exp)
			string_of_var_type_list
      (Cprinter.string_of_formula_exp) 
      (fun e _ -> trans_pure_exp_x e tlist) e0 tlist
...
and trans_pure_exp_list (elist : IP.exp list) tlist : CP.exp list =
  match elist with
    | [] -> []
...
and must_unify (k1 : typ) (k2 : typ) tlist pos : (spec_var_type_list*typ )   =
 	let pr = string_of_typ in
  Debug.no_2 "must_unify" pr pr string_of_var_type_list_type (fun _ _ -> must_unify_x k1 k2 tlist pos) k1 k2
...
and must_unify_expect (k1 : typ) (k2 : typ) tlist pos : (spec_var_type_list*typ)  =
  let pr = string_of_typ in
  Debug.no_2 "must_unify_expect" pr pr string_of_var_type_list_type (fun _ _ -> must_unify_expect_x k1 k2 tlist pos) k1 k2						
...
and unify_type (k1 : spec_var_kind) (k2 : spec_var_kind) tlist :(spec_var_type_list*(typ option) ) =
  let pr = string_of_typ in
  let pr2 = string_of_var_type_list_type_option in
...
and unify_expect_modify (modify_flag:bool) (k1 : spec_var_kind) (k2 : spec_var_kind) tlist : (spec_var_type_list*(typ option)) =
  let pr = string_of_typ in
  Debug.no_2 "unify_expect_modify" pr pr string_of_var_type_list_type_option (fun _ _ -> unify_expect_modify_x modify_flag k1 k2 tlist) k1 k2								
...
and repl_tvar_in unify flag tlist i k =
  Debug.no_eff_4 "repl_tvar_in" [false;false;false;true]
    string_of_bool string_of_int string_of_typ string_of_var_type_list string_of_var_type_list_type_option
    (fun _ _ _ _ -> repl_tvar_in_x unify flag tlist i k) flag i k tlist 		
		
(* TODO : should TVar j be updated to point to i *)
and repl_tvar_in_x unify flag (tlist:spec_var_type_list) i k =
  let test x = 
		let i2 = x.id in 
  	match k with 
    | TVar j -> i2=i || i2=j 
...
and get_var_kind (var : ident) (tlist : spec_var_type_list) =
  try let r = snd(List.find (fun (v,en) -> v=var) tlist) in r.sv_info_kind 
	with | Not_found -> UNK
...
and gather_type_info_var (var : ident) tlist (ex_t : typ) pos : (spec_var_type_list*typ) =
  let pr = string_of_typ in
  Debug.no_eff_3 "gather_type_info_var" [false;true] (fun x -> ("ident: "^x)) string_of_var_type_list pr string_of_var_type_list_type 
...
and gather_type_info_var_x (var : ident) tlist (ex_t : spec_var_kind) pos : (spec_var_type_list*spec_var_kind) =
  begin
    ...
    then (tlist,UNK) (* for vars such as _ and # *)
    else
      try
        let (ident, k) = List.find (fun (a,b) -> a = var )tlist in
        (* if ex_t==UNK then k.sv_info_kind *)
        (* else *)				
        let (n_tlist,tmp) = must_unify(* _expect *)  k.sv_info_kind ex_t tlist pos in 
...
and get_type tlist i = 
  ...
  (try 
    ...
    en.sv_info_kind 
  with _ -> report_error no_pos ("UNEXPECTED : Type Var "^key^" cannot be found in type_var_list"))

and gather_type_info_exp a0 tlist et =	
  Debug.no_eff_3 "gather_type_info_exp" [false;true] 
    Iprinter.string_of_formula_exp string_of_var_type_list string_of_typ
    string_of_var_type_list_type gather_type_info_exp_x a0 tlist et
...
and gather_type_info_exp_x a0 tlist et =
  ...
  | IP.AConst (_,pos) -> 
	...
	| IP.Add (a1, a2, pos) -> 
      ...
      let (new_et, n_tl) = fresh_tvar tlist in			
			...
			let (tmp1,tmp2)=nt in			
      let (n_tl1,t1) = gather_type_info_exp a1 n_tl new_et in (* tvar, Int, Float *)
  ...
  | IP.Mult (a1, a2, pos) |	IP.Div (a1, a2, pos) ->
	...
	| IP.BagDiff (a1,a2,pos) ->
      ...
      let (n_tlist,t1) = gather_type_info_exp_x a1 tlist new_et in 
	...
	| IP.BagIntersect (es,pos) | IP.BagUnion (es,pos) -> (*Need to consider*)
      let (el_t,n_tl) = fresh_tvar tlist in			
      let new_et = must_unify_expect_test (BagT el_t) et n_tl pos in 
    	(*let ts = List.map (fun e -> gather_type_info_exp_x e n_tl new_et) es in*)
			let rec aux es_list type_list =
				match es_list with
				| []->([],type_list)
				| hd::tl -> 
  					let (_tlist,_typ) = gather_type_info_exp_x hd type_list new_et in
	...
	| IP.Bag (es,pos) ->
      let (el_t,n_tl) = fresh_tvar tlist in
      let (n_tlist,t) = List.fold_left (fun (type_list,et) a -> gather_type_info_exp_x a type_list et) (n_tl,el_t) es in
      (n_tlist,BagT t)	
	| IP.Func (id, es, pos) -> (*Need to explain*)
      let t = I.int_type in
	...
	| IP.ArrayAt ((a,p),idx,pos) ->
      ...
      let (n_tl,lt) = gather_type_info_var a tlist new_et pos in
			let rec aux id_list type_list =
				...
				| [] -> type_list
				| hd::tl -> 
						let (n_tl,n_typ) = gather_type_info_exp_x hd type_list Int in
						aux tl n_tl
			in
			let n_tlist = aux idx n_tl in
      (*let _ = List.map (fun i -> gather_type_info_exp_x i n_tl Int) idx in*)
      (match lt with
      | Array (r,_) -> (n_tlist,r)
	...
	| IP.ListTail (a,pos)  | IP.ListReverse (a,pos) -> (* List t -> List t  *)
      ...
      let (n_tlist,lt) = gather_type_info_exp_x a n_tl new_et in
			(n_tlist,lt)
  | IP.ListAppend (es,pos) -> (* [List t] -> List t *)
      let (fv,n_tl) = fresh_tvar tlist in
      let lt = List fv in
      let (n_tl,new_et) = must_unify lt et n_tl pos in
      let (n_tlist,n_type) = List.fold_left (fun (type_list,et) l -> gather_type_info_exp_x l type_list et) (n_tl,new_et) es  in
      (n_tlist,n_type)
	| IP.ListHead (a, pos) ->  (* List t -> t*)
      let (fv,n_tl) = fresh_tvar tlist in
      let new_et = List fv in
      let (n_tl,lt) = gather_type_info_exp_x a n_tl new_et in
	...
	| IP.ListCons (e,es,pos) -> (* t -> List t -> List t *)
      let (fv,n_tl) = fresh_tvar tlist in
      let (n_tl,e1) = gather_type_info_exp_x e n_tl fv in
      let lt = List e1 in
      let (n_tl,new_et) = must_unify lt et n_tl pos in
      let (n_tlist,lt) = gather_type_info_exp_x es n_tl new_et in
      (n_tlist,lt)
	| IP.List (es,pos) -> (* a,..,a -> List a *)
      let (fv,n_tl) = fresh_tvar tlist in
      let (n_tl,r) = List.fold_left (fun (type_list,et) l -> gather_type_info_exp_x l type_list et) (n_tl,fv) es  in
      let lt = List r in
	...
	| IP.ListLength (a, pos) -> (* List t -> Int *)
      ...
      let (n_tlist,_) = gather_type_info_exp_x a n_tl new_et in
      (n_tlist,r)
...
and gather_type_info_pure prog (p0 : IP.formula) (tlist : spec_var_type_list) : spec_var_type_list =
  Debug.no_eff_2 "gather_type_info_pure" [false;true]  (Iprinter.string_of_pure_formula) string_of_var_type_list string_of_var_type_list
...
and gather_type_info_b_formula prog b0 tlist =
  Debug.no_eff_2 "gather_type_info_b_formula" [false;true] 
    Iprinter.string_of_b_formula string_of_var_type_list string_of_var_type_list
...
and gather_type_info_pointer (e0 : IP.exp) (k : spec_var_kind) (tlist:spec_var_type_list) : (spec_var_type_list*typ) =
  ...
  | IP.Var ((sv, sp), pos) -> 
			let (n_tl,n_typ) = gather_type_info_var sv tlist k pos in
			(n_tl,n_typ)
  | _ ->
      Err.report_error
      {
...
and gather_type_info_formula prog f0 tlist filter_res:spec_var_type_list = 
  Debug.no_eff_3 "gather_type_info_formula" [false;true]
    (Iprinter.string_of_formula) string_of_var_type_list 
    string_of_bool string_of_var_type_list
...
and gather_type_info_one_formula prog (f : IF.one_formula) tlist filter_res =
  let n_tl =
    ...
    | None -> tlist
    | Some (v,pr) -> 
				let (n_tl,_)= gather_type_info_var v tlist thread_typ f.IF.formula_pos in n_tl )
	in
  let n_tl = gather_type_info_heap prog f.IF.formula_heap n_tl in
...
and type_store_clean_up (f:CF.struc_formula) tlist = tlist (*if tlist to big,  -> get list of quantified vars, remove them from tlist*)

and gather_type_info_struc_f prog (f0:IF.struc_formula) tlist =
  Debug.no_eff_2 "gather_type_info_struc_f" [false;true]
      Iprinter.string_of_struc_formula string_of_var_type_list string_of_var_type_list
...
and gather_type_info_struc_f_x prog (f0:IF.struc_formula) tlist = 
  let rec inner_collector (f0:IF.struc_formula) tl =  
		match f0 with
    | IF.EAssume (b,_,_)-> let n_tl = gather_type_info_formula prog b tl true in n_tl					
		| IF.ECase b ->  List.fold_left (fun tl (c1,c2)->
	      let n_tl = gather_type_info_pure prog c1 tl in
	      inner_collector c2 n_tl) tl b.IF.formula_case_branches
		| IF.EBase b ->  
    ...
    | IF.EOr b -> 
				let n_tl = inner_collector b.IF.formula_struc_or_f1 tl in
				let n_tl = inner_collector b.IF.formula_struc_or_f2 n_tl in
				n_tl 
	in 
	begin
...
and try_unify_data_type_args prog c ddef v ies tlist pos =
  (*problem detected - have to expand the inline fields as well, fix in look_up_all_fields. *)
  let (n_tl,_) = gather_type_info_var v tlist ((Named c)) pos in
  let fields = I.look_up_all_fields prog ddef
  in 
  (try 
    let f tl arg ((ty,_),_,_) = 
...
and try_unify_view_type_args prog c vdef v ies tlist pos =
  ...
  let n_tl =  
		(if not (dname = "")
		then let (n_tl,_) = gather_type_info_var v tlist ( (Named dname)) pos in n_tl		
  	else tlist) 
  in
...
and gather_type_info_heap prog (h0 : IF.h_formula) tlist = (*rec*)
  Debug.no_eff_2 "gather_type_info_heap" [false;true]
    Iprinter.string_of_h_formula string_of_var_type_list (fun _ -> "()")
      ...
      (try
        ...
        if (List.length args) == (List.length hpdef.I.hp_typed_vars) then
          ...
          let (n_tl,_) = gather_type_info_var r tlist HpT pos in (*Need to consider about  pos*)
					...
					let n_tl = List.fold_left ( fun tl (arg,et) -> fst(gather_type_info_exp_x arg tl et)) n_tl args_expt in
					n_tl        	
        else
          Err.report_error{ Err.error_loc = pos; Err.error_text = ("number of arguments for heap relation "^r^" does not match"); }
      with
...
and case_normalize_renamed_formula_x prog (avail_vars:(ident*primed) list) posib_expl (f:IF.formula):
  ...
  let rec flatten f = 
    ...
    | _ -> [f]
  in
  let rec imm_heap (f : IF.h_formula): IF.h_formula = 
		match f with
    | IF.Star
      {	  
				IF.h_formula_star_h1 = f1;
  ...
  let rec linearize_heap (used_names:((ident*primed) list)) (f : IF.h_formula): ((ident*primed) list) * ((ident*primed) list) * IF.h_formula * Ipure.formula =
    ...
    | IF.HeapNode b ->
        ...
        let isInv, labels = 
            ...
            let flag = 
  						match vdef.I.view_inv_lock with
              | None -> false
              | Some _ -> true
            in
        ...
        in
        let _ = 
					if (List.length b.IF.h_formula_heap_arguments) != (List.length labels) then
  ...
  let rec helper (f:IF.formula):IF.formula* ((ident*primed)list)* ((ident*primed)list) = 
		match f with
    | IF.Or b -> 
...
and case_normalize_struc_formula i prog (h:(ident*primed) list)(p:(ident*primed) list)(f:IF.struc_formula) allow_primes
      allow_post_vars  (lax_implicit:bool) strad_vs :IF.struc_formula* ((ident*primed)list) =
    ...
    | IF.EAssume (b,y,t)-> 
      ...
      | IF.EBase b->		
          ...
          let _ = 
              ...
              Error.report_error {
  							Error.error_loc = b.IF.formula_struc_pos;
          ...
          let _ = 
						if (not allow_primes)&&(List.length (List.filter (fun (c1,c2)-> c2==Primed) (all_expl@posib_impl)))>0 then
          ...
          *)
          ...
          let _ = 
						if not(allow_post_vars) && (List.length (inters (all_expl@posib_impl) p_vars))>0 then 	
          ...
          let _ = 
						if (List.length (diff implvar (IF.heap_fv onb @ fold_opt IF.struc_hp_fv nc)))>0 then 
            	Error.report_error {
								Error.error_loc = pos; Error.error_text = ("malfunction: some implicit vars are not heap_vars\n")} 
						else true 
					in
          (IF.EBase {
...
and rename_exp (ren:(ident*ident) list) (f:Iast.exp):Iast.exp =   
  let rec helper (ren:(ident*ident) list) (f:Iast.exp):Iast.exp =   
    ...
    | Iast.Assert b ->
          ...
          let assert_formula = 
						match b.Iast.exp_assert_asserted_formula with
  	        | None -> None
  	        | Some (f1,f2)-> Some (IF.subst_struc subst_list f1,f2) in
          let assume_formula = 
						match b.Iast.exp_assert_assumed_formula with
  	        | None -> None
    ...
    | Iast.Return b ->  
				Iast.Return {b with Iast.exp_return_val = 
					match b.Iast.exp_return_val with
    	    | None -> None
    ...
    | Iast.While b-> 
        let nw = 
					match b.Iast.exp_while_wrappings with
          | None -> None
    ...
    | Iast.Raise b-> 
        Iast.Raise {b with
          Iast.exp_raise_val = 
						(match b.Iast.exp_raise_val with 
            | None -> None 
            | Some e -> Some (rename_exp ren e));
          Iast.exp_raise_type = 
						(match b.Iast.exp_raise_type with
...
and case_rename_var_decls (f:Iast.exp) : (Iast.exp * ((ident*ident) list)) =  match f with
  ...
  | Iast.Catch b->
	    let ncv,ren = 
				match b.Iast.exp_catch_var with
        | None -> (None,[])
        | Some e-> 
			...
			in
      let ncfv,ren = 
				match b.Iast.exp_catch_flow_var with
        | None -> (None,ren)
        | Some e-> 
            let nn = (Ipure.fresh_old_name e) in
            ((Some nn),(e,nn)::ren)
  ...
  | Iast.VarDecl b -> 		
      let ndecl,nren = List.fold_left (fun (a1,a2) (c1,c2,c3)->
        ...
        let ne = 
          ...
          | Some f-> Some (fst (case_rename_var_decls f)) 
				in
        ((nn,ne,c3)::a1,(c1,nn)::a2)) ([],[]) b.Iast.exp_var_decl_decls 
			in
...
and err_prim_l_vars s l pos= 
  List.iter (fun (c1,c2)-> 
    ...
    | Primed  ->
      ...
      if (c1 = Globals.ls_name || c1 = Globals.lsmu_name || c1 = Globals.waitlevel_name) then () 
			else
      	Error.report_error { 
          Error.error_loc = pos;
...
and check_eprim_in_formula s f = 
	match f with
...
and check_eprim_in_struc_formula_x s f = 
	match f with
...
and case_normalize_exp prog (h: (ident*primed) list) (p: (ident*primed) list)(f:Iast.exp) :
      Iast.exp*((ident*primed) list)*((ident*primed) list) =  
	match f with
  | Iast.Assert b->
      let asrt_nf,nh = 
				match b.Iast.exp_assert_asserted_formula with
      ...
      let assm_nf  = 
        ...
        | Iast.ArrayAlloc b-> (* An Hoa *)
	    	...
	    	| Iast.ArrayAt b-> 
              ...
              (Iast.ArrayAt 
							{ 
								Iast.exp_arrayat_array_base = b.Iast.exp_arrayat_array_base;
                Iast.exp_arrayat_index = new_index;
                Iast.exp_arrayat_pos = b.Iast.exp_arrayat_pos
							},h,p)
	    	| Iast.New b->
        ...
        | Iast.Return b -> 
            (Iast.Return {b with Iast.exp_return_val= 
							match b.Iast.exp_return_val with 
							| None -> None 
							| Some f -> let r,_,_ = (case_normalize_exp prog h p f) in Some r},h,p)
        | Iast.Seq b -> 
        ...
        | Iast.VarDecl b -> 		
            let ndecl = 
							List.fold_left (fun a1 (c1,c2,c3)->
                let ne = 
  								match c2 with
                  | None -> None 
        ...
        | Iast.Raise b-> (Iast.Raise {b with 
	          Iast.exp_raise_val = 
  		        ...
  		        | None -> None
  		        | Some e -> 
									let nc,_,_ = (case_normalize_exp prog h p e) in
          				Some nc)},h,p)
...
and prune_inv_inference_formula_x (cp:C.prog_decl) (v_l : CP.spec_var list) (init_form_lst: (CF.formula*formula_label) list) u_baga u_inv pos: 
      ((Cpure.b_formula * (formula_label list)) list)* (C.ba_prun_cond list) *
      ((formula_label list * (Gen.Baga(CP.PtrSV).baga * Cpure.b_formula list)) list) = 
	let rec get_pure_conj_list (f:CP.formula):(CP.formula * (bool*CP.b_formula) list) = 
		match f with
  ...
  let hull_invs v_l (f:CP.formula):CP.formula list =
    let rec helper acc e_v_l : CP.formula list = 
			match e_v_l with
      | [] ->
  ...
  let compute_invariants v_l (pure_list:(formula_label * (CP.baga_sv * CP.b_formula list)) list) : (formula_label list * (CP.baga_sv * CP.b_formula list)) list= 
    let combine_pures (l1:CP.b_formula list) (l2:CP.b_formula list) :CP.b_formula list = 
      ...
      let lr = 
				match (List.length l1r),(List.length l2r) with 
  ...
  let collect_constr (neg_br:  (formula_label * CP.b_formula) list)
    ...
    let n_c = 
			List.fold_left (fun a (l,c)  ->  
      	if (eq_formula_label l lbl) then a else 
        	let (b,_,_) = TP.imply f (CP.BForm (c,None)) "" false None 
					in
        	if b then c::a  else
          a) [] neg_br in
...
and check_barrier_wf prog bd =   
  ...
  let one_ctx_entail c1 c2 =
    let r = 
      ...
      fst (Solver.heap_entail_init prog false (CF.SuccCtx[c1]) (*wrapp_frac_fv*) c2 no_pos) 
		in
    match r with
  ...
  let prep_t (fs,ts,fl) = 
    ...
    let pres, posts = 
			List.split (List.map (fun f-> 
				match CF.split_struc_formula f with
      	| (p1,p2)::[] -> 
...
and trans_bdecl_x prog bd = 
  if Gen.BList.check_dups_eq (fun (c1,c2,_) (d1,d2,_)-> c1=d1 && c2=d2) bd.I.barrier_tr_list then 
    ...
    let vl = [self] in
		let rec aux1 (tl:spec_var_type_list) (il:Iformula.struc_formula list) = (*need to consider*)
			(match il with 
			| []->tl,[]
			| hd::tail ->
...
and validate_mem_spec (prog : C.prog_decl) (vdef: C.view_decl) = 
	...
	| Some a -> let pos = CF.pos_of_struc_formula vdef.C.view_formula in 
      ...
      in
      if flag then 
				if not (check_mem_formula_guards_disjoint a.CF.mem_formula_guards) then () 
      	else 
  				Err.report_error {
      			Err.error_loc = pos;
