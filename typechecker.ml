                          ...
                          let i_post =
                            if not(infer_post_flag) then spec
                              ...
                              begin
                                ...
                                let post_fml,_ = (*if rels = [] then *)Solver.simplify_post post_fml post_vars prog None [] true
                                  (*                                  else (                                                            *)
                ...
                *)
			    ...
			    let pp, exc = 
                  try (* catch exception to close the section appropriately *)
                    ...
                    let lst_input = List.filter (fun (_,rhs) -> CP.is_Rank_Dec rhs) lst_rank in
                    let rank_res = List.map (fun (lhs,rhs) -> match rhs with
                          | CP.BForm (bf,_) ->
                            (match bf with
                            | (CP.Gt (CP.Func _, CP.Func _,_),_)
                            | (CP.Lt (CP.Func _, CP.Func _,_),_) -> Rank.compute_rank rhs lhs
                            | _ -> report_error no_pos "Unexpected error in rank_dec")
                          | _ -> report_error no_pos "Unexpected error in rank_dec") lst_input 
                    in
                    Debug.info_hprint (add_str "RANK RES: " (pr_list (pr_pair !CP.print_formula (pr_list Rank.print_rel_typ)))) rank_res no_pos;
                      ...
                      begin
                        ...
                        if !dis_post_chk then
                          ...
                          begin
                            ...
                            Debug.dinfo_hprint (add_str "NEW SPECS" pr_spec) new_spec no_pos;
