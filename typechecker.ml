...
and check_exp_a (prog : prog_decl) (proc : proc_decl) (ctx : CF.list_failesc_context) (e0:exp) (post_start_label:formula_label) : CF.list_failesc_context =
  ...
  (* check if the pre of those methods creates a contradiction with the current state - quatifies over all possible arguments *)
          ...
          else let () = y_ninfo_pp "NO REENTRANCY" in ()
        in
        ...
        if (mn_str=Globals.fork_name) then
          ...
          let poly_hash = Globals.create_poly_hash proc.proc_poly_vars pargs in
          (*** performs substitution of poly args types***)
          (* let farg_types = Globals.hsubs_poly_typ poly_hash farg_types in *)
          let farg_types = Globals.subs_poly_typ proc.proc_poly_vars pargs farg_types in
          (**********************************)
