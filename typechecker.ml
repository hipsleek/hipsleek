...
and check_exp_a (prog : prog_decl) (proc : proc_decl) (ctx : CF.list_failesc_context) (e0:exp) (post_start_label:formula_label) : CF.list_failesc_context =
  ...
  let check_exp1 (ctx : CF.list_failesc_context) : CF.list_failesc_context =
          ...
          if not b then ctx
            ...
            let fct c1 =
              ...
              let res = if (CF.subsume_flow_f !norm_flow_int (CF.flow_formula_of_formula c1.CF.es_formula)) then
                  ...
                  (* NOTE: renaming of security context *)
                  ...
                  let () = y_tinfo_hp (add_str "c1.CF.es_formula: " Cprinter.string_of_formula) c1.CF.es_formula in
                  let () = y_tinfo_hp (add_str "compose_es: " Cprinter.string_of_formula) compose_es in
                  ...
                  y_tinfo_hp (add_str "ASSIGN CTX: " Cprinter.string_of_context) compose_ctx;
                  compose_ctx
    ...
    | BConst ({exp_bconst_val = b;
        ...
        try
          ...
          let ctx = if !Globals.ifa (* Information Flow Analysis *)
            then CF.transform_list_failesc_context (idf, idf, CF.prop_const prog.prog_sec_labels res_v pos) ctx
            else if !Globals.eximpf (* Explicit & Implicit Flow  *)
            then CF.transform_list_failesc_context (idf, idf, CF.prop_eximpf_const prog.prog_sec_labels res_v pos) ctx
            else ctx
          in
          (*****************************)
          ...
          if (Gen.is_empty unfolded) then
            ...
            if (CF.isSuccessListFailescCtx_new unfolded) && (not(CF.isSuccessListFailescCtx_new rs))then
              ...
              begin
                ...
                (* Information Flow Analysis *) (* Explicit & Implicit Flow  *)
                let rs = if !Globals.ifa || !Globals.eximpf
    ...
    | Cond ({ exp_cond_type = t;
      ...
      let cond_op () =
        begin
          ...
          (* Information Flow Analysis *) (* Explicit & Implicit Flow  *)
          let ctx = if !Globals.ifa || !Globals.eximpf
    ...
    | IConst ({exp_iconst_val = i;
      ...
      let res_n = CP.mkRes int_type in
      let res_v = CP.Var (res_n, pos) in
      ...
      let ctx = if !Globals.ifa (* Information Flow Analysis *)
        then CF.transform_list_failesc_context (idf, idf, CF.prop_const prog.prog_sec_labels res_n pos) ctx
        else if !Globals.eximpf (* Explicit & Implicit Flow  *)
        then CF.transform_list_failesc_context (idf, idf, CF.prop_eximpf_const prog.prog_sec_labels res_n pos) ctx
        else ctx
      in
      (*****************************)
    ...
    | FConst {exp_fconst_val = f; exp_fconst_pos = pos} ->
      ...
      let res_n = CP.mkRes float_type in
      let res_v = CP.Var (res_n, pos) in
      let c = CP.mkEqExp res_v c_e pos in
      let c =
        if !Globals.infer_lvar_slicing then
    ...
    | Var ({
      ...
      begin
        ...
        let sec_res = CP.mkRes t in
        let ctx = if !Globals.ifa (* Information Flow Analysis *)
          then CF.transform_list_failesc_context (idf, idf, CF.prop_var prog.prog_sec_labels sec_res (CP.mk_spec_var v) pos) ctx
          else if !Globals.eximpf (* Explicit & Implicit Flow  *)
          then CF.transform_list_failesc_context (idf, idf, CF.prop_eximpf_var prog.prog_sec_labels sec_res (CP.mk_spec_var v) pos) ctx
          else ctx
        in
        (*****************************)
    ...
    | Sharp ({exp_sharp_type =t;
      ...
      let nctx = match v with
        | Sharp_var (t,v) ->
          ...
          let ctx = if !Globals.ifa (* Information Flow Analysis *)
            then CF.transform_list_failesc_context (idf, idf, CF.prop_var prog.prog_sec_labels vr vf pos) ctx
            else if !Globals.eximpf (* Explicit & Implicit Flow  *)
            then CF.transform_list_failesc_context (idf, idf, CF.prop_eximpf_var prog.prog_sec_labels vr vf pos) ctx
            else ctx
          in
          (*****************************)
    ...
    begin
      ...
      | Some body ->
        begin
          ...
          let bounds = if !Globals.ifa
            then CF.base_sec_bounds fsvars proc.proc_loc
            else if !Globals.eximpf
            then CF.base_eximpf_sec_bounds prog.prog_sec_labels fsvars proc.proc_loc
