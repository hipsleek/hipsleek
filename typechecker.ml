...
let check_coercion (prog : prog_decl) =
  ...
  let check_left_coercion coer =
    ...
    let lhs = unfold_nth "c1" (prog,None) coer.coercion_head (CP.SpecVar (Named "", self, Unprimed)) true pos in
    let rhs = unfold_nth "c2" (prog,None) coer.coercion_body (CP.SpecVar (Named "", self, Unprimed)) true pos in
      check_entailment lhs rhs in
    (* check_entailment lhs coer.coercion_body in *)
  let check_right_coercion coer =
    let pos = CF.pos_of_formula coer.coercion_head in
    let rhs = unfold_nth "c3" (prog,None) coer.coercion_head (CP.SpecVar (Named "", self, Unprimed)) true pos in
    let lhs = unfold_nth "c4" (prog,None) coer.coercion_body (CP.SpecVar (Named "", self, Unprimed)) true pos in
