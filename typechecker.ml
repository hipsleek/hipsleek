...
(*         		        CF.Ctx {es with CF.es_var_src_ctx = CP.mkAnd es.CF.es_var_src_ctx new_c1 pos_spec}) ctx  in (\*???*\) *)
...
and do_spec_verify_infer (prog : prog_decl) (proc : proc_decl) (ctx : CF.context) e0 (spec: CF.ext_formula)
    ...
    match spec with
	  ...
	  | CF.EVariance v ->
            ...
            (* Termination: Add termination measure into context *)
				    ...
				    let (c,pre,f) = check_specs_infer_a prog proc nctx v.CF.formula_var_continuation e0 in
	          (CF.EVariance {v with CF.formula_var_continuation = c}, pre, f) 
      | CF.EInfer b ->
            Debug.devel_pprint ("check_specs: EInfer: " ^ (Cprinter.string_of_context ctx) ^ "\n") no_pos;
...
let check_prog (prog : prog_decl) =
    ...
    (* Termination: call termination_check from Term module *)
