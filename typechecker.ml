...
(* global option to switch on/off the simplification of context after symbolic excution *)
...
(* Gen.Debug.loop_2_no "check_specs" (Cprinter.string_of_context)        *)
(* (Cprinter.string_of_struc_formula) (string_of_bool) (fun ctx          *)
(* spec_list -> (check_specs_a prog proc ctx spec_list e0)) ctx          *)
(* spec_list                                                             *)

(* and check_specs prog proc ctx spec_list e0 = check_specs_a prog proc    *)
(* ctx spec_list e0                                                        *)
...
and check_specs_a (prog : prog_decl) (proc : proc_decl) (ctx : CF.context) (spec_list: CF.struc_formula) e0 : bool =
	let rec do_spec_verification (spec: Cformula.ext_formula) : bool =
		...
		| Cformula.EBase b ->
				...
				let nctx =
					...
					else (CF.transform_context (CF.normalize_clash_es b.Cformula.formula_ext_base b.Cformula.formula_ext_pos false) ctx)
				in
					...
					if (Cformula.isAnyFalseCtx ctx1) then
						...
						try
							let r =
								...
								(* let _ = print_string ("\n WN 1 :                        *)
								(* "^(Cprinter.string_of_list_partial_context res_ctx)) in *)
	...
	in
	(* let _ = print_string ("\ncheck_specs: " ^ (Cprinter.string_of_context *)
	(* ctx) ^ "\n") in                                                       *)
...
(* and check_exp prog proc ctx e0 label = check_exp_a prog proc ctx e0     *)
(* label                                                                   *)
...
and check_exp_a (prog : prog_decl) (proc : proc_decl) (ctx : CF.list_failesc_context) e0 (post_start_label: formula_label) : CF.list_failesc_context =
	if (exp_to_check e0) then
		...
		(* print_string ("\n false at :"^(Cprinter.string_of_exp e0)) else ()  *)
		(* in                                                                  *)
	...
	let check_exp1 (ctx : CF.list_failesc_context) : CF.list_failesc_context =
		...
		| Bind ({ exp_bind_type = body_t;
			...
			exp_bind_pos = pos }) -> begin
					...
					(* let _ = print_string ("[typechecker.ml, check__exp]: link_pv: *)
					(* " ^ Cprinter.string_of_formula link_pv ^ "\n") in let link_pv *)
					(* = CF.formula_of_pure (CP.mkEqVar v_prim p pos) pos in         *)
					let tmp_ctx =
					...
					(* let unfolded_prim = if !Globals.elim_unsat then elim_unsat    *)
					(* unfolded else unfolded in                                     *)
					...
					(* Test whether fresh_frac is full permission or not writable ->           *)
					(* fresh_frac = full_perm => normally read-only -> fresh_frac != full_perm *)
					(* => in order to detect permission violation We use exp_bind_read_only.   *)
					(* If true -> read only -> 0.0<f<=1.0 Othewiese, false -> write -> f=1.0   *)
					let vheap =
		...
		| New ({ exp_new_class_name = c;
			...
			exp_new_pos = pos }) -> begin
					...
					let heap_node = CF.DataNode ({
								...
								CF.h_formula_data_label = None;
								CF.h_formula_data_pos = pos }) in
					(* c let heap_form = CF.mkExists [ext_var] heap_node ext_null    *)
					(* type_constr pos in                                            *)
		...
		| SCall ({
				...
				begin
					...
					(* Internal function to check pre/post condition of the        *)
					(* function call.                                              *)
					let check_pre_post org_spec (sctx: CF.list_failesc_context) should_output_html : CF.list_failesc_context =
						...
						let stripped_spec = if ir then org_spec else Term.term_strip_variance org_spec in
						
						(* org_spec -> stripped_spec free vars = linking vars that   *)
						(* appear both in pre and are not formal arguments           *)
						...
						(* let _ = print_string (List.fold_left (fun res (p1, p2) -> *)
						(* res ^ "(" ^ (Cprinter.string_of_spec_var p1) ^ "," ^      *)
						(* (Cprinter.string_of_spec_var p2) ^ ") ") "\ncheck_spec:   *)
						(* mapping org_spec to new_spec: \n" st1) in                 *)
						...
						(* let l = (String.index mn '$') in let new_mn = String.create l in let _  *)
						(* = print_string ("mn: " ^ mn ^ "\n") in let _ = String.blit mn 0 new_mn  *)
						...
						let str_debug_variance = if (ir) then
              ...
              Term.term_add_unreachable_state pos  
						...
						(* An Hoa : output the context and new spec before checking  *)
						(* pre-condition                                             *)
						...
						(* The context returned by heap_entail_struc_list_failesc_context_init,    *)
						(* rs, * is the context with unbound existential variables initialized &   *)
						(* matched.                                                                *)
						let _ = PTracer.log_proof prf in
					...
					let check_pre_post org_spec (sctx: CF.list_failesc_context) should_output_html : CF.list_failesc_context =
						...
						Gen.Debug.loop_2_no "check_pre_post" pr3 pr1 pr1 (fun _ _ -> check_pre_post org_spec sctx should_output_html) org_spec sctx
					in
					...
					let res = if (CF.isFailListFailescCtx ctx) then
							let _ =
								if !print_proof && scall_pre_cond_pushed then
									Prooftracer.append_html "Program state is unreachable." in
							ctx
		...
		| Seq ({ exp_seq_type = te2;
			...
			exp_seq_pos = pos }) -> begin
					...
					(* Astsimp ensures that e1 is of type void let _ =             *)
					(* print_endline ("WN                                          *)
					(* C1:"^(Cprinter.string_of_list_failesc_context ctx1)) in     *)
					let ctx2 = check_exp prog proc ctx1 e2 post_start_label (*flow_store*) in
					(* let _ = print_endline ("WN                                  *)
					(* C2:"^(Cprinter.string_of_list_failesc_context ctx2)) in     *)
					ctx2
				end
		...
		| Sharp ({ exp_sharp_type = t;
			...
			exp_sharp_pos = pos })	->
		(* let _ =print_string ("sharp start ctx: "^                   *)
		(* (Cprinter.string_of_list_failesc_context ctx)^"\n") in let  *)
		(* _ = print_string ("raising: "^(Cprinter.string_of_exp       *)
		(* e0)^"\n") in let _ = print_string ("sharp flow type:        *)
		(* "^(Cprinter.string_of_sharp_flow ft)^"\n") in               *)
				let nctx = match v with
					| Sharp_var (t, v) ->
							let t1 = (get_sharp_flow ft) in
							(* let _ = print_endline ("Sharp                   *)
							(* Flow:"^(string_of_flow t1) ^"                   *)
							(* Exc:"^(string_of_flow !raisable_flow_int)) in   *)
	...
	(* let _ = print_endline                                               *)
	(* ("WN:ESCAPE:"^(Cprinter.string_of_list_failesc_context fl)) in let  *)
	(* _ = print_endline                                                   *)
	(* ("WN:CURRENT:"^(Cprinter.string_of_list_failesc_context cl)) in if  *)
	(* (Gen.is_empty cl) then fl else                                      *)
...
and check_post_x (prog : prog_decl) (proc : proc_decl) (ctx : CF.list_partial_context) (post : CF.formula) pos (pid: formula_label) : CF.list_partial_context =
	...
	(* in let _ = print_string ("context before post:                        *)
	(* "^(Cprinter.string_of_list_partial_context ctx)^"\n") in              *)
	...
	(* print_string ("\nLength of List Partial Ctx: " ^                      *)
	(* (Cprinter.summary_list_partial_context(ctx)));                        *)
	let final_state_prim = CF.push_exists_list_partial_context w ctx in
	(* print_string ("\nLength of List Partial Ctx: " ^                      *)
	(* (Cprinter.summary_list_partial_context(final_state_prim))); let _ =   *)
	(* print_flush ("length:"^(string_of_int (List.length                    *)
	(* final_state_prim))) in                                                *)
	let final_state =
	...
	(* Debug.devel_print ("Final state after existential quantifier          *)
	...
	if (CF.isSuccessListPartialCtx rs) then
		...
		(* get source code position of failed branches let locs_of_failures =  *)
		(* List.fold_left (fun res ctx -> res @ (locs_of_partial_context ctx)) *)
		(* [] rs in let string_of_loc_list locs = List.fold_left (fun res l -> *)
		(* res ^ (string_of_loc_by_char_num l) ^ ",") "" locs in               *)
		begin
			...
			Err.report_error {
				...
				(* (Cprinter.string_of_list_partial_context rs) (string_of_loc_list        *)
				(* locs_of_failures)                                                       *)
				}
		end
	end
		...
		begin
			...
			| Some body ->
					begin
						...
						(* add default full permission = 1.0 to ante; need to add type of full     *)
						(* perm to stab                                                            *)
						let init_form =
							if (Perm.allow_perm ()) then
...
let check_proc_wrapper prog proc =
	...
	try
	(* let _ = print_endline ("check_proc_wrapper : proc = " ^               *)
	(* proc.Cast.proc_name) in                                               *)
		check_proc prog proc
	with _ as e ->
			if !Globals.check_all then begin
				...
				raise e
(* let check_view vdef = let ante = vdef.view_formula in let conseq =      *)
(* vdef.view_invariant in let pos = get_constr_pos conseq in let ctx =     *)
(* build_ctx (Ctx (es_empty (get_constr_pos conseq))) ante pos in let ok,  *)
(* res = heap_entail false ctx conseq pos in if ok then true else if       *)
(* !Globals.check_all then begin print_string ("Error detected when        *)
(* checking view " ^ vdef.view_name ^ ": invalid invariant"); false end    *)
(* else report_error pos ("Error detected when checking view " ^           *)
(* vdef.view_name ^ ": invalid invariant") let check_view_wrapper def =    *)
(* match def with | Data _ -> true | View vdef -> check_view vdef          *)
...
let check_coercion (prog : prog_decl) =
	...
	(* combine the 2 lists of coercions into one list of lemmas: - coercions   *)
	(* that have the same name in the left and right list of coercions ->      *)
	(* (Some c1,Some c2) - left coercion c -> (Some c, None) - right coercion  *)
	(* c -> (None, Some c)                                                     *)
	...
	(* add to lemmas the coercions from prog.prog_right_coercions that do    *)
	(* not have a corresponding pair in prog.prog_left_coercions             *)
