...
and mkSeqVar ann element fix_point bounds variation pos : p_formula= 
  SeqVar { seq_ann = ann;
           seq_element = element;
...
and mkPrimTermVar ann pos =
  PrimTermVar { prim_ann = ann; prim_loc = pos}
...
and b_apply_subs sst bf =
  ...
  | VarPerm (ct,ls,pos) -> VarPerm (ct,ls,pos) (*substitue VarPerm using b_apply_subs_varperm *)
    ...
    | SeqVar seq_info -> 
        ...
        let b = e_apply_subs_list sst seq_info.seq_bounds in
...
and e_apply_subs sst e = match e with
  ...
  | Pow (a1, a2, pos) ->
      Pow (e_apply_subs sst a1, e_apply_subs sst a2, pos)
...
and e_apply_one (fr, t) e = match e with
  ...
  | Pow (a1, a2, pos) ->
      Pow (e_apply_one (fr, t) a1, e_apply_one (fr, t) a2, pos)
...
and b_apply_par_term (sst : (spec_var * exp) list) bf =
  ...
  | VarPerm (ct,ls,pos) -> VarPerm (ct,ls,pos) (*TO CHECK: do not substitute*)
    ...
    | SeqVar seq_info -> 
        ...
        let b = a_apply_par_term_list sst seq_info.seq_bounds in
        SeqVar {seq_info with seq_element = e;
                              seq_fix_point = fp;
                              seq_bounds = b; }
    | PrimTermVar _ -> pf
...
and b_apply_one_term ((fr, t) : (spec_var * exp)) bf =
  ...
  | VarPerm (ct,ls,pos) -> VarPerm (ct,ls,pos) (*TO CHECK: do not substitute*)
    ...
    | SeqVar seq_info -> 
        ...
        let b = List.map (a_apply_one_term (fr, t)) seq_info.seq_bounds in
        SeqVar {seq_info with seq_element = e;
                              seq_fix_point = fp;
                              seq_bounds = b;}
    | PrimTermVar _ -> pf
...
and b_apply_one_exp (fr, t) bf =
  ...
  | SeqVar seq_info -> 
      ...
      let b = e_apply_one_list_exp (fr, t) seq_info.seq_bounds in
      SeqVar {seq_info with seq_element = e;
                            seq_fix_point = fp;
                            seq_bounds = b;}
  | PrimTermVar _ -> pf
...
and b_form_simplify_x (b:b_formula) :b_formula = 
  ...
  let npf = match pf with
    |  BConst _ 
    |  SubAnn _ | LexVar _ | SeqVar _ | PrimTermVar _
    |  BVar _ -> pf
    |  Lt (e1, e2, l) ->
	  ...
	  | None  -> let new_arg = f_b_formula_args arg e in 
        ...
        let (npf, opt2) = match pf with	  
          ...
          | SeqVar seq_info -> 
              ...
              let tmp = List.map (helper new_arg) seq_info.seq_bounds in
              let b = List.map fst tmp in
              let r3 = List.map snd tmp in
              let new_seq = {seq_info with seq_element = e;
                                           seq_fix_point = fp;
                                           seq_bounds = b;} in
              (SeqVar new_seq, f_comb ([r1; r2] @ r3))
	...
	| None  ->
	  ...
	  (* bag formulas *)
        ...
        | SeqVar seq_info -> 
            ...
            let b = List.map (transform_exp f_exp) seq_info.seq_bounds in
            SeqVar { seq_info with seq_element = e;
                                   seq_fix_point = fp;
                                   seq_bounds = b; }
        | PrimTermVar _ -> pf
    ...
    let npf = 
      match pf with 
        ...
        | SeqVar seq_info -> 
            ...
            let b = List.map norm_exp seq_info.seq_bounds in
            SeqVar { seq_info with seq_element = e;
                                   seq_fix_point = fp;
                                   seq_bounds = b; }
        | PrimTermVar _ -> pf
...
and has_func_pf (pf: p_formula) : bool = match pf with
  | LexVar _ | SeqVar _ | PrimTermVar _ -> false
  | Lt (e1,e2,_)
  | Lte (e1,e2,_)
  | Gt (e1,e2,_)
  | Gte (e1,e2,_)
...
(** has termination var *)
let rec has_termvar (f: formula) : bool =
  match f with
  | BForm _ -> is_termvar f
  | And (f1, f2, _) -> (has_termvar f1) || (has_termvar f2)
  | AndList b -> exists_l_snd has_termvar b
  | Or (f1, f2, _, _) -> (has_termvar f1) && (has_termvar f2)
  | Not (f, _, _) -> has_termvar f
  | Forall (_, f, _, _)
  | Exists (_, f, _, _) -> has_termvar f
...
let drop_complex_ops =
  let pr_weak b = match b with
        ...
        | PrimTermVar t_info -> Some (mkTrue t_info.prim_loc)
  ...
  let pr_strong b = match b with
        ...
        | PrimTermVar t_info -> Some (mkFalse t_info.prim_loc)
        | RelForm (SpecVar (_, v, _),_,p) ->
            (*provers which can not handle relation => throw exception*)
            if (v="dom") or (v="amodr") or (is_update_array_relation v) then None
            else Some (mkFalse p)
...
(*				let il = t_info.lex_tmp in*)
        ...
        (match t_ann with
          ...
          | _ -> (pf, []))
    (* TRUNG TODO: check from SeqVar & PrimTermVar *)
    | _ -> (pf, [])
  in ((n_pf, ann), pv)
