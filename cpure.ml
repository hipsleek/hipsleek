...
and mkPure (pf: p_formula) : formula = BForm ((pf,None), None, None)
...
and dperm_subst_simpl f = 
	...
	let rec helper flg lsubs f = match f with
	  ...
	  | BForm ((Eq(e1,e2,p1),p2),p3,_) -> if not flg then  f
	     else
      ...
      match r with
        ...
        | Neq (e1,e2,l) ->
              ( match e1,e2 with
                ...
                | _ -> r)
        | _ -> r in  
    let tmp = BForm (((tt relop ae a pos), None),lbl,None) in
    if Gen.is_empty rest then
      tmp
    else
      let tmp1 = helper1 ae rest in
...
and a_apply_par_term (sst : (spec_var * exp) list) e = match e with
  ...
  | Sequence (seq, f, pos) ->
      let newseq = {seq_measure = a_apply_par_term sst seq.seq_measure;
                    ...
                    seq_domain_ub_include = seq.seq_domain_ub_include;} in
      let newf = apply_par_term sst f in
      Sequence (newseq, newf, pos)
  (*| BagEmpty (pos) -> BagEmpty (pos)*)
...
and a_apply_one_term ((fr, t) : (spec_var * exp)) e = match e with
  ...
  | Sequence (seq, f, pos) ->
      let newseq = {seq_measure = a_apply_one_term (fr, t) seq.seq_measure;
                    ...
                    seq_domain_ub_include = seq.seq_domain_ub_include;} in
      let newf = apply_one_term (fr, t)  f in
      Sequence (newseq, newf, pos)
  (*| BagEmpty (pos) -> BagEmpty (pos)*)
...
and elim_of_bformula (f0:formula) ls: formula  =  match f0 with
    | BForm (f, lb, fo) -> if List.mem f0 ls then BForm ((BConst (true,no_pos), None), lb, fo) else f0
...
let add_gte0_for_mona (f0 : formula): (formula)=
  let mkGte_f spec_var lbl pos=
    ...
    BForm (((mkLte zero (Var (spec_var, no_pos)) pos), None), lbl, None) in
  let rec helper f0 =
    match f0 with
      | Exists (qvar, qf, lbl, pos) ->
            begin
...
and e_apply_one_exp (fr, t) e = match e with
  ...
  | Sequence (seq, f, pos) ->
      let newseq = {seq_measure = e_apply_one_exp (fr, t) seq.seq_measure;
                    ...
                    seq_domain_ub_include = seq.seq_domain_ub_include;} in
      let newf = apply_one_exp (fr, t) f in
      Sequence (newseq, newf, pos)
  (*| BagEmpty (pos) -> BagEmpty (pos)*)
  | Bag (alist, pos) ->
...
and add_null f self : formula =  
  mkAnd f (BForm ((mkEq_b (Var (self,no_pos)) (Null no_pos) no_pos, None, None))) no_pos	  
      (*to fully extend*)
...
and purge_mult_x (e :  exp):  exp = match e with
  ...
  |  Sequence (_, _, l) -> report_error l ("Cannot apply purge_mult to sequence")
...
let rec transform_exp f e  = 
  ...
  | None -> match e with
      ...
      | Min (e1,e2,l) ->
          ...
          let ne2 = transform_exp f e2 in
          Min (ne1,ne2,l)
      | Sequence (_, _, l) -> report_error l "Cannot apply transform_exp to sequence"
      | Bag (le,l) -> 
...
let rec get_head e = match e with 
    ...
    | Sequence (_, _, l) -> report_error l "Cannot apply get_head to sequence"
...
let form_formula_eq (v1:spec_var) (v2:spec_var) =
  BForm (((form_bform_eq v1 v2), None), None, None)
...
let form_formula_eq_with_const (v1:spec_var) (v2:spec_var) : formula =
  BForm (((form_bform_eq_with_const v1 v2), None), None, None)
...
let rec remove_redundant_constraints (f : formula) : formula = match f with
	| BForm ((b,a),l,fo) -> BForm ((remove_redundant_constraints_b b,a),l,fo)
	| And (f1,f2,l) -> 
