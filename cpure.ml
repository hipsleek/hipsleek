...
(* An Hoa: Relational formula to capture relations, for instance, s(a,b,c) or
 * t(x+1,y+2,z+3), etc. *)
  ...
  let ri =
    try
      ...
      if (l==0) then slen-1
        ...
        let _tr = String.sub s (n+1) (slen-(n+1)) in
        n
    with  _ -> slen in
...
and check_non_determinism_x (var_name: ident) (f: formula) =
  ...
  let collect_nondet_vars f = (
    ...
    let _todo_var = transform_formula (fh, fm, ff, fb, fe) f in
    !nondet_svs
  ) in
  ...
  if (List.exists (fun x -> eq_str (name_of_sv x) var_name) nondet_svs) then true
    ...
    let rec collect_related_vars vars = (
      ...
      let _todo_unknown = transform_formula (fh, fm, ff, fb, fe) simp_f in
...
and get_subst_equation_b_formula_x (f : b_formula) (v : spec_var) lbl only_vars:
  ((spec_var * exp) list * formula) =
  let (pf,il) = f in
  match pf with
  | Eq (e1, e2, pos) -> begin
      match e1, e2 with
      | Var (sv1, _), Var (sv2, _) ->
        if (eq_spec_var sv1 v) && (not (eq_spec_var sv2 v))
        then ([(v, e2)], mkTrue no_pos)
        else if (eq_spec_var sv2 v) && (not (eq_spec_var sv1 v))
        then ([(v, e1)], mkTrue no_pos )
        else ([], BForm (f,lbl))
      | Var (sv1, _), _  ->
        if only_vars then ([], BForm (f,lbl))
        else if (eq_spec_var sv1 v) &&
                (not (List.exists (fun sv -> eq_spec_var sv v) (afv e2)))
        then ([(v, e2)], mkTrue no_pos )
        else ([], BForm (f,lbl))
      | _ , Var (sv2, _) ->
        if only_vars then ([], BForm (f,lbl))
        else if (eq_spec_var sv2 v) &&
                (not (List.exists (fun sv -> eq_spec_var sv v) (afv e1))) then
          ([(v, e1)], mkTrue no_pos )
        else ([], BForm (f,lbl))
      ...
      | Tsconst (t,_), Add (Tsconst (t1,_), Var (sv,_),_) ->
        if only_vars then ([],BForm(f,lbl))
          ...
          if (Tree_shares.Ts.contains t t1)
          then ([(v,Tsconst (Tree_shares.Ts.subtract t t1, no_pos))],mkTrue no_pos)
          else ([],mkFalse no_pos)
        else ([],BForm (f,lbl))
      | _ ->([], BForm (f,lbl))
    end
...
and get_subst_equation_b_formula (f : b_formula) (v : spec_var) lbl only_vars:
  ((spec_var * exp) list * formula) =
  let pr_out = pr_pair (pr_list (pr_pair !print_sv !print_exp)) !print_formula in
...
and get_all_vv_eqs_x (f0 : formula) : ((spec_var * spec_var) list * formula) =
  let rec helper f0 =  match f0 with
    ...
    | AndList b ->
      ...
      (r1, AndList r2)
    | BForm (((Eq (Var (sv1, _), Var (sv2, _), pos)),_),_) ->
      ([(sv1, sv2)], mkTrue no_pos )
    | _ -> ([], f0)
  in helper f0
...
and prune_perm_bounds f =
  let helper (f,p) = match f with
    | Eq (e1,e2,l) -> if !perm=Dperm && (perm_bounds e1 || perm_bounds e2)
      then  BConst (false, l),p else (f,p)
    | _ -> (f,p) in
  let rec helper_f f = match f with
    ...
    | Exists (v,f,l,pos) -> mkExists [v] (helper_f f) l pos in
  helper_f f
...
and  check_dependent f ls_working: bool=
  ...
  let rec helper_check ls_lvarset =
    ...
    | v::vs ->
      begin
        if List.mem v ls_var then true
        else helper_check vs
      end in
  helper_check ls_varset

and list_of_irr_bformula (ls_lhs:formula list) ls_working:
  ...
  if ((List.length new_ls_lhs_rem) = (List.length ls_lhs)) then
    (*fixpoint*)
    ls_lhs,ls_working
  else list_of_irr_bformula new_ls_lhs_rem new_ls_working
...
let rec imply_disj_orig_x ante_disj conseq t_imply imp_no =
  x_ninfo_hp (add_str "ante: " (pr_list !print_formula)) ante_disj no_pos;
  x_ninfo_hp (add_str "coseq : " ( !print_formula)) conseq no_pos;
  match ante_disj with
  | h :: rest ->
    x_ninfo_hp (add_str "h : " ( !print_formula)) h no_pos;
    let r1,r2,r3 = (t_imply h conseq (string_of_int !imp_no) true None) in
    x_ninfo_hp (add_str "res : " (string_of_bool)) r1 no_pos;
    if r1 then
...
let get_RelForm_arg_list_with_name pf name =
  let _rel_form_list = get_RelForm pf in
  List.fold_left 
    (fun r rel -> 
       match rel with
    ...
    if sflag then
      ...
      if List.length lst <= 1 then None
        ...
        | lhs::rest -> 
          ...
          let _eqlist = find_eq_at_toplevel f in
          let emap = em in
  ...
  let f_arg_f (start_flag,emap,nonlinear) e =
    ...
    | AndList _ ->
      if start_flag then (* add to eqmap *)
        let _eqlist = find_eq_at_toplevel e in
...
let get_vv_eqs (f0 : formula) : (spec_var * spec_var) list =
  let fct p f = match f with
    | BForm ((Eq (Var(vl,_), Var(vr,_),_),_),_) ->
      if p then Some (f,[(vl,vr)]) else Some (f,[])
    | BForm ((Neq(Var(vl,_),Var(vr,_),_),_),_) ->
      if p then Some (f,[]) else Some (f,[(vl,vr)])
    | BForm _ -> Some (f,[])
    | _ -> None in
  ...
  let f_cmb e l = match e with
    ...
    | Or _ ->
      Gen.BList.intersect_eq eq_pair_eq (List.hd l) (List.hd (List.tl l))
    | Forall (sv,_,_,_)
    | Exists (sv,_,_,_) ->
      let n_l = List.concat l in
      let filter_fun (v1,v2) = not (eq_spec_var sv v1 || eq_spec_var sv v2) in
      List.filter filter_fun n_l in
...
let get_neqs (f0 : formula) : ((spec_var * spec_var) list) * (spec_var list) =
  let fct p f = match f with
    ...
    | BForm ((Eq (Var(v ,_),Null _ ,_),_),_) ->
      if p then Some (f,([],[])) else Some (f,([],[v]))
    | BForm ((Neq(Var(v ,_),Null _ ,_),_),_)
    | BForm ((Neq(Null _ ,Var(v ,_),_),_),_) ->
      if p then Some (f,([],[v])) else Some (f,([],[]))
    | BForm _ -> Some (f,([],[]))
    | _ -> None in
  let f_arg arg e = match e with | Not _ -> not arg | _ -> arg in
  let f_cmb e l :((spec_var * spec_var) list) * (spec_var list) = match e with
    | BForm _  | And _ | AndList _  | Not _ ->
      Gen.fold_pair2f List.concat List.concat (List.split l)
    | Or _ ->
...
let drop_neq (aneq,anull) f =
  let f_tr e = match e with
    | BForm ((Neq(Var(vl,_),Var(vr,_),l),_),_) ->
      if List.exists (eq_pair_eq (vl,vr)) aneq
      then Some (mkTrue no_pos) else Some e
    | BForm ((Neq((Null _),Var(v,_),_),_),_)
    | BForm ((Neq(Var(v,_),(Null _),_),_),_) ->
      if (List.exists (eq_spec_var v) anull) then Some (mkTrue no_pos)
      else Some e
    | Not _ -> Some e
    | BForm _ -> Some e
    | _ -> None in
...
let force_all_vv_eqs_x f0 =
  let rec helper b f = match f with
    | BForm ((Eq (Var(vl,_),Var(vr,_),_),_),_) ->
      if b&&(not (eq_spec_var vl vr)) then [vl;vr] else []
    | BForm ((Eq(Var(vl,_),(Null _),_),_),_)
    | BForm ((Eq((Null _),Var(vl,_),_),_),_)-> if b then [vl] else []
    | BForm ((Neq(Var(vl,_),Var(vr,_),_),_),_) ->
      if b&&(not (eq_spec_var vl vr)) then [] else [vl;vr]
  ...
  let helper nf =
    ...
    let f a c = match c with
      | BForm ((Neq(Var(vl,_),Var(vr,_),l),_),_) ->
        ...
        else if (decide_keep vars_to_keep vl) && (decide_keep vars_to_keep vr)
        then Some (c,a)
      ...
      | BForm ((Neq((Null _),Var(v,_),_),_),_)
      | BForm ((Neq(Var(v,_),(Null _),_),_),_) ->
        if (decide_keep vars_to_keep v)
        then Some (c,a) else Some (mkTrue no_pos, a)
      | BForm _ -> Some (c,a)
      | _ -> None  in
    let f_comb =
