...
(** An Hoa : reference to function to solve equations in Redlog
			initialize in redlog.ml
 **)
let solve_equations = ref (fun (eqns : (exp * exp) list) (bv : spec_var list) 
		-> (([],[]) : (((spec_var * spec_var) list) * ((spec_var * string) list))))
...
let rec reduce_pure (f : formula) (bv : spec_var list) 
	...
	(* Take only arithmetic constraints, not object, etc. *)
	...
	let is_b_form_arith f = (match f with
		...
		| _ -> false)
	in
	let bf = List.filter is_b_form_arith bf in*) 
	...
	(*let _ = print_endline "Equality found: " in
