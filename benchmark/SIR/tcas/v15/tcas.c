 ...
 }
{
    ...
    {
      result = Own_Above_Threat()&&(Cur_Vertical_Sep >= 650)&&(Up_Separation >= ALIM(arr, Alt_Layer_Value));
    }
    return result;
}
...
}
{
    ...
    if (upward_preferred)
    {
      result = Own_Below_Threat() && (Cur_Vertical_Sep >= 650) && (Down_Separation >= ALIM(arr, Alt_Layer_Value));
    }
    else
    {
