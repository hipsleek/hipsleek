...
(! (iff (= (length l) 0) (= l nil)) :pattern ((length l)))))
...
(! (= (append (append s0 s1) s2) (append s0 (append s1 s2))) :pattern ((append (append s0 s1) s2)))))
...
(assert (forall ((s (Seq Int)))
(! (= (rev (rev s)) s) :pattern ((rev (rev s))))))

(assert (forall ((s0 (Seq Int)) (s1 (Seq Int)))
(! (iff (= (rev s0) (rev s1)) (= s0 s1)) :pattern ((rev s0) (rev s1)))))
...
(assert (forall ((s0 (Seq Int)) (s1 (Seq Int)))
(! (= (rev (append s0 s1)) (append (rev s1) (rev s0))) :pattern ((append (rev s1) (rev s0))))))
...
(! (iff (isin x (rev s)) (isin x s)) :pattern ((isin x (rev s))))))
...
(assert (forall ((s (Seq Int)) (x Int))
(! (iff (isnotin x (rev s)) (isnotin x s)) :pattern ((isnotin x (rev s))))))

(declare-fun alln ((Int) (Seq (Int))) Bool)
;Alln axioms
(assert (forall ((n Int)) (not (alln n nil))))

(assert (forall ((s (Seq Int)) (v Int) (n Int))
(! (iff (alln n (insert v s)) (and (= v n) (alln n s))) :pattern((alln n (insert v s))))))

(assert (forall ((s (Seq Int)) (n Int))
(! (iff (alln n (rev s)) (alln n s)) :pattern ((alln n (rev s))))))

(assert (forall ((s0 (Seq Int)) (s1 (Seq Int)) (n Int)) 
(! (iff (alln n (append s0 s1)) (and (alln n s0) (alln n s1))) :pattern ((alln n (append s0 s1))))))

(assert (forall ((n Int) (s (Seq Int))) 
