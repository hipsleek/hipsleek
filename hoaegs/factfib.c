...
//checkentail n > 1 & forall(m,fm,gm : m >= n | !(fib(m,fm)) | !(fact(m,gm)) | fm <= gm) & fib(n,f) & fact(n,g) |- f <= g.
...
checkentail n > 1 & forall(m : F[m] = F[m-1] + F[m-2]) & G[n-1] = (n-1) * G[n-2] & G[n] = n * G[n-1] & F[0] = 1 & F[1] = 1 & G[0] = 1 & F[n-1] <= G[n-1] & F[n-2] <= G[n-2] |- F[n] <= G[n].
