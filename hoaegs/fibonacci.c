relation fib(int n, int f) == (n = 0 & f = 1 | n = 1 & f = 1 | n > 1 & exists(f1, f2 : fib(n-1,f1) & fib(n-2,f2) & f = f1 + f2)).

relation fiba(int n, int f).

axiom n = 0 & f = 1 ==> fiba(n,f).
axiom n = 1 & f = 1 ==> fiba(n,f).
axiom fiba(0,f) ==> f = 1.
axiom fiba(1,f) ==> f = 1.
axiom n > 1 & n1 = n - 1 & n2 = n - 2 & fiba(n1,f1) & fiba(n2,f2) & f = f1 + f2 ==> fiba(n,f).
