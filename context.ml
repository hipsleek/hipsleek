  ...
  let r = match c.match_res_type with 
    | Root ->
          ...
          (match lhs_node,rhs_node with
            | DataNode dl, DataNode dr -> !c_strat.root_d_d prog c dl dr
            | ViewNode vl, ViewNode vr -> !c_strat.root_v_v prog c vl vr
            | DataNode dl, ViewNode vr -> !c_strat.root_d_v prog c dl vr
            | ViewNode vl, DataNode dr -> !c_strat.root_v_d prog c vl dr
            | _ -> report_error no_pos "process_one_match unexpected formulas\n"	
          )
    | MaterializedArg (mv,ms) -> 
          (match lhs_node,rhs_node with
            | DataNode dl, DataNode dr -> !c_strat.mater_d_d prog c mv ms dl dr
            | DataNode dl, ViewNode vr -> !c_strat.mater_d_v prog c mv ms dl vr
            | ViewNode vl, ViewNode vr -> !c_strat.mater_v_v prog c mv ms vl vr
            | ViewNode vl, DataNode dr -> !c_strat.mater_v_d prog c mv ms vl dr
            | _ -> report_error no_pos "process_one_match unexpected formulas\n"	
          )
...
and process_matches_x prog lhs_h ((l:match_res list),(rhs_node,rhs_rest)) = match l with
  | [] -> !c_strat.no_match prog lhs_h rhs_node rhs_rest
