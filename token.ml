...
type sleek_token =
  ...
  (*operators*)
  | AND |  ANDAND | AT | LEND | IMM | MUT | DERV | CBRACE | CLIST | COLON | COLONCOLON | COLONCOLONCOLON | COMMA | CPAREN | CSQUARE | DOLLAR | QMARK
...
module Token = struct
  ...
  let to_string k = match k with 
    ...
    | CASE ->"case" | CHECKENTAIL ->"checkentail" | NEG -> "neg" | CAPTURERESIDUE ->"capture_residue" | CLASS ->"class" | CLIST -> "|]" (* | COERCION ->"coercion" *)
  ...
  (*operators*)
    ...
    | DOLLAR ->"$" | QMARK ->"?" | DOT ->"." | DOUBLEQUOTE ->"\"" | DIV -> "/" | EQ ->"=" | EQEQ -> "==" | RIGHTARROW -> "<-"| EQUIV ->"<->" | GT ->">" | GTE ->">= " | HASH ->"#"
