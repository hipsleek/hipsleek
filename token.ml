...
type sleek_token =
  ...
  | LEMMA         of lemma_kind_t
        (*keywords*)
  | ANDLIST
  | ASSERT | ASSERT_EXACT | ASSERT_INEXACT | ASSUME 
  | ALLN | APP | APPEND | AXIOM (* [4/10/2011] An Hoa *)
...
module Token = struct
  ...
  let to_string k = match k with 
    ...
    | ASSERT -> "assert" | ASSERT_EXACT -> "assert_exact" | ASSERT_INEXACT -> "assert_inexact"
    | ASSUME -> "assume" | ALLN-> "alln"
    | APP -> "app" | APPEND -> "append" 
    ...
    | LEMMA TLEM_SPLIT ->"lemma_split" | LEMMA TLEM_PROP ->"lemma_prop"
    | LEMMA TLEM_SAFE ->"lemma_safe" | LEMMA TLEM_INFER ->"lemma_infer"
    | LEMMA TLEM_INFER_PRED ->"lemma_infer_pred" | LET->"let"
    | LIST -> "list"
    | MAX ->"max" | MIN ->"min" | NEW ->"new" | NOTIN ->"notin" | NULL ->"null"
