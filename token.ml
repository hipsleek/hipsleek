...
type sleek_token =
  ...
  (* | PRINT_VIEW *)
  (* | PRINT_VIEW_LONG *)
  | PASS_REF | PASS_REF2 | REL | UNTREL | REQUIRES (*| REQUIRESC*) | RES of string | RETURN
...
module Token = struct
  ...
  let to_string k = match k with
    ...
    |CMP -> "sleek compare" | PASS_REF ->"@R" | PASS_REF2 ->"ref"| REL->"relation" | UNTREL->"untypedrel" |REQUIRES ->"requires" | RES s->"res "^s
