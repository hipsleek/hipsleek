...
type sleek_token =
  ...
  | RELASSUME | SHAPE_INFER | SHAPE_ELIM_USELESS | SHAPE_EXTRACT
  | DTIME
  | ELSE_TT
  | EMPTY
  ...
  (*operators*)  
  | ACCS | AND | ANDSTAR | ANDAND | STARMINUS | AT | ATATSQ | ATAT | LEND | IMM | MUT | DERV | CARET | CBRACE | CLIST | COLON | COLONCOLON | COLONCOLONCOLON | COMMA | CPAREN | CSQUARE | DOLLAR  | VAL | REC
...
module Token = struct
  ...
  let to_string k = match k with 
    ...
    | RELASSUME -> "relAssume" |  SHAPE_INFER -> "shape_infer" | SHAPE_ELIM_USELESS -> "shape_elim_useless" | SHAPE_EXTRACT -> "shape_extract"
  ...
  (*operators*)
    | AND ->"&" |ANDSTAR -> "&*" |  ANDAND ->"&&" | STARMINUS -> "*-" | AT ->"@"  | ATAT -> "@@" |ATATSQ -> "@@[" | LEND->"@L" | ACCS ->"@A" | IMM->"@I"| DERV->"@D"| CARET -> "^"|CBRACE ->"}"| COLON ->":"| COLONCOLON ->"::"| COLONCOLONCOLON -> ":::" | COMMA ->","| CPAREN->")" | CSQUARE ->"]" | VAL ->"@VAL" | REC ->"@REC"
