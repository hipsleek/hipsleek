...
type sleek_token =
  ...
  | IN_T | INT | INTERSECT | INV | INLINE (* An Hoa [22/08/2011] : inline keyword for inline field declaration in structures *)
	| INDUCTION (* An Hoa : keyword for induction spec *)
	| LEMMA | LET
	(*| PLEMMA*)
  | MAX | MIN 
  | NEW | NOTIN | NULL
  ...
  | OP_INC | OP_MOD_ASSIGN | OP_MULT_ASSIGN | OP_SUB_ASSIGN | OR | OROR | PERM | DERIVE | EDERIVE | OSQUARE  | REVERSE | SET | TAIL 
  | PERCENT 
  | PLUS | PRIME 
  | SEMICOLON 
  | STAR | DIV
...
module Token = struct
  ...
  let to_string k = match k with 
    ...
    | IF ->"if" | IN_T ->"in" | INT ->"int"| INTERSECT ->"intersect" | INV->"inv" | INLINE->"inline" (* An Hoa : inline added *)
		| INDUCTION -> "try induction on" (* An Hoa : induction added *)
  ...
  (*operators*)
    | AND ->"&" | ANDAND ->"&&" | AT ->"@"| IMM->"@I"| DERV->"@D"| CBRACE ->"}"| COLON ->":"| COLONCOLON ->"::"| COLONCOLONCOLON -> ":::" | COMMA ->","| CPAREN->")" | CSQUARE ->"]"
    ...
    | DERIVE -> "|-" | EDERIVE -> "|--|" | OSQUARE -> "[" | PERCENT ->"%" | PLUS -> "+" | PRIME -> "'" | SEMICOLON -> ";" | STAR -> "*"
