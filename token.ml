...
type lemma_kind_t = TLEM_TEST | TLEM_TEST_NEW | TLEM | TLEM_UNSAFE | TLEM_INFER
...
module Token = struct
  ...
  let to_string k = match k with 
    ...
    | LEMMA TLEM_INFER ->"lemma_infer" | LET->"let" | MAX ->"max" | MIN ->"min" | NEW ->"new" | NOTIN ->"notin" | NULL ->"null"
