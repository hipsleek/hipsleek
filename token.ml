...
type sleek_token =
  ...
  | OP_INC | OP_MOD_ASSIGN | OP_MULT_ASSIGN | OP_SUB_ASSIGN | OR | OROR | PERM | DERIVE | OSQUARE  | REVERSE | SEQ | SET  | TAIL 
  | PERCENT 
  | PLUS | PRIME 
  | SEMICOLON 
  | STAR | DIV
...
module Token = struct
  ...
  (*operators*)
    ...
    | DERIVE -> "|-" | OSQUARE -> "[" | PERCENT ->"%" | PLUS -> "+" | PRIME -> "'" | SEMICOLON -> ";" | STAR -> "*" | SEQ -> "seq"
