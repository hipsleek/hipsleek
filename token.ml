...
type sleek_token =
  ...
  | RELASSUME | SHAPE_INFER | SHAPE_SPLIT_BASE | SHAPE_ELIM_USELESS | SHAPE_EXTRACT
  | DTIME
  | ELSE_TT
  | EMPTY
  ...
  (*operators*)  
  | ACCS | AND | ANDSTAR | ANDAND | STARMINUS | AT | ATATSQ | ATAT | LEND | IMM | MUT | DERV | CBRACE | CLIST | COLON | COLONCOLON | COLONCOLONCOLON | COMMA | CPAREN | CSQUARE | DOLLAR  | VAL | REC
...
module Token = struct
  ...
  let to_string k = match k with 
    ...
    | RELASSUME -> "relAssume" |  SHAPE_INFER -> "shape_infer" | SHAPE_SPLIT_BASE -> "shape_split_base"
    | SHAPE_ELIM_USELESS -> "shape_elim_useless" | SHAPE_EXTRACT -> "shape_extract"
