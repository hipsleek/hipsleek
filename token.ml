...
type sleek_token =
  ...
  | CASE | CHECKEQ | CHECKENTAIL | CAPTURERESIDUE | CLASS (* | COERCION *) | COMPOSE | CONST | CONTINUE
  ...
  | OP_INC | OP_MOD_ASSIGN | OP_MULT_ASSIGN | OP_SUB_ASSIGN | OR | OROR | PERM | DERIVE | EQV | OSQUARE  | REVERSE | SET | TAIL 
  | PERCENT | PMACRO 
...
module Token = struct
  ...
  let to_string k = match k with 
    ...
    | OFF ->"off" | ON->"on" | ORWORD ->"or" | ANDWORD ->"and" | PRED ->"pred" | DPRINT ->"dprint" |PRINT -> "print" |CMP -> "compare" | REF ->"ref"|REL->"relation" |REQUIRES ->"requires" | RES s->"res "^s 
  ...
  (*operators*)
    ...
    | DERIVE -> "|-" | EQV -> "-|-" |  OSQUARE -> "[" | PERCENT ->"%" | PMACRO -> "PMACRO" | PLUS -> "+" | PRIME -> "'" | SEMICOLON -> ";" | STAR -> "*"
