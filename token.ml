...
type sleek_token =
  ...
  | PRED | PRED_PRIM | PRED_SESS_PROT | PRED_SESS_PROJ | PRED_SESS_TPPROJ | DPRINT | PRED_EXT
  ...
  | TREL_ASSUME
  | INFER_AT_EFA | INFER_AT_DFA | INFER_AT_CLASSIC | INFER_AT_REENTRANCY |  INFER_AT_PAR | INFER_AT_ERRMUST | INFER_AT_ERRMUST_ONLY | INFER_AT_ERRMAY | INFER_AT_DE_EXC | INFER_AT_PREMUST
  | INFER_AT_VER_POST
  ...
  | CARET
  | DOTDOT | ATPOS
  | ACCS | AND | ANDSTAR | ANDAND | UNIONSTAR | STARMINUS | AT | ATATSQ | ATAT | LEND | IMM | MUT | MAT | DERV | SPLIT1Ann | SPLIT2Ann | CBRACE | CLIST | COLON | COLONCOLON | COLONCOLONCOLON | COMMA | CPAREN | CSQUARE | DOLLAR  | INST_PARAM (* | VAL | REC *)
...
module Token = struct
  ...
  let to_string k = match k with
    ...
    | OFF ->"off" | ON->"on" | ORWORD ->"or" | ANDWORD ->"and" | PRED ->"pred" | PRED_PRIM -> "pred_prim" | PRED_SESS_PROT -> "pred_sess_prot" | PRED_SESS_PROJ -> "pred_sess_proj" | PRED_SESS_TPPROJ -> "pred_sess_tpproj" | PRED_EXT ->"pred_extn" | HIP_INCLUDE -> "hip_include"
    ...
    (*operators*)
    | NI ->"@NI" | RO ->"@RO" | ATATSQ -> "@@[" | ACUTE -> "`" | CARET -> "^"
    | DOTDOT ->".."
    ...
    | PRE -> "@pre" | XPRE -> "@xpre" | MUT -> "@M" | MAT -> "@R" | POST -> "@post" | XPOST -> "@xpost" | SUBANN -> "<:" | SAT -> "@S"   | ATT -> "@T" | ATP -> "@P" | INST_PARAM -> "@IP"
    ...
    | INFER_AT_REENTRANCY -> "@reentrancy"
    | INFER_AT_PAR -> "@par"
    | INFER_AT_IMM -> "@imm"
