...
void loop(ref Node l, ref Node result)
	requires l::list<n> * result::list<m>
	ensures result'::list<m+n>;
{
	if (l == null) {
		...
		temp.next = result;
		result = temp;
		return loop(l,result);
	}
}

Node code(Parameter p, ref Transfer t)
	requires p::Parameter<l> * l::list<n> * t::Transfer<from,rcv,amount,0>
	ensures res::list<n> * t'::Transfer<from,rcv,amount,1>;
{
	Node temp = null;
	// LOOP
	loop(p.l,temp);
...
void contract(Parameter p, Transfer t, ref User u1, ref User u2)
	requires p::Parameter<l> * l::list<n> * t::Transfer<from,rcv,amount,0> * u1::user<from,a> * u2::user<rcv,b>
	case {
		a >= amount -> ensures u1'::user<from,a-amount> * u2'::user<rcv,b+amount>;
		a < amount -> ensures u1::user<from,a> * u2::user<rcv,b>;
	}
{
        // Run the code
        t.result = 0;
        if (u1.w.balance >= t.amount) {
                Node result = code(p,t);
