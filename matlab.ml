  ...
  let parse s = (
    try
      ...
      if s.[0] = '{' then (
        ...
        if (abs_float res) = infinity then None
        else Some res
      )
      else None
    with _ -> None
...
let rec partition_by_var e v =
  match e with
  | CP.Var (sp, _) ->
      if CP.eq_spec_var sp v then (1, None)
      else (0, Some e)
  | CP.Add (e1, e2, _) ->
      ...
      let without_v =
        match e1_rest, e2_rest with
        | None, None -> None
        | Some e, None -> e1_rest
        | None, Some e -> e2_rest
        | Some e1, Some e2 -> Some (CP.Add (e1, e2, no_pos)) in
      (with_v, without_v)
  | CP.Subtract (e1, e2, _) ->
      let e1_v, e1_rest = partition_by_var e1 v in
      let e2_v, e2_rest = partition_by_var e2 v in
      ...
      let without_v =
        ...
        | Some e1, Some e2 -> Some (CP.Subtract (e1, e2, no_pos)) in
      (with_v, without_v)
  | _ -> (0, Some e)
...
let elim_exists_with_ineq f =
  Debug.no_1 "elim_exists_with_ineq" !print_formula !print_formula elim_exists_with_ineq f
...
let imply_no_cache_ops pr_w pr_s (f : CP.formula) (imp_no: string) : bool * float =
  ...
  let elim_eq f =
    if !no_elim_exists then f
    else elim_exist_quantifier f in
  let valid f = 
    let wf =
      if (!no_pseudo_ops || CP.is_float_formula f) then f
...
let simplify (f: CP.formula) : CP.formula =
  if (is_linear_formula f  && not (CP.is_float_formula f)) then Omega.simplify f
  else
...
let rec enum_str_prefix prefix n =
  if (n == 0) then []
    ...
    let l = enum_str_prefix prefix (n-1) in
...
let matlab_vars_map (vars : CP.spec_var list) (bv : CP.spec_var list) =
  ...
  let numvars = List.length vars in
    ...
    (newvars, vars_map, vars_rev_map) in
...
let parse_assignment (assignment : string) : (string * string) =
  ...
  with
  | Not_found ->
    let _ = print_string ("parse_assignment is called with input " ^ assignment) in
    ("","")
  ...
  let rec partition (a : (string * string) list) (res : (string * (string list)) list) = 
    ...
    | (lhs,rhs)::a1 -> (
        ...
        | h::t ->
            ...
            let l = snd h in
            let newres =
              if (String.compare rhs r == 0) then
                List.append [(r, List.append [lhs] l)] t
              else List.append [(rhs,[lhs])] res in
            (partition a1 newres)
      ) in
...
let parse_matlab_solution solution (bv : CP.spec_var list) (revmap : (string * CP.spec_var) list) : (CP.spec_var * CP.spec_var) list * (CP.spec_var * string) list=
  ...
  if (l = 0 || solution.[0] = '*') then ([],[]) 
    ...
    let strrep =
      ...
      with
      | Not_found ->
          let _ = print_endline "Assoc NotFound at strrep" in
          [] in
    ...
    let select_sub_cand (c : CP.spec_var list) =
      ...
      if (intc == []) then List.hd c
      else List.hd intc in
...
let solve_eqns (eqns : (CP.exp * CP.exp) list) (bv : CP.spec_var list) =
  ...
  let rec matlab_of_exp varsmap e =
    ...
    | CP.Var (v, _) -> (
        try List.assoc v varsmap
        with 
        | Not_found -> 
            let _ = print_endline ("Variable " ^(CP.string_of_spec_var v) ^ " cannot be found!") in
