...
and cvcl_of_exp a = match a with
  ...
  | CP.IAbs _ -> failwith ("[cvclite.ml]: IAbs is not supported.")
  | CP.FAbs _ -> failwith ("[cvclite.ml]: FAbs is not supported.")
...
and cvcl_of_b_formula b =
  ...
  | CP.PrimTermVar _ -> failwith ("PrimTermVar are not supported in cvclite")
