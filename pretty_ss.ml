...
let print_ident_enhanced f id =
  let name = id_string id in
  if Env.mem name !env then pp_print_string f (name) else pp_print_string f ("_this."^name)
...
and print_data f c =
  ...
  fprintf f "data %a@\n{@\n%a@\n}@\n" pp_print_string (c.cl_name.id) (print_decls c.cl_name.id) field_list
...
(*and print_method f m class_name=
  ...
  fprintf f "%a(%a)"
      (print_method_name_ss class_name) m.m_var (print_comma_list print_var) m.m_formals;
    if m.m_throws <> [] then
      fprintf f " throws %a" (print_comma_list print_name) m.m_throws;
    fprintf f "@\n%a" print_stmt m.m_body
*)

and print_method f m class_name=
  (*let new_formals = enhance_formals m.m_formals class_name in*)
  env := Env.empty;
  let new_formals = {v_mods =[];
    v_type = (TypeName [{id = class_name;pos = -1}]);
    v_name = {id = "_this";pos= -1}}::m.m_formals in
