...
let process_primitives (file_list: string list) : Iast.prog_decl list =
  ...
  if List.for_all (fun x -> Sys.file_exists x) new_names    (* CHECK: all file must exists *)
  then List.map (fun x -> parse_file_full x true) new_names (* - Load files                *)
  else if !Globals.is_ifa
  then [(parse_file_full "./prelude_flow.ss" true)]         (* IFA: load prelude_flow.ss   *)
  else [(parse_file_full "./prelude.ss" true)]              (* DEFAULT: load prelude.ss    *)
