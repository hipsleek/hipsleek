...
let process_source_full source =
  ...
  if (!Scriptarguments.parse_only) then
	...
	let _ = List.map (fun crdef -> Smtsolver.add_relation crdef.Cast.rel_name crdef.Cast.rel_vars crdef.Cast.rel_formula crdef.Cast.rel_induction_values) (List.rev cprog.Cast.prog_rel_decls) in
	let _ = List.map (fun cadef -> Smtsolver.add_axiom cadef.Cast.axiom_hypothesis cadef.Cast.axiom_derive_dir cadef.Cast.axiom_conclusion) (List.rev cprog.Cast.prog_axiom_decls) in
