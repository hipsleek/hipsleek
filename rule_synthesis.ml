...
let choose_rule_assign_end_node (goal: goal) : rule list =
  ...
  let post_vars = post_cond |> get_heap_variables in
...
let choose_rule_assign_end_node goal =
  Debug.no_1 "choose_rule_assign_end_node" pr_goal pr_rules
...
let check_func_arguments goal proc_decl (args: CP.spec_var list) =
  ...
  let called = is_fcall_ever_called goal.gl_trace proc_decl.C.proc_name in
...
let choose_rule_frame_pred goal =
  ...
  let () = x_tinfo_hp (add_str "exists_var" pr_vars) exists_vars no_pos in
  let filter (lhs, rhs) =
...
let choose_synthesis_rules goal : rule list =
  let rules =
    try
      ...
      let _ = choose_rule_assign_end goal |> raise_rules in
