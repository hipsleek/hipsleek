...
let check_pure_entail_sleek prog ante conseq =
  let ante = CF.mkBase_simp CF.HEmp (mix_of_pure ante) in
  let conseq = CF.mkBase_simp CF.HEmp (mix_of_pure conseq) in
  check_entail_sleek prog ante conseq
  |> fst

let check_pure_entail_sleek prog ante conseq =
  Debug.no_2 "check_pure_entail_sleek" Syn.pr_pf Syn.pr_pf string_of_bool
    (fun _ _ -> check_pure_entail_sleek prog ante conseq) ante conseq
...
let check_pure_entail_x ante conseq =
  let sb_ante = SB.translate_pf ante in
  let sb_conseq = SB.translate_pf conseq in
  ...
  let () = x_tinfo_hp (add_str "ent" SBC.pr_ent) ent no_pos in
...
let check_pure_entail ante conseq =
  Debug.no_2 "check_pure_entail" Syn.pr_pf Syn.pr_pf string_of_bool
...
let choose_rule_assign_end_node (goal: Syn.goal) : Syn.rule list =
  ...
  let filter_fun x = CP.mem x all_vars ||
                     CP.is_res_sv x in
...
let choose_rule_assign goal : Syn.rule list =
  ...
  let post_vars = CF.fv post in
  let () = x_binfo_hp (add_str "vars" Syn.pr_vars) post_vars no_pos in
    ...
    let replaced_value =
      if List.mem n_val all_vars then n_val
        ...
        let aux_check_entail replaced_var =
          ...
          let ante = post |> Syn.remove_exists |>  CF.get_pure in
          check_pure_entail_sleek goal.gl_prog ante conseq in
...
let choose_rule_unfold_pre goal =
  ...
  let helper vnode =
    ...
    let () = x_binfo_hp (add_str "nf" (pr_list_mln Syn.pr_f)) nf no_pos in
...
let choose_rule_unfold_post goal =
  Debug.no_1 "choose_rule_unfold_post" Syn.pr_goal Syn.pr_rules
    (fun _ -> choose_rule_unfold_post goal) goal
...
let choose_rule_numeric_end goal =
  ...
  let  aux_pairs cur_var rhs =
    ...
    let () = x_tinfo_hp (add_str "pair" (pr_pair Syn.pr_var Syn.pr_exp))
        (cur_var, rhs) no_pos in
    ...
    let () = x_binfo_hp (add_str "n_pre" Syn.pr_f) n_pre no_pos in
  ...
  let create_templ all_vars cur_var =
    ...
    let pre_pf = CF.get_pure pre |> CP.fv in
    let other_vars = other_vars |> List.filter (fun x -> CP.mem x pre_pf) in
  ...
  let () = x_tinfo_hp (add_str "list1" Syn.pr_rules) list1 no_pos in
  if list1 = [] then
    let list2 = aux_list_vars vars_lhs in
    let () = x_tinfo_hp (add_str "list2" Syn.pr_rules) list2 no_pos in
    list2
  else list1
...
let choose_rule_fread goal =
  ...
  let fix_type_data prog dn_name dn_args =
    try
      let data = List.find (fun x -> eq_str x.CA.data_name dn_name)
          prog.CA.prog_data_decls in
      let fields = data.CA.data_fields |> List.map fst in
      let pairs = List.map2
          (fun x y ->
             let name = CP.name_of_sv x in
             let typ = fst y in
             CP.mk_typed_sv typ name
          ) dn_args fields in
      pairs
    with _ -> failwith "data is not in declared data" in
  let rec helper_hf (hf:CF.h_formula) = match hf with
    | CF.DataNode dnode ->
      ...
      if List.exists (fun x -> CP.eq_spec_var x dn_var) vars then
        ...
        let dn_args = fix_type_data goal.Syn.gl_prog dn_name dn_args in
        [(dn_var, dn_name, dn_args)]
      else []
    | CF.Star sf ->
      let hf1, hf2 = sf.h_formula_star_h1, sf.h_formula_star_h2 in
...
let find_frame_node_var goal all_vars post_var =
  ...
  let pre_pf = pre |> Syn.remove_exists |> CF.get_pure in
  let post_pf = post |> Syn.remove_exists |> CF.get_pure in
  let post_vars = post |> Syn.remove_exists |> CF.fv in
  let post_pf_vars = post_pf |> CP.fv in
  let pf = CP.mkAnd pre_pf post_pf no_pos in
  let helper_arg pre_arg post_arg =
    if CP.mem post_arg post_pf_vars then
      let conseq = CP.mkEqVar pre_arg post_arg no_pos in
      check_pure_entail_sleek goal.gl_prog pf conseq
    else true in
  let helper_arg pre_arg post_arg =
    Debug.no_2 "helper_arg" Syn.pr_var Syn.pr_var string_of_bool
      (fun _ _ -> helper_arg pre_arg post_arg) pre_arg post_arg in
  let helper_hf hf1 hf2 = match hf1, hf2 with
    ...
    | _ -> false in
  let helper pre_var =
    if CP.mem pre_var post_vars
    && not(CP.eq_sv pre_var post_var) then false
    else
      let pre_f = Syn.extract_var_f pre pre_var in
      let post_f = Syn.extract_var_f post post_var in
      match pre_f, post_f with
      | Some f1, Some f2 ->
        let () = x_tinfo_hp (add_str "pre f1" Syn.pr_f) f1 no_pos in
        let () = x_tinfo_hp (add_str "pre f2" Syn.pr_f) f2 no_pos in
        begin
          match f1, f2 with
  ...
  let () = x_tinfo_hp (add_str "frame vars" Syn.pr_vars) frame_vars no_pos in
...
let choose_rule_allocate goal : Syn.rule list =
  ...
  if check_allocate goal pre post then
    ...
    let () = x_tinfo_hp (add_str "rules before" (pr_list Syn.pr_rule_alloc)) rules no_pos in
    let rules = rules |> List.filter filter_eq_var in
    let () = x_tinfo_hp (add_str "rules after" (pr_list Syn.pr_rule_alloc)) rules no_pos in
...
let aux_post_assign goal =
  ...
  let filter_fun (x,y) =
    ...
    not(Syn.is_unk_type_var x) && vars = [] in
  ...
  if Syn.has_mk_null goal.gl_trace then []
    ...
    if List.length trace > 2 then []
      ...
      let filter_rule (rule : Syn.rule_mk_null) =
        ...
        let () = x_binfo_hp (add_str "var" Syn.pr_var) var no_pos in
        (* let n_rules = [] in
         * let n_rules = n_rules @ (choose_rule_allocate_return n_goal) in
         * (\* let n_rules = n_rules @ (choose_rule_func_call n_goal) in *\)
         * let n_rules = n_rules @ (choose_rule_fwrite n_goal) in
      ...
      let () = x_binfo_hp (add_str "list1" (pr_list Syn.pr_rule_mk_null)) list1 no_pos in
      let list2 = aux_post_assign goal in
      let () = x_binfo_hp (add_str "list2" (pr_list Syn.pr_rule_mk_null)) list2 no_pos in
      (* let list2 = [] in *)
      let list = if list2 = [] then list1 else list2 in
      let list = list |> List.map filter_rule
...
let choose_rule_skip goal =
  Debug.no_1 "choose_rule_skip" Syn.pr_goal Syn.pr_rules
    (fun _ -> choose_rule_skip goal) goal
