...
{
	if (x == null) return 0;
	else return 1 + get_mem_count(x.next);
}
...
requires  pq0::ll<n>*pq1::ll<n1> * pq2::ll<n2> * pq3::ll<n3> & next_pid=0 & curJob=null & Term
ensures true;
{
    int command, prio;
    int ratio;
...
int new_job(int prio,ref int npid,ref node curJob,ref node pq1, ref node pq2, ref node pq3)
  requires pq1::ll<n1>*pq2::ll<n2>*pq3::ll<n3> & prio>=1 & prio<=3 & Term
case {
...
int upgrade_prio(int prio,int ratio,ref node curJob,ref node pq1,ref node pq2,ref node pq3)
  requires pq1::ll<n1>*pq2::ll<n2>*pq3::ll<n3> & Term
  case{
    prio = 1 -> case {
...
int block(ref node curJob, ref node pq0, ref node pq1, ref node pq2, ref node pq3)
      requires pq0::ll<n>*pq1::ll<n1>*pq2::ll<n2>*pq3::ll<n3> & Term
...
void free(ref node job)
  requires job::node<_,_,null> & Term
  ensures job'=null;//'
...
int flush(ref node curJob, ref node pq1, ref node pq2, ref node pq3)
  requires pq1::ll<n1>*pq2::ll<n2>*pq3::ll<n3>
 case {
  curJob = null & n1=0 & n2=0 & n3=0 -> requires Term ensures pq1'=null & pq2'=null & pq3'=null &
  curJob' = null & res=0;//'
  curJob != null | n1!=0 | n2!=0 | n3!=0 -> requires curJob::node<_,_,null> & Term[n1+n2+n3]
  ensures pq1'::ll<n4>*pq2'::ll<n5>*pq3'::ll<n6> &
      n4+n5+n6<=n1+n2+n3 & res=0;//'
}
{
...
int schedule(int command, int prio, int ratio,ref node curJob,ref node pq0, ref node pq1, ref node pq2, ref node pq3)
    requires pq0::ll<n>*pq1::ll<n1>*pq2::ll<n2>*pq3::ll<n3> & command >= 1 & command <= 7 & prio>=1 & prio<=3 & Term
 case {
  command = 1 ->  case {
    curJob=null -> ensures true;
...
node lput_end(node x, node y)
 ...
 case {
  x = null -> requires Term ensures y::node<v1,v2,null> & res = y;//'
  x != null -> requires x::ll<n> & Term[n] ensures res::lseg<y,n>*y::node<v1,v2,null>;//'
    }
{
  if (x==null)
    {
...
node del_ele(ref node x, node ele)
  requires (exists j,q: x::lseg<ele,j> * ele::node<v1,v2,q> * q::ll<m> & Term) //&  v2 >=0 & v2 <= 3
