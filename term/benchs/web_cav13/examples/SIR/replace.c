...
bool addstr(int c, ref int [] outset, ref int j, int maxset)
  requires dom(outset,0,maxset) & maxset>=0 & Term
 case {
...
int esc(ref int[] s, ref int i)
  requires dom(s,0,100) & i>=0 & i<100 & Term
 case {
...
void dodash_loop1(int[] src, ref int i, int [] dest, ref int j, int maxset,
 ...
 case {
  k<=src[i+1] -> requires Term[src[i+1]-k] ensures j'>=j & j'<=maxset & i'=i; //'
  k>src[i+1] -> requires Term ensures j'=j & i'=i;//'
}
{
  if (k<=src[i+1]){
...
void dodash(int delim, ref int[] src, ref int i, ref int [] dest, ref int j, int maxset)
  requires dom(src,0,100) & dom(dest,0,100) & src[100]=0 & i>=1 & i<=100 & maxset=100
                             & delim = 93 & j>=0 & j<=maxset & Term[100-i]
case{
  i=100 -> case {
...
bool getccl(int [] arg, ref int i, ref int[] pat, ref int j)
  requires dom(arg,0,100) & dom(pat,0,100) & arg[100]=0 & i>=0 & i<100 & j>=0 & j<=100 & Term
  ensures true;
{
    int	jstart;
    bool junk;
...
void stclose_loop(ref int[] pat, int lastj, ref int jp)
  requires dom(pat,0,100) & jp>=-1 & jp<=99 & lastj>=0 & lastj<=100 & Term[jp+1]
 case {
...
void stclose(ref int[] pat, ref int j, int lastj)
  requires dom(pat,0,100) & lastj>=0 & lastj<=100 & j>=0 & j<=100 & Term
...
start>=0 & start<=100 & delim=0 & Term
  ensures true;
{
    int	result;
    int	i, j, lastj, lj;
...
bool getpat(int[] arg,ref int[] pat)
  requires dom(arg, 0, 100) &dom(pat, 0, 100)& arg[100]=0 & Term
  ensures true;
{
    int	makeres;
...
void makesub_loop(ref int[] arg, int from, int delim, ref int[] sub,ref int i, ref int j)
 ...
 case{
  ...
  i<100 -> requires Term[100-i] case {
...
int makesub(int[] arg, int from, int delim, ref int[] sub)
  requires dom(arg,0,100) & dom(sub,0,100) & arg[100]=0 & from=0 & delim=0 & Term
  ensures true;
{
    int  result;
    int	i, j;
...
bool getsub(int[] arg, ref int[] sub)
  requires dom(arg,0,100) & dom(sub,0,100) & arg[100]=0 & Term
  ensures true;
{
    int	makeres;
...
requires dom(pat,0,100) & offset>=0 & offset<=100 & (i <= offset+pat[offset]) & (offset+pat[offset]<100) & !flag
 case{
  i>offset -> case {
    c=pat[i] -> requires Term ensures dom(pat',0,100) & flag' & i'>offset;//'
    c!=pat[i] -> requires Term[i] ensures dom(pat',0,100) & (flag' & i'>offset)|(!flag' & i'=offset);//'
  }
  i<=offset -> requires Term case {
...
bool locate(int c, int[] pat, int offset)
  requires dom(pat,0,100) & (offset+1<100) & offset>=0 & Term
  case {
...
bool omatch(int [] lin, ref int i, int[] pat, int j)
  requires dom(lin,0,100) & dom(pat,0,100) & i>=0 & i<=100 & j>=0 & j<98 & Term
 case{
lin[i]=0 -> ensures !res;
lin[i]!=0 -> case {
...
int patsize(ref int [] pat, int n)
  requires dom(pat,0,100) & n>=0 & n<99 & Term
case {
...
int amatch(int[] lin, int offset, ref int[] pat, int j)
  requires Term
  ensures res>=-1 & res<=100 & res>=offset;
/*
{
    int	i, k;
    bool result, done;
...
case {
  ...
  i<100 -> requires Term[100-i] ensures (i'>=0 & i'<=100);
...
void subline(int [] lin, ref int[] pat, ref int[] sub)
  requires dom(lin,0,100) & lin[100]=0 & dom(sub,0,100) & Term
  ensures true;
{
  int i, lastm, m=-1;
