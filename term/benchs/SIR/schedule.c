...
node append_ele(ref node x, node a, int prio)
  requires x::ll<n> * a::node<v1,v2,null> & v2=prio & Term
...
node find_nth_helper(node l, int j)
  requires l::lseg<p,n> & n>=j & j>=1 & Term[j]
  ensures res::lseg<p,m> * l::lseg<res,j-1>  & (m=n-j+1) & res!=null;
{
  if (j>1){
    //assume false;
...
node find_nth(node f_list, int j)
  requires f_list::ll<n> & n>=j & j>=1 & Term
...
node find_nth2(node f_list, int j)
 requires f_list::ll<n> & j>=1 & n>=1 & Term
 case {
...
node del_ele(ref node x, node ele)
  requires (exists j,q: x::lseg<ele,j> * ele::node<v1,v2,q> * q::ll<m> & Term) //&  v2 >=1 & v2 <= 3
...
void finish_process(ref node pq1,ref node pq2,ref node pq3, ref node cur_proc, ref int num_processes)
  requires  pq1::ll<n1> * pq2::ll<n2> * pq3::ll<n3> * cur_proc::ll<n> & Term
case{
  n3 > 0 ->
...
requires pq1::ll<n1> * pq2::ll<n2> & ratio >=1 & Term
  case {
    n1 > 0 ->  case {
...
requires pq1::ll<n1> * pq2::ll<n2> * pq3::ll<n3> & prio>0 & prio <=3 & ratio >=1 & Term
 case {
  prio = 1 -> case {
    n1 > 0 ->  case {
...
void unblock_process(int ratio, ref node bq, ref node pq1, ref node pq2, ref node pq3)
 requires pq1::ll<n1> * pq2::ll<n2> * pq3::ll<n3> & ratio >=1 & Term
  case {
  bq != null -> requires bq::ll<m> & m >0
 case{
...
void quantum_expire(ref node cur_proc, ref node pq1, ref node pq2, ref node pq3)
  requires cur_proc::ll<n> * pq1::ll<n1> * pq2::ll<n2> * pq3::ll<n3> & Term
 case {
...
void block_process(ref node cur_proc, ref node block_queue, ref node pq1, ref node pq2, ref node pq3)
  requires cur_proc::ll<n> * block_queue::ll<n> * pq1::ll<n1> * pq2::ll<n2> * pq3::ll<n3> & Term
case {
...
void add_process(int prio, ref int alloc_proc_num, ref int num_processes, ref node pq1, ref node pq2, ref node pq3)
  requires pq1::ll<n1> * pq2::ll<n2> * pq3::ll<n3> & prio>0 & prio<=3 & Term
     case {
...
node init_prio_queue_helper(node queue,int prio,int i,int n/*, ref int alloc_proc_num, ref int num_processes*/)
      requires queue::ll<i> & prio>0 & prio<=3 & n >= 0 & i<=n & Term[n-i]
  ensures res::ll<n> ;
...
void init_prio_queue(int prio, int num_proc, ref node pq1, ref node pq2, ref node pq3)
  /*ref int alloc_proc_num, ref int num_processes,*/
   requires pq1::ll<n1> * pq2::ll<n2> * pq3::ll<n3> & prio>0 & prio<=3 & num_proc>=0 & Term
  case {
...
void main_helper(int i, int maxprio, ref node pq1, ref node pq2, ref node pq3)
  requires pq1::ll<n1> * pq2::ll<n2> * pq3::ll<n3> & i >= 0 & i <= maxprio & maxprio = 3 & Term[i]
...
void main(int argc)
  requires prio_queue1::ll<n1> * prio_queue2::ll<n2> * prio_queue3::ll<n3> & Term
  ensures true;
{
    int command;
    int prio;
