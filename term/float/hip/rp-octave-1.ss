...
void loop1 (float y)
  case
  {
    ...
    y > 0.0 & y < 0.5   ->  requires Term[SeqConDec(-y, -infinity, -1.0 / 65536.0 / 65536.0)] ensures true;
  }
{
  if (y <= 0.0)
    return;
...
void loop2 (float y)
  case
  {
    y <= 0.0            ->  requires Term[] ensures true;
    y >= 0.5            ->  requires Term[] ensures true;
    y > 0.0 & y < 0.5   ->  requires Term[SeqConDec(-y, -infinity, y > 1.0 / 65536.0 / 65536.0)] ensures true;
  }
{
  if (y <= 0.0)
    return;
