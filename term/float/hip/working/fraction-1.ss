...
void loop1f(float x)
  case {
    x > 1.99     -> requires Term[SeqDec{x-2.0, (0, +infinity)}] ensures true;
...
void tmploop1(float x)
  case {
    ...
    0 > x > -0.1 -> requires Term[SeqDec{-x, (0, 0.1)}] ensures true;
    x = 0 -> requires Loop ensures false;
    x > 0 -> requires Term[SeqDec{x, (0, +infinity)}] ensures true;
  }
{
  if (x > -0.1)
    tmploop1(x/2.0);
