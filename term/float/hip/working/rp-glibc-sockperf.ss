...
void loop1(float z)
  case
  {
    ...
    (1 + __sqrt(5))/2 < z < 4 -> requires Term[SeqDec{-z, (-infinity, -(1+__sqrt(5))/2), __abs(z) >= 4 }] ensures true;
    ...
    -4 < z < (1 - __sqrt(5))/2 -> requires Term[SeqDec{-__abs(z), (-infinity, -(1+__sqrt(5))/2), __abs(z) >= 4 }] ensures true;
    z <= -4 -> requires Term ensures true;
  }
{
  if (abs(z) < 4.0)
