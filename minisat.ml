...
(* | VarPerm _ -> Error.report_no_pattern () *)
  | Security _ | ExplicitFlow _ | ImplicitFlow _ -> illegal_format "Security formula not supported by MiniSAT"
...
let return_pure bf f= match bf with
  | (pf,_)-> match pf with
    ...
    | Security _ | ExplicitFlow _ | ImplicitFlow _ -> failwith "MiniSAT cannot handle security formula"
...
let rec can_minisat_handle_expression (exp: Cpure.exp) : bool =
  ...
  | Cpure.BExpr f -> can_minisat_handle_formula f
