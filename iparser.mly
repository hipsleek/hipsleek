...
(*let rec split_specs specs = match specs with
  ...
  let rec split_specs specs = (specs,[])

  (*let rec remove_spec_qualifier (_, pre, post) = (pre, post)*)
	let remove_spec_qualifier (p:Iast.spec) :Iast.spec = p
%}
%token AND
%token ANDAND
%token ASSERT
	...
	| CASE OBRACE branch_list CBRACE 
		{
			Iast.SCase 
				{
						Iast.scase_branches = $3; 
						Iast.scase_pos = get_pos 1; 
				}
			} 
;
...
assert_statement
 ...
 /* | ASSERT extended_constr ASSUME SEMICOLON {
	  Assert { exp_assert_asserted_formula = Some $2;
...
while_statement
  : WHILE OPAREN boolean_expression CPAREN embedded_statement {
	  While { exp_while_condition = $3;
			  exp_while_body = $5;
			  exp_while_specs = Iast.mkSpecTrue no_pos;
