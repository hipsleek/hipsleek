...
bst <sm, lg, n> == self = null & sm <= lg & n = 0 
	or (exists pl,qs: self::node2<v, p, q> * p::bst<sm, pl, n1> * q::bst<qs, lg, n2> & pl <= v & qs >= v & n=1+n1+n2)
	inv n>=0 & sm <= lg;
...
node2 insert(node2 x, int a)
	...
	requires x::bst<sm, lg, n>
    variance (1) [n]
	ensures res::bst<mi, ma, _> & res != null & mi = min(sm, a) & ma = max(lg, a);
...
int remove_min(ref node2 x)
	...
	requires x::bst<s, b, n> & x != null
    variance (1) [n]
	ensures x'::bst<s1, b, _> & s <= res <= s1;
