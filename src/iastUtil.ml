  ...
  let rec helper (in_arg:'b) (e:exp) :(exp* 'a) =
    ...
    | None  ->
      ...
      | CallNRecv b ->
        ...
        let e2l,r2l = List.split ler in
        let ler_ex = List.map (helper n_arg) b.exp_call_nrecv_extra_arg in
        let e2l_ex,r2l_ex = List.split ler_ex in
        let r = comb_f (r2l@r2l_ex) in
        (CallNRecv {b with exp_call_nrecv_arguments = e2l;
                           exp_call_nrecv_extra_arg = e2l_ex;
                   },r)
      | Cast b ->
...
let rename_exp2 e subs =
  ...
  let rec rename_exp (e:exp) ((bvars,subs):(IS.t)*((ident * ident) list)) : exp =
    ...
    let f (bvars, subs) e =
      ...
      | CallNRecv b ->
        (* Andreeac: Why not push below into map_exp_args? The way it is built now this avoids renaming of args in favor of renaming ho_args *)
        if (List.length b.exp_call_nrecv_ho_arg > 0) then
...
let add_free_var_to_proc gvdefs ht proc =
  ...
  Debug.no_1 "add_free_var_to_proc" pr pr (fun _ -> add_free_var_to_proc gvdefs ht proc) proc
