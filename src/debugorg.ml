(*
  Debugging module from Heifer system

  Provides org mode output for call trace and CTF for Perfetto
*)
...
(* TRISTAN : commented out because it doesn't work for now
...
module Query = struct
  ...
  (* TRISTAN : set user_query statically *)
  let user_query : query ref = ref [(Show, (LogLevel 1), true)]
  ...
  (* TRISTAN : query_action * query_on * bool *)
...
let debug_print at title s =
  ...
  | true ->
    ...
    | false ->
      ...
      in
      begin
        (* TRISTAN : change from true to false to prevent collapsing of span
           resulting in levels of call trace to be off *)
        let should_buffer = false in
...
let span show k =
  ...
  is_opening := true;
  (* TRISTAN : to print on entry of function *)
  show None;
  is_opening := false;
  (* in *)
  stack := (!last_title, start) :: !stack;
  ...
  is_closing := true;
  (* TRISTAN : to print on exit of function *)
  show (Some r);
  is_closing := false;
...
let init ctf_output query to_file =
  ...
  if ctf_output then (
    ...
    Format.fprintf (!trace_out |> Option.get) "[@.");
  (*
  TRISTAN : Str.string_match always return false in parse_query,
  therefore unable to parse_query properly.
  Current solution : set user_query statically to print all logs.
  This is fine since control on which logs to print is managed by original
  Debug.ml
