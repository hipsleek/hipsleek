...
let sub_type = ref (fun (t1:typ) (t2:typ) -> false)
...
let rec filter_chr_dependencies_x efv f =
  ...
  let chr_free_vars = chr_free_vars @ efv in
...
let filter_chr_dependencies efv f =
  let pr = !CP.print_formula in
  let pr_bool = (add_str "contains CHR rels" string_of_bool) in
  let pr_out = pr_triple pr_bool pr pr in
  Debug.no_1 "filter_chr_dependencies" pr pr_out (filter_chr_dependencies_x efv) f

let preprocess_ord2sleek efv pform =
  let form =  Sess_orders_relation.trans_ord2sleek_rels_in_pure_formula pform in
  filter_chr_dependencies efv form

let wrapper_enable_ord2sleek ?extrafv:(efv=[]) f =
...
let wrapper_remove_tvar ?sat:(sat = true) form =
  let fp bform =
    let (pf,il) = bform in
    match pf with
    | CP.TVar (var,ty,pos) ->
      let ty_var = CP.get_exp_type var in
      if (!sub_type ty_var ty) || sat then
        Some (CP.mkTrue_p pos,il)
      else raise Type_mismatch (* failwith "Type mismatch" *)
    | _ -> None
  in
...
let wrapper_remove_tvar ?sat:(sat = true) form =
  let pr = !CP.print_formula in
  Debug.no_1 "wrapper_remove_tvar" pr pr (fun f -> wrapper_remove_tvar ~sat:sat f) form
...
let sat_label_filter fct f =
  ...
  let rec helper_x f = match f with
    | AndList b ->
      ...
      let res = List.exists (fun (lbl,f) -> (test ~lbl:lbl f)=false) fs in
      not(res)
...
let imply_label_filter ante conseq =
  ...
  | AndList ba, _ ->
    (* TODO Andreea: how should we settle the label in this case? *)
    (* let lbl = List.fold_left (fun a (lbl,_) -> if 0 == LO.compare lbl (LO.unlabelled) then a else lbl ) LO.unlabelled ba in *)
    (* let res = List.map (fun (lante,ante) -> (lbl, ante,conseq)) ba in *)
    (* res  *)
    [(LO.unlabelled,andl_to_and ante,conseq)]
  (*if (List.for_all (fun (_,c)-> no_andl c) ba)&& no_andl conseq then () else print_string s;*)
  | _ , AndList bc ->  List.map (fun (lbl,c)-> (lbl,ante,c)) bc
...
let tp_imply ante conseq old_imp_no timeout process =
  try
    tp_imply ante conseq old_imp_no timeout process
...
let imply_timeout_univ univ_vars ante0 conseq0 imp_no timeout process =
    ...
    let (b,_,_) as res = x_add imply_timeout ante0 new_conseq imp_no timeout process in
    let () = y_dinfo_hp (add_str "imply_timeout_univ: b " string_of_bool) b in
    if b then
      let () = univ_rhs_store # set conseq0 in res
    else res
    (* res *)
