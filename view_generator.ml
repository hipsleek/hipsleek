...
let rec gen_ext_view (prog : C.prog_decl) (c : C.data_decl) : C.proc_decl =
  ...
  if (String.compare c.C.data_parent_name "Object") != 0 then
    begin
      ...
      let lhs_base_heap = CF.ViewNode({ CF.h_formula_view_node = CP.SpecVar(mkNamedTyp(c.C.data_name),
                                                                            fn2,
                                                                            Unprimed);
      ...
      let lhs_base_type = CF.TypeSub({
          CF.t_formula_sub_type_var = CP.SpecVar(mkNamedTyp(c.C.data_name),
                                                 actual_type_var,
                                                 Unprimed);
      ...
      let extension_arg = CP.SpecVar(mkNamedTyp("Ext" ^ c.C.data_name),
                                     extension_var,
                                     Unprimed) in
      let h_formula_star_h1 = CF.ViewNode({
          CF.h_formula_view_node = CP.SpecVar(mkNamedTyp(parent.C.data_name),
                                              fn1,
                                              Unprimed);
      ...
      let new_proc = {
        ...
        C.proc_args = [(mkNamedTyp(c.C.data_name), fn2)];
