...
let string_of_proc_decl p =
  ...
  ^ p.proc_name
  ^ (match p.proc_poly_vars with | [] -> "" | ha -> " " ^ (pr_list pr_id ha))
  ^ "(" ^ (string_of_param_list p.proc_args) ^ ")"
  ^ (match p.proc_ho_arg with | [] -> "" | ha -> " with " ^ (pr_list string_of_param ha))
  ^ (match p.proc_extra_arg with | [] -> "" | ha -> " with " ^ (pr_list string_of_param ha))
  ^ "[" ^ p.proc_mingled_name ^ "]"
  ^ "\n"
