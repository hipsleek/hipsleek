...
and string_of_h_formula_repair = function
  | F.Star ({F.h_formula_star_h1 = f1;
             F.h_formula_star_h2 = f2;
             F.h_formula_star_pos = l} ) ->
    (* if is_bool_f f1 then *)
  ...
  | F.Phase ({F.h_formula_phase_rd = f1;
    ...
    if is_bool_f f1 then
      if is_bool_f f2 then ""
      else string_of_h_formula_repair f2
    else
    if is_bool_f f2 then string_of_h_formula_repair f1
    else
      "(" ^ (string_of_h_formula f1) ^ ") ; (" ^ (string_of_h_formula f2) ^ ")"
  | F.HeapNode ({F.h_formula_heap_node = x;
                 F.h_formula_heap_name = id;
    ...
    ((string_of_id x) ^ "::" ^ id ^ !deref_str ^ perm_str
     ^ "<" ^ (string_of_data_param_list pl ann_param) ^ ">" ^ (string_of_imm imm))
  | F.HeapNode2 ({F.h_formula_heap2_node = xid;
                  F.h_formula_heap2_name = id;
                  F.h_formula_heap2_deref = deref;
                  F.h_formula_heap2_label = pi;
...
and string_of_formula_repair = function
  | Iast.F.Base ({F.formula_base_heap = hf;
                  F.formula_base_pure = pf;
                  F.formula_base_vperm = vp;
                  F.formula_base_flow = fl;
...
and string_of_struc_formula_repair c = match c with
  ...
  | F.EBase {
    ...
    let c = match cont with
      | None -> ""
      | Some l -> string_of_struc_formula_repair l in
    "requires " ^ b ^ "\n" ^ c ^ ";"
  | F.EAssume {
      F.formula_assume_simpl = b;
      F.formula_assume_struc = s;
...
let rec string_of_exp_repair = function
  ...
  | Catch c ->
    ("catch (" ^
     (match c.exp_catch_var with
      | Some x-> x
      | None -> "") ^ ": " ^ c.exp_catch_flow_type ^ ")\n"
     ^ (string_of_exp_repair c.exp_catch_body)
    )

  | Empty l -> ""
  | Finally c->  ("finally " ^ (string_of_exp_repair c.exp_finally_body))
  | Unary {exp_unary_op = o;
            exp_unary_exp = e;
            exp_unary_pos = l} ->
    (match o with
     | OpPostInc | OpPostDec ->
       (if need_parenthesis2 e then
          (parenthesis (string_of_exp_repair e)) ^ (string_of_unary_op o)
        else (string_of_exp_repair e) ^ (string_of_unary_op o))
     | _ ->
       (if need_parenthesis2 e then
          (string_of_unary_op o) ^ (parenthesis (string_of_exp_repair e))
        else (string_of_unary_op o) ^ (string_of_exp_repair e)))
  | Binary {exp_binary_op = o;
    ...
    if need_parenthesis2 e1 then
      if need_parenthesis2 e2 then
        ...
        ^ (parenthesis (string_of_exp_repair e2))
      else (parenthesis (string_of_exp_repair e1)) ^ (string_of_binary_op o)
           ^ (string_of_exp_repair e2)
    else if need_parenthesis2 e2
    then (string_of_exp_repair e1) ^ (string_of_binary_op o)
         ^ (parenthesis (string_of_exp_repair e2))
    else (string_of_exp_repair e1) ^ (string_of_binary_op o)
         ^ (string_of_exp_repair e2)
  ...
  | CallRecv {exp_call_recv_receiver = recv;
    ...
    exp_call_recv_arguments = el} ->
        ((string_of_exp_repair recv) ^ "." ^ id ^ "(" ^ (string_of_exp_list el ",") ^ ")")
  | ArrayAlloc ({exp_aalloc_etype_name = elm_type;
                 exp_aalloc_dimensions = dims})
    -> "new " ^ elm_type ^ "[" ^ (string_of_exp_list dims ",") ^ "]"
  | New ({exp_new_class_name = id;
          exp_new_arguments = el})
    -> "new " ^ id ^ "(" ^ (string_of_exp_list el ",") ^ ")"
  | Var ({exp_var_name = v}) -> v
  | Member {exp_member_base = e; exp_member_fields = idl} ->
    let base_str = (
      if (need_parenthesis2 e) then ("(" ^ (string_of_exp_repair e) ^ ")")
      else (string_of_exp_repair e)
    ) in
    let newexp = (
      match idl with
      | _ ->
  ...
  | Assign {exp_assign_op = op;
            exp_assign_lhs = e1;
            exp_assign_rhs = e2} ->
    (string_of_exp_repair e1) ^ (string_of_assign_op op) ^
    (string_of_exp_repair e2) ^ ";"
  ...
  | Cond {exp_cond_condition = e1;
    ...
    ("if " ^ (parenthesis (string_of_exp_repair e1))
     ...
     (match e3 with
      | Empty ll -> ""
      | _ ->
        (if (string_of_exp_repair e3 = "") then ""
             else " else {\n" ^ (string_of_exp_repair e3) ^ "\n}")))

  | While {exp_while_condition = e1;
           exp_while_body = e2;
           exp_while_jump_label = lb;
           exp_while_specs = li} ->
    " while " ^ (parenthesis (string_of_exp_repair e1)) ^ " \n"
  ...
  | Raise {exp_raise_type = tb;
    ...
    string_of_control_path_id_opt pid
      ("raise "^(match b with
           | None -> ft
           | Some bs-> "EXPR:" ^ ft ^ (string_of_exp_repair bs)) ^ "\n")
  | Try ({	exp_try_block = bl;
           exp_catch_clauses = cl;
           exp_finally_clause = fl;})
    -> "try {" ^ (string_of_exp_repair bl) ^ "\n}" ^
       (List.fold_left (fun a b -> a^"\n"^(string_of_exp_repair b)) "" cl) ^
       (List.fold_left (fun a b -> a^"\n"^(string_of_exp_repair b)) "" fl)
  ...
  | Par ({ exp_par_vperm = vps; exp_par_lend_heap = lh; exp_par_cases = cl }) ->
        let string_of_par_case c =
          ...
          in
          cond_str ^ (string_of_exp_repair c.exp_par_case_body)
        in
...
and string_of_exp_list l c = match l with
        | []                          -> ""
        | h::[]                       -> string_of_exp_repair h
        | h::t 	                   -> (string_of_exp_repair h) ^ c ^ " "
                                      ^ (string_of_exp_list t c)
...
and string_of_assigning_list l = match l with
        ...
        | (id, eo, l)::t 	           -> id ^ (match eo with
            | None    -> ""
            | Some e  -> " = " ^ (string_of_exp_repair e))
                                        ^ ", " ^ (string_of_assigning_list t)
and string_of_cassigning_list l = match l with
        | []                          -> ""
        | (id, e, l)::[]              -> id ^ "=" ^ (string_of_exp_repair e)
        | (id, e, l)::t 	           -> id ^ "=" ^ (string_of_exp_repair e)
                                       ^ ", " ^ (string_of_cassigning_list t)
...
let string_of_proc_decl_repair p =
  let body = match p.proc_body with
    | None     -> ""
    | Some e   -> "{\n" ^ (string_of_exp_repair e) ^ "\n}" in
...
let string_of_exp_decl exp_decl =
  let pr = pr_list (fun (x, y) -> (string_of_typ x) ^ " " ^ y) in
  let body = match exp_decl.exp_body with
    | ExpForm e -> string_of_formula_exp e
    | ExpUnk -> "?."
  in
  let exp_name = exp_decl.exp_name in
  let typ_str = string_of_typ exp_decl.exp_ret_typ in
  "func " ^ typ_str ^ " " ^ exp_name ^ "(" ^ (pr exp_decl.exp_typed_params) ^ ") == " ^ body
...
let string_of_data_repair cdef =
  let meth_str =
    let dd=cdef.data_methods in
    if dd==[] then ""
    else "\n"^(String.concat "\n" (List.map string_of_proc_decl dd)) in
