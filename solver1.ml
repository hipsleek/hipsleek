 ...
 *   - otherwise raise exception
 *)
let compute_wellformed_condition (f: CF.formula) : CP.formula =
  (* compute wellformed condition of a h_formula *)
  let wellformed_condition_of_hformula (hf: CF.h_formula) : CP.formula = (
  ...
  | CF.Base bf ->
      ...
      let heap = bf.CF.formula_base_heap in
      let wellformed_heap = wellformed_condition_of_hformula heap in
      let pure = MCP.pure_of_mix (bf.CF.formula_base_pure) in
      (CP.mkAnd wellformed_heap pure pos)
  | CF.Exists _ ->
...
let match_spatial_formula (tbl_stack: (CP.spec_var, int) Hashtbl.t) 
    (alloc: (CP.spec_var -> CP.formula)) (f1: CF.formula) (f2: CF.formula)
    : CF.formula =
 ...
 *)
let rec preprocess_hformula (hf: CF.h_formula) : boolean =
  match hf with
  | CF.Star sf ->
      let f1 = sf.CF.h_formula_star_h1 in
      let f2 = sf.CF.h_formula_star_h2 in
      (proprocess_hformula f1) && (preprocess_hformula f2)
  | CF.ViewNode _ -> true
  | CF.DataNode _ -> true
  | CF.HEmp -> true
  | _ -> false
...
let preprocess_formula (f: CF.formula) : boolean =
  match f with
  | CF.Base bf ->
