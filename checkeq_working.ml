...
and checkeq_formulas_one (hvars: ident list) (f1: CF.formula) (f2: CF.formula)(mtl: (map_table list)): (bool*(map_table list))=
  ...
  match f1 with
	...
	|CF.Or ({CF.formula_or_f1 = f21;
	  ...
	  check_or f11 f12 f21 f22 hvars mtl
	)
	|_ -> (false,[]))
...
and check_or f1 f2 hvars mtl =
  ...
  let check_one2 f1 f2 hvars mt = 
    match f2 with 
      | Or ({CF.formula_or_f1 = f21;
	     CF.formula_or_f2 = f22})  -> (
	let (res1, mtl1) = check_one2 f1 f21 hvars mt in
	let (res2, mtl2) = check_one2 f1 f22 hvars mt in
	let new_mix_mtl mlt f2 = List.map (fun (mt,f) -> if isStrictConstTrue f then else (mt,f2) if isStrictConstTrue f2 then (mt,f) else (mt, mkOr f f2 no_pos)) mtl in 
	if(res1 && res2) then 
	  (
	    let mix_mtl1 = new_mix_mtl mlt1 f22 in 
	    let mix_mtl2 = new_mix_mtl mlt2 f21 in 
	    (true, mix_mtl1@mix_mtl2)
	  )
	else if(res1) then let mix_mtl1 = new_mix_mtl mlt1 f22 in (true,mix_mtl1)
	else if(res2) then let mix_mtl2 = new_mix_mtl mlt2 f21 in (true,mix_mtl2)
	else (false,[])
      )
      | _ ->  let (res,mtl) = checkeq_formulas_a hvars f1 f2 [mt] in
	      (res, new_mtl mtl (CF.mkTrue_nf no_pos))
  in
  let check_one1 f1 hvars mix_mtl=
    match f1 with
      | Or ({CF.formula_or_f1 = f11;
	...
	if(res1) then (
      ...
      | _ -> (
	let res_list = List_map (fun (mt,f2) -> let r,l = check_one2 f1 f2 hvars mt in (r, (mt,l))) mix_mtl in
...
let checkeq_defs_with_diff_x hvars (defs: (CP.rel_cat * CF.h_formula * CF.formula) list) ( infile_defs: (CF.formula * CF.formula) list) inf_vars :  (bool*(((CF.formula * CF.formula) *  (CF.formula * CF.formula) * ((map_table * (CF.formula * CF.formula)*(CF.formula * CF.formula)) list)) list))=
  ...
  if(res) then (res,[])
    ...
    let rec check_hps pairs = (
      let find_hpdef p defs infile_defs =
	let (v1,v2) = p in
	try (
	  let check_hp hp v =
	...
	with Not_found -> report_error no_pos "Diff HP not found in either defs or infile_defs"
      in
      let check_each_pair p defs infile_defs =
	let (def1,def2) = find_hpdef p defs infile_defs in
	let b,mtl =  check_equiv_constr_with_diff hvars def1 def2 in
	if(b) then (b,[])
	else 
      in
      let hps = List.map (fun p -> find_hpdef p defs infile_defs) pairs in
      let res_list = List.map (fun (d1,d2) -> let b,mtl =  check_equiv_constr_with_diff hvars d1 d2 in (d1,d2,mtl)) hps in
      res_list
    )
    in
    let pairs = List.map (fun v -> (v,v)) remain_vars in
    (res,check_hps pairs)   
  )
