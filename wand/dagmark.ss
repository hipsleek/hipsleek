...
relation d(node x,int v,node l,node r,bag(node) G).
relation reach(bag(node) G, node x, bag(node) R).

dag<G> == self = null
       or self::node<v,l,r> * l::dag<G> U* r::dag<G> & d(self,v,l,r,G);

axiom d(x,1,l,r,G1) ==> mark1(G,x,G1).

axiom mark(G,x,G1) ==> reach(G1,x,R1) & reach(G1,l,R1) & R subset R1.
...
//ramification lemma dag(l,G) --@ (dag(l,G) U* dag(r,G)) * dag(l,G1) -> dag(l,G1) U* dag(r,G1)
