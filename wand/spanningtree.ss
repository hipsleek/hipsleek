...
relation spanning_tree(abstract G, node x, abstract G1).
...
axiom lookup(G,x,_,_,_) ==> x != null.

axiom true ==> spanning_tree(G,null,G).
...
requires x::graph<G> & lookup(G,x,_,_,_)
ensures x::graph<G1> & spanning_tree(G,x,G1);
{
  node l,r;
  l = x.left;
  r = x.right;
