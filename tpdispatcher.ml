...
let rec is_array_exp e = match e with
  ...
  | CP.BExpr f -> Some (is_array_constraint f)
...
let rec is_list_exp e = match e with
  ...
  | CP.BExpr f -> Some (is_list_constraint f)
...
and is_list_b_formula (pf,_) = match pf with
  | CP.BConst _
  | CP.BVar _
  | CP.BagMin _
  | CP.BagMax _
...
and is_list_constraint (e: CP.formula) : bool =
...
let is_sat f sat_no do_cache =
  let f = Cpure.translate_security_formula f in
  if !external_prover then
