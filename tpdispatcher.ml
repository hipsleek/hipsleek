  ...
  if (CP.is_concrete_rel conseq) then
    ...
    let ante,conseq =
      if (!Globals.allow_locklevel) then
        ...
        let () = x_ninfo_hp (add_str "After translate_: ante = " Cprinter.string_of_pure_formula) ante no_pos in
        let () = x_ninfo_hp (add_str "After translate_: conseq = " Cprinter.string_of_pure_formula) conseq no_pos in
        (ante,conseq)
      else
        (ante,conseq)
    in (None, ante, conseq)
...
let tp_imply_no_cache ante conseq imp_no timeout process =
  let () = x_dinfo_pp "marking transfer to prover \n" no_pos in
...
let tp_imply_no_cache ante conseq imp_no timeout process =
  ...
  Debug.no_4 "tp_imply_no_cache" pr pr (fun s -> s) string_of_prover string_of_bool
     ...
     if res1 then
       ...
       let res1 =
         if (not (CP.is_formula_arith ante))&& (CP.is_formula_arith conseq) then
           let res1 = x_add tp_imply (CP.drop_bag_formula ante) conseq imp_no timeout process in
           if res1 then res1
           else x_add tp_imply ante conseq imp_no timeout process
         else
...
let imply_timeout (ante0 : CP.formula) (conseq0 : CP.formula) (old_imp_no : string) timeout process
  : bool*(formula_label option * formula_label option )list
    * (formula_label option) =
  let (imp_num,old_imp_no) = (0,old_imp_no) in
