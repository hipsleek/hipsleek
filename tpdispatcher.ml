...
let rec is_list_constraint(f : CP.formula) : bool = match f with
  | CP.BForm(bf) -> (is_list_constraint_b_formula bf)
  | CP.And(f1, f2, _) -> (is_list_constraint f1) || (is_list_constraint f2)
  | CP.Or(f1, f2, _) -> (is_list_constraint f1) || (is_list_constraint f2)
  | CP.Not(f1, _) -> (is_list_constraint f1)
  | CP.Forall(_, f1, _) -> (is_list_constraint f1)
  | CP.Exists(_, f1, _) -> (is_list_constraint f1)
