...
let rec strengthen_formula f0 =
  match f0 with
  | CP.BForm ((pf,il),lbl, llbl) -> 
      let r = match pf with
        | CP.Lt (e1, e2, l) -> CP.BForm ((CP.Lte (e1, CP.Add(e2, CP.IConst (-1, no_pos), l), l), il), lbl,llbl)
        | CP.Gt (e1, e2, l) -> CP.BForm ((CP.Gte (e1, CP.Add(e2, CP.IConst (1, no_pos), l), l), il), lbl, llbl)
        | CP.Neq (e1, e2, l) ->
            let lp = CP.Lte (e1, CP.Add(e2, CP.IConst (-1, no_pos), l), l) in
            let rp = CP.Gte (e1, CP.Add(e2, CP.IConst (1, no_pos), l), l) in
            CP.Or (CP.BForm ((lp,il), lbl, llbl), CP.BForm ((rp,il), lbl, llbl), lbl, l)
        | _ -> f0
      in r
...
and strengthen2 f0 =
  let f_f f =
	match f with
	| CP.BForm ((CP.Neq (e1, e2, l), il), lbl, llbl) ->
        let lp = CP.Lte (e1, CP.Add(e2, CP.IConst (-1, no_pos), l), l) in
        let rp = CP.Gte (e1, CP.Add(e2, CP.IConst (1, no_pos), l), l) in
        Some (CP.Or (CP.BForm ((lp, il), lbl, llbl), CP.BForm ((rp, il), lbl, llbl), lbl, l))
    | _ -> None
  in
  let f_bf bf =
	let (pf,il) = bf in
...
and weaken_formula f0 =
  ...
  | CP.BForm ((pf,il),lbl, llbl) ->
      let r = match pf with
        | CP.Lte (e1, e2, l) -> CP.BForm ((CP.Lt (e1, CP.Add(e2, CP.IConst (1, no_pos), l),l), il),lbl, llbl)
        | CP.Gte (e1, e2, l) -> CP.BForm ((CP.Gt (e1, CP.Add(e2, CP.IConst (-1, no_pos), l),l), il),lbl, llbl)
        | CP.Eq (e1, e2, l) ->
            let lp = CP.Gt (e1, CP.Add(e2, CP.IConst (-1, no_pos), l), l) in
            let rp = CP.Lt (e1, CP.Add(e2, CP.IConst (1, no_pos), l), l) in
            CP.And (CP.BForm ((lp,il),lbl, llbl), CP.BForm ((rp,il),lbl, llbl), l)
        | _ -> f0
      in r
...
and weaken2 f0 =
  let f_f f = match f with
    | CP.BForm ((CP.Eq (e1, e2, l),il), lbl, llbl) ->
        let lp = CP.Gt (e1, CP.Add(e2, CP.IConst (-1, no_pos), l), l) in
        let rp = CP.Lt (e1, CP.Add(e2, CP.IConst (1, no_pos), l), l) in
        Some (CP.And (CP.BForm ((lp,il),lbl, llbl), CP.BForm ((rp,il),lbl, llbl), l))
    | _ -> None
  in
  let f_bf bf =
	let (pf,il) = bf in
...
and get_subst_min_b_formula (bf,lbl, llbl) v =
  let (pf,il) = bf in
  match pf with
  | CP.EqMin (e0, e1, e2, pos) ->
    if CP.is_var e0 then
      ...
      else ([], CP.BForm (bf,lbl, llbl))
    else ([], CP.BForm (bf,lbl, llbl))
  | _ -> ([], CP.BForm (bf,lbl, llbl))
...
and get_subst_max_b_formula (bf,lbl, llbl) v =
  let (pf,_) = bf in
  match pf with
...
let get_subst_equation_b_formula bf0 v lbl llbl=
  let (pf,il) = bf0 in
  match pf with
  | CP.Eq (e1, e2, pos) -> 
      ...
      if with_v = 1 then
        ...
        ([], CP.BForm (bf0, lbl, llbl))
  | _ -> ([], CP.BForm (bf0,lbl, llbl))
	  ...
	  let st1, rf1 = get_subst_equation f1 v in
		if not (Gen.is_empty st1) then
		  ...
		  let st2, rf2 = get_subst_equation f2 v in
			(st2, CP.mkAnd f1 rf2 pos)
  | CP.BForm (bf, lbl, llbl) -> get_subst_equation_b_formula bf v lbl llbl
  | CP.AndList _ -> Gen.report_error no_pos "redlog.ml: encountered AndList, should have been already handled"
  | _ -> ([], f0)
...
and elim_exists_min f0 =
  let core qvar qf lbl pos =
    ...
    if not (Gen.is_empty st) then
      ...
      let tmp1 = 
        CP.mkOr 
        (CP.mkAnd (CP.mkAnd (CP.mkEqExp (CP.mkVar v pos) e1 pos) (CP.BForm (((CP.mkLte e1 e2 pos), None),None, [[pos]])) pos) pp1 pos)
        (CP.mkAnd (CP.mkAnd (CP.mkEqExp (CP.mkVar v pos) e2 pos) (CP.BForm (((CP.mkGt e1 e2 pos), None),None, [[pos]])) pos) pp1 pos)
        None
        pos
      in
...
and elim_exists_max f0 =
  let core qvar qf lbl pos =
    ...
    if not (Gen.is_empty st) then
      ...
      let tmp1 = 
        CP.mkOr 
        (CP.mkAnd (CP.mkAnd (CP.mkEqExp (CP.mkVar v pos) e1 pos) (CP.BForm (((CP.mkGte e1 e2 pos), None),None, [[pos]]) ) pos) pp1 pos)
        (CP.mkAnd (CP.mkAnd (CP.mkEqExp (CP.mkVar v pos) e2 pos) (CP.BForm (((CP.mkLt e1 e2 pos), None),None, [[pos]]) ) pos) pp1 pos)
        None
        pos
      in
      let tmp2 = CP.elim_exists (CP.mkExists [qvar] tmp1 lbl pos) in
...
let rec negate_formula f0 = match f0 with
  | CP.BForm (bf, lbl, llbl) ->
    ...
    | Some new_bf -> CP.BForm (new_bf, lbl,llbl)
    | None -> CP.Not (CP.BForm (bf, lbl, llbl), None, no_pos)
    in res
