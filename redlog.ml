...
(****************************************************************
					SOLVING EQUATION FACILITY
						AUTHOR:    VU AN HOA
 ****************************************************************)



(** An Hoa : Helper function to create a list of strings of form 
			[prefix{n}, prefix{n-1},...,prefix{1}]
 @remark  Since reduce poses restrictions on the name of variables, this
		function is necessary to standardize variables to a safe form. In
		particular, we convert base variables to y1,y2,... (these variables 
		will serve as parameters) and the rest to x1,x2,... (for unknowns).
 **)
...
(** An Hoa : parse the solution given out by reduce	
			Assume that "{  { (var = exp)* }+ }" & ONLY one root is obtained!
 **)
