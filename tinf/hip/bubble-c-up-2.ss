...
uls<n, sm,lg, tl> ==
  ...
  self::node<v, q> * q::uls<n-1, sm1,lg1, tl> & lg = max(v, lg1)  & sm=min(sm1,v)
  inv n>=0;
...
bool bubble(node xs)
	...
	*/
	...
	// unchanged if xs is sorted; otherwise, changed
     requires xs::uls<n1, sm1,lg, p> * p::sll<n2, sm> & lg <= sm & Term[n1, n2]
	case {
		n1 = 0 -> requires n2 > 0 ensures xs::sll<n2, sm> & !res;
		n1 != 0 -> ensures xs::uls<n1-n, sm2,lg1, q> * q::sll<n2+n, lg> & lg1 <= lg & 0 < n <= n1
                    ;
	}
{
	bool tmp, flag; 
