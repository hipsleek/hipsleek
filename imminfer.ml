...
let infer_imm_ann (prog: C.prog_decl) : C.prog_decl =
  ...
  let (new_proc_decls, rel_list) =
    let helper id proc (proc_decls, rel_list) =
      ...
      ((id, proc_decl)::proc_decls, rel_list_2) in
