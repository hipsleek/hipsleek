...
void _g_return_val_if_fail(gint val, GList *list)
  ...
  if (!list)
    {
      list = g_list_alloc ();
      list->data = data;
      if (sibling == NULL) tst = 1;
      else tst = 0;
      _g_return_val_if_fail (tst, list);
      return list;
    }
  else if (sibling)
    {
      ...
      if (node->prev)
        ...
        {
          _g_return_val_if_fail (sibling == list, node);
          return node;
        }
    }
  else
...
*/
{
  ...
  while (list)
    {
      if (! g_compare_func(list->data, data))
        return list;
      list = list->next;
    }
  ...
  while (list)
    {
      ...
      list->data = g_copy_func (list->data, user_data);
      list = next;
    }
}
                           ...
                           gpointer  user_data)
{
  ...
  cmp = g_compare_func (data, tmp_list->data);
...
*/
{
  return g_list_insert_sorted_real (list, data,  NULL);
}
...
*/
{
  return g_list_insert_sorted_real (list, data, user_data);
}
...
*/
{
  ...
  while (l1 && l2)
    {
      cmp =  g_compare_func (l1->data, l2->data);
...
*/
{
  ...
  return g_list_sort_merge (g_list_sort_real (list,  user_data),
                            g_list_sort_real (l2,  user_data),
                            user_data);
}
...
*/
{
  return g_list_sort_real (list, //(GFunc) compare_func, 
                           NULL);
}
