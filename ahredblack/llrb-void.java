...
node delete_internal(node h, int v)
	...
	case {
		c = 2 -> ensures res::rbc<n1,1,bh,2> & n-1 <= n1 <= n 
						or res::rbc<n1,1,bh,1> & n-1 <= n1 <= n;
		c = 3 -> ensures res::rbc<n1,1,bh,3> & n-1 <= n1 <= n 
						or res::rbc<n1,1,bh,2> & n-1 <= n1 <= n;
		(c < 2 | c >= 4) -> ensures res::rbc<n1,0,bh,4> & n-1 <= n1 <= n
						or res::rbc<n1,1,bh,3> & n-1 <= n1 <= n 
						or res::rbc<n1,1,bh,2> & n-1 <= n1 <= n 
						or res::rbc<0,1,1,0> & n = 1 & bh = 1 & res = null;
	}
{
	if (v < h.val) {
		// h.left == null means we cannot find v in the tree!
		if (h.left != null) {
			if (rbcase(h.left) == 1)
				move_red_left(h);
		...
		if (rbcase(h) == 2) {
			...
			if (v == h.val) {
				int m;
				h.right = delete_min_internal(h.right, m);
				h.val = m;
			} else
				h.right = delete_internal(h.right, v);
		} else {
			if (h.right == null) {
				node l = h.left;
				assume l' = null; // This should be provable, but currently not!
				if (v == h.val) 
					h = null;
			} else {
				if (rbcase(h.right) == 1)
	...
	}
{
	...
	if (is_red(h.right.left)) {
		...
		// Ensure left-leaning on the right branch! Robert's code is more efficient as it does not requires this step but it is troublesome to write specification.
		if (!is_red(h.right.left))
...
// Flip the color of a node and its two immediate children.
	...
	ensures res = bh;
{
	if (h == null)
		...
		return h.color + rbheight(h.left);
}
...
// Find the number of nodes under h.
int rbsize(node h)
	requires h::rbc<n,cl,bh,c>@I
	ensures res = n;
{
