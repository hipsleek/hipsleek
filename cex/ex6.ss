...
void loop(int x)
 requires cx::cex{[#loop,#inf_loop]} & Loop 
 ensures false;
{
  //st::state<[#loop] * cx::cex{[#loop,#inf_loop] & Loop 
  if (x>0) 
      //st::state<[#loop,#if] * cx::cex{[#loop,#inf_loop] & Loop & x>0 
    x=x+1;
      //st::state<[#loop,#if] * cx::cex{[#loop,#inf_loop] & Loop & x>0 & x'=x+1 
  else 
      //st::state<[#loop,#else] * cx::cex{[#loop,#inf_loop] & Loop & x>0 & x'=x+1 
    x=x+2;
      //st::state<[#loop,#else] * cx::cex{[#loop,#inf_loop] & Loop & x>0 & x'=x+2 
  //st::state<[#loop] * cx::cex{[#loop,#inf_loop] & Loop & x>0 x>0 & x'=x+1 
  // \/ st::state<[#loop] * cx::cex{[#loop,#inf_loop] & Loop & x>0 & x'=x+2
  inf_loop()
