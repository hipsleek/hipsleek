  ...
  | CP.BForm (b_formula, _) ->
    ...
    begin
      ...
      | CP.EqMin (e1, e2, e3, loc) ->
        ...
        SBC.BinRel (SBC.Eq, sb_exp1, max, sb_loc)
      | CP.LexVar lex -> SBC.BConst (true, SBG.no_pos)
      | _ -> report_error no_pos
...
let rec translate_hf hf = match hf with
  ...
  | CF.Star star_hf ->
    let hf1, hf2 = star_hf.CF.h_formula_star_h1, star_hf.CF.h_formula_star_h2 in
    let pos = star_hf.CF.h_formula_star_pos |> translate_loc in
  ...
  | CF.ViewNode view ->  let ann = view.CF.h_formula_view_imm in
    let holes = match ann with
      | CP.ConstAnn Lend -> [hf]
      | _ -> [] in
    let root, name = view.CF.h_formula_view_node, view.CF.h_formula_view_name in
    let args = view.CF.h_formula_view_arguments in
    ...
    let pos = view.CF.h_formula_view_pos |> translate_loc in
...
let create_templ_prog prog ents =
  ...
  let sb_res =
    SBProverH.infer_unknown_functions_w_false_rhs ifp_typ nprog ents in
...
let translate_view_decl (view:Cast.view_decl) =
  ...
  let vars =
    [Cpure.SpecVar (Named view.Cast.view_data_name, "self", VarGen.Unprimed)]
...
let translate_back_vdefns prog (vdefns: SBC.view_defn list) =
  ...
  let helper hps vdefn =
    let hp = List.find (fun x -> x.Cast.hp_name = vdefn.SBC.view_name) hps in
...
let translate_prog (prog:Cast.prog_decl) =
  if !sb_program = None || !enable_repair then
    ...
    let prog = SBC.mk_program "heap_entail" in
...
let get_repair_candidate prog ents cond_op =
  ...
  | Some fun_def_cexp ->
    ...
    let n_prog = {prog with Cast.prog_exp_decls = [n_exp_decl]} in
    begin
      match cond_op with
      | Some cond ->
...
let check_entail_x ?(residue=false) prog ante conseq =
  ...
  if List.length sb_ante != 1 && List.length sb_conseq != 1 then
    ...
    if not(residue) then
      ...
      let () = x_tinfo_hp (add_str "prog" SBC.pr_program) sb_prog no_pos in
      let ptree = SBProverH.check_entailment sb_prog ent in
      let res = ptree.SBProverA.enr_validity in
      match res with
      | SBG.MvlTrue ->
        ...
        let () = x_info_hp (add_str "residue" SBC.pr_f) red no_pos in
...
let check_entail_es ?(pf=None) prog (es:CF.entail_state) (bf:CF.struc_base_formula) =
  ...
  let ents = List.map (fun x -> SBC.mk_entailment ~mode:SBG.PrfEntailResidue x sb_conseq)
      sb_ante in
  let () = x_info_hp (add_str "ents" SBC.pr_ents) ents no_pos in
  ...
  if List.for_all is_valid ptrees then
    ...
    let () = x_info_hp (add_str "residue" pr_formula) residue no_pos in
    (true, Some residue)
  else
...
let rec heap_entail_after_sat_struc_x ?(pf=None) (prog:Cast.prog_decl)
