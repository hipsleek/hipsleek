...
(* This version use merging in RHS *)
...
let contains_evar elst v =
  List.exists (fun item -> exp_contains_var v item) (List.map exp_to_var elst)
;;
...
let get_sorted_puref arrPredlst =
  let rec helper lst lastm flst =
    ...
    | h::tail ->
       ( match h with
         | AsegNE (t,m) ->
            helper tail m ([mkLt t m;mkLte lastm t]@flst)
         | Pointsto (t,v) ->
              ...
              then
                let rhs1 =
                  ( match tail1 with
                    | h2::tail2 ->
                       ( match h2 with
                         | AsegNE (nt,nm) | Aseg (nt,nm) ->
                            head@[mkArrF rhs_e ([mkLt m lhs_m;mkEq m nt]@rhs_p) ((mkAsegNE t nm)::tail2)]@rest
                         | Pointsto (nt,v) ->
                            
                            failwith "exists_aseg_with_existential_var: TO BE IMPLEMENTED" )
                    | [] ->
                       head@[mkArrF rhs_e ([mkEq m lhs_m]@rhs_p) [mkAsegNE t lhs_m]]@rest )
                in
                      ...
                      | AsegNE (t,m) ->
                         ...
                         then helper (mkArrF lhs_e lhs_p ((unfold_AsegNE t m)@tail)) norm_rhs (indent+1)
                           ...
                           ( match (exists_asegne_with_existential_var norm_rhs m) with
                             | Some rhsl ->
                                mkOrlst (List.map (fun rhs_i -> helper lhs rhs_i (indent+1)) rhsl)                                        
                             | None ->
                                let norm_rhs = rhs_align t (remove_emp norm_rhs) in
                                let (all_m, split_rhs) =
                                  List.fold_left
