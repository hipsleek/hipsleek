...
let generate_constraints prog es rhs lhs_b rhs_b1 defined_hps
  ...
  let _ = DD.info_pprint ("  hp_rels: " ^ (let pr = pr_list_ln Cprinter.string_of_hprel in pr hp_rels)) pos in
  ...
  if no_infer_hp_rel es then
    ...
    if CP.intersect ivs (lhrs@rhrs) = [] then
      ...
      begin
        ...
        let _ = DD.info_pprint ("  unmatch before subst: " ^ (Cprinter.string_of_h_formula rhs)) pos in
