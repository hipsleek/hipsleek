    ...
    if not(check_sat) then
      ...
      let new_p,new_p_ass =
        if is_bag_cnt then
          ...
          let () = x_binfo_hp (add_str "lhs_xpure(w ann): " !CP.print_formula) lhs_xpure_ann pos  in
          ...
          let () = x_binfo_hp (add_str "ctr" !CP.print_formula) ctr pos  in
          ...
          let () = x_binfo_hp (add_str "disj_new_p" (pr_list !CP.print_formula)) disj_new_p pos  in
          let () = x_binfo_hp (add_str "disj_new_p_no_contra" (pr_list !CP.print_formula)) disj_new_p_no_contra pos  in
          ...
          let () = x_binfo_hp (add_str "new_p (pairwisecheck)" !CP.print_formula) new_p pos in
          let () = x_binfo_hp (add_str "new_p 3" !CP.print_formula) new_p pos  in
          let () = x_binfo_hp (add_str "new_p_for_assume " !CP.print_formula) new_p_for_assume pos  in
      ...
      if (CP.isConstTrue new_p || CP.isConstFalse new_p) then
        begin
          ...
          if ((lhs_rels==None && unk_heaps==[]) || not(is_contra || rhs_contain_rel)) then
            begin
              x_binfo_pp ">>>>>> infer_pure_m 1 <<<<<<" pos;
              x_binfo_pp "Did not manage to infer a useful precondition" pos;
              x_binfo_hp (add_str "LHS : " !CP.print_formula) lhs_xpure pos;
              x_binfo_hp (add_str "RHS : " !CP.print_formula) rhs_xpure pos;
              x_binfo_hp (add_str "LHS REL : " (pr_opt !CP.print_formula)) lhs_rels pos;
              (* x_dinfo_hp (add_str "new pure: " !CP.print_formula) new_p pos; *)
              x_binfo_hp (add_str "new_p_ass: " !CP.print_formula) new_p_ass pos;
              x_binfo_hp (add_str "new pure: " !CP.print_formula) new_p pos;
            ...
            | None ->
              ...
              then (None,None,[])
                ...
                begin
                  x_binfo_pp ">>>>>> infer_pure_m 2 <<<<<<" pos;
                  DD.binfo_pprint "Adding heap assumption?" pos;
                  DD.binfo_hprint (add_str "unk_heaps" (pr_list !CF.print_h_formula)) unk_heaps pos;
                  DD.binfo_hprint (add_str "lhs_xpure" (!CP.print_formula)) lhs_xpure pos;
                  DD.binfo_hprint (add_str "rhs_xpure" (!CP.print_formula)) rhs_xpure pos;
            ...
            | Some f ->
              x_binfo_pp ">>>>>> infer_pure_m 3 <<<<<<" pos;
              x_binfo_pp "Add relational assumption" pos;
              ...
              let () = y_binfo_hp (add_str "rhs_xpure_orig" !MCP.print_mix_formula) rhs_xpure_orig in
              let () = y_binfo_hp (add_str "rhs_xpure" !CP.print_formula) rhs_xpure in
