...
let get_var var stab = 
  ...
  then 
    ...
    AS.get_spec_var_ident stab var Primed
...
GLOBAL: expression;
  ...
  [ "formula" LEFTA
    ...
    | x = INT; "="; y = INT ->
      ...
      in BForm ((tmp, None), None, [[loc]])
    |	x = exp; "<"; y = exp ->
      if is_res_var y && is_zero x then 
        BForm ((BVar (get_var "res" !stab, loc), None), None, [[loc]]) 
      else if is_res_var x && is_one y then 
        Not (BForm ((BVar (get_var "res" !stab, loc), None), None, [[loc]]), None, loc) 
      else
        let tmp = 
  ...
  [ "specvar" NONA
    ...
    | x = UIDENT -> 
      if is_substr "REC" x 
      then 
        add_prefix (get_var (String.sub x 3 (String.length x - 3)) !stab) "REC"
      else get_var x !stab
    ]
  ]; 
