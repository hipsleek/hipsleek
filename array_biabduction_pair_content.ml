...
let array_biabduction_partial_order_pair_content =
  array_biabduction_partial_order_generic (str_pair !str_sv !str_sv) mkContentEq mkFreshContent
;;

let array_biabduction_full_order_pair_content =
  array_biabduction_full_order_generic mkContentEq mkFreshContent
;;
  
  

let trans_array_entailment_pair_content lhs rhs =
  let match_common = match_common_generic (fun (v11, v12) (v21, v22) -> (mkAnd (mkEqSv v11 v21) (mkEqSv v12 v22))) in
  let trans_array_entailment_generic mkTransformer mkContentEq lhs rhs =
    ...
    let ((new_l_hlst, new_r_hlst), match_pure_lst) = match_common (l_hlst, r_hlst) var_info in
    ...
    ((transLHS#get_root, l_elst, (get_segment_pure l_hlst) @ [lhs_ptr_positive; get_disjoint_pure l_hlst] @ l_plst, new_l_hlst),
     (transRHS#get_root, r_elst, (get_segment_pure r_hlst) @ [rhs_ptr_positive; get_disjoint_pure r_hlst] @ match_pure_lst @ r_plst, new_r_hlst),
     var_info)
  in
