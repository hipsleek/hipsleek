...
HeapPred H1(node x, int v). // non-ptrs are @NI by default
PostPred G(node x, int v). // non-ptrs are @NI by default
PostPred G1(node x, int t). // non-ptrs are @NI by default
*/
/*
sortll<n> == self=null
...
*/
         /*
requires x::sortll<v> ensures  res=1;
*/
{ int r=1;
  while (x)
    /*@
      infer [H1,G1]
      requires H1(x,v) & r=1
      ensures G1(x,v) & x'=null & r'=1;
    */
    /*
      requires x::sortll<v> & r=1 ensures x::sortll<v> &x'=null & (r'=1);
     */
   {
   int t = x->val;
