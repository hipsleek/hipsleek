...
let rec rev_trans_exp e = match e with
  ...
  | CP.BExpr f -> IP.BExpr (rev_trans_pure f)

and rev_trans_pf f = match f with
  | CP.XPure b -> IP.XPure{
...
and rev_trans_pure f = match f with
  ...
  | CP.SecurityForm (lbl, f, pos) -> IP.SecurityForm (rev_trans_sec_label lbl, rev_trans_pure f, pos)
