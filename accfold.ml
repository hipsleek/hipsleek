...
module ViewUtil : sig
  val compute_view_residents : C.view_decl -> C.view_decl list -> CP.spec_var list
  val compute_direction_info_of_views : C.view_decl list -> C.data_decl list
          -> C.view_decl list
  val compute_bound_pointers_of_view : C.view_decl -> CP.spec_var list 
  ...
  (** Compute_bound_pointers_of_view:
  ...
  let rec compute_head_nodes_of_formula_x  (f: CF.formula) (vdecl: C.view_decl)
      : CF.h_formula list =
    let eset = CF.build_eset_of_formula f in
    let view_root = C.get_view_root vdecl in
    let view_root_closure = CF.compute_equiv_closure_of_sv view_root eset in
    let search_f = fun _ -> None in
    let search_mf = Afutil.create_default_mix_formula_searcher false in
    let search_hf hf = (match hf with
      | CF.ViewNode { CF.h_formula_view_node = vnode }->
          if (CP.mem_svl vnode view_root_closure) then Some [hf]
          else Some []
      | CF.DataNode { CF.h_formula_data_node = dnode }->
          if (CP.mem_svl dnode view_root_closure) then Some [hf]
          else Some []
      | _ -> None
    ) in
  ...
  (** compute_head_nodes_of_formula *)
  and compute_head_nodes_of_formula (f: CF.formula) (vdecl: C.view_decl)
      : CF.h_formula list =
    let pr_f = (add_str "f" !CF.print_formula) in
    let pr_v = (add_str "view" C.name_of_view) in
    let pr_res = (add_str "res" (pr_list !CF.print_h_formula)) in
    Debug.no_2 "compute_head_nodes_of_formula" pr_f pr_v pr_res
        (fun _ _ -> compute_head_nodes_of_formula_x f vdecl) f vdecl
  
  
  (** compute_body_nodes_of_formula 
  ...
  *)
  ...
  type direction_pointer_t = (ident * CP.spec_var) (* (view_name, view_var) *)
  type direction_field_t = (ident * ident * ident)         (* (view_name, data_name, data_field) *)
  
  (** string_of_view_direction_info *)
  let string_of_view_direction_info (vdecl: C.view_decl) =
    let vn = vdecl.C.view_name in
    let dn = vdecl.C.view_data_name in
    "  view name: " ^ vdecl.C.view_name ^ "\n"
        ^ "    forward ptrs: "
        ^ (pr_list (fun sv -> vn^"."^(!CP.print_sv sv)) vdecl.C.view_forward_ptrs) ^ "\n"
        ^ "    forward fields: "
        ^ (pr_list (fun fn -> dn^"."^fn) vdecl.C.view_forward_fields) ^ "\n"
        ^ "    backward ptrs: "
        ^ (pr_list (fun sv -> vn^"."^(!CP.print_sv sv)) vdecl.C.view_backward_ptrs) ^ "\n"
        ^ "    backward fields: "
        ^ (pr_list (fun fn -> dn^"."^fn) vdecl.C.view_backward_fields) ^ "\n"
  
  
  (** Compute_dreiction_info_from_formula:
  ...
  let compute_direction_info_from_formula_x (vdecl: C.view_decl) (f: CF.formula)
      (data_decls: C.data_decl list) (view_decls: C.view_decl list)
      : direction_pointer_t list * direction_field_t list
        * direction_pointer_t list * direction_field_t list =
    let get_node_residents node = (
      match node with
      | CF.DataNode dn -> [dn.CF.h_formula_data_node]
      | CF.ViewNode vn ->
          let vd = C.look_up_view_def_raw 0 view_decls vn.CF.h_formula_view_name in
          let residents = List.concat (List.map2 (fun harg vvar ->
            if (CP.mem_svl vvar vd.C.view_residents) then [harg]
            else []
    ...
    let head_nodes = compute_head_nodes_of_formula f vdecl in
    ...
    let body_ptrs = List.concat (List.map get_node_residents body_nodes) in 
      ...
      | CF.DataNode dn ->
          ...
          List.iter2 (fun dn_arg ((_,field_name),_) ->
            ...
            if (CP.is_intersected_svl dn_arg_closure body_ptrs) then (
              let is_existing_fwf = List.exists (fun (x,y,z) ->
                (eq_str x vdecl.C.view_name) && (eq_str y data_name)
                && (eq_str z field_name)
              ) !fwfs in
              if not is_existing_fwf then
                fwfs := !fwfs @ [(vdecl.C.view_name, data_name, field_name)]
            )
    ...
    let head_ptrs = List.concat (List.map get_node_residents head_nodes) in 
    List.iter (fun node -> match node with
      | CF.ViewNode vn ->
          let view_name = vn.CF.h_formula_view_name in
          let view_decl = C.look_up_view_def_raw 0 view_decls view_name in
      ...
      | CF.DataNode dn ->
          ...
          List.iter2 (fun dn_arg ((_,field_name),_) ->
            ...
            if (CP.is_intersected_svl dn_arg_closure head_ptrs) then (
              let is_existing_bwf = List.exists (fun (x,y,z) ->
                (eq_str x vdecl.C.view_name) && (eq_str y data_name)
                && (eq_str z field_name)            ) !bwfs in
              if not is_existing_bwf then
                bwfs := !bwfs @ [(vdecl.C.view_name, data_name, field_name)]
            )
          ) dn.CF.h_formula_data_arguments data_decl.C.data_fields
      | _ -> ()
    ) body_nodes;
  ...
  (** compute_direction_info_from_formula *)
  let compute_direction_info_from_formula (vdecl: C.view_decl) (f: CF.formula)
      (data_decls: C.data_decl list) (view_decls: C.view_decl list)
      : direction_pointer_t list * direction_field_t list
        * direction_pointer_t list * direction_field_t list =
    let pr_v = (add_str "view" string_of_view_direction_info) in
    let pr_f = (add_str "f" !CF.print_formula) in
    let pr_pointers = pr_list (fun (vn,sv) -> vn^"."^(!CP.print_sv sv)) in
    let pr_fields = pr_list (fun (vn,dn,fn) -> "("^vn^")"^dn^"."^fn) in
    let pr_res = (
      add_str "res"
    ...
    Debug.no_2 "compute_direction_info_from_formula" pr_v pr_f pr_res
        (fun _ _ -> compute_direction_info_from_formula_x vdecl f data_decls view_decls)
        vdecl f
  
  (** get_view_base_branches *)
  ...
  (** get_view_inductive_branches_unfolded_by_base_case *)
  ...
  (** get_view_formulas_for_direction_info *)
  ...
  let compute_view_residents_x (vdecl: C.view_decl) (view_decls: C.view_decl list)
    ...
    let collect_pointer_residents vdecl f = (
      ...
      let search_mf = Afutil.create_default_mix_formula_searcher false in
      let search_hf hf = (match hf with
        | CF.ViewNode vn -> Some [vn.CF.h_formula_view_node]
        | CF.DataNode dn -> Some [dn.CF.h_formula_data_node]
        | _ -> None
      ) in
      let searcher = (search_f, search_hf, search_mf) in
      let pointer_vars = Afutil.search_in_formula searcher f in
      let eset = CF.build_eset_of_formula f in
      let residents = List.filter (fun vvar ->
        let vvar_closure = CF.compute_equiv_closure_of_sv vvar eset in
        (CP.is_intersected_svl vvar_closure pointer_vars)
      ) (vdecl.C.view_vars @ [(C.get_view_root vdecl)])in
      residents
    ) in
  ...
  (** compute view vars that reside inside memory allocated to current view *)
  let compute_view_residents (vd: C.view_decl) (view_decls: C.view_decl list) 
      : CP.spec_var list =
    let pr_vd = (add_str "view" C.name_of_view) in
    let pr_res = (add_str "residents" (pr_list !CP.print_sv)) in
    Debug.no_1 "compute_view_residents" pr_vd pr_res
        (fun _ -> compute_view_residents_x vd view_decls) vd
  
  
  (** test direction pointer *)
  ...
  let is_view_forward_pointer (sv: CP.spec_var) (vdecl: C.view_decl) =
    (CP.mem_svl sv vdecl.C.view_forward_ptrs)
  
  (** test backward pointer *)
  let is_view_backward_pointer (sv: CP.spec_var) (vdecl: C.view_decl) =
    (CP.mem_svl sv vdecl.C.view_backward_ptrs)
  
  (** test direction field *)
  ...
  (** test forward field *)
  let is_view_forward_field (field_name: ident) (vdecl: C.view_decl) =
    (mem_str_list field_name vdecl.C.view_forward_fields)
  
  (** test backward field *)
  let is_view_backward_field (field_name: ident) (vdecl: C.view_decl) =
    (mem_str_list field_name vdecl.C.view_backward_fields)
  ...
  (** Compute direction info:
      ...
      direction info *)
  let compute_direction_info_from_view_x (vdecl: C.view_decl) (f: CF.formula)
      (data_decls: C.data_decl list) (view_decls: C.view_decl list)
      : direction_pointer_t list * direction_field_t list
        * direction_pointer_t list * direction_field_t list =
    let head_and_body_nodes = (compute_head_nodes_of_formula f vdecl)
    ...
    List.iter (fun node -> match node with
      ...
      | CF.DataNode dn ->
          ...
          List.iter2 (fun harg field ->
            ...
            (* current argument of data node is a backward field 
               -> use it to find corresponding backward pointer *)
            else if (is_view_backward_field fname vdecl) then (
              let new_bwp_vars = CP.intersect_svl harg_closure vdecl.C.view_vars in
              let new_bwps = List.map (fun x -> (vdecl.C.view_name, x)) new_bwp_vars in
              bwps := !bwps @ new_bwps
            )
            (* current argument of data node isn't a direction field 
               -> use direction pointers in view to determine direction of current data field *)
            else (
              if (CP.is_intersected_svl harg_closure vdecl.C.view_forward_ptrs) then
                fwfs := !fwfs @ [(vdecl.C.view_name, data_name, fname)]
              else if (CP.is_intersected_svl harg_closure vdecl.C.view_backward_ptrs) then
                bwfs := !bwfs @ [(vdecl.C.view_name, data_name, fname)]
            )
  ...
  (** compute directin info *)
  let compute_direction_info_from_view (vdecl: C.view_decl) (f: CF.formula)
      (data_decls: C.data_decl list) (view_decls: C.view_decl list)
      : direction_pointer_t list * direction_field_t list
        * direction_pointer_t list * direction_field_t list =
    let pr_v = (add_str "view" string_of_view_direction_info) in
    ...
    Debug.no_2 "compute_direction_info_from_view" pr_v pr_f pr_res
        (fun _ _ -> compute_direction_info_from_view_x vdecl f data_decls view_decls)
        vdecl f
  ...
  let compute_direction_info_of_views_x (view_decls: C.view_decl list)
    ...
    let select_pointer vdecl (pointer: direction_pointer_t) : bool = (
      let vname = fst pointer in
      (eq_str vname vdecl.C.view_name)
    ) in
  
    let select_field vdecl (field: direction_field_t) : bool = (
      let (vname, dname, _) = field in
    ...
    let update_direction_info fwps fwfs bwps bwfs vd = (
      ...
      let fwfs = List.filter (select_field vd) fwfs in
      let bwfs = List.filter (select_field vd) bwfs in
      let fwp_vars = snd (List.split fwps) in
      let fwp_vars = CP.remove_dups_svl (fwp_vars @ vd.C.view_forward_ptrs) in
      let bwp_vars = snd (List.split bwps) in
      let bwp_vars = CP.remove_dups_svl (bwp_vars @ vd.C.view_backward_ptrs) in
      let fwf_names = List.map (fun (_,_,fn) -> fn) fwfs in
      let fwf_names = remove_dups_str_list (fwf_names @ vd.C.view_forward_fields) in
      let bwf_names = List.map (fun (_,_,fn) -> fn) bwfs in
      let bwf_names = remove_dups_str_list (bwf_names @ vd.C.view_backward_fields) in
      { vd with C.view_forward_ptrs = fwp_vars;
                C.view_forward_fields = fwf_names;
                C.view_backward_ptrs = bwp_vars;
                C.view_backward_fields = bwf_names; }
    ) in
    ...
    let init_view_direction_info vdecls = (
      ...
      (* update to view *)
      List.map (update_direction_info fwps fwfs bwps bwfs) vdecls
    ) in
  ...
  (** compute direction info of many views *)
  ...
  (** Check well-formed formula:
      a formula is well-formed iff all of its heap nodes must be reached
      from root pointers *)
  ...
  (** test well-formed struc_formula *)
  ...
  (** test well-formed struc formula *) 
...
(*  *)                                                                             *)
(* (* TRUNG TODO: rename this function, its current name's ugly *)                 *)
...
module AtomicView : sig

  val collect_main_heap_chain_in_view: C.view_decl -> CF.formula list
  ...
  let collect_main_heap_chain_in_formula_x (f: CF.formula) (root: CP.spec_var)
    ...
    let rec collect_main_pointers_x f main_ptrs vdecl ddecl fwd_ptr fwd_field eset = (
      let search_f = fun _ -> None in
      let search_mf = Afutil.create_default_mix_formula_searcher false in
      let search_hf hf = (match hf with
        | CF.ViewNode vn -> 
            let vnode = vn.CF.h_formula_view_node in
            let vargs = vn.CF.h_formula_view_arguments in
            let vnode_closure = CF.compute_equiv_closure_of_sv vnode eset in
            if (CP.EMapSV.overlap main_ptrs vnode_closure) then (
              let collected_ptrs = vnode_closure in
              let collected_ptrs = List.fold_left2 (fun ptrs arg var ->
                if (CP.eq_spec_var fwd_ptr var) then arg::ptrs
                else ptrs
              ) collected_ptrs vargs vdecl.C.view_vars in
              Some collected_ptrs
            )
            else Some []
        | CF.DataNode dn ->
            let dnode = dn.CF.h_formula_data_node in
            let dargs = dn.CF.h_formula_data_arguments in
            let dnode_closure = CF.compute_equiv_closure_of_sv dnode eset in
            if (CP.EMapSV.overlap main_ptrs dnode_closure) then (
              let collected_ptrs = dnode_closure in
              let collected_ptrs = List.fold_left2 (fun ptrs arg ((_,fname),_) ->
                if (eq_str fwd_field fname) then arg::ptrs
                else ptrs
              ) collected_ptrs dargs ddecl.C.data_fields in
              Some collected_ptrs
            ) else Some []
        | _ -> None
      ) in
      let searcher = (search_f, search_hf, search_mf) in
      let new_main_ptrs = Afutil.search_in_formula searcher f in
      if (List.length new_main_ptrs = List.length main_ptrs) then main_ptrs
      else collect_main_pointers f new_main_ptrs vdecl ddecl fwd_ptr fwd_field eset
    )
    and collect_main_pointers f main_ptrs vdecl ddecl fwd_ptr fwd_field eset = (
      let pr_f = (add_str "f" !CF.print_formula) in
      let pr_pointers = (add_str "main_ptrs" !CP.print_svl) in
      let pr_view = (add_str "view" !C.print_view_decl_short) in
      let pr_res = (add_str "res" !CP.print_svl) in
      Debug.no_3 "collect_main_pointers" pr_f pr_pointers pr_view pr_res
          (fun _ _ _ -> collect_main_pointers_x f main_ptrs vdecl ddecl fwd_ptr fwd_field eset)
          f main_ptrs vdecl
    ) in
    
    (* extract main heap chain in raw format *)
    let extract_main_heap_chain f pointers : CF.formula = (
      let trans_ef, trans_f = (fun _ -> None), (fun _ -> None) in
      let trans_hf hf = (match hf with
        | CF.ViewNode {CF.h_formula_view_node = vnode} ->
    ...
    let ddecl = (match vdecl.C.view_data_decl with 
      | Some dd -> dd
      | None ->
          let msg = "collect_main_heap_chain: view \'" ^ vdecl.C.view_name ^ "\' "
                     ^ "doesn't have an associating data decl" in
  ...
  (** count number of nodes of main heap chains *)
  (* let count_nodes_of_main_heap_chain *)
