#!/usr/bin/swipl -q
:- module(orders, [ev/2,hb/4,cb/4,snot/1,test/0,infer/4,looplabel/4,looprole/4,inferhb/3]).
:- use_module(library(chr)).

%:- initialization start.
:- chr_constraint ev/2,hb/4,cb/4,snot/1,test/0,infer/4,looplabel/4,looprole/4,inferhb/3.
...
% hbhb   @ hb(A,L1,B,L2), hb(B,L2,C,L4) ==> B\=C,L1\=L4 | hb(A,L1,C,L4).
% % CB transitivity 
...
test :-    cb(s,iC,r,iC),hb(a,iA,a,i1),hb(b,iB,b,i1),cb(a,i1,b,i1).
...
looprole(A,T,[],L).
looprole(A,T,[R|Tail],L):-  A,inferhb(T,R,L);looprole(A,T,Tail,L).

looplabel(A,T,R,[]).
looplabel(A,T,R,[L|Tail]):- looprole(A,T,R,L);looplabel(A,T,R,Tail).

infer(A,B,Roles,Labels):-   looplabel(A,B,Roles,Labels).
