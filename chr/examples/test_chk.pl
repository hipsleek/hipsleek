hb(event(X,1),event(Y,2)),hb(event(X,0),event(Y,2)),hb(event(Y,2),event(Z,3)),hb(event(X,1),event(Z,3)).

hb(event(X,1),event(Y,2)),hb(event(X,0),event(Y,2)),hb(event(Y,2),event(Z,3)),hb(event(Z,3),event(X,1)).

hb(event(X,1),event(Y,2)),hb(event(X,0),event(Y,2)),hb(event(Y,2),event(Z,3)),hb(event(Z,3),event(X,5)).


%hb(event(X,1),event(Y,2)),hb(event(X,0),event(Y,2)),hb(event(Y,2),event(Z,3)) ; (hb(event(X,1),event(Z,3))) .

%hb(event(X,1),event(Y,2)),hb(event(X,0),event(Y,2)),hb(event(Y,2),event(Z,3)) .


%hb(event(X,1),event(Y,2)),hb(event(X,0),event(Y,2)),hb(event(Y,2),event(Z,3)), not(hb(event(X,1),event(Z,9)) ; (hb(event(X,7),event(Z,3)))) .
%hb(event(X,1),event(Y,2)),hb(event(X,0),event(Y,2)),hb(event(Y,2),event(Z,3)), (hb(event(X,1),event(Z,9)) ; (hb(event(X,7),event(Z,3)))) .
%hb(event(X,1),event(Y,2)),hb(event(X,0),event(Y,2)),hb(event(Y,2),event(Z,3)),not(hb(event(X,1),event(Z,3))); hb(event(X,1),event(Y,8)).

%,guard(hb(event(X,1),event(Z,3))).

%guard(hb(event(X,1),event(Z,3))),cb(event(X,1),event(Y,1)),hb(event(X,0),event(Y,1)),hb(event(Y,1),event(Z,3)),hb(event(Y,4),event(Z,5)).
%cb(event(X,1),event(Y,1)),hb(event(Y,1),event(Z,3)),hb(event(Y,4),event(Z,5)),guard(hb(event(X,1),event(Z,3))).
%hb(event(X,1),event(Y,2)),hb(event(Y,2),event(Z,3)),hb(event(Y,4),event(Z,5)),guard(hb(event(X,1),event(Z,3))).
%hb(event(X,1),event(Y,2)),hb(event(Y,2),event(Z,3)),guard(hb(event(X,1),event(Z,3))).
