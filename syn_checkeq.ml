...
let rec check_eq_formula args is_strict f10 f20=
  ...
  try
    ...
    let hf1,mf1,_,_,_,_,_ = Cformula.split_components base_f1 in
    let hf2,mf2,_,_,_,_,_ = Cformula.split_components base_f2 in
...
let syntax_contrb_lemma_end_null_x prog lhs rhs=
  ...
  in
  (*****************************************)
  let ( _,rmf,_,_,_,_,_) = Cformula.split_components rhs in
  let r_eqnull_svl =  MCP.get_null_ptrs rmf in
  if r_eqnull_svl = [] then
    (lhs, rhs)
  else
    let ( _,lmf,_,_,_,_,_) = Cformula.split_components lhs in
