...
type comment = { mutable buffer : string; mutable pos : int; mutable spec:bool }
...
let new_comment spec_or_not =
  let com = { buffer = ""; pos = -1 ;spec = spec_or_not} in
  comment_list := com :: !comment_list;
  com

let current_comment () = List.hd !comment_list

let begin_comment buf spec_or_not=
  let comment = new_comment spec_or_not in
...
let eol_comment buf =
  let comment = new_comment false in
  comment.pos <- lexeme_pos buf;
