...
type nameType = 
  | MethodName (* Put _ instead of . *)
  | VarName (* Put this. at front *)


type higherName = (name * nameType option)
type names = higherName list

type typ =
  | TypeName of higherName
  | ArrayType of typ

let named_type str = TypeName ([synth_id str],None)

let no_type = TypeName ([],None)
...
let rec name_list_to_higher_name_list name_list =
    match name_list with
    | head::rest -> (fst head)::(name_list_to_higher_name_list rest)
    | [] -> []
...
let expr_super_invocation expr args =
  Expr (Call (Dot (expr, super_ident,false), args))
...
let rec type_pos t =
  match t with
  | TypeName name -> id_pos (List.hd (fst name))
