...
(declare-const t_emp RefDll_t)

(assert 
		(and 
			(distinct x_emp w_emp)
			(distinct w_emp t_emp)
			(distinct z_emp w_emp)
		(sep 
			(pto w_emp (c_Dll_t t_emp u_emp ))
			(dll x_emp u_emp (as nil RefDll_t) w_emp )
			(dll t_emp y_emp w_emp z_emp )
		)
