...
(* type fv_cache_value = (int * bool) *)
(* type fv_cache_key = Globals.ident *)
(* let fv_cache = Hashtbl.create 400 (\* fv_cache_key * fv_cache_value*\) *)
...
let equal_type (t1:CP.typ) (t2:CP.typ) : bool =
  match t1 with
    | CP.OType _ ->
	      begin
	        match t2 with
