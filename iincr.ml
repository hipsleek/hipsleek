...
let get_infer_const_scc scc =
  let infs = List.fold_left (fun acc proc ->
      acc@(get_infer_const proc.Cast.proc_stk_of_static_specs # top) ) [] scc in
...
let update_infer_const_proc add_infs minus_infs proc=
  ...
  let () = x_tinfo_hp (add_str "new_spec" Cprinter.string_of_struc_formula)
      new_spec no_pos in
...
let add_pre_shape_relation ?(trans_size_to_extn=false) prog proc spec=
  ...
  let rec recf sf rel_name rel_vars=match sf with
    ...
    | CF.EBase eb ->
      ...
      let pre_simpl0 = (CF.formula_of_heap
                          (CF.HRel (CP.SpecVar (HpT, rel_name, Unprimed),
                                    pre_eargs, pos)) pos) in
    ...
    | CF.EAssume ea -> sf
    | CF.EInfer ei ->
      let rel_name = Globals.hp_default_prefix_name
                     ^ (string_of_int (Globals.fresh_int())) in
      let proc_args = List.map
          (fun (t,id) -> CP.mk_typed_spec_var t id) proc.Cast.proc_args in
      ...
      if rel_vars = [] then sf else
        ...
        in
        let () = x_tinfo_hp
            (add_str ("generate unknown predicate for Pre synthesis of " ^
                      proc.Cast.proc_name ^ ": ") pr_id)
        ...
        in
        let () = DD.ninfo_hprint (add_str "rel_args"
                                    Cprinter.string_of_typed_spec_var_list)
            rel_vars no_pos in
...
let add_post_shape_relation_x prog proc spec=
  ...
  let rec recf sf rel_name rel_vars=match sf with
    ...
    | CF.EBase eb -> begin
        ...
        | None -> sf
        | Some cont -> CF.EBase {eb with CF.formula_struc_continuation = Some
                                             (recf cont rel_name rel_vars)}
      end
    | CF.EAssume ea ->
      let args = rel_vars in
      let post_eargs = List.map (fun sv -> CP.Var (sv,pos)) args in
      let post_simpl0 = (CF.formula_of_heap
                           (CF.HRel (CP.SpecVar (HpT, rel_name, Unprimed),
                                     post_eargs, pos)) pos) in
    ...
    | CF.EInfer ei ->
      let rel_name = Globals.hppost_default_prefix_name
                     ^ (string_of_int (Globals.fresh_int())) in
      ...
      if rel_vars = [] &&  not (is_node_typ proc.Cast.proc_return) then sf else
        ...
        let rel_res_vars = if is_node_typ proc.Cast.proc_return
          then [(CP.SpecVar (proc.Cast.proc_return, res_name, Unprimed))]
          else [] in
        ...
        in
        let () = Debug.info_hprint
            (add_str ("generate unknown predicate for Post synthesis of "
                      ^ proc.Cast.proc_name ^ ": ") pr_id)
        ...
        CF.EInfer {ei with
                   CF.formula_inf_vars =
                     CP.remove_dups_svl (ei.CF.formula_inf_vars@[post_inf_sv]);
                   CF.formula_inf_continuation = new_cont}
    | CF.ECase ec -> CF.ECase { ec with
                                CF.formula_case_branches = List.map (fun (pf,sf) ->
                                    let rel_name = fresh_any_name rel_name in
...
let add_post_shape_relation prog proc sf =
  let pr = Cprinter.string_of_struc_formula in
  Debug.no_1 "add_post_shape_relation" pr pr (fun _ ->
      add_post_shape_relation_x prog proc sf) sf
...
let get_post_preds_x spec=
  let rec recf sf =match sf with
    ...
    | CF.EAssume ea ->
      (List.map fst (CF.get_HRels_f ea.CF.formula_assume_simpl))
      @(recf ea.CF.formula_assume_struc)
    | CF.EInfer ei ->
...
let proc_extract_inf_props prog proc_name=
  try
    ...
    extract_inf_props prog [proc.Cast.proc_stk_of_static_specs # top]
  with _ -> []
...
let extend_views iprog prog rev_formula_fnc trans_view_fnc ext_pred_names proc=
  ...
  let vns1 = Gen.BList.remove_dups_eq string_eq
      (List.map (fun vn -> vn.h_formula_view_name) vns) in
  ...
  let vdcls = List.map (x_add Cast.look_up_view_def_raw x_loc
                          prog.Cast.prog_view_decls) ( rev_cl_vns1) in
  let pure_extn_views = List.map (Cast.look_up_view_def_raw x_loc
                                    prog.Cast.prog_view_decls) ext_pred_names in
  let old_view_scc = !Astsimp.view_scc in
  let () = Astsimp.view_scc := [] in
  let map_ext_views = x_add Derive.expose_pure_extn iprog prog rev_formula_fnc
      trans_view_fnc vdcls pure_extn_views in
...
let extend_inf iprog prog map_views proc=
  ...
  let new_t_spec = Cfutil.subst_views_struc map_views t_spec in
  let () =  Debug.ninfo_hprint (add_str "derived top spec"
                                  (Cprinter.string_of_struc_formula)) new_t_spec
      no_pos in
  let () = proc.Cast.proc_stk_of_static_specs # push new_t_spec in
  let n_static_spec = new_t_spec in
  ...
  let proc0 = proc in
  let n_dyn_spec = Cfutil.subst_views_struc map_views proc0.Cast.proc_dynamic_specs in
  let () =  Debug.ninfo_hprint (add_str "derived dynamic spec"
                                  (Cprinter.string_of_struc_formula)) n_dyn_spec
      no_pos in
...
let extend_pure_props_view iprog cprog rev_formula_fnc trans_view_fnc proc=
  ...
  Debug.no_1 "extend_pure_props_view" pr2 pr2
    (fun _ -> extend_pure_props_view iprog cprog rev_formula_fnc
        trans_view_fnc proc) proc
