...
void printOffset(PlotData &plot, const TOffset off, const int from, const int to)
{
    const bool isAboveRoot = (off < 0);
//    const char *color = (isAboveRoot)
//        ? "red"
...
void printVar(PlotData &plot, const TValId rootAt) {
    if (VAL_ADDR_OF_RET == rootAt) {
        plot.out << "OBJ_RETURN";
        return;
    }
...
void printFieldPlacement(PlotData &plot, const TObjId obj, TObjType clt) {
    SymHeap &sh = plot.sh;
...
void printObject(PlotData &plot, const TObjId obj, const bool lonely) {
    SymHeap &sh = plot.sh;
    ...
    const TObjType cltRoot = sh.valLastKnownTypeOfTarget(root);
    if (cltRoot)
        printFieldPlacement(plot, obj, cltRoot);

    if (not(lonely && isProgramVar(code)))
        plot.out << " " << obj;
}

void printRootValue(PlotData &plot, const TValId val, const char *color) {
    SymHeap &sh = plot.sh;
...
void printAtomicObj(PlotData &plot, const AtomicObject ao, const bool lonely)
{
    SymHeap &sh = plot.sh;
    ...
    const char *props = ", style=dashed";

    const EObjectClass code = ao.code;
    switch (code) {
        case OC_VOID:
            CL_BREAK_IF("printAtomicObj() got an object of class OC_VOID");
            return;
...
void printUniformBlocks(PlotData &plot, const TValId root) {
    SymHeap &sh = plot.sh;
...
void printInnerObjects(PlotData &plot, const TValId at, const TObjList &liveObjs)
{
    SymHeap &sh = plot.sh;
    ...
    BOOST_FOREACH(TAtomByOff::const_reference item, objByOff) {
        ...
        BOOST_FOREACH(const AtomicObject &ao, /* TAtomList */ item.second) {
            // plot a single object
            printAtomicObj(plot, ao, /* lonely */ false);

            // connect the inner object with the root by an offset edge
            printOffset(plot, off, at, ao.obj);
        }
    }
}
...
void printCompositeObj(PlotData &plot, const TValId at, const TObjList &liveObjs)
{
    SymHeap &sh = plot.sh;
    ...
    switch (code) {
        ...
        default:
            CL_BREAK_IF("printCompositeObj() got invalid root object");
            return;
    }
    ...
    // plot the root value
    printRootValue(plot, at, color);

    // plot all uniform blocks
    printUniformBlocks(plot, at);

    // plot all atomic objects inside
    printInnerObjects(plot, at, liveObjs);
...
void printDlSeg(PlotData &plot, const TValId seg, const TObjList &liveObjs) {
    SymHeap &sh = plot.sh;
    ...
    // plot the given root
    printCompositeObj(plot, seg, liveObjs);
    ...
    if (OK_DLS == sh.valTargetKind(peer)) {
        ...
        printCompositeObj(plot, peer, liveObjsAtPeer);
    }
...
void printRootObjects(PlotData &plot) {
    SymHeap &sh = plot.sh;
    ...
    BOOST_FOREACH(PlotData::TValues::const_reference item, plot.values) {
        ...
        switch (kind) {
            ...
            case OK_CONCRETE:
                if (1 == liveObjs.size()) {
                    ...
                    printAtomicObj(plot, ao, /* lonely */ true);
                    continue;
                }
                // fall through!
        ...
        printCompositeObj(plot, root, liveObjs);
    }
}
...
void shapeDescribeInt(PlotData &plot, const long num) {
    plot.out << ", label=[int] " << num << "";
}

void shapeDescribeReal(PlotData &plot, const float fpn) {
    plot.out << ", label=[real] " << fpn << "";
}

void shapeDescribeFnc(PlotData &plot, const int uid) {
    TStorRef stor = plot.sh.stor();
    const CodeStorage::Fnc *fnc = stor.fncs[uid];
    CL_BREAK_IF(!fnc);

    const std::string name = nameOf(*fnc);
    plot.out << ", label=[Func]" << name << "()";
}

void shapeDescribeStr(PlotData &plot, const char *str) {
...
void printCustomValue(PlotData &plot, const TObjId obj, const TValId val) {
    SymHeap &sh = plot.sh;
    ...
    switch (code) {
        case CV_INVALID:
            plot.out << ", label=CV_INVALID";
            break;

        case CV_INT:
            shapeDescribeInt(plot, data.num);
            break;

        case CV_REAL:
            shapeDescribeReal(plot, data.fpn);
            break;

        case CV_FNC:
            shapeDescribeFnc(plot, data.uid);
            break;

        case CV_STRING:
            shapeDescribeStr(plot, data.str);
            break;
    }
...
void printValue(PlotData &plot, const TValId val)
{
    SymHeap &sh = plot.sh;
    ...
    switch (code) {
        case VT_CUSTOM:
            // skipt it, custom values are now handled in printHasValue()
            return;
...
void printPointsTo(PlotData &plot, const TValId val, const TObjId target) {
    plot.out << "\t" << SL_QUOTE(val)
        << " -> " << SL_QUOTE(target)
        << " [label=PointsTo];\n";
}
...
void printNonRootValues(PlotData &plot) {
    SymHeap &sh = plot.sh;
...
void printAuxValue(PlotData &plot, const int node, const TValId val, bool isObj)
{
...
void printHasValue(PlotData &plot, const TObjId obj) {
    SymHeap &sh = plot.sh;

    const TValId val = sh.valueOf(obj);
    if (val <= 0) {
        printAuxValue(plot, obj, val, /* isObj */ true);
        return;
    }

    const EValueTarget code = sh.valTarget(val);
    if (VT_CUSTOM == code) {
        printCustomValue(plot, obj, val);
        return;
    }
...
void printNeqZero(PlotData &plot, const TValId val) {
    const int id = ++plot.last;
    plot.out << "\t" << SL_QUOTE("lonely" << id)
        << " [label=NULL];\n";
...
void printNeq(std::ostream &out, const TValId v1, const TValId v2) {
    out << "\t" << SL_QUOTE(v1)
        << " -> " << SL_QUOTE(v2)
        << " [label=neq];\n";
}
...
void printEverything(PlotData &plot) {
    ...
    BOOST_FOREACH(PlotData::TDangValues::const_reference item, plot.dangVals) {
        ...
        if (val <= 0) {
            printAuxValue(plot, id, val, /* isObj */ false);
            continue;
        }
        ...
        const bool                      digForward)
{
    PlotEnumerator *pe = PlotEnumerator::instance();
    std::string plotName(pe->decorate(name));
    std::string fileName(plotName + ".shape");
    ...
    // initialize an instance of PlotData
    CL_DEBUG("printSummary: created shape file '" << fileName << "'");
    PlotData plot(sh, out);

    // do our stuff
    digValues(plot, startingPoints, digForward);
    printEverything(plot);
        ...
        const std::string               &name)
{
    TValList roots;
    sh.gatherRootObjects(roots);
    return printSummary(sh, name, roots);
}
