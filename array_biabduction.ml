...
let str_norm_pre_condition_base norm =
  ...
  | NormBaseImply (uset, eset, lhs_p, rhs_p, frame, antiframe) ->
     let inners = (str_list !str_pformula lhs_p) ^ "=>"
                  ...
                  ^ " @" ^ (str_asegPredplus_lst frame)
                  ^ " * " ^ (str_asegPredplus_lst antiframe) in
     let s =
       if List.length eset = 0
       then inners
       else "Exists " ^ (str_list !str_sv eset) ^ ": " ^ inners
...
(*   let aux_entry (lhs_p, lhs_h) (rhs_p, rhs_h) = *)
(*     match lhs_h, rhs_h with *)
(*     | [], [] -> base_case0 *)
(*     | [], _::_ -> base_case1 *)
(*     | _::_, [] -> base_case2 *)
(*     | _::_, _::_ -> *)
(*        let sorted_lhs_lst = get_sorted lhs_p lhs_h in *)
(*        List.map (fun item -> aux_lhs_sorted item (rhs_p, rhs_h)) sorted_lhs_lst *)
(*   in *)
(*   let aux_lhs_sorted sorted_lhs (rhs_p, rhs_h) = *)
(*     let sorted_rhs_lst = get_sorted rhs_p rhs_h in *)
(*     List.map (aux_sorted sorted_lhs) sorted_rhs_lst *)
(*   in *)
(*   let aux_sorted (lh, ltail) (rh, rtail) = *)
(*     match lh, rh with *)
(*     | Aseg_p _, _               *)
(*       | _, Aseg_p _-> *)
(*        failwith "Aseg_p: Invalid cases" *)
...
(*        then *)
(*          let f,norm = helper orig_lhs_p (lhs_p, ltail) ((mkEqSv lv rv)::rhs_p, rtail) vset uqset uqset_eq frame antiframe (indent+1) in *)
(*          let f, norm = aux_entry (lhs_p, ltail) ((mkEqSv lv rv)::rhs_p, rtail) in *)
(*          (print_and_return f indent,norm) *)
(*        else *)
(*          let (uset,vsetprime) = mkUsetandVsetprime [ls;rs] vset in *)
(*          let case1 = mkEqSv ls rs in *)
