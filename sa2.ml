...
and check_dropable_paras_constr(constr: CF.formula * CF.formula):(hp_para list) =
  Debug.info_hprint (add_str "CONSTRAINT: " (pr_list_ln Cprinter.string_of_hprel_lhs_rhs)) [constr] no_pos;
...
and check_dropable_paras_RHS f = 
  ...
  let check_dropable_each_pred hr f =
    let (svar, el, l) = hr in
    let _ = Debug.info_pprint ("Check HeapPred:  " ^ (CP.name_of_spec_var svar)) no_pos in 
    ...
    let _ = Debug.info_pprint ("Drop loc list: " ^ (str_of_para_loc_lst res)) no_pos in
    res
  in
