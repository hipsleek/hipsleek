...
let filter_var prog svl f=
  ...
  let keep_ptrs = loop_up_closed_ptr_args prog hd_nodes hv_nodes svl in
...
let keep_data_view_hrel_nodes_two_fbs prog f1 f2 hd_nodes hv_nodes eqs keep_rootvars lrootvars keep_hrels=
  ...
  let ps10 = CP.list_of_conjs (MCP.pure_of_mix nf1.CF.formula_base_pure) in
  ...
  let nf11 = {nf1 with CF.formula_base_pure = MCP.mix_of_pure new_p13} in
...
let process_one_f org_args args hp_subst sh_ldns com_eqNulls com_hps (ldns, f)=
  ...
  let nf7,_ = CF.drop_hrel_f nf6 com_hps in
...
let get_min_number_new prog args ll_ldns=
  ...
  let helper_pure_hprels f =
	...
	eqNulls,hprels
  in
  let rec helper ll r_min r_hns r_eqNulls r_hprels=
  match ll with
    | [] -> r_min,r_hns,r_eqNulls,r_hprels
    | (lnds,f)::lls ->
        let ns,nhds = helper1 lnds in
		let eqNulls,hprels = helper_pure_hprels f in
        ...
        if ns < r_min then
          helper lls ns nhds new_eqNulls new_hprels
        else helper lls r_min r_hns new_eqNulls new_hprels
  in
  (*start with length of the first one*)
  let fmin, fdns = helper1 (fst (List.hd ll_ldns)) in
  let eqNull,hprels = helper_pure_hprels (snd (List.hd ll_ldns)) in
  helper (List.tl ll_ldns) fmin fdns eqNull hprels
...
let mk_orig_hprel_def prog hp args sh_ldns eqNulls hprels=
...
let get_longest_common_hnodes_list_x prog hp args fs=
 if List.length fs <= 1 then
   ...
   let min,sh_ldns,eqNulls,hprels = get_min_number_new prog args lldns in
   ...
   if min = 0 then
     ...
     let orig_hpdef, hp_subst, new_hp, n_args,sh_ldns2 = mk_orig_hprel_def prog hp args sh_ldns eqNulls hprels in
     ...
     let n_fs1 = Gen.BList.remove_dups_eq (fun f1 f2 -> check_relaxeq_formula f1 f2) n_fs in
     let new_hpdef =  mk_hprel_def new_hp n_args n_fs1 no_pos in
...
let elim_irr_eq_exps args f=
  match f with
    | CF.Base fb ->
        ...
        let new_h = filter_unconnected_hf args fb. CF.formula_base_heap in
...
*)
let succ_susbt_x nrec_grps unk_hps allow_rec_subst (hp,args,f)=
...
let succ_susbt nrec_grps unk_hps allow_rec_subst (hp,args,f)=
   ...
   Debug.no_3 "succ_susbt" pr1 string_of_bool pr2 pr3
       (fun _ _  _ -> succ_susbt_x nrec_grps unk_hps allow_rec_subst (hp,args,f)) nrec_grps allow_rec_subst (hp,args,f)


let succ_susbt_with_rec_indp_x rec_indp_grps unk_hps depend_grps=
  ...
  *)
    ...
    if rec_branches = [] || base_branches = [] then grp else
      begin
          ...
          let new_rec_bracnhes = List.concat (snd (List.split (
              List.map (succ_susbt [base_branches] unk_hps true) rec_branches))) in
          match new_rec_bracnhes with
            | [] -> base_branches
            | (hp1,args1,_)::_ ->
                let new_rec_fss = refine_grp_helper hp1 args1 (List.map (fun (_,_,f)-> f) new_rec_bracnhes) in
                let new_rec_bracnhes1 =  List.map (fun f -> (hp1,args1,f)) new_rec_fss in
  ...
  let preprocess_rec_indp grp=
    let pr1 =   (pr_list_ln string_of_par_def_w_name_short) in
    Debug.ho_1 "preprocess_rec_indp" pr1 pr1
    ...
    let new_pardefs=
      ...
      *)
      then
        ...
        let _, fss = succ_susbt rec_indp_grps1 unk_hps false (hp,args,f) in
        ...
        let fss1 = refine_grp_helper hp args (List.map (fun (_,_,f) -> f) fss) in
        let fss3 = List.map (fun f1 -> (hp,args,f1)) fss1 in
        fss3
    in
    new_pardefs
  in
  let succ_susbt_one_grp rec_indp_hps grp=
    let pardefs =
      if List.length grp > 1 then
        List.concat (List.map (succ_susbt_one_pardef rec_indp_hps) grp)
      else grp
    in
    pardefs
  in
  ...
  List.map (succ_susbt_one_grp rec_indp_hps) depend_grps

let succ_susbt_with_rec_indp rec_indp_grps unk_hps depend_grps=
  let pr1 = pr_list_ln (pr_list_ln string_of_par_def_w_name_short) in
  Debug.ho_3 "succ_susbt_with_rec_indp" pr1 !CP.print_svl pr1 pr1
      (fun _ _ _ -> succ_susbt_with_rec_indp_x rec_indp_grps unk_hps depend_grps)
...
let succ_susbt_hpdef_x nrec_hpdefs all_succ_hp (hp,args,f)=
...
let succ_susbt_hpdef nrec_hpdefs all_succ_hp (hp,args,f)=
  ...
  Debug.no_3 " succ_susbt_hpdef" pr1 pr2 pr3 pr4
      (fun _ _ _ -> succ_susbt_hpdef_x nrec_hpdefs all_succ_hp (hp,args,f))
...
let remove_dups_recursive_x hp args unk_hps defs=
  let is_rec_f f=
    let hps = CF.get_hp_rel_name_formula f in
    (CP.mem_svl hp hps)
  in
  ...
  let match_with_rec rec_ls_dns f=
    ...
    if not b then ([],[f])
      ...
      ([(f,new_residue)],[])
  in
  let match_with_base_x poss_base_fs base_fs=
    let process_helper (f,residue) =
      if List.exists
        (fun base_f -> check_relaxeq_formula base_f residue) base_fs then
        []
      else [f]
    in
  ...
  in
  (*for debugging*)
  let match_with_base poss_base_fs base_fs=
    let pr1 = pr_list_ln Cprinter.prtt_string_of_formula in
    let pr2 = pr_list_ln (pr_pair Cprinter.prtt_string_of_formula Cprinter.prtt_string_of_formula) in
    Debug.no_2 "match_with_base" pr2 pr1 pr1
        (fun _ _ -> match_with_base_x poss_base_fs base_fs) poss_base_fs base_fs
  in
  (*END for debugging*)
  ...
  if (List.length indep_fs > 1) then
    ...
    let new_base_fs = match_with_base poss_base_fs base_fs in
    (rec_fs@dep_fs@base_fs@new_base_fs)
  else
    defs

let remove_dups_recursive hp args unk_hps defs=
  let pr1 = pr_list_ln Cprinter.prtt_string_of_formula in
  Debug.ho_3 "remove_dups_recursive" !CP.print_sv !CP.print_svl pr1 pr1
