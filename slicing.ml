...
and slice = atomic_constraint list
    ...
    fun (c, s, a) ctr -> match ctr with
      | Const_B b -> (match get_bform_eq_args_with_const b with
        ...
        | _ -> let pos = { memo_formula = b; memo_status = status } in 
          (pos::c, s, a))
      | Const_R f -> (c, f::s, a)) ([], [], empty_var_aset) s in
  let vars = List.fold_left (fun av ctr -> av @ (fv_atom ctr)) [] s in
  {
    memo_group_fv = Gen.BList.remove_dups_eq eq_spec_var vars;
