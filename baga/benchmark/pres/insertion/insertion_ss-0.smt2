(set-info :source  loris-7.ddns.comp.nus.edu.sg/~project/hip/) 
;Variables declarations
(declare-fun v_bool_32_1401_primed () Int)
(declare-fun v_bool_27_1402_primed () Int)
(declare-fun tmp_null_primed () Int)
(declare-fun v () Int)
(declare-fun x () Int)
(declare-fun xl () Int)
(declare-fun sm_1466 () Int)
(declare-fun xs () Int)
(declare-fun n () Int)
(declare-fun v_node_34_1395_primed () Int)
(declare-fun q_1469 () Int)
(declare-fun flted_15_1468 () Int)
(declare-fun qs_1470 () Int)
(declare-fun lg_1467 () Int)
(declare-fun v_primed () Int)
(declare-fun n_1498 () Int)
(declare-fun xs_1499 () Int)
(declare-fun xl_1500 () Int)
(declare-fun sres_1506 () Int)
(declare-fun lres_1507 () Int)
(declare-fun flted_21_1505 () Int)
(declare-fun x_primed () Int)
(declare-fun res () Int)
;Relations declarations
;Axioms assertions
;Antecedent
(assert (> v_bool_32_1401_primed 0))
(assert (> q_1469 0))
(assert (not (> v_bool_27_1402_primed 0)))
(assert (< sm_1466 v_primed))
(assert (< tmp_null_primed 1))
(assert (< 0 n))
(assert (= v_primed v))
(assert (= x_primed x))
(assert (= lg_1467 xl))
(assert (= sm_1466 xs))
(assert (<= xs qs_1470))
(assert (= (+ flted_15_1468 1) n))
(assert (= v_node_34_1395_primed q_1469))
(assert (= n_1498 flted_15_1468))
(assert (= xs_1499 qs_1470))
(assert (= xl_1500 lg_1467))
(assert (<= 1 flted_15_1468))
(assert (<= qs_1470 lg_1467))
(assert (= flted_21_1505 (+ 1 n_1498)))
(assert (or (and (= sres_1506 v_primed) (< v_primed xs_1499)) (and (= sres_1506 xs_1499) (>= v_primed xs_1499))))
(assert (or (and (= lres_1507 v_primed) (>= v_primed xl_1500)) (and (= lres_1507 xl_1500) (< v_primed xl_1500))))
(assert (<= 1 n_1498))
(assert (<= xs_1499 xl_1500))
(assert (= x_primed 1))
(assert (or (and (and (= lres_1507 sres_1506) (= flted_21_1505 1)) (> res 0)) (and (and (<= sres_1506 lres_1507) (<= 2 flted_21_1505)) (> res 0))))
(assert (not (= x_primed res)))
;Negation of Consequence
(assert (not false))
(check-sat)