(set-info :source  loris-7.ddns.comp.nus.edu.sg/~project/hip/) 
;Variables declarations
(declare-fun right_36_1815_primed () Int)
(declare-fun left_36_1814_primed () Int)
(declare-fun nright_36_1813_primed () Int)
(declare-fun nleft_36_1812_primed () Int)
(declare-fun val_36_1811_primed () Int)
(declare-fun mx_3887 () Int)
(declare-fun n_3886 () Int)
(declare-fun tmpv_primed () Int)
(declare-fun tmp_primed () Int)
(declare-fun l_3850 () Int)
(declare-fun t () Int)
(declare-fun v () Int)
(declare-fun tmp_null_primed () Int)
(declare-fun t_primed () Int)
(declare-fun v_bool_27_1962_primed () Int)
(declare-fun m1_3846 () Int)
(declare-fun m3_3848 () Int)
(declare-fun mx () Int)
(declare-fun mx1_3852 () Int)
(declare-fun n () Int)
(declare-fun d_3849 () Int)
(declare-fun v_primed () Int)
(declare-fun v_bool_30_1961_primed () Int)
(declare-fun m1_3851 () Int)
(declare-fun m2_3854 () Int)
(declare-fun v_bool_32_1871_primed () Int)
(declare-fun mx2_3855 () Int)
(declare-fun m2_3847 () Int)
(declare-fun r_3853 () Int)
(declare-fun ma_3900 () Int)
(declare-fun n1_3899 () Int)
(declare-fun v_node_36_1810_primed () Int)
;Relations declarations
;Axioms assertions
;Antecedent
(assert (= right_36_1815_primed r_3853))
(assert (= left_36_1814_primed l_3850))
(assert (= nright_36_1813_primed m2_3854))
(assert (= nleft_36_1812_primed m1_3851))
(assert (= val_36_1811_primed d_3849))
(assert (<= 0 mx_3887))
(assert (<= 0 n_3886))
(assert (or (and (<= mx_3887 tmpv_primed) (= ma_3900 tmpv_primed)) (= ma_3900 mx_3887)))
(assert (= n1_3899 (+ 1 n_3886)))
(assert (<= 0 mx1_3852))
(assert (<= 0 m1_3846))
(assert (= mx_3887 mx1_3852))
(assert (= n_3886 m1_3846))
(assert (= tmpv_primed d_3849))
(assert (= tmp_primed l_3850))
(assert (= t_primed t))
(assert (= v_primed v))
(assert (<= 0 v))
(assert (< tmp_null_primed 1))
(assert (> t_primed 0))
(assert (not (> v_bool_27_1962_primed 0)))
(assert (= m2_3847 m2_3854))
(assert (= m1_3846 m1_3851))
(assert (<= m3_3848 1))
(assert (<= 0 m3_3848))
(assert (= (+ m3_3848 m2_3854) m1_3851))
(assert (<= d_3849 mx))
(assert (<= mx2_3855 d_3849))
(assert (<= mx1_3852 d_3849))
(assert (<= 0 d_3849))
(assert (= n (+ (+ m2_3854 1) m1_3851)))
(assert (< d_3849 v_primed))
(assert (> v_bool_30_1961_primed 0))
(assert (<= m1_3851 m2_3854))
(assert (> v_bool_32_1871_primed 0))
(assert (or (and (and (< r_3853 1) (= m2_3847 0)) (= mx2_3855 0)) (and (and (<= 0 mx2_3855) (<= 1 m2_3847)) (> r_3853 0))))
(assert (or (and (and (< v_node_36_1810_primed 1) (= n1_3899 0)) (= ma_3900 0)) (and (and (<= 0 ma_3900) (<= 1 n1_3899)) (> v_node_36_1810_primed 0))))
;Negation of Consequence
(assert (not false))
(check-sat)