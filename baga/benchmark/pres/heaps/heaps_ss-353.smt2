(set-info :source  loris-7.ddns.comp.nus.edu.sg/~project/hip/) 
;Variables declarations
(declare-fun v_bool_156_1510_primed () Int)
(declare-fun v_bool_154_1565_primed () Int)
(declare-fun d_2528 () Int)
(declare-fun m3_2527 () Int)
(declare-fun m1_2530 () Int)
(declare-fun m2_2533 () Int)
(declare-fun m3_2480 () Int)
(declare-fun v_bool_141_1566_primed () Int)
(declare-fun v_bool_132_1567_primed () Int)
(declare-fun m2_2365 () Int)
(declare-fun m1_2364 () Int)
(declare-fun mx2 () Int)
(declare-fun mx1 () Int)
(declare-fun r () Int)
(declare-fun l () Int)
(declare-fun m2_primed () Int)
(declare-fun m2 () Int)
(declare-fun m1_primed () Int)
(declare-fun m1 () Int)
(declare-fun v () Int)
(declare-fun d_2557 () Int)
(declare-fun d () Int)
(declare-fun d_primed () Int)
(declare-fun d_2481 () Int)
(declare-fun m2_2479 () Int)
(declare-fun mx2_2487 () Int)
(declare-fun m1_2478 () Int)
(declare-fun mx1_2484 () Int)
(declare-fun v_primed () Int)
(declare-fun max1_2599 () Int)
(declare-fun max2_2600 () Int)
(declare-fun v_int_161_1509_primed () Int)
(declare-fun mx1_2578 () Int)
(declare-fun mx2_2579 () Int)
(declare-fun mx1_2531 () Int)
(declare-fun m1_2525 () Int)
(declare-fun l_2529 () Int)
(declare-fun mx2_2534 () Int)
(declare-fun m2_2526 () Int)
(declare-fun r_2532 () Int)
(declare-fun mx3_2601 () Int)
(declare-fun m1_2483 () Int)
(declare-fun l_2482 () Int)
(declare-fun mx4_2602 () Int)
(declare-fun m2_2486 () Int)
(declare-fun r_2485 () Int)
(declare-fun l_primed () Int)
(declare-fun r_primed () Int)
;Relations declarations
;Axioms assertions
;Antecedent
(assert (not (> v_bool_156_1510_primed 0)))
(assert (< v_primed d_2481))
(assert (> v_bool_154_1565_primed 0))
(assert (<= d_2528 d_2481))
(assert (= m2_2365 (+ (+ m2_2533 1) m1_2530)))
(assert (<= 0 d_2528))
(assert (<= mx1_2531 d_2528))
(assert (<= mx2_2534 d_2528))
(assert (<= d_2528 mx2))
(assert (= (+ m3_2527 m2_2533) m1_2530))
(assert (<= 0 m3_2527))
(assert (<= m3_2527 1))
(assert (= m1_2525 m1_2530))
(assert (= m2_2526 m2_2533))
(assert (= m1_2364 (+ (+ m2_2486 1) m1_2483)))
(assert (<= 0 d_2481))
(assert (<= mx1_2484 d_2481))
(assert (<= mx2_2487 d_2481))
(assert (<= d_2481 mx1))
(assert (= (+ m3_2480 m2_2486) m1_2483))
(assert (<= 0 m3_2480))
(assert (<= m3_2480 1))
(assert (= m1_2478 m1_2483))
(assert (= m2_2479 m2_2486))
(assert (not (> v_bool_141_1566_primed 0)))
(assert (not (= m2_primed 0)))
(assert (not (> v_bool_132_1567_primed 0)))
(assert (not (= m1_primed 0)))
(assert (= m2_2365 m2))
(assert (= m1_2364 m1))
(assert (<= v d))
(assert (<= 0 v))
(assert (<= mx2 d))
(assert (<= mx1 d))
(assert (<= m1 (+ 1 m2)))
(assert (<= (+ 0 m2) m1))
(assert (= r_primed r))
(assert (= l_primed l))
(assert (= m2_primed m2))
(assert (= m1_primed m1))
(assert (= v_primed v))
(assert (= d_2557 d))
(assert (= d_primed d_2481))
(assert (= mx1_2578 mx1_2484))
(assert (= mx2_2579 mx2_2487))
(assert (<= 0 m2_2479))
(assert (<= 0 mx2_2487))
(assert (<= 0 m1_2478))
(assert (<= 0 mx1_2484))
(assert (<= mx3_2601 mx1_2578))
(assert (<= mx4_2602 mx2_2579))
(assert (or (and (= max1_2599 mx1_2578) (>= mx1_2578 v_primed)) (and (= max1_2599 v_primed) (< mx1_2578 v_primed))))
(assert (or (and (= max2_2600 mx2_2579) (>= mx2_2579 max1_2599)) (and (= max2_2600 max1_2599) (< mx2_2579 max1_2599))))
(assert (<= v_int_161_1509_primed max2_2600))
(assert (<= mx3_2601 v_int_161_1509_primed))
(assert (<= mx4_2602 v_int_161_1509_primed))
(assert (<= 0 v_int_161_1509_primed))
(assert (exists ((m2_65 Int)) (<= 0 m2_65)))
(assert (exists ((m1_64 Int)) (<= 0 m1_64)))
(assert (<= 0 mx1_2578))
(assert (<= 0 mx2_2579))
(assert (= l_primed 1))
(assert (= r_primed 2))
(assert (or (and (and (< l_2529 1) (= m1_2525 0)) (= mx1_2531 0)) (and (and (<= 0 mx1_2531) (<= 1 m1_2525)) (> l_2529 0))))
(assert (or (and (and (< r_2532 1) (= m2_2526 0)) (= mx2_2534 0)) (and (and (<= 0 mx2_2534) (<= 1 m2_2526)) (> r_2532 0))))
(assert (or (and (and (< l_2482 1) (= m1_2483 0)) (= mx3_2601 0)) (and (and (<= 0 mx3_2601) (<= 1 m1_2483)) (> l_2482 0))))
(assert (or (and (and (< r_2485 1) (= m2_2486 0)) (= mx4_2602 0)) (and (and (<= 0 mx4_2602) (<= 1 m2_2486)) (> r_2485 0))))
(assert (not (= l_primed r_primed)))
;Negation of Consequence
(assert (not false))
(check-sat)