(set-info :source  loris-7.ddns.comp.nus.edu.sg/~project/hip/) 
;Variables declarations
(declare-fun right_189_1285_primed () Int)
(declare-fun left_189_1284_primed () Int)
(declare-fun nright_189_1283_primed () Int)
(declare-fun nleft_189_1282_primed () Int)
(declare-fun val_189_1281_primed () Int)
(declare-fun d_3243 () Int)
(declare-fun mx () Int)
(declare-fun m3_3242 () Int)
(declare-fun m1_3245 () Int)
(declare-fun m2_3248 () Int)
(declare-fun t_primed () Int)
(declare-fun t () Int)
(declare-fun n () Int)
(declare-fun mx1_3246 () Int)
(declare-fun m1_3240 () Int)
(declare-fun l_3244 () Int)
(declare-fun mx2_3249 () Int)
(declare-fun m2_3241 () Int)
(declare-fun r_3247 () Int)
;Relations declarations
;Axioms assertions
;Antecedent
(assert (= right_189_1285_primed r_3247))
(assert (= left_189_1284_primed l_3244))
(assert (= nright_189_1283_primed m2_3248))
(assert (= nleft_189_1282_primed m1_3245))
(assert (= val_189_1281_primed d_3243))
(assert (= n (+ (+ m2_3248 1) m1_3245)))
(assert (<= 0 d_3243))
(assert (<= mx1_3246 d_3243))
(assert (<= mx2_3249 d_3243))
(assert (<= d_3243 mx))
(assert (= (+ m3_3242 m2_3248) m1_3245))
(assert (<= 0 m3_3242))
(assert (<= m3_3242 1))
(assert (= m1_3240 m1_3245))
(assert (= m2_3241 m2_3248))
(assert (= t_primed t))
(assert (< 0 n))
(assert (or (and (and (< l_3244 1) (= m1_3240 0)) (= mx1_3246 0)) (and (and (<= 0 mx1_3246) (<= 1 m1_3240)) (> l_3244 0))))
(assert (or (and (and (< r_3247 1) (= m2_3241 0)) (= mx2_3249 0)) (and (and (<= 0 mx2_3249) (<= 1 m2_3241)) (> r_3247 0))))
;Negation of Consequence
(assert (not false))
(check-sat)