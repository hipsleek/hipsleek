(set-info :source  loris-7.ddns.comp.nus.edu.sg/~project/hip/) 
;Variables declarations
(declare-fun n () Int)
(declare-fun nmin () Int)
(declare-fun nmin_2235 () Int)
(declare-fun n_2216 () Int)
(declare-fun flted_25_2114 () Int)
(declare-fun nmin2_2120 () Int)
(declare-fun nmin1_2118 () Int)
(declare-fun flted_25_2115 () Int)
(declare-fun nmin_2217 () Int)
(declare-fun nmin1_2481 () Int)
(declare-fun nmin2_2483 () Int)
(declare-fun nmin_2133 () Int)
(declare-fun n_2132 () Int)
(declare-fun n_2234 () Int)
(declare-fun nmin_2260 () Int)
(declare-fun nmin1_2289 () Int)
(declare-fun nmin_2151 () Int)
(declare-fun n_2259 () Int)
(declare-fun n_2150 () Int)
;Relations declarations
;Axioms assertions
;Antecedent
(assert (< nmin n))
(assert (exists ((min_2529 Int)) (or (and (= min_2529 nmin1_2481) (< nmin1_2481 nmin2_2483)) (and (= min_2529 nmin2_2483) (>= nmin1_2481 nmin2_2483)))))
(assert (= (+ flted_25_2115 1) n))
(assert (= (+ flted_25_2114 2) n))
(assert (exists ((min_2528 Int)) (and (= nmin (+ 1 min_2528)) (or (and (= min_2528 nmin1_2118) (< nmin1_2118 nmin2_2120)) (and (= min_2528 nmin2_2120) (>= nmin1_2118 nmin2_2120))))))
(assert (= n_2216 flted_25_2114))
(assert (<= 0 nmin_2151))
(assert (<= 0 nmin1_2289))
(assert (<= 0 nmin_2260))
(assert (or (= nmin1_2289 nmin_2260) (= nmin1_2289 (+ 1 nmin_2260))))
(assert (<= nmin_2235 n_2234))
(assert (<= 0 nmin_2235))
(assert (= nmin_2260 nmin_2235))
(assert (<= nmin_2217 n_2216))
(assert (<= 0 nmin_2217))
(assert (= nmin_2235 nmin_2217))
(assert (= n_2234 n_2216))
(assert (<= nmin2_2120 flted_25_2114))
(assert (<= 0 nmin2_2120))
(assert (= nmin_2217 nmin2_2120))
(assert (<= nmin_2133 n_2132))
(assert (<= 0 nmin_2133))
(assert (= nmin_2151 nmin_2133))
(assert (<= nmin1_2118 flted_25_2115))
(assert (<= 0 nmin1_2118))
(assert (= nmin_2133 nmin1_2118))
(assert (= n_2132 flted_25_2115))
(assert (< nmin_2217 n_2234))
(assert (= nmin1_2481 nmin_2151))
(assert (= nmin2_2483 nmin1_2289))
(assert (= (+ n_2150 1) (+ n_2259 2)))
(assert (<= n_2150 nmin_2133))
(assert (= n_2150 n_2132))
(assert (= n_2259 n_2234))
(assert (<= nmin_2260 n_2259))
(assert (<= nmin1_2289 n_2259))
(assert (<= nmin_2151 n_2150))
;Negation of Consequence
(assert (not (= (+ 2 n_2259) (+ 1 n_2150))))
(check-sat)