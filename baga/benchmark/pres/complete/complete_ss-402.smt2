(set-info :source  loris-7.ddns.comp.nus.edu.sg/~project/hip/) 
;Variables declarations
(declare-fun nmin1_2432 () Int)
(declare-fun nmin1_2207 () Int)
(declare-fun nmin_2177 () Int)
(declare-fun nmin_2151 () Int)
(declare-fun n_2150 () Int)
(declare-fun nmin_2133 () Int)
(declare-fun n_2132 () Int)
(declare-fun nmin2_2434 () Int)
(declare-fun n_2176 () Int)
(declare-fun flted_26_2123 () Int)
(declare-fun flted_26_2122 () Int)
(declare-fun nmin1_2126 () Int)
(declare-fun nmin2_2128 () Int)
(declare-fun nmin () Int)
(declare-fun n () Int)
;Relations declarations
;Axioms assertions
;Antecedent
(assert (exists ((min_2469 Int)) (or (and (= min_2469 nmin1_2432) (< nmin1_2432 nmin2_2434)) (and (= min_2469 nmin2_2434) (>= nmin1_2432 nmin2_2434)))))
(assert (= nmin1_2432 nmin1_2207))
(assert (< nmin_2133 n_2150))
(assert (<= nmin1_2207 n_2176))
(assert (<= 0 nmin1_2207))
(assert (<= nmin2_2128 flted_26_2122))
(assert (<= 0 nmin2_2128))
(assert (<= nmin_2177 n_2176))
(assert (<= 0 nmin_2177))
(assert (or (= nmin1_2207 nmin_2177) (= nmin1_2207 (+ 1 nmin_2177))))
(assert (<= nmin_2151 n_2150))
(assert (<= 0 nmin_2151))
(assert (= nmin_2177 nmin_2151))
(assert (= n_2176 n_2150))
(assert (<= nmin_2133 n_2132))
(assert (<= 0 nmin_2133))
(assert (= nmin_2151 nmin_2133))
(assert (= n_2150 n_2132))
(assert (<= nmin1_2126 flted_26_2123))
(assert (<= 0 nmin1_2126))
(assert (= nmin_2133 nmin1_2126))
(assert (= n_2132 flted_26_2123))
(assert (= nmin2_2434 nmin2_2128))
(assert (= (+ n_2176 1) (+ flted_26_2122 1)))
(assert (= (+ flted_26_2123 1) n))
(assert (= (+ flted_26_2122 1) n))
(assert (exists ((min_2468 Int)) (and (= nmin (+ 1 min_2468)) (or (and (= min_2468 nmin1_2126) (< nmin1_2126 nmin2_2128)) (and (= min_2468 nmin2_2128) (>= nmin1_2126 nmin2_2128))))))
(assert (< nmin n))
;Negation of Consequence
(assert (not (<= nmin n)))
(check-sat)