(set-info :source  loris-7.ddns.comp.nus.edu.sg/~project/hip/) 
;Variables declarations
(declare-fun n_5041 () Int)
(declare-fun n_5024 () Int)
(declare-fun flted_161_4931 () Int)
(declare-fun n_4918 () Int)
(declare-fun m_4917 () Int)
(declare-fun height_4545 () Int)
(declare-fun m_4940 () Int)
(declare-fun height1_5017 () Int)
(declare-fun height2_5020 () Int)
(declare-fun height_5013 () Int)
(declare-fun m () Int)
(declare-fun size2_4551 () Int)
(declare-fun n () Int)
(declare-fun height2_4552 () Int)
(declare-fun Anon_4932 () Int)
(declare-fun size1_4548 () Int)
(declare-fun height1_4549 () Int)
(declare-fun n_4941 () Int)
(declare-fun size2_5019 () Int)
(declare-fun size1_5016 () Int)
(declare-fun m_4952 () Int)
(declare-fun n_4953 () Int)
(declare-fun m_5040 () Int)
(declare-fun m_5023 () Int)
(declare-fun res () Int)
(declare-fun ln () Int)
(declare-fun rrm () Int)
(declare-fun lm () Int)
(declare-fun rlm () Int)
(declare-fun flted_34_5345 () Int)
(declare-fun flted_34_5346 () Int)
(declare-fun v_node_200_2792_primed () Int)
;Relations declarations
;Axioms assertions
;Antecedent
(assert (< n_5041 n_5024))
(assert (<= 0 n_4953))
(assert (<= 0 m_4952))
(assert (<= 0 n_5041))
(assert (<= 0 m_5040))
(assert (<= 0 n_5024))
(assert (<= 0 m_5023))
(assert (<= 0 height1_5017))
(assert (<= 0 size1_5016))
(assert (= n_5041 height1_5017))
(assert (<= 0 height2_5020))
(assert (<= 0 size2_5019))
(assert (= n_5024 height2_5020))
(assert (<= 0 height1_4549))
(assert (<= 0 size1_4548))
(assert (<= 0 m_4940))
(assert (<= 0 Anon_4932))
(assert (<= 0 flted_161_4931))
(assert (= m_4940 flted_161_4931))
(assert (<= 0 n_4918))
(assert (<= 0 m_4917))
(assert (= flted_161_4931 (+ 1 m_4917)))
(assert (<= 0 height2_4552))
(assert (<= 0 size2_4551))
(assert (= n_4918 height2_4552))
(assert (= m_4917 size2_4551))
(assert (= height_4545 n))
(assert (= m_4940 (+ (+ size2_5019 1) size1_5016)))
(assert (<= height2_5020 (+ 1 height1_5017)))
(assert (<= height1_5017 (+ 1 height2_5020)))
(assert (exists ((max_5349 Int)) (and (= n_4941 (+ 1 max_5349)) (or (and (= max_5349 height1_5017) (>= height1_5017 height2_5020)) (and (= max_5349 height2_5020) (< height1_5017 height2_5020))))))
(assert (= height_5013 n_4941))
(assert (= m (+ (+ size2_4551 1) size1_4548)))
(assert (<= height2_4552 (+ 1 height1_4549)))
(assert (<= height1_4549 (+ 1 height2_4552)))
(assert (exists ((max_5350 Int)) (and (= n (+ 1 max_5350)) (or (and (= max_5350 height1_4549) (>= height1_4549 height2_4552)) (and (= max_5350 height2_4552) (< height1_4549 height2_4552))))))
(assert (= n_4941 Anon_4932))
(assert (<= 0 n_4941))
(assert (= m_4952 size1_4548))
(assert (= n_4953 height1_4549))
(assert (= (+ 2 n_4953) n_4941))
(assert (= m_5023 size2_5019))
(assert (= m_5040 size1_5016))
(assert (= lm m_4952))
(assert (= ln n_4953))
(assert (= rlm m_5040))
(assert (= rrm m_5023))
(assert (<= 0 lm))
(assert (<= 0 ln))
(assert (<= 0 rlm))
(assert (<= 0 rrm))
(assert (= res v_node_200_2792_primed))
(assert (= flted_34_5346 (+ ln 2)))
(assert (= flted_34_5345 (+ (+ (+ rrm lm) 2) rlm)))
(assert (or (and (and (< v_node_200_2792_primed 1) (= flted_34_5345 0)) (= flted_34_5346 0)) (and (and (<= 1 flted_34_5346) (<= 1 flted_34_5345)) (> v_node_200_2792_primed 0))))
;Negation of Consequence
(assert (not (or (= flted_34_5345 0) (or (= flted_34_5346 0) (< v_node_200_2792_primed 1)))))
(check-sat)