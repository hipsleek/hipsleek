(set-info :source  loris-7.ddns.comp.nus.edu.sg/~project/hip/) 
;Variables declarations
(declare-fun r_10423 () Int)
(declare-fun tmp_10404 () Int)
(declare-fun l_10419 () Int)
(declare-fun v_10418 () Int)
(declare-fun v_int_144_10400 () Int)
(declare-fun color_primed () Int)
(declare-fun color () Int)
(declare-fun c () Int)
(declare-fun b () Int)
(declare-fun a_primed () Int)
(declare-fun a () Int)
(declare-fun ha () Int)
(declare-fun na () Int)
(declare-fun flted_137_9879 () Int)
(declare-fun color_144_9880 () Int)
(declare-fun flted_12_9868 () Int)
(declare-fun v_int_145_10402 () Int)
(declare-fun v_int_147_10403 () Int)
(declare-fun res () Int)
(declare-fun v_node_147_4593_primed () Int)
(declare-fun v_10584 () Int)
(declare-fun v_int_145_10401 () Int)
(declare-fun l_10585 () Int)
(declare-fun b_primed () Int)
(declare-fun r_10589 () Int)
(declare-fun c_primed () Int)
(declare-fun nb () Int)
(declare-fun Anon_10587 () Int)
(declare-fun Anon_20 () Int)
(declare-fun ha_9876 () Int)
(declare-fun nc () Int)
(declare-fun Anon_10591 () Int)
(declare-fun flted_137_9878 () Int)
(declare-fun ha_9877 () Int)
(declare-fun bhr_10592 () Int)
(declare-fun bhr_10426 () Int)
(declare-fun bhl_10588 () Int)
(declare-fun nr_10424 () Int)
(declare-fun nr_10590 () Int)
(declare-fun nl_10586 () Int)
(declare-fun Anon_10425 () Int)
(declare-fun v_10618 () Int)
(declare-fun v_9869 () Int)
(declare-fun l_10619 () Int)
(declare-fun l_9870 () Int)
(declare-fun r_10623 () Int)
(declare-fun r_9873 () Int)
(declare-fun nl_9871 () Int)
(declare-fun Anon_10621 () Int)
(declare-fun flted_12_9867 () Int)
(declare-fun bhl_9872 () Int)
(declare-fun nr_9874 () Int)
(declare-fun Anon_10625 () Int)
(declare-fun flted_12_9866 () Int)
(declare-fun bhr_9875 () Int)
(declare-fun bhr_10626 () Int)
(declare-fun bhl_10422 () Int)
(declare-fun bhl_10622 () Int)
(declare-fun nl_10420 () Int)
(declare-fun nr_10624 () Int)
(declare-fun nl_10620 () Int)
(declare-fun Anon_10421 () Int)
;Relations declarations
;Axioms assertions
;Antecedent
(assert (= r_10423 tmp_10404))
(assert (= l_10419 a_primed))
(assert (= v_10418 v_int_147_10403))
(assert (= v_int_144_10400 0))
(assert (= ha_9877 ha))
(assert (= ha_9876 ha))
(assert (= color 1))
(assert (= flted_137_9878 0))
(assert (= color_primed color))
(assert (= c_primed c))
(assert (= b_primed b))
(assert (= a_primed a))
(assert (= bhr_9875 ha))
(assert (= bhl_9872 ha))
(assert (= na (+ (+ nr_9874 1) nl_9871)))
(assert (= flted_137_9879 1))
(assert (= flted_12_9866 0))
(assert (= flted_12_9867 0))
(assert (= flted_12_9868 1))
(assert (= color_144_9880 flted_12_9868))
(assert (= v_int_145_10401 0))
(assert (= v_int_145_10402 0))
(assert (= v_int_147_10403 0))
(assert (= res v_node_147_4593_primed))
(assert (= v_10584 v_int_145_10401))
(assert (= l_10585 b_primed))
(assert (= r_10589 c_primed))
(assert (= nl_10586 nb))
(assert (= Anon_10587 Anon_20))
(assert (= bhl_10588 ha_9876))
(assert (= nr_10590 nc))
(assert (= Anon_10591 flted_137_9878))
(assert (= bhr_10592 ha_9877))
(assert (= bhl_10588 bhr_10592))
(assert (= bhr_10426 (+ bhl_10588 1)))
(assert (= nr_10424 (+ (+ nr_10590 1) nl_10586)))
(assert (= Anon_10425 0))
(assert (= v_10618 v_9869))
(assert (= l_10619 l_9870))
(assert (= r_10623 r_9873))
(assert (= nl_10620 nl_9871))
(assert (= Anon_10621 flted_12_9867))
(assert (= bhl_10622 bhl_9872))
(assert (= nr_10624 nr_9874))
(assert (= Anon_10625 flted_12_9866))
(assert (= bhr_10626 bhr_9875))
(assert (= bhl_10622 bhr_10626))
(assert (= bhl_10422 (+ bhl_10622 1)))
(assert (= nl_10420 (+ (+ nr_10624 1) nl_10620)))
(assert (= Anon_10421 0))
;Negation of Consequence
(assert (not false))
(check-sat)