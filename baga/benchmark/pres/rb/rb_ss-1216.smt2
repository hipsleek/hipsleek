(set-info :source  loris-7.ddns.comp.nus.edu.sg/~project/hip/) 
;Variables declarations
(declare-fun bhr_10558 () Int)
(declare-fun Anon_10557 () Int)
(declare-fun nr_10556 () Int)
(declare-fun nc () Int)
(declare-fun bhl_10554 () Int)
(declare-fun Anon_10553 () Int)
(declare-fun Anon_20 () Int)
(declare-fun nl_10552 () Int)
(declare-fun nb () Int)
(declare-fun r_10555 () Int)
(declare-fun l_10551 () Int)
(declare-fun v_10550 () Int)
(declare-fun Anon_10421 () Int)
(declare-fun nl_10420 () Int)
(declare-fun bhl_10422 () Int)
(declare-fun bhr_10524 () Int)
(declare-fun Anon_10523 () Int)
(declare-fun nr_10522 () Int)
(declare-fun bhl_10520 () Int)
(declare-fun Anon_10519 () Int)
(declare-fun nl_10518 () Int)
(declare-fun r_10521 () Int)
(declare-fun r_9873 () Int)
(declare-fun l_10517 () Int)
(declare-fun l_9870 () Int)
(declare-fun v_10516 () Int)
(declare-fun v_9869 () Int)
(declare-fun res () Int)
(declare-fun v_node_147_4593_primed () Int)
(declare-fun v_int_145_10402 () Int)
(declare-fun v_int_145_10401 () Int)
(declare-fun color_144_9880 () Int)
(declare-fun flted_12_9868 () Int)
(declare-fun flted_12_9867 () Int)
(declare-fun flted_12_9866 () Int)
(declare-fun flted_137_9879 () Int)
(declare-fun na () Int)
(declare-fun nr_9874 () Int)
(declare-fun nl_9871 () Int)
(declare-fun bhl_9872 () Int)
(declare-fun bhr_9875 () Int)
(declare-fun a () Int)
(declare-fun b_primed () Int)
(declare-fun b () Int)
(declare-fun c_primed () Int)
(declare-fun c () Int)
(declare-fun color_primed () Int)
(declare-fun flted_137_9878 () Int)
(declare-fun color () Int)
(declare-fun ha_9876 () Int)
(declare-fun ha_9877 () Int)
(declare-fun ha () Int)
(declare-fun v_int_144_10400 () Int)
(declare-fun v_10418 () Int)
(declare-fun v_int_147_10403 () Int)
(declare-fun l_10419 () Int)
(declare-fun a_primed () Int)
(declare-fun r_10423 () Int)
(declare-fun tmp_10404 () Int)
;Relations declarations
;Axioms assertions
;Antecedent
(assert (= bhr_10558 ha_9877))
(assert (= Anon_10557 flted_137_9878))
(assert (= nr_10556 nc))
(assert (= bhl_10554 ha_9876))
(assert (= Anon_10553 Anon_20))
(assert (= nl_10552 nb))
(assert (= r_10555 c_primed))
(assert (= l_10551 b_primed))
(assert (= v_10550 v_int_145_10401))
(assert (= Anon_10421 0))
(assert (= nl_10420 (+ (+ nr_10522 1) nl_10518)))
(assert (= bhl_10422 (+ bhl_10520 1)))
(assert (= bhl_10520 bhr_10524))
(assert (= bhr_10524 bhr_9875))
(assert (= Anon_10523 flted_12_9866))
(assert (= nr_10522 nr_9874))
(assert (= bhl_10520 bhl_9872))
(assert (= Anon_10519 flted_12_9867))
(assert (= nl_10518 nl_9871))
(assert (= r_10521 r_9873))
(assert (= l_10517 l_9870))
(assert (= v_10516 v_9869))
(assert (= res v_node_147_4593_primed))
(assert (= v_int_147_10403 0))
(assert (= v_int_145_10402 0))
(assert (= v_int_145_10401 0))
(assert (= color_144_9880 flted_12_9868))
(assert (= flted_12_9868 1))
(assert (= flted_12_9867 0))
(assert (= flted_12_9866 0))
(assert (= flted_137_9879 1))
(assert (= na (+ (+ nr_9874 1) nl_9871)))
(assert (= bhl_9872 ha))
(assert (= bhr_9875 ha))
(assert (= a_primed a))
(assert (= b_primed b))
(assert (= c_primed c))
(assert (= color_primed color))
(assert (= flted_137_9878 0))
(assert (= color 1))
(assert (= ha_9876 ha))
(assert (= ha_9877 ha))
(assert (= v_int_144_10400 0))
(assert (= v_10418 v_int_147_10403))
(assert (= l_10419 a_primed))
(assert (= r_10423 tmp_10404))
;Negation of Consequence
(assert (not false))
(check-sat)