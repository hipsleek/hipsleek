(set-info :source  loris-7.ddns.comp.nus.edu.sg/~project/hip/) 
;Variables declarations
(declare-fun r_7037 () Int)
(declare-fun l_7033 () Int)
(declare-fun tmp_7018 () Int)
(declare-fun v_7032 () Int)
(declare-fun v_int_127_7014 () Int)
(declare-fun color_primed () Int)
(declare-fun color () Int)
(declare-fun c_primed () Int)
(declare-fun c () Int)
(declare-fun b () Int)
(declare-fun a () Int)
(declare-fun h_6233 () Int)
(declare-fun nc () Int)
(declare-fun flted_120_6234 () Int)
(declare-fun color_127_6236 () Int)
(declare-fun flted_12_6224 () Int)
(declare-fun v_int_128_7016 () Int)
(declare-fun v_int_130_7017 () Int)
(declare-fun res () Int)
(declare-fun v_node_130_4726_primed () Int)
(declare-fun v_7130 () Int)
(declare-fun v_int_128_7015 () Int)
(declare-fun l_7131 () Int)
(declare-fun a_primed () Int)
(declare-fun r_7135 () Int)
(declare-fun b_primed () Int)
(declare-fun na () Int)
(declare-fun Anon_7133 () Int)
(declare-fun flted_120_6235 () Int)
(declare-fun h () Int)
(declare-fun nb () Int)
(declare-fun Anon_7137 () Int)
(declare-fun Anon_18 () Int)
(declare-fun h_6232 () Int)
(declare-fun bhr_7138 () Int)
(declare-fun bhl_7036 () Int)
(declare-fun bhl_7134 () Int)
(declare-fun nl_7034 () Int)
(declare-fun nr_7136 () Int)
(declare-fun nl_7132 () Int)
(declare-fun Anon_7035 () Int)
(declare-fun v_7164 () Int)
(declare-fun v_6225 () Int)
(declare-fun l_7165 () Int)
(declare-fun l_6226 () Int)
(declare-fun r_7169 () Int)
(declare-fun r_6229 () Int)
(declare-fun nl_6227 () Int)
(declare-fun Anon_7167 () Int)
(declare-fun flted_12_6223 () Int)
(declare-fun bhl_6228 () Int)
(declare-fun nr_6230 () Int)
(declare-fun Anon_7171 () Int)
(declare-fun flted_12_6222 () Int)
(declare-fun bhr_6231 () Int)
(declare-fun bhr_7172 () Int)
(declare-fun bhr_7040 () Int)
(declare-fun bhl_7168 () Int)
(declare-fun nr_7038 () Int)
(declare-fun nr_7170 () Int)
(declare-fun nl_7166 () Int)
(declare-fun Anon_7039 () Int)
;Relations declarations
;Axioms assertions
;Antecedent
(assert (= r_7037 c_primed))
(assert (= l_7033 tmp_7018))
(assert (= v_7032 v_int_130_7017))
(assert (= v_int_127_7014 0))
(assert (= h_6233 h))
(assert (= h_6232 h))
(assert (= color 1))
(assert (= flted_120_6235 0))
(assert (= color_primed color))
(assert (= c_primed c))
(assert (= b_primed b))
(assert (= a_primed a))
(assert (= bhr_6231 h_6233))
(assert (= bhl_6228 h_6233))
(assert (= nc (+ (+ nr_6230 1) nl_6227)))
(assert (= flted_120_6234 1))
(assert (= flted_12_6222 0))
(assert (= flted_12_6223 0))
(assert (= flted_12_6224 1))
(assert (= color_127_6236 flted_12_6224))
(assert (= v_int_128_7015 0))
(assert (= v_int_128_7016 0))
(assert (= v_int_130_7017 0))
(assert (= res v_node_130_4726_primed))
(assert (= v_7130 v_int_128_7015))
(assert (= l_7131 a_primed))
(assert (= r_7135 b_primed))
(assert (= nl_7132 na))
(assert (= Anon_7133 flted_120_6235))
(assert (= bhl_7134 h))
(assert (= nr_7136 nb))
(assert (= Anon_7137 Anon_18))
(assert (= bhr_7138 h_6232))
(assert (= bhl_7134 bhr_7138))
(assert (= bhl_7036 (+ bhl_7134 1)))
(assert (= nl_7034 (+ (+ nr_7136 1) nl_7132)))
(assert (= Anon_7035 0))
(assert (= v_7164 v_6225))
(assert (= l_7165 l_6226))
(assert (= r_7169 r_6229))
(assert (= nl_7166 nl_6227))
(assert (= Anon_7167 flted_12_6223))
(assert (= bhl_7168 bhl_6228))
(assert (= nr_7170 nr_6230))
(assert (= Anon_7171 flted_12_6222))
(assert (= bhr_7172 bhr_6231))
(assert (= bhl_7168 bhr_7172))
(assert (= bhr_7040 (+ bhl_7168 1)))
(assert (= nr_7038 (+ (+ nr_7170 1) nl_7166)))
(assert (= Anon_7039 0))
;Negation of Consequence
(assert (not false))
(check-sat)