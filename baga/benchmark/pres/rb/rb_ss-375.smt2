(set-info :source  loris-7.ddns.comp.nus.edu.sg/~project/hip/) 
;Variables declarations
(declare-fun r_6779 () Int)
(declare-fun l_6775 () Int)
(declare-fun tmp_6760 () Int)
(declare-fun v_6774 () Int)
(declare-fun v_int_127_6756 () Int)
(declare-fun color_primed () Int)
(declare-fun color () Int)
(declare-fun c_primed () Int)
(declare-fun c () Int)
(declare-fun b () Int)
(declare-fun a () Int)
(declare-fun h_6233 () Int)
(declare-fun nc () Int)
(declare-fun flted_120_6234 () Int)
(declare-fun color_127_6236 () Int)
(declare-fun flted_12_6224 () Int)
(declare-fun v_int_128_6758 () Int)
(declare-fun v_int_130_6759 () Int)
(declare-fun res () Int)
(declare-fun v_node_130_4726_primed () Int)
(declare-fun v_6872 () Int)
(declare-fun v_int_128_6757 () Int)
(declare-fun l_6873 () Int)
(declare-fun a_primed () Int)
(declare-fun r_6877 () Int)
(declare-fun b_primed () Int)
(declare-fun na () Int)
(declare-fun Anon_6875 () Int)
(declare-fun flted_120_6235 () Int)
(declare-fun h () Int)
(declare-fun nb () Int)
(declare-fun Anon_6879 () Int)
(declare-fun Anon_18 () Int)
(declare-fun h_6232 () Int)
(declare-fun bhr_6880 () Int)
(declare-fun bhl_6778 () Int)
(declare-fun bhl_6876 () Int)
(declare-fun nl_6776 () Int)
(declare-fun nr_6878 () Int)
(declare-fun nl_6874 () Int)
(declare-fun Anon_6777 () Int)
(declare-fun v_6906 () Int)
(declare-fun v_6225 () Int)
(declare-fun l_6907 () Int)
(declare-fun l_6226 () Int)
(declare-fun r_6911 () Int)
(declare-fun r_6229 () Int)
(declare-fun nl_6227 () Int)
(declare-fun Anon_6909 () Int)
(declare-fun flted_12_6223 () Int)
(declare-fun bhl_6228 () Int)
(declare-fun nr_6230 () Int)
(declare-fun Anon_6913 () Int)
(declare-fun flted_12_6222 () Int)
(declare-fun bhr_6231 () Int)
(declare-fun bhr_6914 () Int)
(declare-fun bhr_6782 () Int)
(declare-fun bhl_6910 () Int)
(declare-fun nr_6780 () Int)
(declare-fun nr_6912 () Int)
(declare-fun nl_6908 () Int)
(declare-fun Anon_6781 () Int)
;Relations declarations
;Axioms assertions
;Antecedent
(assert (= r_6779 c_primed))
(assert (= l_6775 tmp_6760))
(assert (= v_6774 v_int_130_6759))
(assert (= v_int_127_6756 0))
(assert (= h_6233 h))
(assert (= h_6232 h))
(assert (= color 1))
(assert (= flted_120_6235 0))
(assert (= color_primed color))
(assert (= c_primed c))
(assert (= b_primed b))
(assert (= a_primed a))
(assert (= bhr_6231 h_6233))
(assert (= bhl_6228 h_6233))
(assert (= nc (+ (+ nr_6230 1) nl_6227)))
(assert (= flted_120_6234 1))
(assert (= flted_12_6222 0))
(assert (= flted_12_6223 0))
(assert (= flted_12_6224 1))
(assert (= color_127_6236 flted_12_6224))
(assert (= v_int_128_6757 0))
(assert (= v_int_128_6758 0))
(assert (= v_int_130_6759 0))
(assert (= res v_node_130_4726_primed))
(assert (= v_6872 v_int_128_6757))
(assert (= l_6873 a_primed))
(assert (= r_6877 b_primed))
(assert (= nl_6874 na))
(assert (= Anon_6875 flted_120_6235))
(assert (= bhl_6876 h))
(assert (= nr_6878 nb))
(assert (= Anon_6879 Anon_18))
(assert (= bhr_6880 h_6232))
(assert (= bhl_6876 bhr_6880))
(assert (= bhl_6778 (+ bhl_6876 1)))
(assert (= nl_6776 (+ (+ nr_6878 1) nl_6874)))
(assert (= Anon_6777 0))
(assert (= v_6906 v_6225))
(assert (= l_6907 l_6226))
(assert (= r_6911 r_6229))
(assert (= nl_6908 nl_6227))
(assert (= Anon_6909 flted_12_6223))
(assert (= bhl_6910 bhl_6228))
(assert (= nr_6912 nr_6230))
(assert (= Anon_6913 flted_12_6222))
(assert (= bhr_6914 bhr_6231))
(assert (= bhl_6910 bhr_6914))
(assert (= bhr_6782 (+ bhl_6910 1)))
(assert (= nr_6780 (+ (+ nr_6912 1) nl_6908)))
(assert (= Anon_6781 0))
;Negation of Consequence
(assert (not false))
(check-sat)