(set-info :source  loris-7.ddns.comp.nus.edu.sg/~project/hip/) 
;Variables declarations
(declare-fun bhr_7722 () Int)
(declare-fun nr_7721 () Int)
(declare-fun bhl_7719 () Int)
(declare-fun nl_7718 () Int)
(declare-fun r_7720 () Int)
(declare-fun l_7717 () Int)
(declare-fun v_7716 () Int)
(declare-fun v_int_161_7709 () Int)
(declare-fun v_int_161_7708 () Int)
(declare-fun color () Int)
(declare-fun d_primed () Int)
(declare-fun d () Int)
(declare-fun c_primed () Int)
(declare-fun c () Int)
(declare-fun b_primed () Int)
(declare-fun b () Int)
(declare-fun a_primed () Int)
(declare-fun a () Int)
(declare-fun nd () Int)
(declare-fun flted_154_7704 () Int)
(declare-fun nc () Int)
(declare-fun flted_154_7705 () Int)
(declare-fun tmp_primed () Int)
(declare-fun nb () Int)
(declare-fun flted_154_7706 () Int)
(declare-fun na () Int)
(declare-fun h () Int)
(declare-fun flted_154_7707 () Int)
(declare-fun color_primed () Int)
(declare-fun Anon_17 () Int)
(declare-fun na_7282 () Int)
(declare-fun h_7285 () Int)
(declare-fun nb_7283 () Int)
(declare-fun Anon_18 () Int)
(declare-fun nc_7284 () Int)
(declare-fun flted_121_7749 () Int)
(declare-fun flted_121_7748 () Int)
(declare-fun flted_121_7750 () Int)
(declare-fun res () Int)
;Relations declarations
;Axioms assertions
;Antecedent
(assert (= nc_7284 (+ (+ nr_7721 1) nl_7718)))
(assert (= bhl_7719 bhr_7722))
(assert (= bhr_7722 h))
(assert (= nr_7721 nd))
(assert (= bhl_7719 h))
(assert (= nl_7718 nc))
(assert (= r_7720 d_primed))
(assert (= l_7717 c_primed))
(assert (= v_7716 v_int_161_7708))
(assert (= v_int_161_7709 1))
(assert (= v_int_161_7708 0))
(assert (= color 1))
(assert (= flted_154_7704 0))
(assert (= flted_154_7705 0))
(assert (= flted_154_7706 0))
(assert (= flted_154_7707 0))
(assert (= color_primed color))
(assert (= d_primed d))
(assert (= c_primed c))
(assert (= b_primed b))
(assert (= a_primed a))
(assert (= na_7282 na))
(assert (= h_7285 h))
(assert (= nb_7283 nb))
(assert (= Anon_18 flted_154_7706))
(assert (<= 0 nd))
(assert (<= 0 flted_154_7704))
(assert (<= flted_154_7704 1))
(assert (<= 0 nc))
(assert (<= 0 flted_154_7705))
(assert (<= flted_154_7705 1))
(assert (> tmp_primed 0))
(assert (<= 0 nb))
(assert (<= 0 flted_154_7706))
(assert (<= flted_154_7706 1))
(assert (<= 0 na))
(assert (<= 1 h))
(assert (<= 0 flted_154_7707))
(assert (<= flted_154_7707 1))
(assert (= flted_121_7750 (+ (+ (+ 2 nb_7283) na_7282) nc_7284)))
(assert (= flted_121_7749 0))
(assert (= flted_121_7748 (+ 2 h_7285)))
(assert (= color_primed 0))
(assert (or (and (and (and (and (and (and (and (and (and (exists ((flted_119_400 Int)) (and (<= 0 flted_119_400) (<= flted_119_400 1))) (exists ((flted_119_399 Int)) (and (<= 0 flted_119_399) (<= flted_119_399 1)))) (exists ((h_402 Int)) (<= 1 h_402))) (exists ((h_401 Int)) (<= 1 h_401))) (<= 0 na_7282)) (<= 1 h_7285)) (<= 0 nb_7283)) (<= 0 Anon_17)) (<= Anon_17 1)) (<= 0 nc_7284)) (and (and (and (and (and (and (and (and (and (exists ((flted_120_398 Int)) (and (<= 0 flted_120_398) (<= flted_120_398 1))) (exists ((flted_120_397 Int)) (and (<= 0 flted_120_397) (<= flted_120_397 1)))) (exists ((h_404 Int)) (<= 1 h_404))) (exists ((h_403 Int)) (<= 1 h_403))) (<= 0 na_7282)) (<= 1 h_7285)) (<= 0 nb_7283)) (<= 0 Anon_18)) (<= Anon_18 1)) (<= 0 nc_7284))))
(assert (or (and (and (and (= flted_121_7749 0) (<= 2 flted_121_7748)) (<= 1 flted_121_7750)) (> res 0)) (or (and (and (and (< res 1) (= flted_121_7750 0)) (= flted_121_7748 1)) (= flted_121_7749 0)) (and (and (and (= flted_121_7749 1) (<= 1 flted_121_7748)) (<= 1 flted_121_7750)) (> res 0)))))
;Negation of Consequence
(assert (not false))
(check-sat)