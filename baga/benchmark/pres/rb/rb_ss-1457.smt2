(set-info :source  loris-7.ddns.comp.nus.edu.sg/~project/hip/) 
;Variables declarations
(declare-fun nc_10928 () Int)
(declare-fun nd () Int)
(declare-fun a () Int)
(declare-fun b () Int)
(declare-fun c_primed () Int)
(declare-fun d_primed () Int)
(declare-fun d () Int)
(declare-fun color_primed () Int)
(declare-fun flted_169_11176 () Int)
(declare-fun flted_169_11175 () Int)
(declare-fun flted_169_11173 () Int)
(declare-fun color () Int)
(declare-fun v_int_175_11178 () Int)
(declare-fun v_11292 () Int)
(declare-fun v_int_175_11177 () Int)
(declare-fun l_11293 () Int)
(declare-fun a_primed () Int)
(declare-fun r_11296 () Int)
(declare-fun b_primed () Int)
(declare-fun na () Int)
(declare-fun nb () Int)
(declare-fun bhr_11298 () Int)
(declare-fun bhl_11295 () Int)
(declare-fun ha () Int)
(declare-fun na_10926 () Int)
(declare-fun nr_11297 () Int)
(declare-fun nl_11294 () Int)
(declare-fun flted_169_11174 () Int)
(declare-fun h () Int)
(declare-fun nc () Int)
(declare-fun c () Int)
;Relations declarations
;Axioms assertions
;Antecedent
(assert (= nc_10928 nd))
(assert (= a_primed a))
(assert (= b_primed b))
(assert (= c_primed c))
(assert (= d_primed d))
(assert (= color_primed color))
(assert (= flted_169_11176 0))
(assert (= flted_169_11175 0))
(assert (= flted_169_11174 0))
(assert (= flted_169_11173 0))
(assert (= color 1))
(assert (= v_int_175_11177 0))
(assert (= v_int_175_11178 1))
(assert (= v_11292 v_int_175_11177))
(assert (= l_11293 a_primed))
(assert (= r_11296 b_primed))
(assert (= nl_11294 na))
(assert (= bhl_11295 h))
(assert (= nr_11297 nb))
(assert (= bhr_11298 h))
(assert (= bhr_11298 bhl_11295))
(assert (= bhr_11298 ha))
(assert (= bhl_11295 ha))
(assert (= na_10926 (+ (+ nr_11297 1) nl_11294)))
(assert (or (and (and (and (= flted_169_11174 0) (<= 2 h)) (<= 1 nc)) (> c 0)) (or (and (and (and (< c 1) (= nc 0)) (= h 1)) (= flted_169_11174 0)) (and (and (and (= flted_169_11174 1) (<= 1 h)) (<= 1 nc)) (> c 0)))))
;Negation of Consequence
(assert (not false))
(check-sat)