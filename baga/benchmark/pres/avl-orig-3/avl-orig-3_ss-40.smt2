(set-info :source  loris-7.ddns.comp.nus.edu.sg/~project/hip/) 
;Variables declarations
(declare-fun k2 () Int)
(declare-fun left_133_2645 () Int)
(declare-fun l () Int)
(declare-fun right_134_2648 () Int)
(declare-fun bm () Int)
(declare-fun bn () Int)
(declare-fun ba2 () Int)
(declare-fun cm () Int)
(declare-fun cn () Int)
(declare-fun ba3 () Int)
(declare-fun v_int_135_2694 () Int)
(declare-fun height_135_2689 () Int)
(declare-fun Anon_41 () Int)
(declare-fun ll () Int)
(declare-fun am () Int)
(declare-fun an () Int)
(declare-fun ba1 () Int)
(declare-fun res () Int)
(declare-fun m_2700 () Int)
(declare-fun n_2701 () Int)
(declare-fun b_2702 () Int)
(declare-fun n () Int)
(declare-fun b () Int)
(declare-fun m () Int)
(declare-fun lr () Int)
(declare-fun n_2664 () Int)
(declare-fun b_2665 () Int)
(declare-fun m_2663 () Int)
(declare-fun r () Int)
(declare-fun n_2706 () Int)
(declare-fun b_2707 () Int)
(declare-fun m_2705 () Int)
(declare-fun v_node_136_1719_primed () Int)
(declare-fun k1_primed () Int)
(declare-fun k2_primed () Int)
;Relations declarations
;Axioms assertions
;Antecedent
(assert (= k2_primed k2))
(assert (= k1_primed l))
(assert (= left_133_2645 l))
(assert (= right_134_2648 lr))
(assert (= m bm))
(assert (= n bn))
(assert (= b ba2))
(assert (<= 0 bm))
(assert (<= 0 bn))
(assert (<= 0 ba2))
(assert (<= ba2 2))
(assert (<= 0 m))
(assert (<= 0 n))
(assert (<= 0 b))
(assert (<= b 2))
(assert (= m_2663 cm))
(assert (= n_2664 cn))
(assert (= b_2665 ba3))
(assert (<= 0 cm))
(assert (<= 0 cn))
(assert (<= 0 ba3))
(assert (<= ba3 2))
(assert (<= 0 m_2663))
(assert (<= 0 n_2664))
(assert (<= 0 b_2665))
(assert (<= b_2665 2))
(assert (<= n_2664 n))
(assert (= v_int_135_2694 (+ 1 n)))
(assert (= height_135_2689 Anon_41))
(assert (= v_node_136_1719_primed ll))
(assert (= m_2700 am))
(assert (= n_2701 an))
(assert (= b_2702 ba1))
(assert (<= 0 am))
(assert (<= 0 an))
(assert (<= 0 ba1))
(assert (<= ba1 2))
(assert (= res n_2701))
(assert (= m_2705 m_2700))
(assert (= n_2706 n_2701))
(assert (= b_2707 b_2702))
(assert (<= 0 m_2700))
(assert (<= 0 n_2701))
(assert (<= 0 b_2702))
(assert (<= b_2702 2))
(assert (= k1_primed 1))
(assert (or (and (and (and (< lr 1) (= m 0)) (= n 0)) (= b 1)) (and (and (and (and (and (<= 0 b) (<= (+ (- 0 n) 2) b)) (<= b n)) (<= b 2)) (<= 1 m)) (> lr 0))))
(assert (or (and (and (and (< r 1) (= m_2663 0)) (= n_2664 0)) (= b_2665 1)) (and (and (and (and (and (<= 0 b_2665) (<= (+ (- 0 n_2664) 2) b_2665)) (<= b_2665 n_2664)) (<= b_2665 2)) (<= 1 m_2663)) (> r 0))))
(assert (= k2_primed 2))
(assert (or (and (and (and (< v_node_136_1719_primed 1) (= m_2705 0)) (= n_2706 0)) (= b_2707 1)) (and (and (and (and (and (<= 0 b_2707) (<= (+ (- 0 n_2706) 2) b_2707)) (<= b_2707 n_2706)) (<= b_2707 2)) (<= 1 m_2705)) (> v_node_136_1719_primed 0))))
(assert (not (= k1_primed k2_primed)))
;Negation of Consequence
(assert (not false))
(check-sat)