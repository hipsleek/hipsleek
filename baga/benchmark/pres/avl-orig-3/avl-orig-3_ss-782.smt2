(set-info :source  loris-7.ddns.comp.nus.edu.sg/~project/hip/) 
;Variables declarations
(declare-fun Anon_5318 () Int)
(declare-fun n2_5317 () Int)
(declare-fun m2_5316 () Int)
(declare-fun Anon_5314 () Int)
(declare-fun n1_5313 () Int)
(declare-fun m1_5312 () Int)
(declare-fun q_5315 () Int)
(declare-fun p_5311 () Int)
(declare-fun tm () Int)
(declare-fun m1_3778 () Int)
(declare-fun tm_3797 () Int)
(declare-fun flted_43_3821 () Int)
(declare-fun m2_3782 () Int)
(declare-fun Anon_3784 () Int)
(declare-fun m () Int)
(declare-fun m_3858 () Int)
(declare-fun b_3860 () Int)
(declare-fun m_4499 () Int)
(declare-fun v_node_52_3833 () Int)
(declare-fun b_4612 () Int)
(declare-fun m_4610 () Int)
(declare-fun q_3781 () Int)
(declare-fun height_79_4751 () Int)
(declare-fun v_int_79_5306 () Int)
(declare-fun n_4611 () Int)
(declare-fun b_4501 () Int)
(declare-fun n_4500 () Int)
(declare-fun v_int_54_4458 () Int)
(declare-fun n_3859 () Int)
(declare-fun b_3839 () Int)
(declare-fun n () Int)
(declare-fun resn_3822 () Int)
(declare-fun resb_3823 () Int)
(declare-fun tn_3798 () Int)
(declare-fun n_3775 () Int)
(declare-fun tn () Int)
(declare-fun n2_3783 () Int)
(declare-fun n1_3779 () Int)
(declare-fun b () Int)
;Relations declarations
;Axioms assertions
;Antecedent
(assert (<= b_3860 2))
(assert (<= 0 b_3860))
(assert (<= 0 m_3858))
(assert (<= 0 m))
(assert (<= Anon_3784 2))
(assert (<= 0 Anon_3784))
(assert (<= 0 m2_3782))
(assert (<= 0 flted_43_3821))
(assert (<= 0 tm_3797))
(assert (< 0 tm_3797))
(assert (<= 0 m1_3778))
(assert (exists ((flted_44_5363 Int)) (= (+ flted_44_5363 n2_5317) (+ 1 n1_5313))))
(assert (exists ((max_5362 Int)) (and (= v_int_79_5306 (+ 1 max_5362)) (or (and (= max_5362 n1_5313) (>= n1_5313 n2_5317)) (and (= max_5362 n2_5317) (< n1_5313 n2_5317))))))
(assert (= Anon_5318 b_4612))
(assert (= n2_5317 n_4611))
(assert (= m2_5316 m_4610))
(assert (= Anon_5314 b_4501))
(assert (= n1_5313 n_4500))
(assert (= m1_5312 m_4499))
(assert (= q_5315 q_3781))
(assert (= p_5311 v_node_52_3833))
(assert (not (= v_int_54_4458 2)))
(assert (= tm (+ (+ m2_3782 1) m1_3778)))
(assert (= tm_3797 m1_3778))
(assert (= flted_43_3821 (+ 1 tm_3797)))
(assert (= m flted_43_3821))
(assert (= m_3858 m2_3782))
(assert (= b_3860 Anon_3784))
(assert (= m_4499 m))
(assert (<= 0 m_4499))
(assert (= m_4610 m_3858))
(assert (= b_4612 b_3860))
(assert (<= 0 m_4610))
(assert (<= 0 b_4612))
(assert (<= b_4612 2))
(assert (or (and (and (and (< v_node_52_3833 1) (= m_4499 0)) (= n_4500 0)) (= b_4501 1)) (and (and (and (and (and (<= 0 b_4501) (<= (+ (- 0 n_4500) 2) b_4501)) (<= b_4501 n_4500)) (<= b_4501 2)) (<= 1 m_4499)) (> v_node_52_3833 0))))
(assert (or (and (and (and (< q_3781 1) (= m_4610 0)) (= n_4611 0)) (= b_4612 1)) (and (and (and (and (and (<= 0 b_4612) (<= (+ (- 0 n_4611) 2) b_4612)) (<= b_4612 n_4611)) (<= b_4612 2)) (<= 1 m_4610)) (> q_3781 0))))
(assert (= height_79_4751 n_3775))
(assert (= v_int_79_5306 (+ 1 n_4500)))
(assert (<= n_4611 n_4500))
(assert (<= 0 n_4611))
(assert (<= 0 n_3859))
(assert (= n_4611 n_3859))
(assert (<= b_4501 2))
(assert (<= 0 b_4501))
(assert (<= 0 n_4500))
(assert (<= b_3839 2))
(assert (<= 0 b_3839))
(assert (<= 0 n))
(assert (= b_4501 b_3839))
(assert (= n_4500 n))
(assert (= (+ v_int_54_4458 n_3859) n))
(assert (<= 0 n2_3783))
(assert (= n_3859 n2_3783))
(assert (<= resb_3823 2))
(assert (<= 0 resb_3823))
(assert (<= 0 resn_3822))
(assert (= b_3839 resb_3823))
(assert (= n resn_3822))
(assert (<= 0 tn_3798))
(assert (or (= tn_3798 resn_3822) (and (= resn_3822 (+ 1 tn_3798)) (not (= resb_3823 1)))))
(assert (< 0 tn_3798))
(assert (<= 0 n1_3779))
(assert (= tn_3798 n1_3779))
(assert (= n_3775 tn))
(assert (<= n1_3779 (+ n2_3783 1)))
(assert (<= n2_3783 (+ 1 n1_3779)))
(assert (exists ((max_5361 Int)) (and (= tn (+ 1 max_5361)) (or (and (= max_5361 n1_3779) (>= n1_3779 n2_3783)) (and (= max_5361 n2_3783) (< n1_3779 n2_3783))))))
(assert (= (+ b n2_3783) (+ 1 n1_3779)))
;Negation of Consequence
(assert (not (<= b 2)))
(check-sat)