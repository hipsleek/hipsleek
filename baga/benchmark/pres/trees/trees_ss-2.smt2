(set-info :source  loris-7.ddns.comp.nus.edu.sg/~project/hip/) 
;Variables declarations
(declare-fun right_50_1673_primed () Int)
(declare-fun q_1758 () Int)
(declare-fun left_50_1672_primed () Int)
(declare-fun val_50_1671_primed () Int)
(declare-fun Anon_1757 () Int)
(declare-fun n_1766 () Int)
(declare-fun n () Int)
(declare-fun Anon_1765 () Int)
(declare-fun Anon_19 () Int)
(declare-fun m_1764 () Int)
(declare-fun Anon_1763 () Int)
(declare-fun x () Int)
(declare-fun y_primed () Int)
(declare-fun y () Int)
(declare-fun v_bool_45_1681_primed () Int)
(declare-fun self_1756 () Int)
(declare-fun x_primed () Int)
(declare-fun p_1755 () Int)
(declare-fun Anon_18 () Int)
(declare-fun m () Int)
(declare-fun n1_1759 () Int)
(declare-fun flted_40_1778 () Int)
(declare-fun z_primed () Int)
;Relations declarations
;Axioms assertions
;Antecedent
(assert (= right_50_1673_primed q_1758))
(assert (= left_50_1672_primed p_1755))
(assert (= val_50_1671_primed Anon_1757))
(assert (<= 0 n_1766))
(assert (<= 0 m_1764))
(assert (= flted_40_1778 (+ n_1766 m_1764)))
(assert (<= 0 n1_1759))
(assert (<= 0 n))
(assert (= n_1766 n))
(assert (= Anon_1765 Anon_19))
(assert (= m_1764 n1_1759))
(assert (= Anon_1763 self_1756))
(assert (= x_primed x))
(assert (= y_primed y))
(assert (> x_primed 0))
(assert (not (> v_bool_45_1681_primed 0)))
(assert (= self_1756 x_primed))
(assert (= p_1755 Anon_18))
(assert (= m (+ 1 n1_1759)))
(assert (or (and (< z_primed 1) (= flted_40_1778 0)) (and (<= 1 flted_40_1778) (> z_primed 0))))
;Negation of Consequence
(assert (not false))
(check-sat)