(set-info :source  loris-7.ddns.comp.nus.edu.sg/~project/hip/) 
;Variables declarations
(declare-fun right_239_1488_primed () Int)
(declare-fun left_239_1487_primed () Int)
(declare-fun height_239_1486_primed () Int)
(declare-fun Anon_57 () Int)
(declare-fun ele_239_1485_primed () Int)
(declare-fun Anon_56 () Int)
(declare-fun v_int_239_1495_primed () Int)
(declare-fun Anon_58 () Int)
(declare-fun an () Int)
(declare-fun am () Int)
(declare-fun left_238_2907 () Int)
(declare-fun right_237_2904 () Int)
(declare-fun r () Int)
(declare-fun k1_primed () Int)
(declare-fun k1 () Int)
(declare-fun bn () Int)
(declare-fun Anon_61 () Int)
(declare-fun bm () Int)
(declare-fun rl () Int)
(declare-fun cn () Int)
(declare-fun Anon_62 () Int)
(declare-fun cm () Int)
(declare-fun rr () Int)
(declare-fun k2_primed () Int)
(declare-fun n () Int)
(declare-fun b () Int)
(declare-fun m () Int)
(declare-fun l () Int)
;Relations declarations
;Axioms assertions
;Antecedent
(assert (= right_239_1488_primed rl))
(assert (= left_239_1487_primed l))
(assert (= height_239_1486_primed Anon_57))
(assert (= ele_239_1485_primed Anon_56))
(assert (<= b 2))
(assert (<= 0 b))
(assert (<= 0 n))
(assert (<= 0 m))
(assert (= v_int_239_1495_primed n))
(assert (<= Anon_58 2))
(assert (<= 0 Anon_58))
(assert (<= 0 an))
(assert (<= 0 am))
(assert (= b Anon_58))
(assert (= n an))
(assert (= m am))
(assert (= left_238_2907 rl))
(assert (= right_237_2904 r))
(assert (= k2_primed r))
(assert (= k1_primed k1))
(assert (or (and (and (and (< rl 1) (= bm 0)) (= bn 0)) (= Anon_61 1)) (and (and (and (and (and (<= 0 Anon_61) (<= (+ (- 0 bn) 2) Anon_61)) (<= Anon_61 bn)) (<= Anon_61 2)) (<= 1 bm)) (> rl 0))))
(assert (or (and (and (and (< rr 1) (= cm 0)) (= cn 0)) (= Anon_62 1)) (and (and (and (and (and (<= 0 Anon_62) (<= (+ (- 0 cn) 2) Anon_62)) (<= Anon_62 cn)) (<= Anon_62 2)) (<= 1 cm)) (> rr 0))))
(assert (= k2_primed 1))
(assert (or (and (and (and (< l 1) (= m 0)) (= n 0)) (= b 1)) (and (and (and (and (and (<= 0 b) (<= (+ (- 0 n) 2) b)) (<= b n)) (<= b 2)) (<= 1 m)) (> l 0))))
;Negation of Consequence
(assert (not false))
(check-sat)