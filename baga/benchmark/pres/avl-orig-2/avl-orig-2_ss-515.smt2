(set-info :source  loris-7.ddns.comp.nus.edu.sg/~project/hip/) 
;Variables declarations
(declare-fun v_bool_89_2132_primed () Int)
(declare-fun v_bool_86_2211_primed () Int)
(declare-fun Anon_3751 () Int)
(declare-fun tm () Int)
(declare-fun b () Int)
(declare-fun n_3750 () Int)
(declare-fun tn () Int)
(declare-fun v_bool_80_2243_primed () Int)
(declare-fun tmp_primed () Int)
(declare-fun x_primed () Int)
(declare-fun x () Int)
(declare-fun t () Int)
(declare-fun m1_3753 () Int)
(declare-fun n1_3754 () Int)
(declare-fun Anon_3755 () Int)
(declare-fun tm_3772 () Int)
(declare-fun tn_3773 () Int)
(declare-fun b_3774 () Int)
(declare-fun left_87_3802 () Int)
(declare-fun p_3752 () Int)
(declare-fun flted_76_3801 () Int)
(declare-fun flted_76_3800 () Int)
(declare-fun flted_76_3799 () Int)
(declare-fun m2_3757 () Int)
(declare-fun n2_3758 () Int)
(declare-fun Anon_3759 () Int)
(declare-fun v_int_89_4444 () Int)
(declare-fun m () Int)
(declare-fun n () Int)
(declare-fun b_3814 () Int)
(declare-fun v_int_118_2232_primed () Int)
(declare-fun q_3756 () Int)
(declare-fun m_3833 () Int)
(declare-fun n_3834 () Int)
(declare-fun b_3835 () Int)
(declare-fun res () Int)
(declare-fun m_4597 () Int)
(declare-fun n_4598 () Int)
(declare-fun b_4599 () Int)
(declare-fun t_primed () Int)
(declare-fun n_4487 () Int)
(declare-fun b_4488 () Int)
(declare-fun m_4486 () Int)
(declare-fun v_node_87_3810 () Int)
(declare-fun n_4661 () Int)
(declare-fun b_4662 () Int)
(declare-fun m_4660 () Int)
(declare-fun v_node_118_2227_primed () Int)
;Relations declarations
;Axioms assertions
;Antecedent
(assert (not (> v_bool_89_2132_primed 0)))
(assert (not (= v_int_89_4444 2)))
(assert (> v_bool_86_2211_primed 0))
(assert (< x_primed Anon_3751))
(assert (= tm (+ (+ m2_3757 1) m1_3753)))
(assert (exists ((max_79 Int)) (and (= tn (+ max_79 1)) (or (and (= max_79 n1_3754) (>= n1_3754 n2_3758)) (and (= max_79 n2_3758) (< n1_3754 n2_3758))))))
(assert (<= (+ 0 n2_3758) (+ n1_3754 1)))
(assert (<= n1_3754 (+ 1 n2_3758)))
(assert (= (+ b n2_3758) (+ 1 n1_3754)))
(assert (= n_3750 tn))
(assert (not (> v_bool_80_2243_primed 0)))
(assert (> t_primed 0))
(assert (< tmp_primed 1))
(assert (= x_primed x))
(assert (= t_primed t))
(assert (= tm_3772 m1_3753))
(assert (= tn_3773 n1_3754))
(assert (= b_3774 Anon_3755))
(assert (<= 0 m1_3753))
(assert (<= 0 n1_3754))
(assert (<= 0 Anon_3755))
(assert (<= Anon_3755 2))
(assert (= flted_76_3801 1))
(assert (= flted_76_3800 1))
(assert (= flted_76_3799 1))
(assert (= tn_3773 0))
(assert (= tm_3772 0))
(assert (< p_3752 1))
(assert (<= 0 tm_3772))
(assert (<= 0 tn_3773))
(assert (<= 0 b_3774))
(assert (<= b_3774 2))
(assert (= left_87_3802 p_3752))
(assert (= m flted_76_3801))
(assert (= n flted_76_3800))
(assert (= b_3814 flted_76_3799))
(assert (<= 0 flted_76_3801))
(assert (<= 0 flted_76_3800))
(assert (<= 0 flted_76_3799))
(assert (<= flted_76_3799 2))
(assert (= m_3833 m2_3757))
(assert (= n_3834 n2_3758))
(assert (= b_3835 Anon_3759))
(assert (<= 0 m2_3757))
(assert (<= 0 n2_3758))
(assert (<= 0 Anon_3759))
(assert (<= Anon_3759 2))
(assert (= (+ v_int_89_4444 n_3834) n))
(assert (= m_4486 m))
(assert (= n_4487 n))
(assert (= b_4488 b_3814))
(assert (<= 0 m))
(assert (<= 0 n))
(assert (<= 0 b_3814))
(assert (<= b_3814 2))
(assert (= v_int_118_2232_primed n_4487))
(assert (<= 0 m_4486))
(assert (<= 0 n_4487))
(assert (<= 0 b_4488))
(assert (<= b_4488 2))
(assert (= v_node_118_2227_primed q_3756))
(assert (= m_4597 m_3833))
(assert (= n_4598 n_3834))
(assert (= b_4599 b_3835))
(assert (<= 0 m_3833))
(assert (<= 0 n_3834))
(assert (<= 0 b_3835))
(assert (<= b_3835 2))
(assert (= res n_4598))
(assert (= m_4660 m_4597))
(assert (= n_4661 n_4598))
(assert (= b_4662 b_4599))
(assert (<= 0 m_4597))
(assert (<= 0 n_4598))
(assert (<= 0 b_4599))
(assert (<= b_4599 2))
(assert (= t_primed 1))
(assert (or (and (and (and (< v_node_87_3810 1) (= m_4486 0)) (= n_4487 0)) (= b_4488 1)) (and (and (and (and (and (<= 0 b_4488) (<= (+ (- 0 n_4487) 2) b_4488)) (<= b_4488 n_4487)) (<= b_4488 2)) (<= 1 m_4486)) (> v_node_87_3810 0))))
(assert (or (and (and (and (< v_node_118_2227_primed 1) (= m_4660 0)) (= n_4661 0)) (= b_4662 1)) (and (and (and (and (and (<= 0 b_4662) (<= (+ (- 0 n_4661) 2) b_4662)) (<= b_4662 n_4661)) (<= b_4662 2)) (<= 1 m_4660)) (> v_node_118_2227_primed 0))))
;Negation of Consequence
(assert (not false))
(check-sat)