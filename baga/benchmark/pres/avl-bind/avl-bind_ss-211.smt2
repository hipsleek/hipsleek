(set-info :source  loris-7.ddns.comp.nus.edu.sg/~project/hip/) 
;Variables declarations
(declare-fun v_primed () Int)
(declare-fun r () Int)
(declare-fun lr_primed () Int)
(declare-fun ll_primed () Int)
(declare-fun rm () Int)
(declare-fun flted_49_3265 () Int)
(declare-fun res () Int)
(declare-fun v_int_56_2290_primed () Int)
(declare-fun v_int_56_2291_primed () Int)
(declare-fun lln () Int)
(declare-fun llm () Int)
(declare-fun ll () Int)
(declare-fun flted_49_3266 () Int)
(declare-fun lrm () Int)
(declare-fun lr () Int)
(declare-fun n () Int)
(declare-fun m () Int)
(declare-fun r_primed () Int)
;Relations declarations
;Axioms assertions
;Antecedent
(assert (= v_primed 10))
(assert (= (+ flted_49_3265 1) lln))
(assert (= (+ flted_49_3266 1) lln))
(assert (= r_primed r))
(assert (= lr_primed lr))
(assert (= ll_primed ll))
(assert (= m rm))
(assert (= n flted_49_3265))
(assert (<= 0 rm))
(assert (<= 0 flted_49_3265))
(assert (= v_int_56_2291_primed n))
(assert (<= 0 m))
(assert (<= 0 n))
(assert (= v_int_56_2290_primed 1))
(assert (= res (+ v_int_56_2290_primed v_int_56_2291_primed)))
(assert (or (and (and (< ll 1) (= llm 0)) (= lln 0)) (and (and (<= 1 lln) (<= 1 llm)) (> ll 0))))
(assert (or (and (and (< lr 1) (= lrm 0)) (= flted_49_3266 0)) (and (and (<= 1 flted_49_3266) (<= 1 lrm)) (> lr 0))))
(assert (or (and (and (< r_primed 1) (= m 0)) (= n 0)) (and (and (<= 1 n) (<= 1 m)) (> r_primed 0))))
;Negation of Consequence
(assert (not false))
(check-sat)