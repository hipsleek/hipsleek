(set-info :source  loris-7.ddns.comp.nus.edu.sg/~project/hip/) 
;Variables declarations
(declare-fun n_3821 () Int)
(declare-fun v_bool_207_1721_primed () Int)
(declare-fun v_bool_201_1722_primed () Int)
(declare-fun n_3699 () Int)
(declare-fun v_bool_198_1732_primed () Int)
(declare-fun v_bool_170_1733_primed () Int)
(declare-fun Anon_3368 () Int)
(declare-fun m () Int)
(declare-fun n_3367 () Int)
(declare-fun n () Int)
(declare-fun v_bool_165_1734_primed () Int)
(declare-fun a_primed () Int)
(declare-fun a () Int)
(declare-fun x () Int)
(declare-fun xright_3639 () Int)
(declare-fun q_3372 () Int)
(declare-fun m2_3373 () Int)
(declare-fun n2_3374 () Int)
(declare-fun m_3630 () Int)
(declare-fun n_3631 () Int)
(declare-fun flted_160_3644 () Int)
(declare-fun Anon_3645 () Int)
(declare-fun m_3640 () Int)
(declare-fun m1_3370 () Int)
(declare-fun n1_3371 () Int)
(declare-fun n_3641 () Int)
(declare-fun m2_3705 () Int)
(declare-fun n2_3706 () Int)
(declare-fun m1_3702 () Int)
(declare-fun n1_3703 () Int)
(declare-fun m_3716 () Int)
(declare-fun n_3717 () Int)
(declare-fun m_3754 () Int)
(declare-fun n_3755 () Int)
(declare-fun m_3707 () Int)
(declare-fun n_3708 () Int)
(declare-fun m_3769 () Int)
(declare-fun n_3770 () Int)
(declare-fun m2_3827 () Int)
(declare-fun n2_3828 () Int)
(declare-fun m1_3824 () Int)
(declare-fun n1_3825 () Int)
(declare-fun m_3650 () Int)
(declare-fun n_3651 () Int)
(declare-fun am () Int)
(declare-fun an () Int)
(declare-fun bm () Int)
(declare-fun bn () Int)
(declare-fun cm () Int)
(declare-fun cn () Int)
(declare-fun dm () Int)
(declare-fun res () Int)
(declare-fun flted_105_3878 () Int)
(declare-fun flted_105_3879 () Int)
(declare-fun v_node_208_1707_primed () Int)
(declare-fun p_3701 () Int)
(declare-fun xright_primed () Int)
(declare-fun x_primed () Int)
;Relations declarations
;Axioms assertions
;Antecedent
(assert (= m_3754 (+ (+ m2_3827 1) m1_3824)))
(assert (<= (+ 0 n2_3828) (+ n1_3825 1)))
(assert (<= n1_3825 (+ 1 n2_3828)))
(assert (exists ((max_32 Int)) (and (= n_3755 (+ 1 max_32)) (or (and (= max_32 n1_3825) (>= n1_3825 n2_3828)) (and (= max_32 n2_3828) (< n1_3825 n2_3828))))))
(assert (= n_3821 n_3755))
(assert (> v_bool_207_1721_primed 0))
(assert (not (> v_bool_201_1722_primed 0)))
(assert (<= n_3708 n_3717))
(assert (= m_3640 (+ (+ m2_3705 1) m1_3702)))
(assert (<= (+ 0 n2_3706) (+ n1_3703 1)))
(assert (<= n1_3703 (+ 1 n2_3706)))
(assert (exists ((max_32 Int)) (and (= n_3641 (+ 1 max_32)) (or (and (= max_32 n1_3703) (>= n1_3703 n2_3706)) (and (= max_32 n2_3706) (< n1_3703 n2_3706))))))
(assert (= n_3699 n_3641))
(assert (> v_bool_198_1732_primed 0))
(assert (not (> v_bool_170_1733_primed 0)))
(assert (< Anon_3368 a_primed))
(assert (= m (+ (+ m2_3373 1) m1_3370)))
(assert (<= (+ 0 n2_3374) (+ n1_3371 1)))
(assert (<= n1_3371 (+ 1 n2_3374)))
(assert (exists ((max_32 Int)) (and (= n (+ 1 max_32)) (or (and (= max_32 n1_3371) (>= n1_3371 n2_3374)) (and (= max_32 n2_3374) (< n1_3371 n2_3374))))))
(assert (= n_3367 n))
(assert (not (> v_bool_165_1734_primed 0)))
(assert (> x_primed 0))
(assert (= a_primed a))
(assert (= x_primed x))
(assert (= xright_3639 q_3372))
(assert (= m_3630 m2_3373))
(assert (= n_3631 n2_3374))
(assert (<= 0 m2_3373))
(assert (<= 0 n2_3374))
(assert (= flted_160_3644 (+ 1 m_3630)))
(assert (<= 0 m_3630))
(assert (<= 0 n_3631))
(assert (= m_3640 flted_160_3644))
(assert (= n_3641 Anon_3645))
(assert (<= 0 flted_160_3644))
(assert (<= 0 Anon_3645))
(assert (<= 0 m_3640))
(assert (<= 0 n_3641))
(assert (= m_3650 m1_3370))
(assert (= n_3651 n1_3371))
(assert (<= 0 m1_3370))
(assert (<= 0 n1_3371))
(assert (= (+ 2 n_3651) n_3641))
(assert (= m_3707 m2_3705))
(assert (= n_3708 n2_3706))
(assert (<= 0 m2_3705))
(assert (<= 0 n2_3706))
(assert (= m_3716 m1_3702))
(assert (= n_3717 n1_3703))
(assert (<= 0 m1_3702))
(assert (<= 0 n1_3703))
(assert (= m_3754 m_3716))
(assert (= n_3755 n_3717))
(assert (<= 0 m_3716))
(assert (<= 0 n_3717))
(assert (<= 0 m_3754))
(assert (<= 0 n_3755))
(assert (= (+ n_3770 1) n_3755))
(assert (= m_3769 m_3707))
(assert (= n_3770 n_3708))
(assert (<= 0 m_3707))
(assert (<= 0 n_3708))
(assert (= am m_3650))
(assert (= an n_3651))
(assert (= bm m1_3824))
(assert (= bn n1_3825))
(assert (= cm m2_3827))
(assert (= cn n2_3828))
(assert (= dm m_3769))
(assert (<= 0 m_3769))
(assert (<= 0 n_3770))
(assert (<= 0 m2_3827))
(assert (<= 0 n2_3828))
(assert (<= 0 m1_3824))
(assert (<= 0 n1_3825))
(assert (<= 0 m_3650))
(assert (<= 0 n_3651))
(assert (= flted_105_3879 (+ (+ (+ (+ dm bm) 3) am) cm)))
(assert (= flted_105_3878 (+ an 2)))
(assert (exists ((an_91 Int)) (<= 0 an_91)))
(assert (<= 0 am))
(assert (<= 0 an))
(assert (<= 0 bm))
(assert (<= 0 bn))
(assert (<= 0 cm))
(assert (<= 0 cn))
(assert (<= 0 dm))
(assert (= res v_node_208_1707_primed))
(assert (= p_3701 1))
(assert (or (and (and (< v_node_208_1707_primed 1) (= flted_105_3879 0)) (= flted_105_3878 0)) (and (and (<= 1 flted_105_3878) (<= 1 flted_105_3879)) (> v_node_208_1707_primed 0))))
(assert (= xright_primed 2))
(assert (= x_primed 3))
(assert (not (= p_3701 x_primed)))
(assert (not (= p_3701 xright_primed)))
(assert (not (= xright_primed x_primed)))
;Negation of Consequence
(assert (not false))
(check-sat)