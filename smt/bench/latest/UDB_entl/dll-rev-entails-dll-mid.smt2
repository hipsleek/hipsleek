...
;; DLLmid(hd,p,tl,n)::= hd->n,p & hd=tl | 
;;	hd->tl,p * tl->n,hd |
;;	\E x,y,z. x->y,z * DLL(y,x,tl,n) * DLLrev(hd,p,z,x)

(define-fun DLL_plus_mid ((?hd DLL_t) (?p DLL_t) (?tl DLL_t) (?n DLL_t)) Space
  (tospace (or
    (and (tobool (pto ?hd (sref (ref next ?n) (ref prev ?p)))) (= ?hd ?tl))
    (tobool (ssep (pto ?hd (sref (ref next ?tl) (ref prev ?p))) (points_to ?tl ?n ?hd)))
    (exists ((?x DLL_t) (?y DLL_t) (?z DLL_t)) (tobool (ssep (pto ?x (sref (ref next ?y) (ref prev ?z)))
    	    	 	    	       	       	       	     (DLL_plus ?y ?x ?tl ?n)
...
;; DLL_plus_rev(x,nil,y,nil) |- DLL_plus_mid(x,nil,y,nil)

(assert (tobool (DLL_plus_rev x nil y nil)
))

(assert (not (tobool
	(DLL_plus_mid x nil y nil)
)))
