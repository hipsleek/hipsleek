...
(tospace (or 
	...
	(exists ((?x1 GenTyp) (?x2 GenTyp) (?x3 GenTyp))
		
		 (tobool 
	(sep (succ3circuit ?x1 ?x2 ?x3 ?y1 ?y2 ?y3)
		(Q ?x1 ?x2 ?x3)
	)
...
;;(assert (tobool (index alpha1 (succ3circuit  x0 x1 x2 x3 x4 x5))))
;;(define-fun x0 () GenTyp)
;;(define-fun x1 () GenTyp)
;;(define-fun x2 () GenTyp)
;;(assert (tobool (index alpha1 (P  x0 x1 x2))))
(define-fun x0 () GenTyp)
(define-fun x1 () GenTyp)
(define-fun x2 () GenTyp)
(assert (tobool (index alpha1 (Q  x0 x1 x2))))
