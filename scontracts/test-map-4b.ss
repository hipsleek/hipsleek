...
void foo27(address id, int amount)
     requires [ub] userBalances::Map<ub>
     ensures  (exists ub1: userBalances'::Map<ub1> & ub1[id] = ub[id] + amount);
{
    userBalances[id] = userBalances[id] + amount;
}

void foo28(address sender, address receiver, int amount)
     requires [ub] userBalances::Map<ub>
     ensures  (exists ub1: userBalances'::Map<ub1> & ub1[sender] = ub[sender] - amount & ub1[receiver] = ub[receiver] + amount);
{
    userBalances[sender] = userBalances[sender] - amount;
    userBalances[receiver] = userBalances[receiver] + amount;
}
