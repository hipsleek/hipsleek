...
// mapping(address => uint) private userBalances;
 ...
 pred MapE<key,val,q> == self::Map<q> * key::Entry<val,self>;

int getUserBalance(int userid)
   requires  userBalances::Map<q>@L * userid::Entry<n,userBalances>@L
   ensures   userBalances = userBalances' & res=n;

void setUserBalance(int userid, int amount)
   requires  userBalances::Map<q> * userid::Entry<n,userBalances>
   ensures   userBalances::Map<q> * userid::Entry<amount,userBalances>;
...
void withdrawBalance_a()
   //infer [@reentrancy]
   requires  msg::message<_,_,id,_,_>@L * userBalances::MapE<id,n,q> & n>0 & bal>=n
   ensures   userBalances::MapE<id,0,q> & bal'=bal-n;
   requires  msg::message<_,_,id,_,_>@L * userBalances::MapE<id,n,q>@L & n=0 & bal>=n
