...
node get_next(node x)
	...
	requires x::ll<L> & len(L) > 0 & alln(0, L)
	ensures x::lz<L1> * res::lz<L2> & len(L1) = 1 & len(L2) = len(L) - 1; /*  & L1 = [| 0 |] */
...
node get_next_next(node x) 
	...
	ensures res::lz<L2> & len(L2) = len(L1) - 2;
...
void insert(node x, int a)
	...
	ensures x::lz<L2> & len(L2) = len(L1) + 1;
