...
ll1<n, S> == self =  null & n = 0 & S = {} 
	or self::node<v, r> * r::ll1<n1, S1> & n = 1 + n1 & S = union(S1, {v})
	inv n >= 0;

sll1<n, S> == self::node<v1, null> & n = 1 & S = {v1}
	or self::node<v2, r> * r::sll1<n1, S1> & r != null 
	& S = union(S1, {v2}) &	forall(x: (x notin S1 | v2 <= x)) & n = n1 + 1
  inv n >= 1 & self !=null;

void partition1(node x, ref node y, ref node z, int c)
	requires x::ll1<n, S> 
    ensures y'::ll1<n1, S1> * z'::ll1<n2, S2> & S = union(S1, S2) &
...
node append_bll1(node x, node y)
	requires x::sll1<n1, S1> * y::sll1<n2, S2> & 
	forall (a, b:(a notin S1 | b notin S2 | a <= b | a>0 & a<=0))
	ensures res::sll1<n3, S3> & S3 = union(S1, S2) & n3 = n1 + n2;
