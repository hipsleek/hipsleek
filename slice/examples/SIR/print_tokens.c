 ...
 * ****************************************************************** */
int fgets(ref int [] s, int end, int fp)
 requires true
	ensures dom(s',0,80) & s'[80]=0 & res=1;

int get_char(ref cstream stream_ptr)
  requires stream_ptr::cstream<fp,sind,s> & dom(s,0,80) & sind>=0 & sind<=80 & s[80]=0
      case{
  sind=80 ->ensures stream_ptr'::cstream<fp,sind1,s1> & dom(s1,0,80)& (sind1=6) & //'
    res=s1[5] & s1[80]=0;//'res=-2 or s1[5]
  sind<80 ->
 case {
    s[sind]=0 -> ensures stream_ptr'::cstream<fp,sind1,s1> & dom(s1,0,80)& (sind1=6)&
     res=s1[5]  & s1[80]=0 ;//'res=-2 or s1[5]
    s[sind]!=0 -> ensures stream_ptr'::cstream<fp,sind1,s> & (sind1=sind+1)& sind1>=1 & sind1<=80 & res=s[sind];//'
 }
 ...
 }
{
  ...
  {
    if(fgets(stream_ptr.stream,80-5,stream_ptr.fp) == 0)//START 5
...
bool is_end_of_character_stream(cstream stream_ptr)
  requires stream_ptr::cstream<fp,sind,s> & dom(s,0,80) & sind>=1 & sind<=80 & s[80]=0
 case{
...
void unget_char(int ch, ref cstream stream_ptr)
  requires stream_ptr::cstream<fp,sind,s> & sind>=0 & sind<=80 & dom(s,0,80) & s[80]=0
 case {
...
token get_token_loop(ref cstream tstream_ptr,ref token token_ptr, ref int ch,ref int cu_state,ref int token_ind)
 requires tstream_ptr::cstream<fp,sind,s>*token_ptr::token<_,ts> & dom(s,0,80) & s[80]=0 & sind>=1 & sind<=80 & cu_state>=0 & cu_state<=54
		    & token_ind>=0 & token_ind<=80 & dom(ts,0,80)
case{
   token_ind<80 -> case {
     cu_state=31 -> case {
 ...
 * ****************************************************************** */
 void numeric_case_loop (ref int token_ind, ref cstream tstream_ptr, ref int [] token_str,ref int ch)
  requires tstream_ptr::cstream<fp,sind,s> & dom(s, 0, 80) & s[80]=0 & sind>=1 & sind<=80
   & dom(token_str, 0, 80) & token_ind>=1 & token_ind<=80
 case{
...
void strcpy(ref int [] des, int [] src)
  requires dom(des, 0, 80) & dom(src, 0, 80)
...
token numeric_case(ref cstream tstream_ptr, ref token token_ptr, int ch, int[] token_str, int token_ind)
  requires tstream_ptr::cstream<fp,sind,s>*token_ptr::token<id,tstring> & dom(s, 0, 80)&
	 ...
	 dom(tstring, 0, 80) & dom(token_str, 0, 80)
 case{
...
token error_or_eof_case(ref cstream tstream_ptr, ref token token_ptr,int cu_state,int[] token_str,int token_ind, int ch) 
  requires tstream_ptr::cstream<fp,sind,s>*token_ptr::token<_, tstring>  & s[80]=0 & sind>=1 & sind<=80 & token_ind>=1
        & token_ind<=80 & dom(token_str, 0, 80) & dom(s, 0, 80) & dom(tstring, 0, 80)
case{
  s[sind-1]=-2 /*EOF*/->
 ...
 * ******************************************************************* */
void skip_loop (ref int c, ref cstream stream_ptr)
  requires stream_ptr::cstream<fp,sind,s> & dom(s,0,80) & sind>=1 & sind<=80 & s[80]=0
  case {
  c=10 | s[sind-1]=-2 ->ensures stream_ptr'::cstream<fp,sind,s> & c'=c;
  c!=10 &  s[sind-1]!=-2 ->ensures stream_ptr'::cstream<fp,sind1,s1> & dom(s1,0,80) & sind1>=1 & sind1<=80 & s1[80]=0;//'
 }
{
  /*
...
int next_state(int state, int ch) 
 requires state>=-3 & state<=54 & ch>=-2 & ch<=126 //-2 EOF
  ...
  state=10 | state=17 |state=51 -> case{
  ...
  state>0 & state!=5&state!=10&state!=17&state!=51&state!=28 & state!=31 -> ensures res>=-3 & res<=32 & res!=7 & res!=28;
}
{
  int tmp;
  if(state < 0)
...
requires t::token<id, _>//& dom(tstring, 0, 79)
  case {
  id=0 -> ensures res;
  id!=0 -> ensures !res;
 }
...
bool print_token(token token_ptr)
 requires token_ptr::token<id, _> //& dom(tstring, 0, 79)
  case{
...
void gat_loop1(ref int [] token_str,ref int ind)
 requires dom(token_str, 0, 80) & ind>=0 & ind<=80
...
*/
 ...
 requires dom(token_str, 0, 80) & ind>=0 & ind<=upb & upb >=0 & upb <=80
