...
requires (str_com_id=0 | str_com_id=1|str_com_id=2)&ch>=-1 &ch<=126
case {
  ch=-1 ->  ensures res;
  ch=10 ->  ensures res;
  ch=34 -> case{
...
requires dom(tok, 0, 80)& tok[80]=0
ensures true;
{
...
bool is_num_constant_loop(int[] str, ref int i)
  requires dom(str, 0, 80) & str[80]=0 & i>=1 & i<=80
  ensures i'>=i;//'
{
  /*
...
bool is_num_constant(int[] str)
  requires dom(str, 0, 80) & str[80]=0
  ensures true;
{
  int i=1;
