...
let rec contains_inf_eq_b_formula (bf: CP.b_formula) : bool =
  ...
  | CP.BagMax _
  (* | CP.VarPerm _ *)
  | CP.RelForm _ | CP.Security _ | CP.ExplicitFlow _ | CP.ImplicitFlow _ -> false
...
let rec sub_inf_list_exp (exp: CP.exp) (vars: CP.spec_var list) (is_neg: bool) : CP.exp =
  ...
  | CP.BExpr f -> CP.BExpr (sub_inf_list f vars is_neg false)

and sub_inf_list_b_formula (bf:CP.b_formula) (vl: CP.spec_var list) (is_neg: bool) (is_bound: bool)
  ...
  let p_f_conv,tbf = if is_bound && is_neg
    ...
    then (match p_f with
      ...
      (match p_f with
       ...
       | CP.BagMax _
       (* | CP.VarPerm _ *)
       | CP.RelForm _  | CP.Security _ | CP.ExplicitFlow _ | CP.ImplicitFlow _ -> p_f,tbf
      ) in (p_f_conv,bf_ann),(tbf,bf_ann)

(*
substitute all variables in vl with \inf in f
*)
and sub_inf_list (f:CP.formula) (vl: CP.spec_var list) (is_neg: bool) (is_bound: bool) : CP.formula =
