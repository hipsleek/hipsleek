...
let trans_specs_hprel_2_cview iprog cprog proc_name unk_hps to_unfold_hps hpdefs chprels_decl =
  ...
  let formula_subst_dangling_pred dang_hps to_unfold_hps post_hps f0=
    let _ =  Debug.info_hprint (add_str "f0" (Cprinter.string_of_formula)) f0 no_pos in
    let _ =  Debug.info_hprint (add_str "dang_hps" (!CP.print_svl)) dang_hps no_pos in
          ...
          if CP.mem_svl hp to_unfold_hps then
            try
              ...
              let f5,_ = CF.drop_hrel_f f4 dang_hps in
               let _ =  Debug.info_hprint (add_str "f5" (Cprinter.string_of_formula)) f5 no_pos in
              f5
            with _ -> f0
          else
            f0
