...
(*This seems similar to peek_and*)
 ...
 let peek_tpprojection_send =
   SHGram.Entry.of_parser "peek_tpprojection_send"
       (fun strm ->
           match Stream.npeek 1 strm with
             | [NOT,_] -> ()
             | _ -> raise Stream.Failure)

 let peek_tpprojection_receive =
   SHGram.Entry.of_parser "peek_tpprojection_receive"
       (fun strm ->
           match Stream.npeek 1 strm with
             | [QUERY,_] -> ()
             | _ -> raise Stream.Failure)
  ...
  [[ 
    vh = view_header; `EQEQ; `EXISTS; vars = cid_list; `COLON; s = protocol_formula ->  
      ...
      { vh with
           ...
           view_session = Iast.mk_session_projection (Session.ProtocolSession session_formula)}
          
  | vh = view_header; `EQEQ; s = protocol_formula
          -> { vh with
               view_session_info = Some (mk_view_session_info ~sk:Protocol ());
               view_session = Iast.mk_session_projection (Session.ProtocolSession s)}
  ]];
  ...
  [ "semicolon" RIGHTA
    ...
    |
      ...
      | peek_protocol_base; first = cid; `LEFTARROW; second = cid; `COLON; msg_var = OPT session_msg_var; c = session_message ->
        ...
        let mv = session_extract_msg_var msg_var loc in
        let suid = Session.IProtocol_base.def_suid in 
        Session.IProtocol.SBase (Session.IProtocol.mk_base (first, second, None, mv, loc, suid) c)
...
proj_view_decl:
  [[ vh = view_header; `EQEQ; f = projection_formula
     -> { vh with
          ...
          view_session = Iast.mk_session_projection (Session.ProjectionSession f)}
   ]];
...
tpproj_view_decl:
  [[ vh = view_header; `EQEQ; f = tpprojection_formula
     -> { vh with
          ...
          view_session = Iast.mk_session_projection (Session.TPProjectionSession f)}
   ]];
...
tpprojection_formula:
  [ "semicolon" RIGHTA
    [
        p1 = tpprojection_formula; `SEMICOLONSEMICOLON; p2 = tpprojection_formula ->
            let loc = (get_pos_camlp4 _loc 1) in
            Session.ITPProjection.mk_session_seq_formula p1 p2 loc
    ]
    | [ p1 = tpprojection_formula; `STAR; p2 = tpprojection_formula ->
            let loc = (get_pos_camlp4 _loc 1) in
            Session.ITPProjection.mk_session_star_formula p1 p2 loc
      | p1 = tpprojection_formula; `ORWORD; p2 = tpprojection_formula ->
            let loc = (get_pos_camlp4 _loc 1) in
            Session.ITPProjection.mk_session_or_formula p1 p2 loc
    ]
