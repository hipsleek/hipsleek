...
let extract_abs_formula_branch_x fs v_base_name v_new_name extn_args ls_ann_infos pure_extn_svl is_spec is_view=
  ...
  let process_one f=
    ...
    (* let _ =  Debug.info_pprint ("  sel_svl_rest: "^ (!CP.print_svl sel_svl_rest)) no_pos in *)
...
(* Same as the above method except that it   *)
(* also tells whether we have unknown LexVar *)
...
let has_unknown_lexvar_struc sf =
  let arg = () in
  let f_arg a _ = a in
  ...
  let f_f _ f =
    let _, has_unknown_lexvar = has_unknown_lexvar_formula f in
    Some (f, has_unknown_lexvar)
  in 
