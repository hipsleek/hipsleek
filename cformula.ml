...
and formula_base = {  formula_base_heap : h_formula;
                      formula_base_pure : CP.memo_pure;
...
and formula_exists = {  formula_exists_qvars : CP.spec_var list;
                        ...
                        formula_exists_pure : CP.memo_pure;
...
and h_formula_data = {  h_formula_data_node : CP.spec_var;
                        ...
                        h_formula_data_pruning_conditions :  (CP.memoised_constraint * formula_label list ) list;
...
and h_formula_view = {  h_formula_view_node : CP.spec_var;
                        ...
                        h_formula_view_pruning_conditions :  (CP.memoised_constraint * formula_label list ) list;
...
let rec formula_of_heap h pos = mkBase h (CP.mkMTrue pos) TypeTrue (mkTrueFlow ()) [] pos
and formula_of_heap_fl h fl pos = mkBase h (CP.mkMTrue pos) TypeTrue fl [] pos
...
and formula_of_memo_pure (p:CP.memo_pure) (pos:loc) :formula= mkBase HTrue p TypeTrue (mkTrueFlow ()) [] pos

and formula_of_pure (p:CP.formula) (pos:loc) :formula= mkBase HTrue (CP.memoise_add_pure [] p) TypeTrue (mkTrueFlow ()) [] pos
...
and formula_of_pure_with_branches p br pos = mkBase HTrue (CP.memoise_add_pure [] p) TypeTrue (mkTrueFlow ()) br pos

and formula_of_memo_pure_with_branches p br pos = mkBase HTrue p TypeTrue (mkTrueFlow ()) br pos

and formula_of_pure_with_branches_fl p br fl pos = mkBase HTrue (CP.memoise_add_pure [] p) TypeTrue fl br pos
...
and isAnyConstFalse f = match f with
  ...
  | Base ({formula_base_heap = h;
		   ...
		   formula_base_flow = fl;}) ->
             (h = HFalse || CP.isConstMFalse p || (List.filter (fun (_,f) -> CP.isConstFalse f) br <> []))||
...
and isStrictConstTrue f = match f with
  ...
  | Base ({formula_base_heap = HTrue;
           ...
           formula_base_branches = br;
           formula_base_flow = fl;}) -> 
             CP.isConstMTrue p && (List.filter (fun (_,f) -> not (CP.isConstTrue f)) br = [])&&(is_true_flow fl.formula_flow_interval)
...
and isAnyConstTrue f = match f with
  ...
  | Base ({formula_base_heap = HTrue;
           ...
           formula_base_branches = br;
		   formula_base_flow = fl;}) -> 
            CP.isConstMTrue p && (List.filter (fun (_,f) -> not (CP.isConstTrue f)) br = [])
	  (* don't need to care about formula_base_type  *)
  | _ -> false
...
and mkTrue (flowt: flow_formula) pos = Base ({formula_base_heap = HTrue; 
						formula_base_pure = CP.mkMTrue pos; 
...
and mkFalse (flowt: flow_formula) pos = Base ({formula_base_heap = HFalse; 
						 formula_base_pure = CP.mkMFalse pos; 
...
and mkBase (h : h_formula) (p : CP.memo_pure) (t : t_formula) (fl : flow_formula) b (pos : loc) : formula= 
  if CP.isConstMFalse p || h = HFalse || (is_false_flow fl.formula_flow_interval)  then 
	mkFalse fl pos
  else 
	Base ({formula_base_heap = h; 
		   formula_base_pure = p; 
...
let rec mkStar (f1 : formula) (f2 : formula) flow_tr (pos : loc) =
  ...
  let p = CP.merge_mems p1 p2 true in
  let t = mkAndType t1 t2 in
  let b = CP.merge_branches b1 b2 in
  let fl = mkAndFlow fl1 fl2 flow_tr in
  mkBase h p t fl b pos
   
and combine_and_pure (f1:formula)(p:CP.memo_pure)(f2:CP.memo_pure):CP.memo_pure*bool = 
	if (isAnyConstFalse f1) then (CP.mkMFalse no_pos,false)
		else if (isAnyConstTrue f1) then (f2,true)
			else 
        let r = CP.merge_mems p f2 true in
        if (CP.isConstMFalse r) then (r,false)
        else if (CP.isConstMTrue r) then (r,false)
        else (r,true)      
      (*(sem_and p f2)
and sem_and (p:memo_pure)(f2:memo_pure):(memo_pure*bool) = 
...
and sintactic_search (f:formula)(p:Cpure.formula):bool = match f with
	...
	| Exists _-> 
		...
		(CP.memo_is_member_pure p pl)||(List.exists (fun (_,c)->Cpure.is_member_pure p c) br)
...
and mkAnd_pure_and_branch (f1 : formula) (p2 : CP.memo_pure) b2 (pos : loc):formula = 
  if (isAnyConstFalse f1) then f1
   else 
		let h1, p1, fl1, b1, t1 = split_components f1 in		
		if (CP.isConstMTrue p1) then mkBase h1 p2 t1 fl1 (CP.merge_branches b1 b2) pos
			else mkBase h1 (CP.merge_mems p1 p2 true) t1 fl1 (CP.merge_branches b1 b2) pos

   
and mkExists (svs : CP.spec_var list) (h : h_formula) (p : CP.memo_pure) (t : t_formula) (fl:flow_formula) b (pos : loc) =
...
and fv (f : formula) : CP.spec_var list = match f with
  ...
  | Base ({formula_base_heap = h; 
		   ...
		   formula_base_branches = br;
		   formula_base_type = t}) -> 
	  Util.remove_dups (List.fold_left (fun a (c1,c2)-> (CP.fv c2)@a) (h_fv h @ CP.mfv p) br)
  | Exists ({formula_exists_qvars = qvars; 
			 formula_exists_heap = h; 
			 formula_exists_pure = p; 
			 formula_exists_type = t;
...
and apply_one ((fr, t) as s : (CP.spec_var * CP.spec_var)) (f : formula) = match f with
  ...
  | Base ({formula_base_heap = h; 
      ...
      Base ({formula_base_heap = h_apply_one s h; 
			 formula_base_pure = CP.regroup_memo_group (CP.m_apply_one s p); 
			 formula_base_type = t;
			 formula_base_flow = fl;
  ...
  | Exists ({formula_exists_qvars = qsv; 
	  ...
	  if List.mem (CP.name_of_spec_var fr) (List.map CP.name_of_spec_var qsv) then f 
					...
					formula_exists_pure = CP.regroup_memo_group (CP.m_apply_one s qp); 
					formula_exists_type = tconstr;
					formula_exists_flow = fl;
...
and h_apply_one ((fr, t) as s : (CP.spec_var * CP.spec_var)) (f : h_formula) = match f with
  ...
  | ViewNode ({h_formula_view_node = x; 
      ...
      ViewNode ({h_formula_view_node = subst_var s x; 
				 ...
				 h_formula_view_label = lbl;
         h_formula_view_remaining_branches = ann;
         h_formula_view_pruning_conditions = List.map (fun (c,c2)-> ({c with CP.memo_formula = CP.b_apply_one s c.CP.memo_formula},c2)) pcond;
				 h_formula_view_pos = pos})
  | DataNode ({h_formula_data_node = x; 
      ...
      DataNode ({h_formula_data_node = subst_var s x; 
				 ...
				 h_formula_data_label = lbl;
         h_formula_data_remaining_branches = ann;
         h_formula_data_pruning_conditions = List.map (fun (c,c2)-> ({c with CP.memo_formula = CP.b_apply_one s c.CP.memo_formula},c2)) pcond;
				 h_formula_data_pos = pos})
  | HTrue -> f
  | HFalse -> f
...
if (isAnyConstFalse f) then (HFalse,(CP.mkMFalse no_pos),(flow_formula_of_formula f),[],TypeFalse)
else match f with
  | Base ({formula_base_heap = h; 
		   formula_base_pure = p; 
       formula_base_branches = b;
...
type entail_state = {
  ...
  es_pure : (CP.memo_pure * (branch_label * CP.formula) list);
...
and combine_and (f1:formula) (f2:CP.memo_pure) :formula*bool = match f1 with
	...
	| Exists ({formula_exists_qvars = evars;
			...
			if (List.length (Util.intersect (Cpure.mfv f2) evars))=0 then
				let r1,r2 = (combine_and_pure f1 p f2) in
...
and normalize_no_rename_context_formula (ctx : context) (p : CP.memo_pure) : context = 
	let rec push_pure (f:formula):formula = match f with
		| Base b-> Base {b with formula_base_pure = CP.merge_mems p b.formula_base_pure true}
		| Exists b -> Exists {b with formula_exists_pure = CP.merge_mems p b.formula_exists_pure true}
		| Or b -> Or {
...
and apply_one_exp ((fr, t) as s : (CP.spec_var * CP.exp)) (f : formula) = match f with
  ...
  | Base ({formula_base_heap = h; 
       ...
       formula_base_branches = b;
		   ...
		   formula_base_pos = pos}) -> 
    Base ({formula_base_heap = h; 
			formula_base_pure = CP.mapply_one_exp s p;
			formula_base_flow = fl;
     	(* TODO: solve this *)
  ...
  | Exists ({formula_exists_qvars = qsv; 
      ...
      Exists ({formula_exists_qvars = qsv; 
					formula_exists_heap =  qh; 
					formula_exists_pure = CP.mapply_one_exp s qp; 
					formula_exists_type = tconstr;
					formula_exists_flow = fl;
        ...
        if (List.length b.formula_case_branches) <=0 then  mkTrue (mkTrueFlow ()) b.formula_case_pos
          ...
          (fun a (c1,c2)-> 
            ...
            (mkOr a (normalize_combine 
                  (mkBase HTrue (CP.memoise_add_pure [] c1) TypeTrue (mkTrueFlow ()) [] b.formula_case_pos ) 
                  (struc_to_formula c2)
...
let replace_memo_pure_label nl f = 
  List.map (fun c-> {c with CP.memo_group_slice = List.map (replace_pure_formula_label nl) c.CP.memo_group_slice;}) f
      
let rec replace_formula_label1 nl f = match f with
	| Base b->Base {b with 
			formula_base_heap = replace_heap_formula_label nl b.formula_base_heap ;
			formula_base_pure = replace_memo_pure_label nl b.formula_base_pure ;
	...
	| Exists b->Exists {b with 
			...
			formula_exists_pure = replace_memo_pure_label nl b.formula_exists_pure ;
...
let rec transform_formula f (e:formula):formula =
	...
	| None  -> match e with	 
		| Base b -> Base{b with 
			...
			formula_base_pure =  CP.transform_memo_formula f_p_t b.formula_base_pure;
		...
		| Exists e -> Exists {e with
			...
			formula_exists_pure = CP.transform_memo_formula f_p_t e.formula_exists_pure;
...
let rename_labels transformer e =
  ...
  let f_m e = None in
	...
	let f_p_f e = 
	 ...
	 transformer (f_e_f,f_f,f_h_f,(f_m,f_p_f,f_b,f_e)) e
...
let rename_labels_formula_ante  e=
  ...
  let f_m e = None in
	...
	transform_formula (f_e_f,f_f,f_h_f,(f_m,f_p_f,f_b,f_e)) e
