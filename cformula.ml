...
let get_sec_in_formula f =
  let rec helper f =
    match f with
    | Base   f -> [MCP.get_sec_in_mcpure f.formula_base_pure  ]
    | Exists f -> [MCP.get_sec_in_mcpure f.formula_exists_pure]
    | Or     f -> (helper f.formula_or_f1)@(helper f.formula_or_f2)
  in
  helper f

let get_sec_in_context ctx =
  let rec helper ctx =
    match ctx with
    | Ctx  es     -> get_sec_in_formula es.es_formula
    | OCtx(c1,c2) -> (helper c1)@(helper c2)
  in
  helper ctx

let get_sec_in_branch_ctx_list bcl =
  List.fold_left (fun acc bc -> let (_,ctx,_) = bc in acc@get_sec_in_context ctx) [] bcl

let get_sec_in_list_failesc_ctx lfc =
  List.fold_left (fun acc fc -> let (_,_,bcl) = fc in acc@get_sec_in_branch_ctx_list bcl) [] lfc

(* NOTE: Remove sec formula *)
let rec filter_sec_form f =
  match f with
  | Base   f -> Base   { f with formula_base_pure   = MCP.filter_out_sec_form f.formula_base_pure   }
  | Exists f -> Exists { f with formula_exists_pure = MCP.filter_out_sec_form f.formula_exists_pure }
...
(* NOTE: Simplification *)
(* OLD ALGORITHM: this does not simplify *)
...
let simpl_sec_form lattice sfl =
  ...
  let replace_one sf v lbl =
    ...
    | _                   -> x_report_error no_pos "replace : not security formula"
  in
  let replace_all sfl v lbl = List.map (fun sf -> replace_one sf v lbl) sfl in
  (* match sfl with
   *   | []     -> []
...
let simpl_eximpf_sec_form lattice sfl =
  ...
  let replace_one_expf sf v lbl =
    match sf with
    | ExplicitFlow (sv,l,loc) -> ExplicitFlow (sv, replace_lbl l v lbl, loc)
    | ImplicitFlow _          -> sf
    | _                       -> x_report_error no_pos "replace : not explicit/implicit formula"
  in
  let replace_all_expf sfl v lbl = List.map (fun sf -> replace_one_expf sf v lbl) sfl in

  let replace_one_impf sf v lbl =
    match sf with
    | ImplicitFlow (sv,l,loc) -> ImplicitFlow (sv, replace_lbl l v lbl, loc)
    | ExplicitFlow _          -> sf
    | _                       -> x_report_error no_pos "replace : not explicit/implicit formula"
  in
  let replace_all_impf sfl v lbl = List.map (fun sf -> replace_one_impf sf v lbl) sfl in

  let rec helper curr prev = List.fold_left (fun acc sf -> (
        match sf with
        | ExplicitFlow (v, lbl, _) -> replace_all_expf acc v lbl
  ...
  let is_trivial sf =
    match sf with
    | ExplicitFlow (v,SecVar sv,_)
    | ImplicitFlow (v,SecVar sv,_) -> eq_spec_var v sv
    | _                            -> false
  in
  let is_same_expf v sf =
    match sf with
    | ExplicitFlow (sv,_,_) -> eq_spec_var v sv
    | _                     -> false
  in
  let is_same_impf v sf =
    match sf with
    | ImplicitFlow (sv,_,_) -> eq_spec_var v sv
    | _                     -> false
  in
  let rec atomize sfl =
    match sfl with
    ...
    | sf::sr ->
      begin
        match sf with
        | ExplicitFlow (v, lbl, loc) ->
          begin
            let svl = List.filter (fun x -> is_same_expf v x) sr in
            ...
            (ExplicitFlow(v,glb,loc))::(atomize srl)
          end
        | ImplicitFlow (v, lbl, loc) ->
          begin
            let svl = List.filter (fun x -> is_same_impf v x) sr in
            ...
            (ImplicitFlow(v,glb,loc))::(atomize srl)
          end
        | _                      -> x_report_error no_pos "helper : not explicit/implicit formula"
      end
  in
