...
let simplify_failesc_context ((l,a,cs) : failesc_context) : failesc_context = 
  let cs = List.filter (fun x -> not(isAnyFalseBranchCtx x)) cs in
  let newcs = List.map (fun (p,c) -> (p, simplify_context c)) cs in
  (l,a,newcs)

let simplify_list_failesc_context (l : list_failesc_context) : list_failesc_context = 
  List.map simplify_failesc_context l

let simplify_list_failesc_context_simplify (l : list_failesc_context) : list_failesc_context = 
  let pr = !print_list_failesc_context in
  Gen.Debug.no_1 "simplify_list_failesc_context" pr pr simplify_list_failesc_context l
...
*)
  ...
  (*context set union*)
(* moved to musterr.ml
let union_list_context_x c1 c2 = 
...
let rank (t:partial_context):float = match t with
  ...
  | ( l1 , l2) -> 
    ...
    sn /.(fn +. sn)
*)
let union_list_partial_context (l1:list_partial_context) (l2:list_partial_context):list_partial_context = remove_dupl_false_pc_list (l1 @ l2)
...
let union_list_partial_context (l1:list_partial_context) (l2:list_partial_context):list_partial_context = 
  let pr x = string_of_int(List.length x) in
  Gen.Debug.no_2 "union_list_partial_context" pr pr pr union_list_partial_context l1 l2

let union_list_failesc_context (l1:list_failesc_context) (l2:list_failesc_context):list_failesc_context = 
  let pr x = string_of_int(List.length x) in
...
*)
let or_list_partial_context (l1:list_partial_context) (l2:list_partial_context) : list_partial_context =
  (* List.concat (List.map (fun pc1-> (List.map (simple_or pc1) l2)) l1) *)
  List.concat (List.map (fun pc1-> (List.map (fun pc2 -> remove_dupl_false_pc (merge_partial_context_or pc1 pc2)) l2)) l1)

let or_list_partial_context (l1:list_partial_context) (l2:list_partial_context) : list_partial_context =
  let pr x = string_of_int (List.length x) in
  Gen.Debug.loop_2_no "or_list_partial_context" pr pr pr or_list_partial_context l1 l2

let or_list_failesc_context f (l1:list_failesc_context) (l2:list_failesc_context) : list_failesc_context = 
...
let or_list_failesc_context f (l1:list_failesc_context) (l2:list_failesc_context) : list_failesc_context = 
  ...
  Gen.Debug.no_2 "or_list_failesc_context"
      pr pr pr
      (fun _ _ -> or_list_failesc_context f l1 l2) l1 l2
...
*)
let add_to_context_with_step (c:context) (s:string) = 
...
let add_to_context_with_step_num i (c:context) (s:string) = 
...
and simplify_context_w_brc (ctx : context) (bv : CP.spec_var list) = 
	match ctx with
