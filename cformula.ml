...
type prune_status = 
  ...
  | Implied           (*constraint that can be deducted from the current state- including predicate invariants*)
  | Unknown of bool   (*pruning constraint with unknown status, -bool indicates if it comes from an invariant
                          true -> if proven becomes Implied, false becomes Implied_dupl*)
  
type memoised_constraint = {
    (*memo_formula_mut: CP.b_formula ref;
    memo_status_mut : prune_status ref ;*)
    memo_formula : CP.b_formula;
...
and formula_base = {  formula_base_heap : h_formula;
                      ...
                      formula_base_memoise: memoised_group list;
...
and formula_exists = {  formula_exists_qvars : CP.spec_var list;
                        ...
                        formula_exists_memoise: memoised_group list;
...
and h_formula_data = {  h_formula_data_node : CP.spec_var;
                        ...
                        h_formula_data_pruning_conditions :  (memoised_constraint * formula_label list ) list;
...
and h_formula_view = {  h_formula_view_node : CP.spec_var;
                        ...
                        h_formula_view_pruning_conditions :  (memoised_constraint * formula_label list ) list;
...
and formula_of_pure (p:CP.formula) (pos:loc) :formula= mkBase HTrue p TypeTrue (mkTrueFlow ()) [] (memoise_add_pure [] p) pos
...
and formula_of_pure_with_branches p br pos = mkBase HTrue p TypeTrue (mkTrueFlow ()) br (memoise_add_pure [] p) pos

and formula_of_pure_with_branches_fl p br fl pos = mkBase HTrue p TypeTrue fl br (memoise_add_pure [] p) pos
...
and mkBase (h : h_formula) (p : CP.formula) (t : t_formula) (fl : flow_formula) b memoise (pos : loc) : formula= 
...
and filter_mem_fct fct lst =  
  ...
  List.filter (fun c-> (List.length c.memo_group_cons)>0) l
...
and filter_mem_triv lst = 
  filter_mem_fct (fun c->match c.memo_formula with 
    ...
    | CP.Gte (e1,e2,l) 
    | CP.Eq  (e1,e2,l) 
    | CP.Neq  (e1,e2,l) -> not (CP.eqExp e1 e2)
    | _ -> true) lst
  
and filter_mem fvs lst  = 
  filter_mem_fct (fun c->List.length(Util.intersect_fct CP.eq_spec_var fvs (CP.bfv c.memo_formula))>0) lst
...
and recompute_unknowns (p:memoised_group list): memoised_group list= p
  ...
  let l_i,l_c,l_u = List.fold_left (fun (a1,a2,a3) c-> match c.memo_status with
    ...
    | Fail_prune -> (a1,c::a2,a3)
    | Unknown b -> (a1,a2,(c,b)::a3)) ([],[],[]) c.memo_group_cons in
  let i_f = List.fold_left (fun a c-> CP.mkAnd a (CP.BForm (c.memo_formula, None, None)) no_pos) (CP.mkTrue no_pos) l_i in
  let sat_subno = ref 0 in
  let n_l_u = List.fold_left (fun acc (c,b)-> 
      let and_is = CP.mkAnd i_f (CP.BForm (c.memo_formula, None, None)) no_pos in
      let sat = TP.is_sat_sub_no and_is sat_subno in
      if (not sat) then 
          [{memo_formula=CP.memo_f_neg c.memo_formula; memo_status = if b then Implied else Implied_dupl};
           {memo_formula=c.memo_formula; memo_status = Fail_prune}]@acc
      else 
        let nf = CP.memo_f_neg c.memo_formula in
        let nand_is = CP.mkAnd i_f (CP.BForm (nf, None, None)) no_pos in
        let sat = TP.is_sat_sub_no nand_is sat_subno in
        if (not sat) then 
          [{memo_formula= c.memo_formula; memo_status = if b then Implied else Implied_dupl};
...
and merge_mems (l1: memoised_group list) (l2: memoised_group list) : memoised_group list = 
  let n_l = List.fold_left (fun a c-> 
    let merged, un_merged = List.partition (fun d-> (List.length(Util.intersect_fct CP.eq_spec_var d.memo_group_fv c.memo_group_fv))>0) a in
...
and memoise_add_memo (l: memoised_group list) (cm:memoised_constraint): memoised_group list =
  ...
  let merged,un_merged = List.partition (fun c-> (List.length (Util.intersect_fct CP.eq_spec_var fv c.memo_group_fv))>0) l in
...
and create_memo_group (l:CP.b_formula list) inv_cons:memoised_group list = 
  ...
  let ll  = List.fold_left ( fun a bf->
    let fv = CP.bfv bf in
    let to_merge, no_merge = List.partition (fun (v,_)-> (List.length (Util.intersect_fct CP.eq_spec_var fv v))>0) a in
    let merg_fv,merg_f  = List.fold_left (fun (a1,a2)(c1,c2)-> (a1@c1,a2@c2)) ([],[]) to_merge in
    let merg_fv = Util.remove_dups (merg_fv@fv) in
    (merg_fv,bf::merg_f)::no_merge)[] l in
  List.map (fun (vars,fs)-> 
...
let rec mkStar (f1 : formula) (f2 : formula) flow_tr (pos : loc) =
  ...
  let h2, p2, fl2, b2, t2, mem2 = split_components f2 in
  ...
  let mem = merge_mems mem1 mem2 in
  mkBase h p t fl b mem pos
...
and mkAnd_pure_and_branch (f1 : formula) (p2 : Cpure.formula) b2 (pos : loc):formula = 
  if (isAnyConstFalse f1) then f1
		...
		let h1, p1, fl1, b1, t1, mem1 = split_components f1 in		
		if (Cpure.isConstTrue p1) then mkBase h1 p2 t1 fl1 (CP.merge_branches b1 b2) (memoise_add_pure mem1 p2) pos
			else mkBase h1 (Cpure.mkAnd p1 p2 pos) t1 fl1 (CP.merge_branches b1 b2) (memoise_add_pure mem1 p2) pos

   
and mkExists (svs : CP.spec_var list) (h : h_formula) (p : CP.formula) (t : t_formula) (fl:flow_formula) b memoise (pos : loc) =
  ...
  let qvars = U.intersect svs fvars in (* used only these for the quantified formula *)
	if U.empty qvars then
	  Base{tmp_b with formula_base_memoise = memoise}
	else
	  Exists ({formula_exists_qvars = qvars; 
         ...
         formula_exists_memoise = memoise;
...
and h_apply_one ((fr, t) as s : (CP.spec_var * CP.spec_var)) (f : h_formula) = match f with
  ...
  | ViewNode ({h_formula_view_node = x; 
      ...
      ViewNode ({h_formula_view_node = subst_var s x; 
				 ...
				 h_formula_view_label = lbl;
         h_formula_view_remaining_branches = ann;
         h_formula_view_pruning_conditions = List.map (fun (c,c2)-> ({c with memo_formula = CP.b_apply_one s c.memo_formula},c2)) pcond;
				 h_formula_view_pos = pos})
  | DataNode ({h_formula_data_node = x; 
      ...
      DataNode ({h_formula_data_node = subst_var s x; 
				 ...
				 h_formula_data_label = lbl;
         h_formula_data_remaining_branches = ann;
         h_formula_data_pruning_conditions = List.map (fun (c,c2)-> ({c with memo_formula = CP.b_apply_one s c.memo_formula},c2)) pcond;
				 h_formula_data_pos = pos})
  | HTrue -> f
  | HFalse -> f
  ...
  | _ -> begin
      match f2 with
		...
		| _ -> begin
			...
			let new_h, new_p, new_fl, b, new_t, mem = split_components new_base in
			let resform = mkExists (qvars1 @ qvars2) new_h new_p new_t new_fl b mem pos in (* qvars[1|2] are fresh vars, hence no duplications *)
			  resform
		  end
    end
...
if (isAnyConstFalse f) then (HFalse,(Cpure.mkFalse no_pos),(flow_formula_of_formula f),[],TypeFalse, [])
  ...
  | Base ({formula_base_heap = h; 
       ...
       formula_base_branches = b;
		   formula_base_flow =fl;
       formula_base_memoise = mem;
		   formula_base_type = t}) -> (h, p, fl, b, t, mem)
  | Exists ({formula_exists_heap = h; 
		   formula_exists_pure = p; 
       formula_exists_branches = b;
		   formula_exists_flow = fl;
       formula_exists_memoise = mem;
		   formula_exists_type = t}) -> (h, p, fl, b, t, mem)
...
and split_quantifiers (f : formula) : (CP.spec_var list * formula) = match f with
  | Exists ({formula_exists_qvars = qvars; 
       ...
       formula_exists_memoise = mem;
			 formula_exists_pos = pos}) -> 
      (qvars, mkBase h p t fl b mem pos)
  | Base _ -> ([], f)
...
and add_quantifiers (qvars : CP.spec_var list) (f : formula) : formula = match f with
  | Base ({formula_base_heap = h; 
       ...
       formula_base_branches = b;
       formula_base_memoise = mem;
		   formula_base_pos = pos}) -> mkExists qvars h p t fl b mem pos
  | Exists ({formula_exists_qvars = qvs; 
	  ...
	  let new_qvars = U.remove_dups (qvs @ qvars) in
		mkExists new_qvars h p t fl b mem pos
...
and remove_quantifiers (qvars : CP.spec_var list) (f : formula) : formula = match f with
  ...
  | Exists ({formula_exists_qvars = qvs; 
	  ...
	  let new_qvars = (List.filter (fun x -> not(List.exists (fun y -> CP.eq_spec_var x y) qvars)) qvs) in
	  	if (List.length new_qvars == 0) then mkBase h p t fl b mem pos
	  	else mkExists new_qvars h p t fl b mem pos
  | _ -> failwith ("add_quantifiers: invalid argument")
(* 19.05.2008 *)
...
let rec group_mem_by_fv (lst: memoised_group list):memoised_group list = 
    let n_l = List.fold_left (fun a d-> 
      let n_l = List.map (fun c->((CP.bfv c.memo_formula),c)) d.memo_group_cons in
      n_l @ a) [] lst in
    let r = List.fold_left (fun acc (v_list, mem) -> 
        let n_acc, b = List.fold_left (fun (a,b) (v_l,m_l) -> 
          if (List.length (Util.intersect_fct CP.eq_spec_var v_l v_list))>0 then ((Util.remove_dups (v_list@v_l),mem::m_l)::a,true)
...
and apply_one_exp ((fr, t) as s : (CP.spec_var * CP.exp)) (f : formula) = match f with
  ...
  | Base ({formula_base_heap = h; 
       ...
       formula_base_memoise = mem;
		   formula_base_pos = pos}) -> 
    let n_mem = List.map (fun c -> 
      { c with memo_group_cons = List.map (fun d->{d with memo_formula = CP.b_apply_one_exp s d.memo_formula}) c.memo_group_cons}) mem in
    Base ({formula_base_heap = h; 
      ...
      formula_base_memoise =  filter_mem_triv (group_mem_by_fv n_mem);
		 	formula_base_pos = pos})
  | Exists ({formula_exists_qvars = qsv; 
			 formula_exists_heap = qh; 
			 formula_exists_pure = qp; 
      ...
      Exists ({formula_exists_qvars = qsv; 
          ...
          formula_exists_memoise = filter_mem_triv (group_mem_by_fv n_mem);
					formula_exists_pos = pos})
        ...
        if (List.length b.formula_case_branches) <=0 then  mkTrue (mkTrueFlow ()) b.formula_case_pos
          ...
          (fun a (c1,c2)-> 
            ...
            (mkOr a (normalize_combine 
                  (mkBase HTrue c1 TypeTrue (mkTrueFlow ()) [] (memoise_add_pure [] c1) b.formula_case_pos ) 
                  (struc_to_formula c2)
...
let rec replace_struc_formula_label1 nl f = List.map (fun f-> match f with
	...
	| ECase b -> 
      let new_br = List.map 
        (fun (c1,c2)-> 
          ((replace_pure_formula_label nl c1), (replace_struc_formula_label1 nl c2))
