...
let default_value (t :typ) pos : exp =
  match t with
    | Int -> IntLit { exp_int_lit_val = 0; exp_int_lit_pos = pos; }
    | Bool -> BoolLit {exp_bool_lit_val = true;  exp_bool_lit_pos = pos;}
    | Float -> FloatLit {exp_float_lit_val = 0.0; exp_float_lit_pos = pos;}
    | StringT -> StringLit {exp_string_lit_val = ""; exp_string_lit_pos = pos;}
    | (TVar _) ->
	      failwith
  ...
  and helper e bvars =
    match e with
      ...
      | Unary u ->
            ...
            let _ = Debug.binfo_hprint (add_str "Unary" string_of_exp) e no_pos in
      ...
      | CallNRecv c ->
            ...
            let args = List.combine proc.proc_args c.exp_call_nrecv_arguments in
  ...
  let rec helper (e:exp) (vars: ident list) : (exp) =
    match e with
      ...
      | VarDecl v ->
          ...
          (* let _ = print_endline ("Vardecl : before E.add") in *)
          ...
          (* let _ = print_endline ("Vardecl : after E.add") in *)
          let decls = List.map (fun (id,e0,pos) -> 
              let e1 = match e0 with
                | None -> None
                | Some e0 -> 
