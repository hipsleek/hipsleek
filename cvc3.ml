...
and cvc3_of_sv_type sv = match sv with
  | CP.SpecVar (Prim (BagT _), _, _) -> "SET"
  | CP.SpecVar (Prim Bool, _, _) -> "INT" (* "BOOLEAN" *)
  | _ -> "INT"
...
and split_vars (vars : CP.spec_var list) = 
  if Gen.is_empty vars then 
	...
	begin
	  ...
	  match var with
		| CP.SpecVar (Prim (BagT _), _, _) -> (ints, bools, var :: bags)
		| CP.SpecVar (Prim Bool, _, _) -> (var :: ints, bools, bags)
