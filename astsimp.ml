...
and case_normalize_formula_x prog (h:(ident*primed) list)(f:IF.formula): IF.formula = 
  ...
  let f = x_add_1 Session.irename_message_pointer f in
  let f = Session.wrap_one_seq f in
  let f = Session.wrap_last_seq_node f in
...
(*        let () = print_endline ("!!!"^(VarGen.string_of_loc (IF.pos_of_formula f))) in
  ...
  let nf = x_add_1 Session.irename_message_pointer_struc nf in
  let nf = Session.wrap_one_seq_struc nf in
  let nf = Session.wrap_last_seq_node_struc nf in
