...
type rank_type =
  | VIEW of (string * CP.spec_var list)  (* view name * view's args *)
  | PRE of C.typed_ident list
  | POST

let rec add_rank_constraint_view (vdef: C.view_decl): C.view_decl =
  let incr_rrel_id = if vdef.C.view_vars = [] then () else incr_rrel_id () in
  let view_form = add_rank_constraint_struc_formula vdef.C.view_formula 
    (VIEW (vdef.C.view_name, vdef.C.view_vars)) in
  ...
  let add_rank_constraint_pure hf pf rtyp =
    ...
    | VIEW (view_id, view_args) ->
        ...
        if is_raw_view then
          ...
          let nhf, rrels, rrel_ivars = CF.collect_rankrel_vars_h_formula hf [view_id] in
          let npf = MCP.memoise_add_pure_N pf (CP.mkRankConstraint_no_fresh rankrel_id 
            (List.map CP.mkRArg_var view_args)) in
          let npf = MCP.memoise_add_pure_N npf (CP.join_conjunctions rrels) in
          nhf, npf, rrel_ivars
    | PRE proc_args ->
        let nhf, rankrel_vars, rankrel_ivars, _ = CF.collect_rankrel_vars_h_formula_raw hf [] in
...
and case_normalize_formula_not_rename prog (h:(ident*primed) list)(f:IF.formula):IF.formula = 
  ...
  let f = convert_heap2 prog f in
  let f = IF.float_out_thread f in
  let f = IF.float_out_exps_from_heap (I.lbl_getter prog) f None in
  let f = IF.float_out_min_max f in
  let f = IF.rename_bound_vars f in
  (* let f,_,_ = case_normalize_renamed_formula prog h [] f in *)
  f
...
and case_normalize_struc_formula_x prog (h_vars:(ident*primed) list)(p_vars:(ident*primed) list)(f:IF.struc_formula) 
  ...
  let nf = IF.float_out_thread_struc_formula (I.lbl_getter prog) nf None in 
  (* let _ = print_string ("\ncase_normalize_struc_formula :: CHECK POINT 1 ==> nf = " ^ Iprinter.string_of_struc_formula nf ) in *)
  let nf = IF.float_out_exps_from_heap_struc (I.lbl_getter prog) nf None in 
...
and prune_inv_inference_formula_x (cp:C.prog_decl) (v_l : CP.spec_var list) (init_form_lst: (CF.formula*formula_label) list) u_baga u_inv pos: 
  ...
  *)
  ...
  let split_one_branch (vl:CP.spec_var list) (uinvl:CP.b_formula list) ((b0,lbl):(CF.formula * Globals.formula_label)) 
	...
	let ms = Solver.formula_2_mem b0 cp in
