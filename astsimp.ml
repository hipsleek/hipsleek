...
let rec convert_heap2_heap prog (h0 : IF.h_formula) : IF.h_formula =
  match h0 with
    ...
    | IF.HRel _
    | IF.HTrue | IF.HFalse | IF.HEmp | IF.HeapNode _ | IF.HeapNodeDeref _ -> h0
...
and linearize_formula_x (prog : I.prog_decl)  (f0 : IF.formula) (tlist : spec_var_type_list) : (spec_var_type_list * CF.formula) =
  ...
  let rec linearize_heap (f : IF.h_formula) pos (tl:spec_var_type_list) : ( CF.h_formula * CF.t_formula) = 
    let res = (*let _ = print_string("H_formula: "^(Iprinter.string_of_h_formula f)^"\n") in*)
      match f with
        | IF.HeapNode2 h2 -> report_error (IF.pos_of_formula f0) "malfunction with convert heap2 to heap node"
        | IF.HeapNodeDeref _ -> report_error (IF.pos_of_formula f0) "TRUNG TODO: code here"
        | IF.HeapNode{
...
and case_normalize_renamed_formula_x prog (avail_vars:(ident*primed) list) posib_expl (f:IF.formula):
  ...
  let rec linearize_heap (used_names:((ident*primed) list)) (f : IF.h_formula): ((ident*primed) list) * ((ident*primed) list) * IF.h_formula * Ipure.formula =
    match f with
      ...
      | IF.HeapNodeDeref b -> report_error b.IF.h_formula_heap_deref_pos "TRUNG TODO: code here" 
      | IF.HeapNode b ->
