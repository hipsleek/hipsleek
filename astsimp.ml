...
let type_infer_flag = ref true

(* this method is to enforce type-checking *)
(* that was previously inferred or given *)
let wrap_type_checking f a =
  let save = !type_infer_flag in
  let _ = type_infer_flag := false in
  try
  let ans = f a in
  let _ = type_infer_flag := save
  in ans
  with _ as e ->
      begin
        type_infer_flag := save;
        raise e
      end

  
let mk_sv_info t i = {sv_info_kind = t; inferrable = !type_infer_flag; id= i}

let mk_sv_info_id t = {sv_info_kind = t; inferrable = !type_infer_flag; id= fresh_int()}

(* do not infer *)
let mk_sv_info_id_no t = {sv_info_kind = t; inferrable = false; id= fresh_int()}
let mk_sv_info_id_yes t = {sv_info_kind = t; inferrable = true; id= fresh_int()}

let string_of_sv_info c2 = 
  let infer_str = if c2.inferrable then "IFR " else "NO " 
  in (infer_str^(string_of_typ c2.sv_info_kind))

class symbol_table =
  object
    val tab = H.create 103
    method add (id:ident) (entry:spec_var_info) 
      = H.add tab id entry
    method find (id:ident) 
      = H.find tab id
    method mem (id:ident)
      = H.mem tab id
  end;;
...
let string_of_stab stab = Hashtbl.fold
		(fun c1 c2 a ->
                    let infer_str = if c2.inferrable then "IFR " else "" in
			a^"; ("^c1^" "^infer_str^
			(
...
and trans_view_x (prog : I.prog_decl) (vdef : I.view_decl) : C.view_decl =
  ...
  Debug.tinfo_hprint (add_str "vtv" (pr_list (pr_pair string_of_typ pr_id))) vtv no_pos;
  List.iter (fun (t,c) -> 
      if t==UNK 
      then () 
      else H.add stab c (mk_sv_info_id_no t)) vtv ;
  H.add stab self (mk_sv_info_id_no  (Named data_name));
  Debug.tinfo_hprint (add_str "stab(after self)" string_of_stab) stab no_pos;
...
and trans_rel (prog : I.prog_decl) (rdef : I.rel_decl) : C.rel_decl =
  ...
  let stab = H.create 103 in
  let _ = List.map (fun (var_type, var_name) -> 
      H.add stab var_name (mk_sv_info_id (trans_type prog var_type pos))) rdef.I.rel_typed_vars in
...
and trans_hp (prog : I.prog_decl) (hpdef : I.hp_decl) : C.hp_decl =
  ...
  let stab = H.create 103 in
  let _ = List.map (fun (var_type, var_name) -> 
      H.add stab var_name (mk_sv_info_id (trans_type prog var_type pos))) hpdef.I.hp_typed_vars in
  (* Need to collect the type information before translating the formula *)
  ...
  let trans_proc_x_op () =
    ...
    if not (Gen.is_empty dup_names) then
      ...
      let add_param p = H.add stab p.I.param_name 
        (mk_sv_info_id (trans_type prog p.I.param_type p.I.param_loc)) in
      (ignore (List.map add_param all_args);

      let _ = H.add stab res_name (mk_sv_info_id cret_type) in
      let _ = H.add stab eres_name (mk_sv_info_id UNK) in
      (* Termination: Add info of logical vars *)
      let add_logical (CP.SpecVar (t, i, _)) = H.add stab i (mk_sv_info_id t) in
      let log_vars = List.concat (List.map (trans_logical_vars) prog.I.prog_logical_var_decls) in 
  ...
  let rec helper ie =
	...
	I.exp_arrayat_pos = pos } ->
      ...
      | I.Assert{
            ...
            in
            (ignore
                (List.map (fun (t, n) -> H.add stab n (mk_sv_info_id t) )
                    all_names);
            let assert_cf_o =
...
and unify_type_modify_x (modify_flag:bool) (k1 : spec_var_kind) (k2 : spec_var_kind) stab : spec_var_kind option =
  ...
  and unify k1 k2 =
    match k1,k2 with
      ...
      | Int, INFInt 
      | INFInt, Int -> Some INFInt (* give more general type *)
      | NUM, Int -> Some Int
...
and unify_type_modify (modify_flag:bool) (k1 : spec_var_kind) (k2 : spec_var_kind) stab : spec_var_kind option =
  let pr = string_of_typ in
  Debug.no_2 "unify_type_modify" pr pr (pr_option pr) (fun _ _ -> unify_type_modify_x modify_flag k1 k2 stab) k1 k2

(* k2 is expected type *)
and unify_expect (k1 : spec_var_kind) (k2 : spec_var_kind) stab : spec_var_kind option =
  unify_expect_modify 1 true k1 k2 stab

and must_unify_expect_test k1 k2 pos = 
  let k = unify_expect_modify 2 false k1 k2 !type_table  in
  match k with
    | Some r -> r
...
and must_unify_expect_test_2 k1 k2 k3 pos = 
  let k = unify_expect_modify 3 false k1 k2 !type_table  in
  match k with
    | Some r -> r
...
and unify_expect_modify i (modify_flag:bool) (k1 : spec_var_kind) (k2 : spec_var_kind) stab : spec_var_kind option =
  let pr = string_of_typ in
  Debug.no_3_num i "unify_expect_modify" string_of_bool pr pr (pr_option pr) (fun _ _ _-> unify_expect_modify_x modify_flag k1 k2 stab) modify_flag k1 k2
...
and unify_expect_modify_x (modify_flag:bool) (k1 : spec_var_kind) (k2 : spec_var_kind) stab : spec_var_kind option =
  ...
  let repl_stab i k = Debug.no_2 "repl_stab" string_of_int pr (pr_opt pr) repl_stab i k in
  let rec unify k1 k2 =
    match k1,k2 with
      | UNK, _ -> Some k2
      | _, UNK -> Some k1
...
and set_var_kind (var : ident) (k : spec_var_kind) (stab : spec_var_table) =
  ...
  with | Not_found -> (H.add stab var (mk_sv_info_id k); H.find stab var)
...
and set_var_kind2 (var1 : ident) (var2 : ident) (k : spec_var_kind) (stab : spec_var_table) =
  ...
  let _ = match (r1,r2) with
    | None, None -> let c = (mk_sv_info_id k) in
...
and gather_type_info_var i (var : ident) stab (ex_t : typ) pos : typ =
  let pr = string_of_typ in
  Debug.no_eff_3_num i "gather_type_info_var" [false;true] (fun x -> ("ident: "^x)) string_of_stab pr pr 
...
and gather_type_info_var_x (var : ident) stab (ex_t : spec_var_kind) pos : spec_var_kind =
  begin
    ...
    then UNK (* for vars such as _ and # *)
      ...
      try
        (* what if var do not allow inference? *)
        let k = H.find stab var in
        (* if ex_t==UNK then k.sv_info_kind *)
        (* else *)
        let s_before=string_of_stab stab in
        let tmp = must_unify(* _expect *)  k.sv_info_kind ex_t stab pos in
        (k.sv_info_kind <- tmp);
        Debug.tinfo_hprint (add_str "var(k)" string_of_sv_info) k no_pos;
        Debug.tinfo_hprint (add_str "var(expected)" string_of_typ) ex_t no_pos;
        Debug.tinfo_hprint (add_str "var(stab-before)" pr_id) s_before  no_pos;
        Debug.tinfo_hprint (add_str "var(stab-after)" string_of_stab) stab  no_pos;
        Debug.tinfo_hprint (add_str "var(res)" string_of_typ) tmp no_pos;
        tmp
      with 
        | Not_found -> 
...
and fresh_proc_var_kind stab et = 
  let r = match et with
    | TVar i -> mk_sv_info et i
          (* | NUM -> fresh_tvar_rec stab *)
    | _ -> mk_sv_info_id et in
  r
...
and gather_type_info_exp i a0 stab et =
  Debug.no_eff_3_num i "gather_type_info_exp" [false;true] 
...
and gather_type_info_exp_x a0 stab et =
  match a0 with
    ...
    | IP.Var ((sv, sp), pos) -> 
          let t = gather_type_info_var 1 sv stab et pos
          in t
    | IP.Tsconst (_,pos) ->
	  let t = Tree_sh in
    ...
    | IP.ArrayAt ((a,p),idx,pos) -> (* t[] -> int -> t *)
	  ...
	  let dim = List.length idx in
          ...
          let lt = gather_type_info_var 2 a stab new_et pos in
...
and gather_type_info_b_formula_x prog b0 stab =
  ...
  match pf with
    | IP.BConst _ -> ()
    | IP.BVar ((bv, bp), pos) ->
	  let _ = gather_type_info_var 3 bv stab (C.bool_type) pos in
          ()
    | IP.SubAnn(a1,a2,pos) ->
	  let _ = gather_type_info_exp 1 a1 stab (Cpure.ann_type) in
	  let _ = gather_type_info_exp 2 a2 stab (Cpure.ann_type) in
          ()
    | IP.LexVar(t_ann, ls1, ls2, pos) ->
	  let _ = List.map (fun e -> gather_type_info_exp 3 e stab (Int)) ls1 in
	  let _ = List.map (fun e -> gather_type_info_exp 4 e stab (Int)) ls2 in
          ()
    | IP.Lt (a1, a2, pos) | IP.Lte (a1, a2, pos) | IP.Gt (a1, a2, pos) |
	      IP.Gte (a1, a2, pos) ->
          let new_et = fresh_tvar stab in
	  let t1 = gather_type_info_exp 5 a1 stab new_et in (* tvar, Int, Float *)
	  let t2 = gather_type_info_exp 6 a2 stab new_et in
    ...
    | IP.EqMin (a1, a2, a3, pos) | IP.EqMax (a1, a2, a3, pos) ->
          let new_et = fresh_tvar stab in
	  let t1 = gather_type_info_exp 7 a1 stab new_et in (* tvar, Int, Float *)
	  let t2 = gather_type_info_exp 7 a2 stab new_et in
	  let t3 = gather_type_info_exp 7 a3 stab new_et in (* tvar, Int, Float *)
          let t1 = must_unify_expect t1 NUM stab pos in
          let t2 = must_unify_expect t2 NUM stab pos in
    ...
    | IP.BagIn ((v, p), e, pos) | IP.BagNotIn ((v, p), e, pos) ->  (* v in e *)
          let new_et = fresh_tvar stab in
          let t1 = gather_type_info_exp 8 e stab (BagT new_et) in
          let t2 = gather_type_info_var 4 v stab new_et pos in
          let _ = must_unify t1 (BagT t2) stab pos in
          ()
    | IP.BagSub (e1, e2, pos) ->
          let new_et = fresh_tvar stab in
          let t1 = gather_type_info_exp 9 e1 stab (BagT new_et) in
          let t2 = gather_type_info_exp 9 e2 stab (BagT new_et) in
          let _ = must_unify t1 t2 stab pos in
          ()
    | IP.Eq (a1, a2, pos) | IP.Neq (a1, a2, pos) ->
          let new_et = fresh_tvar stab in
	  let t1 = gather_type_info_exp 10 a1 stab new_et in (* tvar, Int, Float *)
	  let t2 = gather_type_info_exp 10 a2 stab new_et in
          let _ = must_unify t1 t2 stab pos  in (* UNK, Int, Float, TVar *)
          ()
    | IP.BagMax ((v1, p1), (v2, p2), pos) 
    | IP.BagMin ((v1, p1), (v2, p2), pos) -> (* V1=BagMin(V2) *)
          let et = fresh_tvar stab in
	  let t1 = gather_type_info_var 5 v1 stab et pos in
          let t = must_unify t1 NUM stab pos  in
	  let _ = gather_type_info_var 5 v2 stab (BagT t) pos in
          ()
    | IP.VarPerm _ -> () (*TO CHECK: no type info*)
    | IP.ListIn (e1, e2, pos) | IP.ListNotIn (e1, e2, pos)  | IP.ListAllN (e1, e2, pos) ->
          let new_et = fresh_tvar stab in
          let t1 = gather_type_info_exp 11 e2 stab (List new_et) in
          let t2 = gather_type_info_exp 11 e1 stab new_et in
    ...
    | IP.RelForm (r, args, pos) ->
	    ...
	    let args_ctypes = List.map (fun (t,n) -> trans_type prog t pos) rdef.I.rel_typed_vars in
	    let args_exp_types = List.map (fun t -> (t)) args_ctypes in
            let _ = gather_type_info_var 6 r stab RelT in
	    let _ = List.map2 (fun x y -> gather_type_info_exp 13 x stab y) args args_exp_types in ()
          with
    ...
    | IP.XPure({IP.xpure_view_node = vn ;
      ...
      IP.xpure_view_pos = pos}) -> 
	    ...
	    let hpdef = I.look_up_hp_def_raw prog.I.prog_hp_decls r in
	      ...
	      let args_exp_types = List.map (fun t -> (t)) args_ctypes in
              let _ = gather_type_info_var 7 r stab HpT in
	      let _ = List.map2 (fun x y -> gather_type_info_var 8 x stab y) args args_exp_types in ()
            else
...
and gather_type_info_pointer (e0 : IP.exp) (k : spec_var_kind) stab : typ =
  match e0 with
    | IP.Null _ -> null_type
    | IP.Var ((sv, sp), pos) -> gather_type_info_var 9 sv stab k pos
    | _ ->
	  Err.report_error
              {
                  Err.error_loc = IP.pos_of_exp e0;
...
and gather_type_info_one_formula prog (f : IF.one_formula) stab filter_res =
  (
      (match f.IF.formula_thread with
        | None -> ()
        | Some (v,pr) -> let _ = gather_type_info_var 10 v stab thread_typ f.IF.formula_pos in () );
...
and try_unify_data_type_args prog c ddef v ies stab pos =
  ...
  let _ = gather_type_info_var 11 v stab ((Named c)) pos in
  let fields = I.look_up_all_fields prog ddef
  in 
  (try 
    let f _ arg ((ty,_),_,_) = 
      (let _ = wrap_type_checking (gather_type_info_exp 12 arg stab) ty in ())
...
and try_unify_view_type_args prog c vdef v ies stab pos =
  ...
  let _ =  (if not (dname = "")
  then let _ = gather_type_info_var 12 v stab ( (Named dname)) pos in ()
  else ()) 
  in
  ...
  then
    let _ = (List.map (fun (t, n) -> gather_type_info_var 13 n stab (t) pos) tmp_r) in
    ()
  else begin
    (* below seems wrong to unify against previous var names *)
    (try 
      let _ = (List.map (fun (t, n) -> gather_type_info_var 14 n stab (t) pos) tmp_r) in
      let f _ arg lhs_v = 
        (let et = get_var_kind lhs_v stab  in 
        let new_t = gather_type_info_exp 13 arg stab et in
    ...
    | IF.HeapNode
	    {
          ...
          Debug.trace_hprint (add_str "view" Iprinter.string_of_h_formula) h0 no_pos;
                ...
                let gather_type_info_ann c stab = match c with
                  | IF.ConstAnn _ -> ()
                  | IF.PolyAnn ((i,_),_) -> ignore(gather_type_info_var 15 i stab AnnT pos) in
                let gather_type_info_perm p stab = match p with
                  | None -> ()
                  | Some e -> let _ = gather_type_info_exp 14 e stab ft in () in
		...
		if (c = Parser.generic_pointer_type_name) then 
		  ...
		  if type_found then
		    ...
		    let _ = H.add stab rootptr (mk_sv_info type_rootptr 0) in
		    ...
		    let _ = gather_type_info_exp 15 (List.hd ies) stab ptr_type in ()
		  else ()
    ...
    | IF.HRel (r, args, pos) ->
          (try
            ...
            if (List.length args) == (List.length hpdef.I.hp_typed_vars) then
              let args_ctypes = List.map (fun (t,n) -> trans_type prog t pos) hpdef.I.hp_typed_vars in
	      let args_exp_types = List.map (fun t -> (t)) args_ctypes in
              let _ = gather_type_info_var 16 r stab HpT in
	      let _ = List.map2 (fun x y -> gather_type_info_exp 16 x stab y) args args_exp_types in ()
            else
              Err.report_error{ Err.error_loc = pos; Err.error_text = ("number of arguments for heap relation "^r^" does not match"); }
          with
...
and trans_bdecl_x prog bd = 
  if Gen.BList.check_dups_eq (fun (c1,c2,_) (d1,d2,_)-> c1=d1 && c2=d2) bd.I.barrier_tr_list then 
    ...
    let stab = H.create 103 in
    H.add stab self (mk_sv_info_id (Named bd.I.barrier_name));
    List.iter (fun (t,c)-> H.add stab c (mk_sv_info_id t)) bd.I.barrier_shared_vars ; 
