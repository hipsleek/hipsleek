...
and sat_warnings cprog = 
  let sat_warnings_op () =
    let warn n discard = 
      print_string ("WARNING : the view body for "^n^" contains unsat branch(es) :"^(List.fold_left (fun a c-> a^"\n   "^(Cprinter.string_of_formula c)) "" discard)^"\n") in
...
and trans_one_coercion_x (prog : I.prog_decl) (coer : I.coercion_decl) :
  ...
  let head_pure = if  coer_type = I.Left then c_guard0 else (IP.mkTrue no_pos) in
...
and trans_pure_exp_x (e0 : IP.exp) (tlist:spec_var_type_list) : CP.exp =
  match e0 with
    ...
    | IP.Abs (e, pos) -> CP.Abs (trans_pure_exp e tlist, pos)
    | IP.Sqrt (e, pos) -> CP.Sqrt (trans_pure_exp e tlist, pos)
    | IP.Pow (e1, e2, pos) -> CP.Pow (trans_pure_exp e1 tlist, trans_pure_exp e2 tlist, pos)
    | IP.Sin (e, pos) -> CP.Sin (trans_pure_exp e tlist, pos)
    | IP.Cos (e, pos) -> CP.Cos (trans_pure_exp e tlist, pos)
    | IP.Tan (e, pos) -> CP.Tan (trans_pure_exp e tlist, pos)
    | IP.Cotan (e, pos) -> CP.Cotan (trans_pure_exp e tlist, pos)
    | IP.ArcSin (e, pos) -> CP.ArcSin (trans_pure_exp e tlist, pos)
    | IP.ArcCos (e, pos) -> CP.ArcCos (trans_pure_exp e tlist, pos)
    | IP.ArcTan2 (e1, e2, pos) -> CP.ArcTan2 (trans_pure_exp e1 tlist, trans_pure_exp e2 tlist, pos)
    | IP.ArcCot (e, pos) -> CP.ArcCot (trans_pure_exp e tlist, pos)
...
let rec rev_trans_exp e = match e with
  ...
  | CP.Abs (e, p) -> IP.Abs (rev_trans_exp e, p)
  | CP.Sqrt (e, p) -> IP.Sqrt (rev_trans_exp e, p)
  | CP.Pow (e1, e2, p) -> IP.Pow (rev_trans_exp e1, rev_trans_exp e2, p)
  | CP.Sin (e, p) -> IP.Sin (rev_trans_exp e, p)
  | CP.Cos (e, p) -> IP.Cos (rev_trans_exp e, p)
  | CP.Tan (e, p) -> IP.Tan (rev_trans_exp e, p)
  | CP.Cotan (e, p) -> IP.Cotan (rev_trans_exp e, p)
  | CP.ArcSin (e, p) -> IP.ArcSin (rev_trans_exp e, p)
  | CP.ArcCos (e, p) -> IP.ArcCos (rev_trans_exp e, p)
  | CP.ArcTan2 (e1, e2, p) -> IP.ArcTan2 (rev_trans_exp e1, rev_trans_exp e2, p)
  | CP.ArcCot (e, p) -> IP.ArcCot (rev_trans_exp e, p)
