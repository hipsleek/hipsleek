      ...
      let cviews1 = (* if !Globals.gen_baga_inv then *)
        if !Globals.gen_baga_inv then
          List.map (fun cv ->
            ...
            let _ = Debug.ninfo_hprint (add_str ("infered baga inv("^cv.C.view_name^")") (Cprinter.string_of_ef_pure_disj)) inv no_pos in
  ...
  let rec match_exp (used_names : (ident*primed) list) (hargs : ((IP.exp * bool) * LO.t) list) pos :
    ...
    match hargs with
      | ((e,rel_flag), label) :: rest ->
            ...
            (* Debug.info_hprint (add_str "case-norm hvars" pr_primed_ident_list) hvars pos; *)
            (* Debug.info_hprint (add_str "case-norm evars" pr_primed_ident_list) evars pos; *)
