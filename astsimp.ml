...
and look_for_anonymous_exp (arg : IP.exp) : (ident * primed) list =
  ...
  | IP.CLen (e1,_) -> look_for_anonymous_exp e1
  | _ -> []
...
and trans_exp_x (prog : I.prog_decl) (proc : I.proc_decl) (ie : I.exp) : trans_exp_type =
  ...
  and helper_x ie =
    ...
    | I.StringLit { I.exp_string_lit_val = s; I.exp_string_lit_pos = pos } ->
         ((C.SConst { C.exp_sconst_val = s; C.exp_sconst_pos = pos; }), C.string_type)
    | I.CharLit { I.exp_char_lit_val = s; I.exp_char_lit_pos = pos } ->
         ((C.CConst { C.exp_cconst_val = s; C.exp_cconst_pos = pos; }), C.char_type)
...
and default_value (t :typ) pos : C.exp =
  ...
  been rejected" *)
  | Char ->
    C.CConst { C.exp_cconst_val = '\x00'; C.exp_cconst_pos = pos; }
  | (TVar _) ->
    failwith
...
and trans_pure_exp_x (e0 : IP.exp) (tlist:spec_var_type_list) : CP.exp =
  ...
  | IP.CConst (c, pos) -> CP.CConst (c, pos)
  ...
  | IP.CLen (e, pos) -> CP.CLen (trans_pure_exp_x e tlist, pos)
  | IP.CharAt (e1, e2, pos) -> CP.CharAt (trans_pure_exp_x e1 tlist,
                                          trans_pure_exp_x e2 tlist, pos)
...
and case_rename_var_decls (f:Iast.exp) : (Iast.exp * ((ident*ident) list)) =  match f with
  ...
  | Iast.StringLit _ | Iast.CharLit _
