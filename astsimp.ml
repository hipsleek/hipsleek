...
and trans_pure_exp_x (e0 : IP.exp) stab : CP.exp =
  match e0 with
    ...
    | IP.Sequence (seqs, f, pos) ->
        let newseqs = (
          List.map (
            fun seq -> { CP.seq_element = trans_pure_exp seq.IP.seq_element stab;
                         CP.seq_domain_lb = trans_pure_exp seq.IP.seq_domain_lb stab;
                         CP.seq_domain_ub = trans_pure_exp seq.IP.seq_domain_ub stab;
                         CP.seq_domain_ub_include = seq.IP.seq_domain_ub_include; }
          ) seqs
        ) in
        let newf = trans_pure_formula f stab in
        CP.Sequence (newseqs, newf, pos)
