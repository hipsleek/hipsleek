...
let rec  trans_prog (prog3 : I.prog_decl) : C.prog_decl =
  ...
  then
    ...
    let dups = Util.find_dups all_names in
    let dups = Util.find_dups all_names in
...
and find_materialized_vars prog params (f0 : CF.formula) : CP.spec_var list =
  ...
  (while not !quit_loop do 
    ef := Solver.expand_all_preds prog !ef false;		
      ...
      let tmp2 = Gen.BList.remove_dups_eq (=)_f (tmp1 @ !all_mvars) CP.eq_spec_var in
      let tmp2 = Gen.BList.remove_dups_eq (=)_f (tmp1 @ !all_mvars) CP.eq_spec_var in
...
and find_mvars prog (params : CP.spec_var list) (f0 : CF.formula) :
  ...
  match f0 with
    | CF.Or { CF.formula_or_f1 = f1; CF.formula_or_f2 = f2 } ->
	      ...
	      let mvars = Gen.BList.remove_dups_eq (=)_f (mvars1 @ mvars2) CP.eq_spec_var in
	      let mvars = Gen.BList.remove_dups_eq (=)_f (mvars1 @ mvars2) CP.eq_spec_var in
...
and trans_proc (prog : I.prog_decl) (proc : I.proc_decl) : C.proc_decl =
  ...
  let dup_names = Gen.BList.find_one_dup_eqfind_one_dup (fun a1 a2 -> a1.I.param_name = a2.I.param_name) proc.I.proc_args in
  let dup_names = Gen.BList.find_one_dup_eqfind_one_dup (fun a1 a2 -> a1.I.param_name = a2.I.param_name) proc.I.proc_args in
  let dup_names = Gen.BList.find_one_dup_eqfind_one_dup (fun a1 a2 -> a1.I.param_name = a2.I.param_name) proc.I.proc_args in
  let dup_names = Gen.BList.find_one_dup_eqfind_one_dup (fun a1 a2 -> a1.I.param_name = a2.I.param_name) proc.I.proc_args in
...
and trans_one_coercion (prog : I.prog_decl) (coer : I.coercion_decl) :
  ...
  let compute_univ () =
    ...
    let univ_vars = Gen.BList.difference_eq CP.eq_spec_var pvars hvars in 
    Gen.BList.remove_dups_eq (=)_f univ_vars CP.eq_spec_var in
    Gen.BList.remove_dups_eq (=)_f univ_vars CP.eq_spec_var in
  ...
  match ie with
    ...
    | I.New {
          ...
          if ( != ) nargs (List.length field_types) then
            ...
            if List.exists2 (fun t1 t2 -> not (sub_type t1 t2)) cts parg_types then
              Err.report_error { Err.error_loc = pos; Err.error_text = "argument types do not match";}
            else ( let positions = Gen.Profiling.repeat pos nargs in
            else ( let positions = Gen.Profiling.repeat pos nargs in
            else ( let positions = Gen.Profiling.repeat pos nargs in
            else ( let positions = Gen.Profiling.repeat pos nargs in
            let (local_vars, init_seq, arg_vars) = trans_args (Gen.combine3 cargs cts positions) in
          ...
          if not (Gen.is_some tmp1) then
            ...
            let (bind_body, bind_type) = match rhs_o with
              ...
              | Some rhs_e ->
                    ...
                    if (Util.is_some rhs_t) && (sub_type (Gen.unsome rhs_t) ct) then
...
and prune_inv_inference_formula (cp:C.prog_decl) (v_l : CP.spec_var list) (init_form_lst: (CF.formula*formula_label) list) u_baga u_inv pos:
      ...
      ((formula_label list * (Gen.Baga(Slices.PtrSV).baga * Cpure.b_formula list) ) list)
      ((formula_label list * (Gen.Baga(Slices.PtrSV).baga * Cpure.b_formula list) ) list)
...
and prune_inv_inference_formula_x (cp:C.prog_decl) (v_l : CP.spec_var list) (init_form_lst: (CF.formula*formula_label) list) u_baga u_inv pos: 
  ...
  let filter_pure_conj_list pc  =
    ...
    let r = List.map (fun c-> match c with
      | CP.Eq (e1,e2,l) -> (match e1,e2 with
          ...
          | _-> c) 
      | _ -> c) r in
    Gen.BList.remove_dups_eq (=)_f r CP.eq_b_formula_no_aset in
    Gen.BList.remove_dups_eq (=)_f r CP.eq_b_formula_no_aset in
  ...
  let rec propagate_constraints (p_c:CP.b_formula list) (nl:CP.b_formula list): CP.b_formula list = 
    ...
    let r = Gen.BList.remove_dups_eq (=)_f (new_con_list (p_c@nl)) CP.eq_b_formula_no_aset in    
    let r = Gen.BList.remove_dups_eq (=)_f (new_con_list (p_c@nl)) CP.eq_b_formula_no_aset in    
    let r1 = List.filter ( fun c-> not (List.exists (CP.eq_b_formula_no_aset c) (p_c@nl) ) ) r in
    if (List.length r1)>0 then 
      let n = Gen.BList.remove_dups_eq (=)_f (r1@nl) CP.eq_b_formula_no_aset in    
      let n = Gen.BList.remove_dups_eq (=)_f (r1@nl) CP.eq_b_formula_no_aset in    
...
(* Cristian's fixes which seem very slow *)
      ...
      to_be_added @ (Gen.BList.remove_dups_eq (=)_f lr CP.eq_b_formula_no_aset) in      
      to_be_added @ (Gen.BList.remove_dups_eq (=)_f lr CP.eq_b_formula_no_aset) in      
      ...
      if (!Globals.enable_strong_invariant) then
        List.fold_left 
            (fun (a1,(b1,a2))(c1,(b2,c2))-> 
                if a1=[] then ([c1],(b2,c2))
                else (c1::a1, (Gen.Baga(Slices.PtrSV).or_baga CP.eq_spec_var b1 b2, combine_pures c2 a2))) ([],(Gen.is_empty_baga(),[])) pure_list
                else (c1::a1, (Gen.Baga(Slices.PtrSV).or_baga CP.eq_spec_var b1 b2, combine_pures c2 a2))) ([],(Gen.is_empty_baga(),[])) pure_list
                else (c1::a1, (Gen.Baga(Slices.PtrSV).or_baga CP.eq_spec_var b1 b2, combine_pures c2 a2))) ([],(Gen.is_empty_baga(),[])) pure_list
                else (c1::a1, (Gen.Baga(Slices.PtrSV).or_baga CP.eq_spec_var b1 b2, combine_pures c2 a2))) ([],(Gen.is_empty_baga(),[])) pure_list
      ...
      if i>=l then all :: crt_lst
        ...
        let n_list1 = List.map (
            fun  (c1,(b1,c2)) -> 
                ...
                List.map (fun (d1,(b2,d2))->(d1::c1, (Gen.Baga(Slices.PtrSV).or_baga CP.eq_spec_var b1 b2,combine_pures c2 d2))) rem 
                List.map (fun (d1,(b2,d2))->(d1::c1, (Gen.Baga(Slices.PtrSV).or_baga CP.eq_spec_var b1 b2,combine_pures c2 d2))) rem 
        ) last_lst in          
  ...
  let norm_inv_list = List.map (fun (c1,(b1,c2))-> (c1,(Gen.Baga(Slices.PtrSV).conj_baga CP.eq_spec_var v_l b1,MCP.memo_norm_wrapper c2))) invariant_list in
  let norm_inv_list = List.map (fun (c1,(b1,c2))-> (c1,(Gen.Baga(Slices.PtrSV).conj_baga CP.eq_spec_var v_l b1,MCP.memo_norm_wrapper c2))) invariant_list in
...
and splitter (f_list_init:(Cpure.formula*Cformula.ext_formula) list) (v1:Cpure.spec_var list) : Cformula.struc_formula list =
  ...
  if (List.length v1)<1 then [(snd (List.split f_list_init))]
    ...
    | _ ->
	      ...
	      let constr_list = Gen.BList.remove_dups_eq (=)_f constr_list 
	      let constr_list = Gen.BList.remove_dups_eq (=)_f constr_list 
