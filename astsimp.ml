  ...
  let rec helper n do_not_compute_flag =
    ...
    (if (n > 0 (* && not(vdef.C.view_is_prim) *)) then
       (
         ...
         let drop_f = 
           if false (* not(!Globals.adhoc_flag_3) *) then CP.drop_rel_formula (MCP.pure_of_mix xform)
           else MCP.pure_of_mix xform in
         let xform1 = (x_add Excore.simplify_with_label (TP.simplify_with_pairwise 1) drop_f) in
  ...
  (
    ...
    if (List.mem res_name cf_fv) || (List.mem res_name pf_fv) || (List.mem res_name inv_lock_fv)  then
      ...
      let allow_ex_vs =
        ...
        let () = y_tinfo_hp (add_str "inv_pf" !CP.print_formula) inv_pf in
      ...
      let () = x_tinfo_hp (add_str "vbi" (pr_option pr)) vbc_i no_pos in
      ...
      let () = y_tinfo_hp (add_str "extracted start/end" !CP.print_svl) ans in
      let () = y_tinfo_hp (add_str "view_vars" !CP.print_svl) view_sv in
      let inter_vars = CP.intersect_svl ans view_sv in
      let () = y_tinfo_hp (add_str "inter_vars" !CP.print_svl) inter_vars in
      ...
      let cvdef = {
        ...
        C.view_match_args = inter_vars;
              (* (let () = (y_tinfo_pp "WN : Need to fix match_args") in *)
              (* []); *)
    ...
    let res = ( (*let () = print_string("H_formula: "^(Iprinter.string_of_h_formula f)^"\n") in*)
      ...
      | IF.ThreadNode {IF.h_formula_thread_node = (v, p);
        ...
        let newNode = CF.ThreadNode {
            CF.h_formula_thread_name = x_add_1 CF.get_node_name dataNode2;
