...
let rec (* overriding test *) (* field duplication test *)
  ...
  let check_field_hiding = Chk.no_field_hiding prog0 in
	  ...
	  in
		 ...
		 (print_string
                    ("duplicated top-level name(s): " ^
                       ((String.concat ", " dups) ^ "\n"));
		  failwith ("Error detected: duplicated top-level name(s): " ^ (String.concat ", " dups)))
               else
		 (
    ...
    else failwith "Error detected: !(check_field_dup &&
        (check_method_dup && (check_overridding && check_field_hiding)))"
    ...
    match (List.hd f) with
	  ...
	  let is_sat = if br = [] then 
	    ...
	    if (not is_sat) then None
	      ...
	      let base_case = Cpure.BForm ((Cpure.Eq ((Cpure.Var (self,no_pos)),(Cpure.Null ((Cpure.type_of_spec_var self), no_pos)),no_pos)),None) in
...
and trans_exp (prog : I.prog_decl) (proc : I.proc_decl) (ie : I.exp) :
    trans_exp_type =
  match ie with
    | I.Label (pid, e)-> let e1,t1 = (trans_exp prog proc e) in
    ...
    | I.Member
	{
	  ...
	  I.exp_member_path_id = pid;
          I.exp_member_pos = pos
	} -> flatten_to_bind prog proc e (List.rev fs) None pid pos
    | I.New
	{
    ...
    | I.Unary
	{ I.exp_unary_op = u_op; I.exp_unary_exp = e; I.exp_unary_path_id = pid; I.exp_unary_pos = pos;}
      ->
		 ...
		 I.Seq
		   {
               ...
               in
		 trans_exp prog proc
		   (I.Block { I.exp_block_body = seq2; I.exp_block_jump_label = I.NoJumpLabel; I.exp_block_pos = pos;})
	   | I.OpPostDec -> 
               let fn = (fresh_var_name "int" pos.start_pos.Lexing.pos_lnum) in
               let fn_decl =
		 I.VarDecl
		 ...
		 I.Seq
		   {
               ...
               in
		 trans_exp prog proc
		   (I.Block { I.exp_block_body = seq;I.exp_block_jump_label = I.NoJumpLabel;  I.exp_block_pos = pos;})
	   | I.OpPreDec ->
               let sub1_e =
		 I.Binary
		   {
  ...
  match b0 with
    ...
    | IP.ListAllN (e1, e2, pos) ->
      ...
      let pe2 = trans_pure_exp e2 stab in CP.ListAllN (pe1, pe2, pos)
		| IP.ListFirstOcc (e1, e2, e3, pos) ->
      let pe1 = trans_pure_exp e1 stab in
			let pe2 = trans_pure_exp e2 stab in
      let pe3 = trans_pure_exp e3 stab in CP.ListFirstOcc (pe1, pe2, pe3, pos)
    | IP.ListPerm (e1, e2, pos) ->
      let pe1 = trans_pure_exp e1 stab in
      let pe2 = trans_pure_exp e2 stab in CP.ListPerm (pe1, pe2, pos)
		| IP.ListStable (e1, e2, pos) ->
      let pe1 = trans_pure_exp e1 stab in
      let pe2 = trans_pure_exp e2 stab in CP.ListStable (pe1, pe2, pos)

and trans_pure_exp (e0 : IP.exp) stab : CP.exp =
  match e0 with
    | IP.Null pos -> CP.Null (CP.OType "Obj", pos)        (*ToDo: write type inference for Null*)
    ...
    | IP.ListLength (e, pos) ->
      CP.ListLength (trans_pure_exp e stab, pos)
		| IP.Fst (e, pos) ->
      CP.Fst (trans_pure_exp e stab, pos)
		| IP.Snd (e, pos) ->
      CP.Snd (trans_pure_exp e stab, pos)
		| IP.ListMin (e1, e2, pos) ->
...
and collect_type_info_b_formula b0 stab =
  ...
  | IP.ListFirstOcc (e1, e2, e3, pos) ->
      (collect_type_info_arith e1 stab Unknown;
       collect_type_info_list e2 stab;
		   collect_type_info_arith e3 stab Unknown)
  | IP.ListPerm (e1, e2, pos) ->
...
and collect_type_info_list (e0 : IP.exp) stab =
  ...
  | IP.ListConsP (a1, a2, a3, pos) -> 
      (collect_type_info_arith a1 stab Unknown;
			 collect_type_info_arith a2 stab Unknown;
	     collect_type_info_list a3 stab)
	| IP.ListMin (a1, a2, pos) ->
		   (collect_type_info_list a1 stab;
			  collect_type_info_arith a2 stab Unknown)
	| IP.ListHead (a, pos) ->
		  collect_type_info_arith a stab Unknown
  | IP.ListTail (a, pos) ->
      (collect_type_info_list a stab)
...
and case_normalize_exp prog (h: (ident*primed) list) (p: (ident*primed) list)(f:Iast.exp) :
    Iast.exp*((ident*primed) list)*((ident*primed) list)*((ident*ident) list) =  match f with
      ...
      | Iast.Label (pid,b)-> let nb,_,_,_ =  case_normalize_exp prog h p b in 
