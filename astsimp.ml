...
and trans_view_x (prog : I.prog_decl) mutrec_vnames transed_views ann_typs (vdef : I.view_decl): C.view_decl =
  ...
  (
    ...
    let (n_tl,cf) = trans_I2C_struc_formula 1 prog false true (self :: vdef.I.view_vars) vdef.I.view_formula (orig_tl) false true in
...
and trans_views_x iprog ls_mut_rec_views ls_pr_view_typ =
  ...
  let has_pure_props = List.exists (fun vdecl -> vdecl.C.view_kind=Globals.View_NORM &&
...
and trans_proc_x (prog : I.prog_decl) (proc : I.proc_decl) : C.proc_decl =
  let trans_proc_x_op () =
    ...
    else if not !verify_td && (not check_return_res) && (String.compare proc.I.proc_name "main" != 0) then
      Err.report_error {
