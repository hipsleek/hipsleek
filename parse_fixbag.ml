...
GLOBAL: expression or_formula formula pformula exp specvar;
  ...
  [ "pformula" LEFTA
    [ x = exp; "<="; y = exp -> 
      begin
      match (x,y) with
        | (Var _, Var _) -> BForm ((BagSub (x, y, loc), None), None, None)
        | (Bag _, Var _) -> BForm ((BagSub (x, y, loc), None), None, None)
        | _ -> mkTrue loc
      end
    | x = exp; ">="; y = exp -> 
      begin
      match (x,y) with
        | (Var _, Var _) -> BForm ((BagSub (y, x, loc), None), None, None)
        | (Var _, Bag _) -> BForm ((BagSub (y, x, loc), None), None, None)
        | _ -> mkTrue loc
      end
    | x = exp; "="; y = exp -> 
      begin
      match (x,y) with
        ...
        | _ -> BForm ((Eq (x, y, loc), None), None, None)
      end
    | x = exp; "!="; y = exp -> BForm ((Neq (x, y, loc), None), None, None)
    | "forall"; x = exp; "in"; y = exp; ":"; z = pformula ->
      ...
      match (x,z) with
        | (Var (v1,_), BForm ((Neq(Var(v2,_),Var(v3,_),_),_),_,_)) -> 
          ...
          in BForm ((res,None),None,None)
        | (Var (v1,_), BForm ((Eq(Var(v2,_),Var(v3,_),_),_),_,_)) -> 
          if eq_spec_var v1 v2 then mkForall [v1]
            (mkOr (BForm ((BagNotIn (v1,y,loc),None),None,None)) 
                  (BForm ((Eq (Var (v1,loc),Var (v3,loc),loc),None),None,None)) None loc) None loc else
          if eq_spec_var v1 v3 then mkForall [v1]
            (mkOr (BForm ((BagNotIn (v1,y,loc),None),None,None)) 
                  (BForm ((Eq (Var (v1,loc),Var (v2,loc),loc),None),None,None)) None loc) None loc else mkTrue loc
        | _ -> mkTrue loc
      end
