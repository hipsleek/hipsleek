...
/* view for avl trees */
/**** for omega nd mona ****/
avl2<n, "s":S> == true & ["":self = null & n = 0; "s": S = {} ]
  or (exists n3,tmp: self::node<v, n, p, q> * p::avl2<n1, S1> * q::avl2<n2, S2> & ["": n3 =  max(n1-n2, n2-n1) & n3 <= 1 & tmp = max(n1, n2) & n = tmp + 1; "s": S = union(S1, S2, {v})]) 
  inv true & ["": n >= 0];
...
node rotate_left(node l, node rl, node rr)
  ...
  ensures res::avl2<2+ln, S> & ["s":S = union (S1,S2,S3)]; 
