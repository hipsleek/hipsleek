...
let rec smt_of_exp_x a =
  ...
  | CP.ListAppend ([es; CP.List ([e], _)], _) -> "(insert " ^ (smt_of_exp e) ^ " " ^ (smt_of_exp es) ^ ")"
...
and smt_of_exp (e: CP.exp) : string =
  let pr_e = (add_str "exp" !CP.print_exp) in
  let pr_res = (add_str "res" pr_id) in
  Debug.no_1 "smt_of_exp" pr_e pr_res
      (fun _ -> smt_of_exp_x e) e
...
let check_formula f timeout =
  ...
  let fnc f = (
    ...
    (* Trung:
       - In interactive mode, using continuously scopes like
...
let check_formula f timeout =
  Debug.no_2 "smt_check_formula" idf string_of_float string_of_smt_output
...
and smt_imply pr_weak pr_strong (ante : Cpure.formula) (conseq : Cpure.formula) (prover: smtprover) timeout : bool =
  ...
  Debug.no_3(* _loop *) "smt_imply" pr_ante pr_conseq pr_timeout pr_res
      (fun _ _ _ -> smt_imply_x  pr_weak pr_strong ante conseq prover timeout)
      ante conseq timeout
...
let smt_is_sat pr_weak pr_strong (f : Cpure.formula) (sat_no : string) (prover: smtprover) timeout : bool =
  ...
  if (should_run_smt) then
    ...
    Debug.ninfo_hprint (add_str "smt_is_sat input" pr_id) input no_pos;
