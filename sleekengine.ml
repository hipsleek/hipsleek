...
let process_list_lemma ldef_lst =
  ...
  let res = 
    match ldef_lst.Iast.coercion_list_kind with
      ...
      | LEM_INFER_PRED      -> let r1,_,r2 = Lemma.manage_infer_pred_lemmas lst iprog !cprog Solver.xpure_heap in
        let _ =
          begin
...
let process_pred_split ids=
  ...
  let hp_defs1, split_map = SAC.pred_split_hp iprog !cprog unk_hps Infer.rel_ass_stk CF.rel_def_stk sel_hp_defs in
...
let process_shape_infer_prop pre_hps post_hps=
  ...
  let _ = if not (!Globals.pred_syn_modular) then
    begin
      let rel_defs =  CF.rel_def_stk in
      if not(rel_defs# is_empty) then
        print_endline "";
      print_endline "\n*************************************";
      print_endline "*******relational definition ********";
      print_endline "*************************************";
      print_endline (CF.rel_def_stk # string_of_reverse);
      print_endline "*************************************"
    end
  in
  (* let _ = if(!Globals.cp_test || !Globals.cp_prefile) then *)
