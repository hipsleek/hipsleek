...
let print_residue residue =
  ...
  if (not !Globals.smt_compete_mode) then
    ...
    | Some (ls_ctx, print, local_dfa, local_err_exc) -> begin
        ...
        if not local_dfa (*!Globals.disable_failure_explaining *) then
          (* let () = Debug.info_pprint ("a" ^ (string_of_bool local_err_exc))  no_pos in *)
          let () = if not !Globals.enable_error_as_exc && not local_err_exc then
...
let run_infer_one_pass_set_states itype (ivars: ident list) (iante0s : meta_formula list) (iconseq0 : meta_formula) =
  ...
  | ante::rest ->
    ...
    let _ = CF.residues := Some (comb_rs, r, !Globals.disable_failure_explaining, List.mem INF_ERR_MUST itype ||
     List.mem INF_ERR_MAY itype) in
    ((r, comb_rs, v0), pr0)
...
let process_capture_residue (lvar : ident) =
  let flist = match !CF.residues with
    ...
    | Some (ls_ctx, print, _,_) -> CF.list_formula_of_list_context ls_ctx in
