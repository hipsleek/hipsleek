...
let run_entail_check (iante : meta_formula) (iconseq : meta_formula) (etype: entail_type) =
  let pr = string_of_meta_formula in
  let pr_2 (* (a,b,_) *) =  pr_pair string_of_bool Cprinter.string_of_list_context in
...
let run_sat_check (iante0 : meta_formula) =
  ...
  let _ = Debug.devel_zprint (lazy ("\nrun_sat_check:"
                        ...
                        ^"\n\n")) no_pos in
  let es = CF.empty_es (CF.mkTrueFlow ()) Lab2_List.unlabelled no_pos in
  let ante = Solver.normalize_formula_w_coers 0 !cprog es ante !cprog.C.prog_left_coercions in
