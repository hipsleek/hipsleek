...
and checkeq_formulas_with_diff_mt_x ivars f1 f2 mtl = 
  ...
  let (r,fs) = match f1,f2 with 
    |CF.Or _ ,
  CF.Or _  -> (
    check_or_with_diff f1 f2 ivars mtl
  )
    | _ -> (
      let re_order mt = List.map (fun (a,b) -> (b,a)) mt in
...
let check_equiv_def_x hvars svars (def1: (CF.formula * CF.formula)) (def2: (CF.formula * CF.formula)) hp_map: ((CP.spec_var * CP.spec_var * (((CP.spec_var * CP.spec_var) list)list)) list) =
  ...
  let is_hold p (hp1, hp2) hp_map =
    ...
    if(CP.is_hprel_typ a && CP.is_hprel_typ b) then 
	  ...
	  try
	    ...
	    let b = List.exists (fun (c,_) -> CP.eq_spec_var a c) cands in
	    if(b) then false else true
	  with 
	    |Not_found -> true
    else false
  in
  let rec helper mt (hp1,hp2) hp_map svars = 
    match mt with
      | [] -> ([],[])
      | x::y -> let a = is_hold x (hp1, hp2) hp_map in
		let b = (
		  let (_,x2) = x in 
		  List.exists (fun c -> String.compare c (CP.name_of_spec_var x2) == 0) svars
		)
		in
		let (n1,n2) = helper y (hp1,hp2) hp_map svars in
		if(a && b) then (x::n1,x::n2)
		else if(a) then (x::n1,n2)
		else if(b) then (n1,x::n2)
		else (n1,n2)
  in 
  let hp1,_ = def1 in
  let hp2,_ = def2 in
  ...
  if(m)then
    (
      let rel_mtl = List.map (fun mt -> helper mt (hp1, hp2) hp_map svars ) mtl in 
      (* let _ = Debug.ninfo_pprint ("***Relation. MTL: " ^ (string_of_map_table_list rel_mtl)) no_pos in *)
      let tmp = List.filter (fun (rmt,smt) -> List.length rmt == 0) rel_mtl in
      if(List.length tmp > 0) then (
	let _,smts = List.split tmp in 
	let smts = List.filter (fun c -> List.length c > 0) smts in
	(*TODO: get all min length :D (maybe it's should be same length!!*)
	[(hp1, hp2, smts)]
      )
      else []
    )
...
let check_equiv_def hvars svars def1 def2 hp_map =
  let pr1 = pr_pair Cprinter.prtt_string_of_formula Cprinter.prtt_string_of_formula in
  let pr2 = pr_list_ln (pr_pair Cprinter.string_of_spec_var Cprinter.string_of_spec_var) in
  let pr3 = pr_list_ln (pr_triple Cprinter.string_of_spec_var Cprinter.string_of_spec_var (pr_list_ln pr2)) in
  Debug.no_2 "check_equiv_def" pr1 pr1 (pr3)
    (fun _ _ ->  check_equiv_def_x hvars svars def1 def2 hp_map) def1 def2

let match_def_x hvars svars defs def hp_map =
 let hp,_ = def in
 let _ = if(List.length (CF.get_hp_rel_name_formula hp) == 0) then report_error no_pos "lhs is not only HP" in
 let rec remove_dups_ele ls =
   match ls with
     | [] -> []
     | (hp,t)::y -> if(List.exists (fun (c,_) -> CP.eq_spec_var c hp) y) then remove_dups_ele y else (hp,t)::(remove_dups_ele y)
 in
 let hp =  List.hd (CF.get_hp_rel_name_formula hp) in
  let add_hp_map (hps,hp) hp_map =
   try
     ...
     let new_ele = (remove_dups_ele (cands @ hps), hp) in
     let tmp_hp_map = List.filter (fun (_,hp1) -> not(CP.eq_spec_var hp hp1)) hp_map in
     tmp_hp_map@[new_ele]
   with 
     |Not_found -> hp_map@[(hps,hp)]
 in
 let match_def_helper hvars svars idef def2 hp_map= 
   let (rc, hf,f) = idef in
   let def1 = (CF.formula_of_heap hf no_pos, f) in
   check_equiv_def hvars svars def1 def2 hp_map
 in 
 let ls_res = List.concat (List.map (fun idef -> match_def_helper hvars svars idef def hp_map) defs) in
 if(List.length ls_res > 0) then(
   let hps = List.map (fun (d1,d2,smts) -> (d1,smts)) ls_res in
   add_hp_map (hps,hp) hp_map
 )
 else add_hp_map ([],hp) hp_map

let match_def hvars svars defs def hp_map =
  let pr1 = pr_pair Cprinter.prtt_string_of_formula Cprinter.prtt_string_of_formula in
  ...
  Debug.no_2 "match_def" pr2 pr1 (pr3)
    (fun _ _ -> match_def_x hvars svars defs def hp_map) defs def

let process_svars full_tb svars inf_vars=
  let is_inf hp = List.exists (fun c -> CP.eq_spec_var hp c) inf_vars in
  let spairs = List.map (fun c -> (c,[])) svars in
  let rec  update_spairs map spairs =
    let (sv1,sv2) = map in
    match spairs with
      | [] -> []
      | (x,xl)::y -> if(String.compare x (CP.name_of_spec_var sv2) == 0) then (x,sv1::xl)::y else  (x,xl)::(update_spairs map y)
  in
  let rec helper full_tb spairs =
    match full_tb with
      | [] -> spairs
      | x::y -> (
	let (mix_hps, hp) = x in
	let rec subhelper mix_hps spairs = 
	  match mix_hps with
	  | [] -> spairs
	  | mix_hp::t-> (
	    let (hp1,ll) = mix_hp in
	    (*TODO: here just match the first meet, but if need to *)
	    let l = List.concat ll in
	    let sp_head = List.fold_left (fun sp c -> update_spairs c sp) spairs l in
	    subhelper t sp_head
	  )
	in
	let sp_head2 = subhelper mix_hps spairs in
	helper y sp_head2
      )
  in
  let newspairs = List.map (fun (c,cl) -> (c, CP.remove_dups_svl cl)) (helper full_tb spairs) in
  let rec filter_spairs cl = match cl with
    | [] -> []
    | (x,l)::y -> if(List.length l == 0) then filter_spairs y else (x,List.hd l)::(filter_spairs y)
  in
  let newspairs = filter_spairs (helper full_tb spairs) in
  let pr3 = pr_list_ln ( Cprinter.string_of_spec_var) in
  let  _,x = List.split newspairs in
  print_string (pr3 x ^ "\n");
  let rec mknewtb full_tb newspairs = 
    match full_tb with
      | [] -> []
      | x::y -> (
	let (mix_hps, hp) = x in
	let rec subhelper mix_hps newspairs = 
	  match mix_hps with
	    | [] -> []
	    | mix_hp::t-> (
	      let (hp1,ll) = mix_hp in
	      (*choose hp here*)
	      let rec check_l l newspairs=  
		match l with 
		  | [] -> []
		  | x::y -> let (sv1,sv2) = x in 
			    let head = try (
			      let (s,c) = List.find (fun (sv,cand) -> (String.compare sv (CP.name_of_spec_var sv2) == 0)) newspairs in
			      if(CP.eq_spec_var c sv1) then [] else [x]
			    )
			      with _ -> []
			    in
			    head@(check_l y newspairs)
	      in
	      let check_ll = (List.length ll == 0) || (List.exists (fun c -> List.length c == 0) (List.map (fun l -> check_l l newspairs) ll)) in 
	      if(check_ll) then 
		hp1::(subhelper t newspairs)
	      else (subhelper t newspairs)
	    )
	in
	let sp_head2 = (subhelper mix_hps newspairs,hp) in
	sp_head2::(mknewtb y newspairs)
      )
  in
  (mknewtb full_tb newspairs,newspairs)
...
let checkeq_defs_x hvars svars (defs: (CP.rel_cat * CF.h_formula * CF.formula) list) ( infile_defs: (CF.formula * CF.formula) list) =
  ...
  in
  let rec helper hvars svars defs1 defs2 hp_mt=
    let hp_map1 = List.fold_left (fun piv def -> match_def hvars svars defs1 def piv) hp_mt defs2 in
    if(no_change hp_map1 hp_mt) then  (*terminate condition*)
      hp_map1 
    else 
      helper hvars svars defs1 defs2 hp_map1
  in
  let full_tb = helper hvars svars defs infile_defs [] in
  full_tb
  
let checkeq_defs hvars svars (defs: (CP.rel_cat * CF.h_formula * CF.formula) list) ( infile_defs: (CF.formula * CF.formula) list) =
  let pr1 = pr_list_ln (pr_pair Cprinter.prtt_string_of_formula Cprinter.prtt_string_of_formula) in
  let pr2 = pr_list_ln Cprinter.string_of_hp_rel_def in
  let pr5 = pr_list_ln (pr_pair Cprinter.string_of_spec_var Cprinter.string_of_spec_var) in
  let pr4 = (pr_pair Cprinter.string_of_spec_var (pr_list_ln pr5)) in
  let pr3 = pr_list_ln (pr_pair (pr_list_ln pr4) Cprinter.string_of_spec_var) in
...
let checkeq_defs_bool hvars svars (defs: (CP.rel_cat * CF.h_formula * CF.formula) list) ( infile_defs: (CF.formula * CF.formula) list) inf_vars=
  let full_tb = checkeq_defs hvars svars defs infile_defs in
...
let check_equiv_def_with_diff hvars svars (def1: (CF.formula * CF.formula)) (def2: (CF.formula * CF.formula)) hp_map =
  ...
  let hp2,_ = def2 in
  let (hp1, hp2) =  ((List.hd (CF.get_hp_rel_name_formula hp1)),(List.hd  (CF.get_hp_rel_name_formula hp2))) in
  let m,mtl = check_equiv_constr_with_diff hvars def1 def2 in (*bool, map_table_list*)
  if(m)then
    (
...
let checkeq_defs_with_diff_x hvars svars (defs: (CP.rel_cat * CF.h_formula * CF.formula) list) ( infile_defs: (CF.formula * CF.formula) list) inf_vars :  (bool*(((CF.formula * CF.formula) *  (CF.formula * CF.formula) * ((map_table * (CF.formula * CF.formula)*(CF.formula * CF.formula)) list)) list))=
  ...
  if(res) then (res,[])
	...
	let b,mtl,new_hps =  check_equiv_def_with_diff hvars svars d1 d2 mtb in
	if(b) then
...
let checkeq_defs_with_diff hvars svars (defs: (CP.rel_cat * CF.h_formula * CF.formula) list) ( infile_defs: (CF.formula * CF.formula) list) inf_vars =
  ...
  Debug.ho_2 "checkeq_defs_with_diff" pr2 pr1 (pr_pair pr4 pr7)
