...
and xpure_x (prog : prog_decl) (f0 : formula) : (mix_formula * CP.spec_var list * CF.mem_formula) =
  ...
  if (!Globals.allow_imm)||(!Globals.allow_field_ann) then xpure_symbolic prog f0
  else
        ...
        | ViewNode ({ h_formula_view_node = p;
          ...
          h_formula_view_arguments = vs}) -> begin
            try
	          ...
	          (****)  
              let renamed_view_formula = 
	        if ((isImm imm) || (isLend imm) || (isAccs imm)) && not(!Globals.allow_field_ann) then (* andreeac && not(!Globals.allow_field_ann)*)
		...
		(*  end of helper method *)
	          ...
	          let rhs = conseq in
   	          ...
   	          let rhs_lst = split_linear_node_guided (CP.remove_dups_svl (h_fv h1 @ MCP.mfv p1)) h2 in
    	          ...
    	          let actions = Context.compute_actions prog rhs_eqset h1 p1 p2 posib_r_alias rhs_lst estate.es_is_normalizing pos in
...
and heap_entail_empty_rhs_heap_x (prog : prog_decl) (is_folding : bool)  estate_orig lhs (rhs_p:MCP.mix_formula) pos : (list_context * proof) =
  ...
  *) 
  ...
  in
  let fold_fun_mipt x (rhs_p:MCP.mix_formula) =
    let pr = Cprinter.string_of_mix_formula in
...
and do_match_x prog estate l_node r_node rhs (rhs_matched_set:CP.spec_var list) is_folding pos : 
  ...
  Debug.devel_zprint (lazy ("do_match: using " ^
    ...
    let (r,add_to_lhs,add_to_rhs, ann_subst) = if ((*not(!allow_field_ann) &&*) (!allow_imm)) then subtype_ann_gen es_impl_vars es_evars l_ann r_ann else (true, None, None, ([], []))  (*ignore node ann is field ann enable*) in
    ...
    let (rl, param_ann_lhs, param_ann_rhs, field_ann_subst) =  if (!allow_field_ann) then subtype_ann_list es_impl_vars es_evars l_param_ann r_param_ann else (true, [], [], ([], [])) in
    ...
    let (rvl, view_param_ann_lhs, view_param_ann_rhs) =  if (!allow_field_ann) 
        ...
        else (true, None,None) in       (* andreeac: add ann view for ex *)
    ...
    let (r, ann_lhs, ann_rhs, ex_subst) = (r && rvl,Immutable.mkAndOpt ann_lhs view_param_ann_lhs, Immutable.mkAndOpt ann_rhs view_param_ann_rhs, concat_pair_of_lists ann_subst field_ann_subst) in
	    ...
	    | _ -> 
            ...
            let label_list = try 
              ...
              *)
              ...
              let ex_subst = List.combine (fst ex_subst) (snd ex_subst) in
              (* let ext_subst = ex_subst@ext_subst in *)
              ...
              let ext_subst = if (!Globals.allow_imm)||(!Globals.allow_field_ann) then List.filter (fun (f,t) -> not ( is_ann f &&  is_ann t)) ext_subst else ext_subst in (*andreeac why filter out anns?  *)
              let ext_subst = ext_subst@ex_subst in
...
and heap_entail_non_empty_rhs_heap prog is_folding  ctx0 estate ante conseq lhs_b rhs_b (rhs_h_matched_set:CP.spec_var list) pos : (list_context * proof) =
  (*LDK*)
  Debug.no_4_loop "heap_entail_non_empty_rhs_heap" 
      Cprinter.string_of_context
                ...
                match r with
                  ...
                  | None ->
                        ...
                        if (not res) then
                          ...
                          let n_rhs_b = Base {rhs_b with formula_base_heap = rhs_rest} in
                          Debug.tinfo_hprint (add_str "new_estate(M_unmatched_rhs_data_node)" (Cprinter.string_of_entail_state)) new_estate pos;
                          let res_es0, prf0 = do_match prog new_estate rhs rhs n_rhs_b rhs_h_matched_set is_folding pos in
                          (* let res_ctx = Ctx new_estate  in *)
                          (* (SuccCtx[res_ctx], NoAlias) *)
                          (res_es0,prf0)
...
(* andreeac: can param i be deleted? *)
and process_action i caller prog estate conseq lhs_b rhs_b a (rhs_h_matched_set:CP.spec_var list) is_folding pos =
  ...
  let pr3 = Cprinter.string_of_formula in
  Debug.no_6 "process_action" string_of_int pr1 Cprinter.string_of_entail_state pr3 pr3 pr3 pr2
