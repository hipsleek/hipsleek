    ...
    | Context.M_cyclic ({
      ...
      let new_view_opt = if !Globals.lemma_syn then
          let nview_opt = if lem_type =2 then
              ...
              let lemma_count = List.length (Lem_store.all_lemma # get_left_coercion) in
              let () = x_add Lemsyn.gen_lemma prog (!rev_trans_formula) (!manage_unsafe_lemmas)
                  estate lem_type lhs_node lhs_b rhs_node orig_rhs_b in
              let new_lemma_count = List.length (Lem_store.all_lemma # get_left_coercion) in
              let () = if lemma_count = new_lemma_count then Globals.lemma_syn := false in
              None
          in
          nview_opt
        else None
