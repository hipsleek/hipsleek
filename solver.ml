...
and split_linear_node (h : h_formula) : (h_formula * h_formula) list =
 ...
 *)
and split_linear_node_guided (vars : CP.spec_var list) (h : h_formula) :
...
and split_linear_node_guided_x (vars : CP.spec_var list) (h : h_formula) :
  (h_formula * h_formula) list =
...
and discard_uninteresting_constraint (f : CP.formula) (vvars: CP.spec_var list)
  : CP.formula =
  match f with
  | CP.BForm _ ->
...
and elim_exists_pure_branch_x ?(revflag=false) (w : CP.spec_var list) (f0 :
        ...
        let r1,p1 =
          ...
          if not(contra) then
            ...
            if real_c then
              ...
              let () = Debug.binfo_hprint  (add_str "lc" Cprinter.string_of_list_context) lc no_pos  in
              (lc,prf)
          else
            match (r1,prf) with
  ...
  unexpected for bperm"
      ...
      (match res with
       ...
       | None  -> begin
           ...
           let r = process_before_do_match pure_to_add prog estate conseq lhs_b
               rhs_b rhs_h_matched_set is_folding pos lhs_node lhs_rest rhs_node
               rhs_rest holes in
...
and is_cycle_coer_a (c:coercion_decl) (origs:ident list) : bool =  List.mem
...
and normalize_frac_heap prog h p vp =  (*used after adding back the consumed heap*)
  if !perm=NoPerm then (h, p)
    ...
    let f = normalize_base_perm prog (mkBase h p vp TypeTrue (mkTrueFlow ()) []
                                        no_pos) in
    match f with
    | Or _ -> Error.report_error {Err.error_loc = no_pos;
                                  Err.error_text
                                  = "normalize_frac_heap: adding the consumed
  heap should not yield OR"}
    | _ ->
  ...
  let rec helper f0 =
    ...
    | Exists ({ formula_exists_qvars = qvar :: rest_qvars;
      ...
      if  not(List.exists (fun sv -> CP.eq_spec_var sv qvar) fvh) then
        ...
        if List.length st > 0 then (* if there exists one substitution  -
