...
let pr_mater_source ms = match ms with
  ...
  | Context.Coerc_mater v -> PR.fmt_string ("coerc_defn_mater: "^v)
  
let pr_match_type (e:Context.match_type) =
  match e with
    | Context.Root -> PR.fmt_string "Root"
    | Context.MaterializedArg (mv,ms) -> 
      PR.fmt_string "MaterializedArg "; PR.pr_mater_prop mv;PR.fmt_string " " ;pr_mater_source ms    
...
and find_node_one prog lhs_h lhs_p (p : CP.spec_var) (imm : bool)  rhs_info pos : find_node_result =
  let pr1 x = match x with
    | None -> "None"
    | Some (p1,p2,_) -> pr_pair Cprinter.string_of_mix_formula Cprinter.string_of_spec_var_list (p1,p2) in
    ...
    | ViewNode ({ h_formula_view_node = p2;
      ...
      h_formula_view_pos = pos2}) -> begin
	    ...
	    *)
	    ...
	    let fnode_results = find_node_one prog lhs_h lhs_p p2 imm2 (Some (rhs_p,posib_r_alias,ln2))  pos in
