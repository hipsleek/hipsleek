...
and heap_entail_conjunct_lhs_struc
      p is_folding  has_post ctx conseq (tid: CP.spec_var option)
  ...
  Gen.Debug.no_3 "heap_entail_conjunct_lhs_struc"
      ...
      match f with
        ...
        | EBase ({
              ...
              if (List.length base_exists) > 0 then 
	            ...
	            let n_ctx_list, prf = heap_entail_one_context prog (if (List.length formula_cont)>0 then true else is_folding)   n_ctx formula_base None pos in
			    ...
			    (match n_ctx_list with
	              ...
	              | SuccCtx sc ->
		                if (List.length formula_cont)>0 then
                          let res, n_rpf = heap_entail_struc prog is_folding has_post n_ctx_list formula_cont tid pos pid in
  ...
  if CF.isAnyFalseCtx ctx then (* Unreachable state *)
	...
	if (var_label_lhs = var_label_rhs && var_label_rhs > 0) then (* Case 1: In loop *)
	  ...
	  let fun_check_term lst_measures = (* [(m1,n1),(m2,n2)] -> m1=n1 & m1>=lb1 & m2>n2 & m2>=lb2 *) 
		...
		let (rs, _) = (heap_entail_conjunct_lhs_struc prog false false ctx [mkEBase term_formula loc] None no_pos None) in
...
and heap_entail_empty_rhs_heap_x (prog : prog_decl) (is_folding : bool)  estate lhs (rhs_p:MCP.mix_formula) rhs_p_br pos : (list_context * proof) =
  ...
  if r_rez then begin (* Entailment is valid *)
    ...
    if (tmp1!=[] || tmp2!=[]) then
      begin
          (*FAIL*)
          Debug.devel_pprint ("heap_entail_empty_rhs_heap:" ^ (Cprinter.string_of_spec_var_list (tmp1@tmp2))^ "is " ^(string_of_vp_ann VP_Zero) ^ "\n") pos; (*TO DO: error messages can be more instructive*)
          Debug.devel_pprint ("heap_entail_empty_rhs_heap: failed in entailing " ^ (string_of_vp_ann VP_Value) ^ " and " ^ (string_of_vp_ann VP_Ref) ^ " variable permissions in conseq\n") pos;
    ...
    if (tmp4!=[]) then
      begin
          ...
          (* S@zero |- v@full  --> FAIL *)
          (*FAIL*)
          Debug.devel_pprint ("heap_entail_empty_rhs_heap:" ^ (Cprinter.string_of_spec_var_list tmp4)^ "is " ^(string_of_vp_ann VP_Zero) ^ "\n") pos; (*TO DO: error messages can be more instructive*)
          Debug.devel_pprint ("heap_entail_empty_rhs_heap: failed in entailing " ^ (string_of_vp_ann VP_Full) ^ " variable permissions in conseq\n") pos;
          Debug.devel_pprint ("heap_entail_empty_rhs_heap: formula is not valid\n") pos;
          let rhs_p = List.fold_left (fun mix_f vperm -> memoise_add_pure_N mix_f vperm) rhs_p rhs_vperms in
          (CF.mkFailCtx_in (Basic_Reason ({
		      fc_message = "failed in entailing " ^ (string_of_vp_ann VP_Full) ^ " variable permissions in conseq";
		      fc_current_lhs  = estate;
		      fc_prior_steps = estate.es_prior_steps;
		      fc_orig_conseq  = struc_formula_of_formula (formula_of_mix_formula_with_branches rhs_p rhs_p_br [] pos) pos;
		      fc_current_conseq = CF.formula_of_heap HFalse pos;
    ...
    match r with
      | Some new_estate ->
            ...
            (* let ctx1 = (Ctx new_estate) in *)
            (* let ctx1 = set_unsat_flag ctx1 true in  *)
            let r1, prf = heap_entail_one_context prog is_folding ctx1 (CF.formula_of_mix_formula rhs_p pos) None pos in
            (r1,prf)
      | None ->
            begin
              Debug.devel_pprint ("heap_entail_empty_rhs_heap: formula is not valid\n") pos;
...
let heap_entail_struc_list_failesc_context_init (prog : prog_decl) (is_folding : bool)  (has_post: bool)
  ...
  ^"\n") pos; 
  let res,prf = heap_entail_failesc_prefix_init prog is_folding  has_post cl conseq tid pos pid (rename_labels_struc,Cprinter.string_of_struc_formula,(heap_entail_one_context_struc_nth "2")) in
...
let heap_entail_list_failesc_context_init (prog : prog_decl) (is_folding : bool)  (cl : list_failesc_context)
  ...
  if cl==[] then ([],UnsatAnte)
    ...
    let conseq = prune_preds prog false conseq in
    Gen.Profiling.pop_time "entail_prune";
    let (lfc,prf) = heap_entail_failesc_prefix_init prog is_folding  false cl_after_prune conseq tid pos pid (rename_labels_formula ,Cprinter.string_of_formula,heap_entail_one_context_new) in
