  ...
  match cl with 
    ...
    | SuccCtx cl ->
	  if !Globals.use_set || Gen.is_empty cl then
            ...
            (heap_entail_one_context 4 prog is_folding  (List.hd cl) conseq None pos)
  ...
  Debug.devel_zprint (lazy ("heap_entail_conjunct_helper:\ncontext:\n" ^ (Cprinter.string_of_context ctx0)^ "\nconseq:\n" ^ (Cprinter.string_of_formula conseq))) pos;
    ...
    match ctx0 with
      ...
      | Ctx estate -> 
            ...
            let conseq = Mem.remove_accs_from_formula conseq in (* Make x::node<_,_>@A to x!= null on RHS *)
...
and heap_entail_empty_rhs_heap_x (prog : prog_decl) (is_folding : bool)  estate_orig lhs (rhs_p:MCP.mix_formula) pos : (list_context * proof) =
  ...
  *)
  ...
  (* let _ = print_string ("\n(andreeac)solver.ml  rhs_p: " ^ (Cprinter.string_of_mix_formula rhs_p)) in *)
  (* Termination: Try to prove rhs_wf with inference *)
	...
	| _ -> 
		...
		| _ -> mkStarH rem_l_node l_h pos in
	      ...
	      let imm_n = get_imm r_node in
              let new_consumed = if not(isLend imm_n || isAccs imm_n) (*&& not(!allow_field_ann)*) then  mkStarH consumed_h estate.es_heap pos  else  estate.es_heap in
