...
and xpure_mem_enum (prog : prog_decl) (f0 : formula) : (MCP.mix_formula * (branch_label * CP.formula) list * CF.mem_formula) = 
  Gen.Debug.ho_1 "xpure_mem_enum" Cprinter.string_of_formula (fun (a1,_,a3)->(Cprinter.string_of_mix_formula a1)^"#"
...
and xpure_heap_symbolic_i_orig (prog : prog_decl) (h0 : h_formula) xp_no: (MCP.mix_formula * (branch_label * CP.formula) list * CP.spec_var list) = 
  let rec helper h0 = match h0 with
    | DataNode ({ h_formula_data_node = p;
  ...
  let helper h0 =
	Gen.Debug.ho_1 "xpure_heap_symbolic_i: helper"
...
and xpure_heap_symbolic_i_x(*slicing*) (prog : prog_decl) (h0 : h_formula) xp_no :
	(MCP.mix_formula * (branch_label * CP.formula) list * CP.spec_var list) = 
  let rec helper h0 = match h0 with
    | DataNode ({ h_formula_data_node = p;
				  h_formula_data_label = lbl;
				  h_formula_data_pos = pos }) ->
      let non_zero = CP.BForm ((CP.Neq (CP.Var (p, pos), CP.Null pos, pos), None), lbl) in
      (MCP.memoise_add_pure_N (MCP.mkMTrue pos) non_zero, [], [p])
	  (*let approx = MCP.memoise_add_pure_N (MCP.mkMTrue pos) non_zero in
	  (mkBase HTrue approx TypeTrue (mkTrueFlow ()) [] pos, [], [p])*)
    | ViewNode ({ h_formula_view_node = p;
				  h_formula_view_name = c;
				  h_formula_view_arguments = vs;
				  h_formula_view_remaining_branches = lbl_lst;
      ...
      (match lbl_lst with
        | None -> 
		  ...
		  let _ = print_string ("xpure_heap_symbolic_i: vinv: " ^ (Cprinter.string_of_mix_formula vinv) ^ "\n") in
...
and heap_entail_after_sat_struc prog is_folding  has_post
  ...
  Gen.Debug.ho_1 "heap_entail_after_sat_struc" Cprinter.string_of_context
	(fun (lctx, _) -> Cprinter.string_of_list_context lctx)
	(fun ctx -> heap_entail_after_sat_struc_x prog is_folding has_post ctx conseq pos pid ss) ctx
...
and heap_entail_init (prog : prog_decl) (is_folding : bool)  (cl : list_context) (conseq : formula) pos : (list_context * proof) =
  Gen.Debug.ho_2 "heap_entail_init"
...
and heap_entail_conjunct_lhs prog is_folding  (ctx:context) conseq pos : (list_context * proof) 
  = Gen.Debug.ho_2 "heap_entail_conjunct_lhs" Cprinter.string_of_context Cprinter.string_of_formula Cprinter.(fun (lctx, _) -> Cprinter.string_of_list_context lctx) 
  ...
  let r1,r2 = match a with
    ...
    | Context.M_unfold ({Context.match_res_lhs_node=lhs_node},unfold_num) -> 
          ...
          if (curr_unfold_num>1) then 
            ...
            CF.mk_failure_must "possible infinite unfolding" )),NoAlias)
          else
            let delta1 = unfold_nth 1 (prog,None) estate.es_formula lhs_var true unfold_num pos in (* update unfold_num *)
            let ctx1 = build_context (Ctx estate) delta1 pos in
			let ctx1 = set_unsat_flag ctx1 true in
    ...
    | Context.M_lemma  ({
          ...
          let _ = match ln with
            | None -> () 
            | Some c -> (* print_string ("!!! do_coercion should try directly M? lemma: "^c.coercion_name^"\n") *) () in 
