...
and combine_list_failesc_context_and_unsat_now prog (ctx : list_failesc_context) (f : Cpure.formula) : list_failesc_context = 
  ...
  let rec process_one (ss:CF.steps) fold_rs1 = match fold_rs1 with
    ...
    | Ctx fold_es ->
	    ...
	    let (to_ante, to_ante_br), (to_conseq, to_conseq_br), new_evars = 
            split_universal fold_es.es_pure fold_es.es_evars fold_es.es_gen_expl_vars fold_es.es_gen_impl_vars vs2 pos in
...
and heap_entail_struc_init (prog : prog_decl) (is_folding : bool) (is_universal : bool) (has_post: bool)(cl : list_context) (conseq : struc_formula) pos (pid:control_path_id): (list_context * proof) = 
  ...
  match cl with
    ...
    | SuccCtx _ ->
	    ...
	    let rename_es es = 
        {
          es with es_formula = rename_labels_formula_ante es.es_formula
        }
        in
        ...
        (*
      let _= print_string("\n after transform_list_context in heap_entail_struc_init:\n solver \n" ^ Cprinter.string_of_list_context cl_new^"\n") in
         *)
...
and print_temp tmp = 
 match tmp with
   |head::tail -> (Cprinter.string_of_list_context head   )^"\n and\n" ^ (print_temp tail) 
   |[] -> "\n"
...
and heap_entail_struc (prog : prog_decl) (is_folding : bool) (is_universal : bool) (has_post: bool)(cl : list_context) (conseq : struc_formula) pos pid: (list_context * proof) =
  match cl with 
    ...
    | SuccCtx cl ->
        ...
        let _ = print_string(print_temp tmp2) in(*it is used to observe the context*)
        let _ = print_string("\n end of what the hell is going on\n") in
           *)
