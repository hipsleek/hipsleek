...
and xpure_heap_mem_enum(*_debug*) (prog : prog_decl) (h0 : h_formula) (which_xpure :int) : (MCP.mix_formula * (branch_label * CP.formula) list * CF.mem_formula) =  
  Gen.Debug.no_2 "xpure_heap_mem_enum" 
      ...
      ^(Cprinter.string_of_mem_formula a3)) 
  ...
  let rec helper h0 = match h0 with
    ...
    | Star ({ h_formula_star_h1 = h1;
	  ...
	  h_formula_star_pos = pos}) ->
          ...
          (* let _ = print_string ("xpure_heap_symbolic_i:" *)
          (*                       ^ "\n ### ph1 = " ^ (Cprinter.string_of_mix_formula ph1) *)
          (*                       ^ "\n ### addrs1 = " ^ (Cprinter.string_of_spec_var_list addrs1) *)
          (*                       ^ "\n ### ph2 = " ^ (Cprinter.string_of_mix_formula ph2) *)
          (*                       ^ "\n ### addrs2 = " ^ (Cprinter.string_of_spec_var_list addrs2) *)
          (*                       ^ "\n ### tmp1 = " ^ (Cprinter.string_of_mix_formula tmp1) *)
          (*                       ^ "\n") in *)
...
in
        ...
        (*                        ^ ("\n ### RHS view = " ^ (Cprinter.string_of_h_formula view)) *)
        (*                        ^ ("\n ### view_form = " ^ (Cprinter.string_of_struc_formula view_form)) *)
        (*                        ^ ("\n ### new_ctx = " ^ (Cprinter.string_of_context new_ctx)) *)
        (*                        ^ "\n\n") in *)
        ...
        let tmp_vars = match frac with
          | None -> p :: (estate.es_evars @ vs)
          | Some f -> p :: (f::(estate.es_evars @ vs))
        in
        ...
        let rec process_one_x (ss:CF.steps) rs1  =
          ...
          (* let _  = print_string ("\nfold_op_x1: process_one:" *)
          (*                        ^ ("\n ### (ctx) rs1 = " ^ (Cprinter.string_of_context rs1)) *)
          (*                        ^ "\n\n") in *)
	      ...
	      match rs1 with
	        ...
	        | Ctx es ->
                  ...
                  let mix_f, _ = tmp_pure in
        (* let _  = print_string ("\nfold_op_x1: process_one: after elim_exists es.es_pure" *)
        (*                        ^ ("\n ### es.es_evars = " ^ (Cprinter.string_of_spec_var_list es.es_evars)) *)
        (*                        ^ ("\n ### tmp_vars = " ^ (Cprinter.string_of_spec_var_list w)) *)
        (*                        ^ ("\n ### (args) vs = " ^ (Cprinter.string_of_spec_var_list vs)) *)
        (*                        ^ ("\n ### (svl) w = " ^ (Cprinter.string_of_spec_var_list w)) *)
        (*                        ^ ("\n ### tmp_pure.mix_f = " ^ (Cprinter.string_of_mix_formula mix_f)) *)
        (*                        ^ "\n\n") in *)
  ...
  let rec process_one_x (ss:CF.steps) fold_rs1 = match fold_rs1 with
    ...
    | Ctx fold_es ->
	      ...
	      let (to_ante, to_ante_br), (to_conseq, to_conseq_br), new_evars = 
            ...
            (*                       ^ "\n fold_es = " ^ (Cprinter.string_of_entail_state fold_es) *)
          ...
          (* (\*LDK:*\) *)
          (* let _ = print_string ("process_fold_result : context at end fold" *)
          (*                       ^ "\n ### (bf) new_ctx = "^(Cprinter.string_of_context new_ctx) *)
          (*                       ^ "\n ### (bf) new_conseq = "^(Cprinter.string_of_formula new_conseq) *)
          (*                       (\* ^ "\n ### to_conseq = "^(Cprinter.string_of_pure_formula to_conseq) *\) *)
          (*                       ^ "\n ### (bf) fold_es = "^(Cprinter.string_of_entail_state fold_es) *)
          (*                       ^ "\n ### (af) rest_rs = "^(Cprinter.string_of_list_context rest_rs) *)
          (*                       ^ "\n ### (af) r = "^(Cprinter.string_of_list_context r) *)
          (*                       ^"\n\n") in *)
...
and elim_exists_pure_branch_x (w : CP.spec_var list) (f0 : CP.formula) pos : CP.formula =
  ...
  if (r==[]) then f0
    ...
    (* let lc1 = CP.remove_dupl_conj_eq lc in *)
    (* let (fl,bl)=List.partition (fun e -> CP.intersect r (CP.fv e)==[]) lc1 in *)
    ...
    (* let _ = print_string ("elim_exists_pure_branch:" *)
    (*                       ^ "\n ### w = " ^ (Cprinter.string_of_spec_var_list w) *)
    (*                       ^ "\n ### r = " ^ (Cprinter.string_of_spec_var_list r) *)
    (*                       (\* ^ "\n ### lc1 = " ^ (Cprinter.string_of_pure_formula_list lc1) *\) *)
    (*                       ^ "\n ### fl = " ^ (Cprinter.string_of_pure_formula_list fl) *)
    (*                       ^ "\n ### bl = " ^ (Cprinter.string_of_pure_formula_list bl) *)
    (*                       ^ "\n ### f = " ^ (Cprinter.string_of_pure_formula f) *)
    (*                       ^ "\n ### sf = " ^ (Cprinter.string_of_pure_formula sf) *)
    (*                       ^ "\n ### simplified_f = " ^ (Cprinter.string_of_pure_formula simplified_f) *)
    (*                       ^ "\n\n") in *)
...
and heap_entail_struc_init_bug_orig (prog : prog_decl) (is_folding : bool)  (has_post: bool) (cl : list_context) (conseq : struc_formula) pos (pid:control_path_id): (list_context * proof) =
  ...
  (* let _ = print_string ("\n [Debug] heap_entail_struc_init_bug_orig: ans = "^(Cprinter.string_of_list_context ans)^"\n\n\n") in  (*LDK*) *)
  ...
  match cl with 
    ...
    | SuccCtx cl ->
	      ...
	      if !Globals.use_set || Gen.is_empty cl then
            ...
            (* let _ = print_string "\n [Debug] heap_entail_struc: branch 1 \n" in (\*LDK: error goes this way*\) *)
...
and heap_entail_one_context_struc_nth n p i1 hp cl cs pos pid =
  ...
  Gen.Profiling.do_3_num n str (heap_entail_one_context_struc_debug(* x *) p i1 hp cl) cs pos pid
...
and heap_entail_one_context_struc_debug p i1 hp cl cs pos pid =
  Gen.Debug.no_1 "heap_entail_one_context_struc" Cprinter.string_of_context (fun _ -> "?") (fun cl -> heap_entail_one_context_struc_x p i1 hp cl cs pos pid) cl
    ...
    and heap_entail_one_context_struc_x (prog : prog_decl) (is_folding : bool)  has_post (ctx : context) (conseq : struc_formula) pos pid : (list_context * proof) =
      ...
      (* (\*LDK*\) *)
      (*  print_string ("[Debug] heap_entail_one_context_struc: before" *)
      ...
      Debug.devel_pprint ("heap_entail_one_context_struc:"
        ...
        if isAnyFalseCtx ctx then
          ...
          (* let _ = print_string ("\n [Debug] heap_entail_one_context_struc_x: branch 1") in (\*LDK*\) *)
          ...
          ((SuccCtx [ctx]), UnsatAnte)
              ...
              (* let _ = print_string ("\n [Debug] heap_entail_one_context_struc_x: branch 2") in (\*LDK*\) *)


              ((SuccCtx [ctx]), TrueConseq)
        else

          (* let _ = print_string ("\n [Debug] heap_entail_one_context_struc_x: branch 3") in      (\*LDK: !!! error is on this way *\) *)
      ...
      (* print_string ("[Debug] heap_entail_one_context_struc: after" *)
      (* ^ "\nctx:\n" ^ (Cprinter.string_of_list_context result) *)
      (* ^ "\nconseq:\n" ^ (string_of_proof prf)^"\n\n"); *)
...
and heap_entail_after_sat_struc prog is_folding  has_post
  ...
  match ctx with
    | OCtx (c1, c2) ->
        ...
        (* (\*LDK*\) *)
        (* let _ = print_string ("\n [Debug] heap_entail_after_sat_struc: branch OCtx") in *)
    ...
    | Ctx es -> 
        ...
        (* print_string ("heap_entail_after_sat_struc: invoking heap_entail_conjunct_lhs_struc" *)
		(* ^ "\ncontext:\n" ^ (Cprinter.string_of_context ctx) *)
		(* ^ "\nconseq:\n" ^ (Cprinter.string_of_struc_formula conseq) ^ "\n\n"); *)
  ...
  (*       (\*LDK: trace indicate error*\) *)
  (*       let _ = print_string ("[Debug] heap_entail_after_sat_struc:" *)
...
and heap_entail_conjunct_lhs_struc_x
    ...
    begin
      match f with
        | ECase b   -> 
            ...
            (* let _ = print_string "helper_inner: ECase" in (\*LDK*\) *)
        ...
        | EBase ({
            ...
            (* (\*LDK: error this way *\) *)
            (* let _ = print_string "helper_inner: EBase" in *)
	        (* let _ = print_string ("\nhelper_inner: EBase, start case:"^(Cprinter.string_of_ext_formula f)^"\n") in *)

          if (List.length base_exists)>0 then 

            (* let _ = print_string "helper_inner: EBase branch 1" in (\*LDK*\) *)
	        ...
	        (*   ("\n n_ctx = " ^ Cprinter.string_of_context n_ctx)  *)
            (*    ^ "\n n_ctx_list = " ^ (Cprinter.string_of_list_context n_ctx_list ) *)
            (*    ^"\n conseq: "^ (Cprinter.string_of_ext_formula f)^"\n" *)
	        (*   ) in *)
	        ...
	        (match n_ctx_list with
	          | FailCtx _ -> 
                  ...
                  (* let _ = print_string "ppp 1" in (\*LDK: error is here, failure*\) *)

                  (n_ctx_list, prf)
	          | SuccCtx sc ->

                  (* let _ = print_string "ppp 2" in (\*LDK*\) *)
        ...
        | EAssume (ref_vars, post,(i,y)) -> 
            ...
            (* let _ = print_string "helper_inner: EAssume" in (\*LDK*\) *)
	    ...
	    | EVariance e ->
           ...
           (* let _ = print_string "helper_inner: EVariance" in (\*LDK*\) *)
  ...
  and inner_entailer i (ctx22 : context) (conseq : struc_formula): list_context * proof = 
    ...
    (*   (\*LDK: error this way *\) *)
    (* let _ = print_string "\ninner entailer: _ branch 1 \n" in *)

    (* let _ = print_string ("\ninner entailer: _ branch 1: before \n"  *)
    (*                       ^ "ctx = " ^ (Cprinter.string_of_context ctx22) *)
    (*                       ^"\n") in *)

	  let ctx = CF.add_to_context_num 2 ctx22 "para OR on conseq" in

    (* let _ = print_string ("\ninner entailer: _ branch 1: after \n"  *)
    (*                       ^ "ctx = " ^ (Cprinter.string_of_context ctx) *)
    (*                       ^"\n") in  *)

 (*LDK: before and after are similar*)

	  let r = List.map (helper_inner ctx) conseq in

      (*       (\*LDK: trace below indicate failure*\) *)
      (* let _ = print_string ("\n [Debug] inner entailer: r = " *)
      (*                       ^ "\n list_ctx = "^(Cprinter.string_of_list_context (fst (List.hd r))) *)
      (*                       ^ "\n proof = " ^ (string_of_proof (snd (List.hd r))) *)
      (*                       ^"\n\n\n") in *)
  ...
  if adv_u==[] then
    ...
    (* let _ = print_string ("\n  adv_u: branch 1 \n"  *)
    (*                       ^ "\n ctx_00 = " ^ (Cprinter.string_of_context ctx_00) *)
    (*                       ^ "\n conseq = " ^ (Cprinter.string_of_struc_formula conseq) *)
    (*                       ^"\n") *)
    (* in (\*LDK: error this way*\) *)
...
(* (\*LDK*\) *)
(*   let _ = print_string ("aaa heap_entail_one_context:" *)
(*   ^ "\nctx:\n" ^ (Cprinter.string_of_context ctx) *)
(*   ^ "\nconseq:\n" ^ (Cprinter.string_of_formula conseq)^"\n") in *)
  ...
  ^ "\nconseq:\n" ^ (Cprinter.string_of_formula conseq)^"\n") pos;
    if isAnyFalseCtx ctx then
      ...
      (* let _ = print_string "bbb let abv" in  (\*LDK*\) *)
...
and heap_entail_after_sat prog is_folding  (ctx:CF.context) (conseq:CF.formula) pos
  ...
  (*LDK: error this way*)
  Gen.Debug.no_2 "ccc heap_entail_after_sat" (Cprinter.string_of_context) (Cprinter.string_of_formula) (fun (l,p) -> Cprinter.string_of_list_context l)
    ...
    | _ -> begin
        ...
        let r1,p1 =
	      if !Globals.allow_imm (*(contains_immutable_ctx ctx) or (contains_immutable conseq)*) then
            ...
            (* (\*LDK: error this way*\) *)
            (* let _ = print_string "ffff branch 2 \n" in *)
...
and move_expl_inst_ctx_list_x (ctx:list_context)(f:MCP.mix_formula):list_context = 
    let fct es = 
      ...
      (* (\*LDK: ??? is it safe if I instantiate the ivars in this*\) *)
      let f = MCP.find_rel_constraints f (es.es_gen_expl_vars@es.es_gen_impl_vars@es.es_ivars) in
      let nf = 
      ...
      (*remove duplicated conjs in es_formula which may be introduced
      when adding f2 into the es_formula*)
      let nf = CF.remove_dupl_conj_eq_formula nf in
...
(* 	      end *)
    (* | _ -> Error.report_error {Error.error_loc = no_pos;Error.error_text = "malf(\* unction coer_target recieved non views"} *)
...
and coer_target_a prog (coer : coercion_decl) (node:CF.h_formula) (target_rhs : CF.formula) (lhs : CF.formula) : bool =
  ...
  match node, coer_lhs_heap with
    ...
    | _ -> Error.report_error {Error.error_loc = no_pos; Error.error_text = "malfunction coer_target recieved neither view nodes nor data nodes"}
	      (* given a spec var -> return the entire node *)
	      ...
	      match res_ctx with
	        | SuccCtx (cl) ->
	              ...
	              let res = List.map (fun c -> 
                      ...
                      Debug.devel_pprint ("heap_entail_split_rhs_phases: entail the pure part"
                                          ^ (Cprinter.string_of_context c)
                                          ^ "\nconseq:\n"
                                          ^ (Cprinter.string_of_formula new_conseq)
                                          ^ "\nis_folding:\n"
                                          ^ (string_of_bool is_folding)) pos;
	              ...
	              (res_ctx, res_prf)
	        | FailCtx _ -> 
                Debug.devel_pprint ("heap_entail_split_rhs_phases: entail the pure part: FailCtx") pos;
                (res_ctx, res_prf)	    
  in
...
(*because h is the only phase => prove the heap part + the pure part at the same time*)
...
(*   (\* let _  = print_string("*************************************************\n") in *\) *)
(*     (\* let new_conseq = (func h (MCP.mkMTrue pos)) in *\) *)
(*     let new_conseq = (func h p) in  *)
(*     Debug.devel_pprint ("heap_n_pure_entail: prove the heap + pure *)
(*                             \nante:\n" *)
...
(*                         ^ "\nis_folding:\n" *)
(*                         ^ (string_of_bool is_folding)) pos; *)
(*     heap_entail_split_lhs_phases prog is_folding  ctx0 conseq (contains_mutable_h_formula h) pos *)

(*entail the heap and the pure parts separately*)
and heap_n_pure_entail_x
      (prog : prog_decl)
      (is_folding : bool)
  ...
  (* let _  = print_string("*************************************************\n") in *)
    let new_conseq = (func h (MCP.mkMTrue pos)) in
    (* let new_conseq = (func h p) in *)
    Debug.devel_pprint ("heap_n_pure_entail: prove the heap first
                            \nante:\n"
                      ^ (Cprinter.string_of_context ctx0)
                      ^ "\nconseq:\n"
                      ^ (Cprinter.string_of_formula new_conseq)
  ...
  match entail_h_ctx with
    | FailCtx _ ->
        Debug.devel_pprint ("heap_n_pure_entail: NOT prove the pure (FailCtx) "
                            ^ (Cprinter.string_of_mix_formula p)
                            ^ "\n of the heap \n"
                            ^ (Cprinter.string_of_formula new_conseq)
                            ^ "\nis_folding:\n"
                            ^ (string_of_bool is_folding)) pos;
        (entail_h_ctx, entail_h_prf)
    | SuccCtx(cl) ->
	      (* let _  = print_string("*************************************************\n") in *)
	      (* let _ = print_string("entailing the pure:\n") in *)
	      (* let _  = print_string("*************************************************\n") in *)
          (*prove pure constraints in p*)
        Debug.devel_pprint ("heap_n_pure_entail: prove the pure "
                            ^ (Cprinter.string_of_mix_formula p)
                            ^ "\n of the heap \n"
                            ^ (Cprinter.string_of_formula new_conseq)
                            ^ "\nante:\n"
                            ^ (Cprinter.string_of_context_list cl)
                            ^ "\nconseq:\n"
                            ^ (Cprinter.string_of_mix_formula p)
                            ^ "\nis_folding:\n"
                            ^ (string_of_bool is_folding)) pos;
	      ...
	      match ante with
	        | Exists ({formula_exists_qvars = qvars;
		      ...
		      formula_exists_pos = pos}) ->
                ...
                (* (\*LDK*\) *)
                (* let _ = print_string "heap_entail_conjunct_helper: EXISTS in ante \n" in *)
                  ...
                  (* (\*LDK*\) *)
                  (* let _ = print_string ( "jjj" ^ (Cprinter.string_of_context new_ctx) ^ "\n") in *)
                  ...
                  (* (\*LDK: trace below indicate failure*\) *)
                  (* let _ = print_string ("\n [Debug] hhh" *)
                  (*                       ^ "\n rs = "^(Cprinter.string_of_list_context rs) *)
                  (*                       ^ "\n proof1 = " ^ (string_of_proof prf1) *)
                  (*                       ^"\n\n\n") in *)
                  ...
                  (* (\*LDK: trace below indicate failure*\) *)
                  (* let _ = print_string ("[Debug] hhh" *)
                  (*                       ^ "\n new_rs = "^(Cprinter.string_of_list_context new_rs) *)
                  (*                       ^ "\n proof = " ^ (string_of_proof prf) *)
                  (*                       ^"\n\n\n") in *)
	        ...
	        | _ -> begin
                ...
                (* (\*LDK*\) *)
                (* let _ = print_string "heap_entail_conjunct_helper: no EXISTS in ante \n" in *)

                (* (\*LDK: without existential variables in ante*\) *)
                (* let _ = print_string "kkk \n" in *)
                (* let _ = print_string ("heap_entail_conjunct_helper:" *)
                (*                       ^ "\nconseq:\n" ^ (Cprinter.string_of_formula conseq)) in *)
		        ...
		        match conseq with
		          | Exists ({formula_exists_qvars = qvars;
			        ...
			        formula_exists_pos = pos}) ->
                      ...
                      (* (\*LDK: without existential variables in ante*\) *)
                      (* let _ = print_string "kkk, heap_entail_conjunct_helper: Exists in conseq \n" in *)
                        ...
                        (* (\*LDK*\) *)
                        (* let _ = print_string ("heap_entail_conjunct_helper:"  *)
                        (*                       ^ "\n baref = " ^ (Cprinter.string_of_formula baref) *)
                        (*                       ^ "\n\n") in *)

                        (*LDK: replace existential variables by fresh global variables*)
		                let new_baref = subst st baref in

                        (* let _ = print_string ("heap_entail_conjunct_helper:"  *)
                        (*                       ^ "\n new_baref = " ^ (Cprinter.string_of_formula new_baref) *)
                        (*                       ^ "\n\n") in *)

                        (*LDK: add new fresh varialbes to list of existential varialbes: EX-R*)
		                let new_ctx = Ctx {estate with es_evars = ws @ estate.es_evars} in
		          ...
		          | _ ->
                      ...
                      (* let _ = print_string "kkk: No Exists in conseq \n" in *)
                        ...
                        (* (\*LDK: output is ok*\) *)
                        (* let _ = print_string ("kkk, heap_entail_conjunct_helper: No Exists in conseq" *)
			            ...
			            if (isAnyConstFalse ante)&&(CF.subsume_flow_ff 1 fl2 fl1) then 
			              ...
			              if (not(CF.is_false_flow fl2.formula_flow_interval)) && not(CF.subsume_flow_ff 2 fl2 fl1) then begin
                              ...
                              (* (\*LDK: error not this way*\) *)
                              (* let _ = print_string "kkk: No Exists in conseq, 1 \n" in *)
                            ...
                            (* (\*LDK: Error going this way*\) *)
                            (* let _ = print_string "kkk: No Exists in conseq, 2 \n" in *)

			                match h2 with
			                  | HFalse (* -> (--[], UnsatConseq)  entailment fails *)
			                  | HTrue -> 

                                  (* let _ = print_string "kkk: No Exists in conseq, 2, HFalse or HTrue \n" in *)
                                  ...
                                  (* (\*LDK*\) *)
                                  (* let _ = print_string ("heap_entail_conjunct: before heap_entail_empty_rhs_heap: before remove_dupl \n") in *)
                                  ...
                                  (* (\*LDK: not really neccessary*\) *)
                                  (* let _ = print_string ("heap_entail_conjunct: before heap_entail_empty_rhs_heap: after remove_dupl \n"                                                        ^ "\n ### rhs, p2 = " ^ (Cprinter.string_of_mix_formula p2) *)
                                  (*                       ^ "\n\n") in *)
...
and heap_entail_build_mix_formula_check_a (evars : CP.spec_var list) (ante : MCP.mix_formula) (conseq : MCP.mix_formula) pos : (MCP.mix_formula * MCP.mix_formula) =
  ...
  (*let _ = print_string ("\nheap_entail_build_mix_formula_check: conseq: "^(Cprinter.string_of_mix_formula conseq)) in*)
	...
	(* let _ = print_string ("heap_entail_build_mix_formula_check:"  *)
    (*                       ^ "\n (evars from the conseq) evars = " ^ (String.concat "," (List.map Cprinter.string_of_spec_var evars)) *)
    (*                       ^ "\n (free vars of the ante) avars = " ^ (Cprinter.string_of_spec_var_list avars) *)
    (*                       ^ "\n outer_vars = " ^ (Cprinter.string_of_spec_var_list outer_vars) *)
    (*                       ^ "\n inner_vars = " ^ (Cprinter.string_of_spec_var_list inner_vars) *)
    (*                       ^ "\n ANTECEDENT = " ^ (Cprinter.string_of_mix_formula ante) *)
    (*                       ^ "\n CONSEQUENCE = " ^ (Cprinter.string_of_mix_formula conseq)   *)
    (*                       ^ "\n\n") in   *)

    (*using MCP.prune_status to prune*)
    let conseq = if !no_RHS_prop_drop then 
          conseq 
        else  
          MCP.mix_cons_filter conseq MCP.isImplT in
  let tmp1 = (*MCP.memo_pure_push_exists*) elim_exists_mix_formula inner_vars conseq no_pos in (*inner_vars are vars that do not exist in the ante*)
  let tmp11 = MCP.memo_pure_push_exists outer_vars tmp1 in

	(* let _ = print_string ("heap_entail_build_mix_formula_check: after filter conseq " *)
    (*                       ^ "\n !no_RHS_prop_drop = " ^ (string_of_bool !no_RHS_prop_drop) *)
    (*                       ^ "\n (after filer) CONSEQUENCE conseq = " ^ (Cprinter.string_of_mix_formula conseq) *)
    (*                       ^ "\n (after elim_exists) CONSEQUENCE tmp1= " ^ (Cprinter.string_of_mix_formula tmp1) *)
    (*                       ^ "\n (after push_exists) CONSEQUENCE tmp11= " ^ (Cprinter.string_of_mix_formula tmp11) *)
    (*                       ^ "\n\n") in *)
...
in
        ...
        (* (\*LDK*\) *)
        (* let _ = print_string ("heap_entail_empty_rhs_heap_x: before" *)
  ...
  (*   let _ = print_string ("After folding :  " ^  (string_of_bool r_rez) ^ "\n") in *)
    if r_rez then 
      begin (* Entailment is valid *)
          ...
          (*                       ^ "\n ### is_folding  = " ^ (string_of_bool is_folding) *)
...
and imply_mix_formula_x ante_m0 ante_m1 conseq_m imp_no memset 
  ...
  (* (\*LDK*\) *)
  (* let _ = print_string ("imply_mix_formula_x:" *)
  (*                       ^ "\n conseq_m  = " ^ (Cprinter.string_of_mix_formula conseq_m) *)
  (*                       ^ "\n\n") in *)
  ...
  match ante_m0,ante_m1,conseq_m with
    | MCP.MemoF a, MCP.MemoF a1, MCP.MemoF c ->
          ...
          (* (\*LDK*\) *)
          (* let _ = print_string ("imply_mix_formula_x: MCP.MemoF a, MCP.MemoF a1, MCP.MemoF c \n") in *)
    ...
    | MCP.OnePF a0, MCP.OnePF a1 ,MCP.OnePF c ->
          ...
          (* (\*LDK*\) *)
          (* let _ = print_string ("imply_mix_formula_x: MCP.OnePF a0, MCP.OnePF a1 ,MCP.OnePF c \n") in *)
          let (a0_l,a1_l,c_l) = ((TP.split_disjunctions a0),
                                  (TP.split_disjunctions a1),
                                  (TP.split_conjunctions c))
          in
          (* let _ = print_string ("imply_mix_formula_x: "  *)
          (*                       ^ "\n (ante0) a0_l = " ^ (Cprinter.string_of_pure_formula_list a0_l) *)
          (*                       ^ "\n (ante1) a1_l = " ^ (Cprinter.string_of_pure_formula_list a1_l) *)
          (*                       ^ "\n (conseq) c_l = " ^ (Cprinter.string_of_pure_formula_list c_l) *)
          (*                       ^ "\n\n") in *)
...
and do_base_case_unfold_only_x prog ante conseq estate lhs_node rhs_node is_folding pos rhs_b =
    (* let _ = print_string ("do_base_case_unfold_only_x: Enter" *)
    (*                       ^ "\n\n") in *)
  if (is_data lhs_node) then 
    (* let _ = print_string ("do_base_case_unfold_only_x: is_data lhs_node" *)
    (*                         ^ "\n\n") in *)
    None
  else begin
    ...
    let fold_ctx = Ctx {(empty_es (mkTrueFlow ()) pos) with es_formula = ante;
      ...
      (*                       ^ "\n ### fold_ctx = " ^ (Cprinter.string_of_context fold_ctx) *)
    ...
    let _ = Gen.Profiling.pop_time "empty_predicate_testing" in
    if (isFailCtx na) then
      (* let _ = print_string ("do_base_case_unfold_only_x: Fail" *)
      (*                       ^ "\n\n") in *)
      None
    else 
      ...
      (* let _ = print_string ("do_base_case_unfold_only_x:" *)
      (*                       ^ "\n ### cx = " ^ (Cprinter.string_of_context cx) *)
...
and do_match_x prog estate l_node r_node rhs (rhs_matched_set:CP.spec_var list) is_folding pos : 
    ...
    (* let _ = print_string ("\ndo_match: using \n" ^ *)
	(*                            (Cprinter.string_of_h_formula l_node)	^ " \nto prove\n " ^ *)
	(*                            (Cprinter.string_of_h_formula r_node)) in *)
    (* let _ = print_string ("\ndo_match: source LHS: "^ (Cprinter.string_of_entail_state estate)) in *)
    (* let _ = print_string ("\ndo_match: source RHS: "^ (Cprinter.string_of_formula rhs) ^ "\n\n")in *)
    ...
    (*               h_formula_data_arguments = l_args} *)
    (*   | ViewNode {h_formula_view_name = l_node_name; *)
    (*               h_formula_view_arguments = l_args} -> (l_args, l_node_name) *)
    (*   | _ -> report_error no_pos "[solver.ml]: do_match non view input\n" in *)
    (* let r_args, r_node_name, r_var = match r_node with *)
    ...
    *)
    ...
    (* (\*LDK*\) *)
    (* let _ = print_string ("do_match_x: after subs_to_inst_vars" *)
    ...
    *)
    ...
    (*                       ^ "\nRHS, new_conseq:" ^ (Cprinter.string_of_formula tmp_conseq) *)
    (*                       ^ "\n\n") in *)
    ...
    (* (\*LDK*\) *)
    (* let _ = print_string ("do_match:  before split_components :" *)
    (*                       ^ "\n r_subs = "^ (Cprinter.string_of_spec_var_list r_subs) *)
    (*                       ^ "\n l_sub = "^ (Cprinter.string_of_spec_var_list l_sub) *)
    (*                       ^ "\nLHS, new_ante = "^ (Cprinter.string_of_formula new_ante) *)
    ...
    *)
    ...
    (*                       ^ "\nLHS, new_ctx = "^ (Cprinter.string_of_context new_ctx) *)
    ...
    let res_es1, prf1 = (*heap_entail_split_rhs_phases*) heap_entail_conjunct prog is_folding  new_ctx new_conseq
      ...
      (* (\*LDK*\) *)
      (* let _ = print_string ("\n do_match: after all, before add_to_subst" *)
      (*                       ^ "\n res_es1 = "^(Cprinter.string_of_list_context res_es1) *)
      (*                       ^ "\n  prf1 = " ^ (string_of_proof  prf1) *)
      (*                       ^ "\n  r_subs = " ^ (Cprinter.string_of_spec_var_list  r_subs) *)
      (*                       ^ "\n  l_sub = " ^ (Cprinter.string_of_spec_var_list  l_sub) *)
      (*                       ^"\n\n\n") in *)
...
and heap_entail_non_empty_rhs_heap_x prog is_folding  ctx0 estate ante conseq lhs_b rhs_b (rhs_h_matched_set:CP.spec_var list) pos : (list_context * proof) =
  ...
  (* let _ = print_string ("\n heap_entail_non_empty_rhs_heap_x: before compute_action \n") in *)
  ...
  (* let _ = print_string ("\n heap_entail_non_empty_rhs_heap_x: before process_action \n") in *)
...
and do_fold_w_ctx_x fold_ctx prog estate conseq ln2 vd resth2 rhs_b is_folding pos = 
  ...
  (* let fold_rs, fold_prf = fold_op prog fold_ctx view_to_fold vd (\* false *\) use_case pos in *)
      ...
      (*       (\*LDK: trace below indicate failure*\) *)
      (* let _ = print_string ("do_fold_w_ctx_x: before  process_fold_result" *)
      (*                       ^ "\n ln2 = "^(Cprinter.string_of_h_formula ln2) *)
  ...
  if not (CF.isFailCtx fold_rs) then
    ...
    (*   (\*LDK:*\) *)
    (* let _ = print_string ("\ndo_fold_w_ctx_x: after process_fold_result" *)
    (*                       ^ "\n ln2 = "^(Cprinter.string_of_h_formula ln2) *)
    (*                       ^ "\n conseq = "^(Cprinter.string_of_formula conseq) *)
    (*                       ^ "\n tmp = "^(Cprinter.string_of_list_context tmp) *)
...
and comp_act_x prog (estate:entail_state) (rhs:formula) : (Context.action_wt) =
  ...
  let posib_r_alias = (estate.es_evars @ estate.es_gen_impl_vars @ estate.es_gen_expl_vars) in
  let rhs_eqset = estate.es_rhs_eqset in
   (0,Context.compute_actions prog rhs_eqset lhs_h lhs_p rhs_p posib_r_alias rhs_lst estate.es_is_normalizing no_pos)
...
and process_action_x prog estate conseq lhs_b rhs_b a (rhs_h_matched_set:CP.spec_var list) is_folding pos = 
  ...
  (*add tracing into the entailment state*)
	...
	(* let _ = print_string ("process_action: after adding into es_trace" *)
    (*                       ^ "\n ### estate = " ^ ( Cprinter.string_of_entail_state estate) *)
    (*                       ^ "\n\n") in *)
  ...
  let r1,r2 = match a with
    | Context.M_match {
        ...
        (* (\*LDK*\) *)
        (* let _ = print_string "process_action_x: M_match \n" in *)
    ...
    | Context.M_lemma  ({
      ...
      (* (\*LDK: trace below indicate failure*\) *)
      (*     let r22=Search r2 in *)
      (*     let _ = print_string ("\n [Debug] zzz After do_coercion" *)
      (*                           ^ "\n r1 = "^ (Cprinter.string_of_list_context r1) *)
      (*                           (\* ^ "\n r2 = " ^ (List.fold_right (fun prf str -> (string_of_proof prf) ^ str ^ "\n") r2 "") *\) *)
      (*                           ^ "\n r22 = " ^ (string_of_proof r22) *)
      (*                           ^"\n\n\n") in *)
...
and do_universal_x prog estate (node:CF.h_formula) rest_of_lhs coer anode lhs_b rhs_b conseq is_folding pos: (list_context * proof) =
  begin
    ...
    (* let lhs_heap , _, _, _ , _  = split_components coer_lhs in *)
	      ...
	      *)
	      ...
	      then
            ...
            (* let _ = print_string "do_universal: 2 \n" in *)
              ...
              (* let rest_of_lhs = CF.add_mix_formula_to_formula frac_to_lhs rest_of_lhs  in *)
              ...
              (* (\*LDK: add fractional permission constraint to the ante*\) *)
              (* let l_h,l_p,l_fl,l_b,l_t = split_components estate.es_formula in *)
              (* let new_ante_p = (MCP.memoise_add_pure_N l_p to_lhs ) in *)
              (* let new_ante = mkBase l_h new_ante_p l_t l_fl l_b pos in *)
...
(* rewrite_coercion *)
(*******************************************************************************************************************************************************************************************)
and rewrite_coercion prog ctx estate node f coer lhs_b rhs_b target_b weaken pos : (bool * formula) =
  ...
  Gen.Debug.no_4 "rewrite_coercion" Cprinter.string_of_h_formula  p1 Cprinter.string_of_coercion Cprinter.string_of_entail_state
      p2 (fun _ _ _ _ -> rewrite_coercion_x prog ctx estate node f coer lhs_b rhs_b target_b weaken pos) node f coer estate

(*LDK: return the a new formula (new_f) after apply coercion into f*)
and rewrite_coercion_x prog ctx estate node f coer lhs_b rhs_b target_b weaken pos : (bool * formula) =
...
begin
          ...
          (*     in *)
	        ...
	        (*************************************************************)
              ...
              (* (\*LDK: this way*\) *)
              (* let _ = print_string "www, I am coming \n" in *)
              

	        let apply_coer = (coer_target prog coer node (CF.formula_of_base target_b (* rhs_b *)) (CF.formula_of_base lhs_b)) in

              (* let _ = print_string ("www, apply_coer= \n"^ (string_of_bool apply_coer)) in (\*LDK: true*\) *)
	        ...
	        *)
            ...
            let flag = if !Globals.allow_imm then false else not(apply_coer) in
              ...
              (* let _ = print_string ("www, flag= \n"^ (string_of_bool flag)) in (\*LDK: false*\) *)
	        ...
	        then
              ...
              (* let _ = print_string ("rewrite_coercion: before subst_avoid_capture" *)
              (*                       ^ "\n ### coer_rhs = " ^ (Cprinter.string_of_formula coer_rhs) *)
              (*                       ^ "\n\n") in *)

                (*LDK: not CORRECT at the moment*)
		        (* let coer_rhs_new1 = match frac1,frac2 with *)
                (*   | Some f1, Some f2 ->  *)
                (*       if (is_propagated) then *)
                (*         subst_avoid_capture (p2 :: ps2) (p1 :: ps1) coer_rhs *)
              ...
              (* let _ = print_string ("rewrite_coercion: before add_origins" *)
              (*                       ^ "\n ### coer_rhs_new1 = " ^ (Cprinter.string_of_formula coer_rhs_new1) *)
              (*                       ^ "\n\n") in *)
              ...
              (* let _ = print_string ("rewrite_coercion: after add_origins" *)
              (*                       ^ "\n ### coer_rhs_new = " ^ (Cprinter.string_of_formula coer_rhs_new) *)
              (*                       ^ "\n\n") in *)
              ...
              (*                       ^ "\n\n") in *)
                ...
                (*LDK:
                  create a binding between frac1 and frac2.
                  If RHS frac var f1 is an existential var -> keep the binding at RHS
                  if.... an implicit var -> instantiate to LHS
                  ???
                *)
              ...
              (*                       ^ "\n ### new_f = " ^ (Cprinter.string_of_formula f) *)
              (*                       ^ "\n\n") in *)
...
(* rewrite_coercion prog ctx0 estate anode f coer lhs_b rhs_b rhs_b true pos in *)
(* rewrite_coercion prog ctx0 estate anode f coer lhs_b rhs_b rhs_b true pos in *)
	    ...
	    then
          ...
          let rec process_one_x (ss:CF.steps) res = match res with
            ...
            | Ctx es ->
	            ...
	            Debug.devel_pprint ("apply_left_coercion_complex: process_one: resume entail check: conseq = " ^ (Cprinter.string_of_formula conseq)) pos;


	            let rest_rs, prf = heap_entail_one_context prog is_folding new_ctx conseq pos in


	            Debug.devel_pprint ("apply_left_coercion_complex: process_one: after resume entail check: rest_res =  " ^ (Cprinter.string_of_list_context check_res)) pos;
          ...
          let process_one (ss:CF.steps) res = 
            let pr1 = Cprinter.string_of_context  in
            let pr2 (c,_) = Cprinter.string_of_list_context c in
            Gen.Debug.no_1 "apply_left_coercion_complex: process_one" pr1 pr2 (fun _ -> process_one_x (ss:CF.steps) res) res in
...
and do_coercion_x prog c_opt estate conseq resth1 resth2 anode lhs_b rhs_b ln2 is_folding pos : (CF.list_context * proof list) =
  ...
  (* let _ = print_string "Checkpoint 1" in (\*LDK*\) *)
  ...
  (* (\*LDK*\) *)
  (* let _ = print_string ("do_coercion_x: yyy" *)
  (*                       ^ "\n coers1 = " ^ (Cprinter.string_of_coerc_decl_list coers1) *)
  (*                       ^ "\n coers2 = " ^ (Cprinter.string_of_coerc_decl_list coers2) *)
  (*                       ^ "\n univ_coers = " ^ (Cprinter.string_of_coerc_decl_list univ_coers) *)
  (*                       ^ "\n\n") in *)
  ...
  then (CF.mkFailCtx_in(Trivial_Reason "no lemma found in both LHS and RHS nodes (do coercion)"), [])
    ...
    let left_r = 
      if (List.length coers1)>0 then
        ...
        (* (\*LDK: trace show fail context*\) *)
        (* let _ = print_string ("nnn, left_res = " ^ (Cprinter.string_of_list_context_list left_res) ^ "\n") in *)
...
and apply_right_coercion_a estate coer prog (conseq:CF.formula) ctx0 resth2 ln2 (*rhs_p rhs_t rhs_fl*) lhs_b rhs_b (c2:ident) is_folding pos =
  ...
  let ok, new_rhs = rewrite_coercion prog ctx0 estate ln2 f coer lhs_b rhs_b lhs_b false pos in

  (* let _ = print_string ( "apply_right_coercion: after rewrite_coercion"  *)
  (*                        ^ "\n ### ok = "^ (string_of_bool ok) *)
