...
and fold_op_x prog (ctx : context) (view : h_formula) vd (rhs_p: MCP.mix_formula) (use_case:bool) (pos : loc): (list_context * proof) =
  ...
  Gen.Debug.no_2 "fold_op" 
      pr2 id pr
      (fun _ _ -> fold_op_x1  prog (ctx : context) (view : h_formula) vd rhs_p (use_case:bool) (pos : loc)) vd ans
...
in
        ...
        (*                        ^ ("\n ### (args) vsf = " ^ (Cprinter.string_of_spec_var_list vsf)) *)
...
and heap_entail_empty_rhs_heap_x (prog : prog_decl) (is_folding : bool)  estate lhs (rhs_p:MCP.mix_formula) rhs_p_br pos : (list_context * proof) =
  ...
  let fold_fun (is_ok,succs,fails, (fc_kind,(contra_list, must_list, may_list))) ((branch_id, rhs_p):string*MCP.mix_formula) =
	...
	(* "NEW CONSEQUENCE = " ^ (Cprinter.string_of_mix_formula new_conseq0)  ^ "\n") in *)
        ...
        (*LDK*)
        (* let _ = print_string ("heap_entail_empty_rhs_heap_x: before Globals.omega_simpl" *)
        ...
        (*LDK*)
        (* let _ = print_string ("\nheap_entail_empty_rhs_heap_x: before" *)
        (*                       ^ "\n new_conseq0 = " ^ (Cprinter.string_of_mix_formula new_conseq0) *)
        (*                       ^ "\n\n") in *)
        ...
        (*LDK*)
        (* let _ = print_string ("\nheap_entail_empty_rhs_heap_x: before imply_mix_formula" *)
...
and do_fold_w_ctx_x fold_ctx prog estate conseq ln2 vd resth2 rhs_b is_folding pos = 
  ...
  *)
  ...
  let fold_rs, fold_prf = fold_op prog (Ctx estate) view_to_fold vd rhs_p (* false *) use_case pos in
...
(* rewrite_coercion *)
(*******************************************************************************************************************************************************************************************)
and rewrite_coercion prog ctx estate node f coer lhs_b rhs_b target_b weaken pos : (int * formula) =
...
and rewrite_coercion_x prog ctx estate node f coer lhs_b rhs_b target_b weaken pos : (int * formula) =
