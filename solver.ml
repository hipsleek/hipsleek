...
and process_fold_result_x prog is_folding estate (fold_rs0:list_context) p2 vs2 base2 pos : (list_context * proof list) =
  ...
  let rec process_one_x (ss:CF.steps) fold_rs1 = match fold_rs1 with
    ...
    | Ctx fold_es ->
	      ...
	      let tmp_conseq = mkBase resth2 pure2 type2 flow2 perm2 branches2 pos in
...
and unsat_base_x prog (sat_subno:  int ref) f  : bool= 
  match f with
    ...
    | Base ({ formula_base_heap = h;
	  ...
	  formula_base_branches = br;
	  formula_base_pos = pos}) ->
          is_unsat_with_branches (fun f-> let a,b,_,_ = xpure_heap 1 prog f 1 in (a,b)) [] h p pr br pos sat_subno
    | Exists ({ formula_exists_qvars = qvars;
      formula_exists_heap = qh;
      formula_exists_pure = qp;
	  formula_exists_perm = pr;
      formula_exists_branches = br;
      formula_exists_pos = pos}) ->
          is_unsat_with_branches (fun f-> let a,b,_,_ = xpure_heap 1 prog f 1 in (a,b)) qvars qh qp pr br pos sat_subno
			              ...
			              if (not(CF.is_false_flow fl2.formula_flow_interval)) && not(CF.subsume_flow_ff fl2 fl1) then begin
			                ...
			                match h2 with
			                  ...
			                  | HTrue -> begin
				                  ...
				                  (*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*)
				                  let ctx, proof = 
									if (Cperm.imply pr1 pr2) then heap_entail_empty_rhs_heap prog is_folding  estate b1 p2 br2 pos 
...
and do_match_x prog estate l_node r_node rhs is_folding pos : 
  ...
  Debug.devel_pprint ("do_match: using " ^
    ...
    let new_ante = mkBase l_h new_ante_p l_t l_fl l_pr (CP.merge_branches l_b to_lhs_br) pos in
    let tmp_conseq = mkBase r_h new_conseq_p r_t r_fl r_pr (CP.merge_branches r_b to_rhs_br) pos  in
    ...
    let new_conseq = mkBase tmp_h2 tmp_p2 r_t r_fl r_pr tmp_b2 pos in
