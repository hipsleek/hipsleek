...
type find_node_result =
  ...
  | Match of ((*Context.context*) (h_formula * h_formula * Context.match_type * Context.phase_type option * h_formula * int) list) (* found p1::c1<V1> such that p1=p2 *)
...
(*---------------*)
let rec xpure (prog : prog_decl) (f0 : formula) : (MCP.mix_formula * (branch_label * CP.formula) list * CF.mem_formula) = 
  ...
  let rec xpure_helper  (prog : prog_decl) (f0 : formula) : (MCP.mix_formula * (branch_label * CP.formula) list) = 
    match f0 with
      | Or ({ formula_or_f1 = f1;
	      formula_or_f2 = f2;
	      formula_or_pos = pos}) ->
          let pf1, pf1b, _ = xpure prog f1 in
          let pf2, pf2b, _ = xpure prog f2 in
      ...
      | Base ({ formula_base_heap = h;
		...
		formula_base_branches = br;
		formula_base_pos = pos}) ->
          let (ph, phb, _) = xpure_heap prog h 1 in
      ...
      | Exists ({ formula_exists_qvars = qvars;
		  ...
		  formula_exists_branches = br;
		  formula_exists_pos = pos}) ->
          let (pqh, pqhb, _) = xpure_heap prog qh 1 in
...
and xpure_heap (prog : prog_decl) (h0 : h_formula) (use_xpure0 :int) : (MCP.mix_formula * (branch_label * CP.formula) list * CF.mem_formula) = 
  (* let _ = print_string("Calling xpure_heap for f = " ^ (Cprinter.string_of_h_formula h0) ^ "\n") in *)
  let memset = h_formula_2_mem h0 [] prog in

  let rec xpure_heap_helper (prog : prog_decl) (h0 : h_formula) (use_xpure0 :int) : (MCP.mix_formula * (branch_label * CP.formula) list) = 
    match h0 with
      | DataNode ({h_formula_data_node = p;
      ...
      | Conj ({h_formula_conj_h1 = h1;
	       h_formula_conj_h2 = h2;
	       h_formula_conj_pos = pos}) ->
          let (ph1, ph1b, _) = xpure_heap prog h1 use_xpure0 in
          let (ph2, ph2b, _) = xpure_heap prog h2 use_xpure0 in
      ...
      | Hole _ -> failwith "[solver.ml]: An immutability marker was encountered in the formula\n"   
  in
  let ph, pb = xpure_heap_helper prog h0 use_xpure0 in
    (ph, pb, memset)
...
and xpure_consumed_pre_heap (prog : prog_decl) (h0 : h_formula) : (CP.formula * (branch_label * CP.formula) list) = match h0 with
  ...
  | Hole _ -> failwith "[solver.ml]: Immutability annotation encountered\n"
...
and prune_preds prog (simp_b:bool) (f:formula):formula =   
  (* only pure checks - no need for the memset *)
                   ...
                   if ((List.length (Util.intersect pr_branches rem_br))=0) then (yes_prune, no_prune,new_mem)
                     ...
                     let corr = MCP.memo_find_relevant_slice fv new_mem in
                       if not (MCP.memo_changed corr) then (yes_prune,(p_cond, pr_branches)::no_prune,new_mem)
                         ...
                         let y_p = (Util.inc_counter "syn_memo_count";
...
and split_linear_node_debug h phase = Util.ho_debug_2 "split_linear_node"
(Cprinter.string_of_h_formula)
(fun _ -> "?")
(fun ((a,b),c) -> (Cprinter.string_of_h_formula a) ^ " and " ^ (Cprinter.string_of_h_formula c))
split_linear_node h phase

and split_linear_node (h : h_formula) (phase : Context.phase_type option) : ((h_formula * Context.phase_type option) * h_formula) = 
  match h with
    | HTrue -> ((HTrue, phase), HTrue)
    | HFalse -> ((HFalse, phase), HFalse)
    | Hole _ -> failwith "Immutability annotation encountered\n"   
    | DataNode _ | ViewNode _ -> ((h, phase), HTrue)
    | Star ({h_formula_star_h1 = h1;
	     h_formula_star_h2 = h2;
	     h_formula_star_pos = pos}) ->
	begin
	  match h1 with
	    | HTrue -> print_string ("\n\n!!!This shouldn't happen!!!\n\n"); split_linear_node h2 (Some Context.Spatial) (* this shouldn't happen anyway *)
	    | _ ->
		let (ln1, c1), r1 = split_linear_node h1 (Some Context.Spatial) in
	          ((ln1, c1), mkStarH r1 h2 pos)
	end
    | Phase ({h_formula_phase_rd = h1;
	      h_formula_phase_rw = h2;
	      h_formula_phase_pos = pos}) ->
	begin
	  match h1 with
	    | HTrue -> print_string ("\n\n!!!This shouldn't happen!!!\n\n"); split_linear_node h2 (Some Context.Classic) (* this shouldn't happen anyway *)
	    | _ ->
		let (ln1, c1), r1 = split_linear_node h1 (Some Context.Classic) in
	          ((ln1, c1), mkPhaseH r1 h2 pos)
	end
    | Conj ({h_formula_conj_h1 = h1;
	     h_formula_conj_h2 = h2;
	     h_formula_conj_pos = pos}) -> 
	begin
	  match h1 with
	    ...
	    | _ ->
		let (ln1, c1), r1 = split_linear_node h1 (Some Context.Classic) in
	          ((ln1, c1), mkConjH r1 h2 pos)
	end

and split_linear_node_guided_debug v h phase = Util.ho_debug_3 "split_linear_node_guided"
(fun _ -> "?")
(Cprinter.string_of_h_formula)
(fun _ -> "?")
(fun ((a,b),c) -> (Cprinter.string_of_h_formula a) ^ " and " ^ (Cprinter.string_of_h_formula c))
split_linear_node_guided v h phase

	  
and split_linear_node_guided (vars : CP.spec_var list) (h : h_formula) (phase : Context.phase_type option) : ((h_formula * Context.phase_type option) * h_formula) = 
  (*  let _ = print_string("in split_linear_node_guided with h = " ^ (Cprinter.string_of_h_formula h) ^ "\n") in
      let _ = print_string("in split_linear_node_guided with input phase = " ^ (string_of_phase phase) ^ "\n") in*)
  match h with
    | HTrue -> ((HTrue, phase), HTrue)
    | HFalse -> ((HFalse, phase), HFalse)
    | Hole _ -> failwith "[solver.ml]: Immutability annotation encountered\n"	
	...
	(*let conj = 
	(*if imm then Context.Classic else*) conj 
	  in*)
	if (List.exists (CP.eq_spec_var root) vars) then ((h, phase), HTrue)
	else ((HTrue, phase),h)
    | Conj ({h_formula_conj_h1 = h1;
	     h_formula_conj_h2 = h2;
	     h_formula_conj_pos = pos}) ->
	begin
	  match h1 with
	    | HTrue -> print_string ("\n\n!!!This shouldn't happen!!!\n\n"); split_linear_node_guided vars h2 (Some Context.Classic) (* this shouldn't happen anyway *)
	    | _ ->
		let (ln1, c1), r1 = split_linear_node_guided vars h1 (Some Context.Classic) in
		  match ln1 with
		    | HTrue -> let (ln2, c2), r2 = split_linear_node_guided vars h2 (Some Context.Classic) in
			((ln2, c2), mkConjH h1 r2 pos)
		    | _     ->  ((ln1, c1), mkConjH r1 h2 pos)
	end
    | Phase ({h_formula_phase_rd = h1;
	      h_formula_phase_rw = h2;
	      h_formula_phase_pos = pos}) ->
	begin
	  match h1 with
		...
		let (ln1, c1), r1 = split_linear_node_guided vars h1 (Some Context.Classic) in
		  match ln1 with
		    | HTrue -> let (ln2, c2), r2 = split_linear_node_guided vars h2 (Some Context.Classic) in
			((ln2, c2), mkPhaseH h1 r2 pos)
		    | _     ->  ((ln1, c1), mkPhaseH r1 h2 pos)
	end
	...
	begin
	  match h1 with
	    | HTrue -> print_string ("\n\n!!!This shouldn't happen!!!\n\n"); split_linear_node_guided vars h2 (Some Context.Spatial) (* this shouldn't happen anyway *)
	    | _ ->
		let (ln1, c1), r1 = split_linear_node_guided vars h1 (Some Context.Spatial) in
		  match ln1 with
		    | HTrue -> let (ln2, c2), r2 = split_linear_node_guided vars h2 (Some Context.Spatial) in
			((ln2,c2), mkStarH h1 r2 pos)
		    | _     ->  ((ln1,c1), mkStarH r1 h2 pos)
	end
...
and find_node prog lhs_h (lhs_p : MCP.mix_formula) (ps : CP.spec_var list) pos : find_node_result =
  ...
  let tmp1 = List.map (fun p -> find_node_one prog lhs_h lhs_p p true (* todo: change the true!! *) None(*--(Some Context.Classic)*) None pos) ps in
...
and find_node_one prog lhs_h lhs_p (p : CP.spec_var) (imm : bool) (phase : Context.phase_type option) rhs_info pos : find_node_result =
  ...
  let matches = Context.choose_context prog lhs_h lhs_p p imm phase rhs_info pos in 
    (*=======
...
and elim_exists_pure_branch_debug w f0 p = 
  Util.ho_debug_2 "elim_exists_pure_branch" Cprinter.string_of_spec_var_list Cprinter.string_of_pure_formula Cprinter.string_of_pure_formula 
    (fun w f0 -> elim_exists_pure_branch w f0 p) w f0
...
and unsat_base prog (sat_subno:  int ref) f  : bool= 
  match f with
    ...
    | Base ({ formula_base_heap = h;
	      ...
	      formula_base_branches = br;
	      formula_base_pos = pos}) ->
        is_unsat_with_branches (fun f-> let a,b,_ = xpure_heap prog f 1 in (a,b)) [] h p br pos sat_subno
    | Exists ({ formula_exists_qvars = qvars;
                formula_exists_heap = qh;
                formula_exists_pure = qp;
                formula_exists_branches = br;
                formula_exists_pos = pos}) ->
        is_unsat_with_branches (fun f-> let a,b,_ = xpure_heap prog f 1 in (a,b)) qvars qh qp br pos sat_subno
...
and elim_unsat_es (prog : prog_decl) (sat_subno:  int ref) (es : entail_state) : context =
  if (es.es_unsat_flag) then Ctx es
    ...
    let _ = reset_int2 () in
      (* <<<<<<< solver.ml *)
      (*     let (pf, pfb, memset) = xpure prog f in *)
...
and heap_entail_conjunct_lhs prog is_folding is_universal (ctx:context) conseq pos : (list_context * proof) = match conseq with
  ...
  | _ -> begin
      ...
      let r1,p1 = heap_entail_split_phases prog is_folding is_universal ctx conseq pos in      
	(r1,p1)
    end
...
and check_one_target prog (target : CP.spec_var) (lhs_pure : MCP.mix_formula) (rhs_p : MCP.mix_formula) (rhs_h : CF.h_formula) (coer_rhs_h : CF.h_formula)
  ...
  let fnode_results = (find_node prog rhs_h rhs_p lhs_targetasets no_pos) in
	...
	| Match (matches) ->
	    begin
	      let _ = print_string("No no no\n") in
	      match matches with
		  (* before:	      | (anode, r_flag, _, resth1, new_ctx) :: rest ->*)
		| (resth1, anode, r_flag, phase, ctx, hole_id) :: rest ->	  
		    begin
...
and get_wr_phase (h : h_formula) : h_formula = 
  match h with
      | Star ({h_formula_star_h1 = h1;
		h_formula_star_h2 = h2;
		h_formula_star_pos = pos}) -> 
	  (match h2 with
		...
		get_wr_phase (CF.mkStarH (CF.mkStarH h1 sh1 pos) sh2 pos)
	    | _ -> h)
      | Conj _ -> failwith ("[solver.ml] : Conjunction should not appear at this level \n")
      | Phase({h_formula_phase_rd = h1;
	       h_formula_phase_rw = h2;
	       h_formula_phase_pos = pos}) ->
	  get_wr_phase h2
...
and get_next_phase (h : h_formula) : h_formula = 
  match h with
      | Star ({h_formula_star_h1 = h1;
		h_formula_star_h2 = h2;
		h_formula_star_pos = pos}) -> 
	  (match h2 with
	    ...
	    | Star ({h_formula_star_h1 = sh1;
		    h_formula_star_h2 = sh2;
		    h_formula_star_pos = spos}) ->
		get_next_phase (CF.mkStarH (CF.mkStarH h1 sh1 pos) sh2 pos)
	    | _ -> HTrue)
      | Conj _ -> failwith ("[solver.ml] : Conjunction should not appear at this level \n")
      | Phase({h_formula_phase_rd = h1;
	       h_formula_phase_rw = h2;
...
(*
  heap_entail_lhs_conjunct
  D |-I KR * R,hm  <-----
  //drop_read_phase(subs_hole(R,hm)) |- KW * (R1,hm1)
  subs_hole(R,hm) |- KW * (R1,hm1)
  R1 |- K * (R2,hm2)
  --------------------------------------
  D |- (KR ; KW * K) * subs_hole(R2,hm1@hm2)
*)


and heap_entail_split_phases 
    (prog : prog_decl) 
    (is_folding : bool) 
    (is_universal : bool)
    (ctx0 : context) 
  ...
  (* let _ = print_string("in split phases\n") in *)
  match conseq with 
    | Base(bf) -> 
	let (h, p, fl, b, t) = CF.split_components conseq in
	  (match h with
		...
		(* let _ = print_string("first phase = " ^ (Cprinter.string_of_h_formula h1) ^ "\n") in *)
		(* let _ = print_string("B4 rd heap = " ^ (Cprinter.string_of_context ctx0) ^ "\n") in *)
		let new_conseq = CF.mkBase h1 (MCP.mkMTrue pos) t fl b pos in
		(* let _ = print_string("new_conseq = " ^ (Cprinter.string_of_formula new_conseq) ^ "\n") in *)
		  
		(* let ctx0 = Context.update_ctx_es_orig_conseq ctx0 (CF.struc_formula_of_formula new_conseq pos) in *)
		let (after_rd_ctx, prf) = heap_entail_conjunct prog is_folding is_universal ctx0 new_conseq pos in
		  (match after_rd_ctx with
                     |FailCtx _ -> (after_rd_ctx, prf)
		     | SuccCtx (cl) -> 
			 (* let _ = print_string("After rd heap = " ^ (Cprinter.string_of_context (List.hd cl)) ^ "\n") in *)
			 (* let _ = print_string("h2 = " ^ (Cprinter.string_of_h_formula h2) ^ "\n") in *)
			 (* let _ = print_string("wr phase = " ^ (Cprinter.string_of_h_formula (get_wr_phase h2)) ^ "\n") in *)
			 (* input the frame and entail the write phase *)
			 let after_wr = List.map (fun c -> heap_entail_conjunct prog is_folding is_universal (Context.input_ctx_frame c) (CF.mkBase (get_wr_phase h2) (MCP.mkMTrue pos) t fl b pos) pos) cl in
			 ...
			 (match after_wr_ctx with
			    ...
			    | SuccCtx (cl) -> 
				(* entail the rest of the conseq - also add the pure part  *)
				(* let _ = print_string("Entail the rest of the conseq = " ^ (Cprinter.string_of_context (List.hd cl)) ^ "\n") in *)
				(* let _ = print_string("h2 = " ^ (Cprinter.string_of_h_formula h2) ^ "\n") in *)
				(* let _ = print_string("next phase = " ^ (Cprinter.string_of_h_formula (get_next_phase h2)) ^ "\n") in *)
				let (ctx, pr) =  heap_entail prog is_folding is_universal (Context.input_list_ctx_frame after_wr_ctx) (CF.mkBase (get_next_phase h2) p t fl b pos) pos
	   in ((Context.input_list_ctx_frame ctx), prf)))
	    | Conj _ -> failwith ("[solver.ml] : No conj should reach this point\n")
	    | _ -> let (ctx, prf) = heap_entail_conjunct prog is_folding is_universal (Context.input_ctx_frame ctx0) conseq pos
                      in  ((Context.input_list_ctx_frame ctx), prf))
    ...
    | Exists({formula_exists_qvars = qvars;
	  ...
	  (match h with
	    | Phase ({h_formula_phase_rd = h1;
		      ...
		      h_formula_phase_pos = pos}) ->
		(* entail the read phase heap *)
		let (after_rd_ctx, prf) = heap_entail_conjunct prog is_folding is_universal ctx0 (CF.mkExists qvars h1 (MCP.mkMTrue pos) t fl b pos) pos in
		  (match after_rd_ctx with
                     |FailCtx _ -> (after_rd_ctx, prf)
		     | SuccCtx (cl) -> 
			 (* 	let _ = print_string("h2 = " ^ (Cprinter.string_of_h_formula h2) ^ "\n") in *)
			 ...
			 (* input the frame and entail the write phase *)
			 let after_wr = List.map (fun c -> heap_entail_conjunct prog is_folding is_universal (Context.input_ctx_frame c) (CF.mkExists qvars (get_wr_phase h2) (MCP.mkMTrue pos) t fl b pos) pos) cl in
			 let after_wr_ctx, after_wr_prfs = List.split after_wr in
			 let after_wr_prfs = mkContextList cl (Cformula.struc_formula_of_formula conseq pos) after_wr_prfs in
			 let after_wr_ctx = fold_context_left after_wr_ctx in 
			   (match after_wr_ctx with
				...
				let (ctx, pr) =  heap_entail prog is_folding is_universal (Context.input_list_ctx_frame after_wr_ctx) (CF.mkExists qvars (get_next_phase h2) p t fl b pos) pos
	   in ((Context.input_list_ctx_frame ctx), prf)))
 
(*				  let (ctx, prf) =  heap_entail prog is_folding is_universal (Context.input_list_ctx_frame after_wr_ctx) (CF.mkExists qvars (get_next_phase h2) p t fl b pos) pos in
	    	   in ((Context.input_list_ctx_frame ctx), prf)))*)
	    | Conj _ -> failwith ("[solver.ml] : No RHS conj should reach this point\n")
	    | _ -> let (ctx, prf) = heap_entail_conjunct prog is_folding is_universal (Context.input_ctx_frame ctx0) conseq pos
                      in  ((Context.input_list_ctx_frame ctx), prf))
    | _ -> failwith ("solver.ml: No RHS disj should reach this point\n  ")
...
and heap_entail_build_mix_formula_check (evars : CP.spec_var list) (ante : MCP.mix_formula) (conseq : MCP.mix_formula) pos : (MCP.mix_formula * MCP.mix_formula) =
  ...
  let conseq = MCP.mix_cons_filter conseq MCP.isImplT in
  let tmp1 = (*MCP.memo_pure_push_exists*) elim_exists_mix_formula inner_vars conseq no_pos in
    (ante,tmp1)
      
and heap_entail_build_mix_formula_check_debug ev an cq pos =
  Util.ho_debug_1 "heap_entail_build_mix_formula_check" Cprinter.string_of_mix_formula (fun (_,f) -> Cprinter.string_of_mix_formula f)
    (fun cq -> heap_entail_build_mix_formula_check ev an cq pos) cq
...
and xpure_imply (prog : prog_decl) (is_folding : bool) (is_universal : bool)  lhs rhs_p timeout : bool = 
  ...
  let _ = reset_int2 () in
  let xpure_lhs_h, xpure_lhs_h_b, memset = xpure_heap prog (mkStarH lhs_h estate.es_heap pos) 1 in
...
and heap_entail_empty_rhs_heap (prog : prog_decl) (is_folding : bool) (is_universal : bool) estate lhs (rhs_p:MCP.mix_formula) rhs_p_br pos : (list_context * proof) =
  ...
  let _ = reset_int2 () in
  let xpure_lhs_h0, xpure_lhs_h0_b, memset = xpure_heap prog (mkStarH lhs_h estate.es_heap pos) 0 in
  let xpure_lhs_h1, xpure_lhs_h1_b, memset = xpure_heap prog (mkStarH lhs_h estate.es_heap pos) 1 in
...
and imply_formula_no_memo new_ante new_conseq imp_no memset =   
  ...
  let res,_,_ = TP.imply new_ante new_conseq ((string_of_int imp_no)) false in
...
and do_match_debug prog estate l_args r_args l_node_name r_node_name l_node r_node rhs is_folding is_universal r_var pos =
  Util.ho_debug_3 "do_match"
...
*)
    ...
    (fun estate l_args r_args -> do_match prog estate l_args r_args l_node_name r_node_name l_node r_node rhs is_folding is_universal r_var pos)
     estate l_args r_args
...
and do_match prog estate l_args r_args l_node_name r_node_name l_node r_node rhs is_folding is_universal r_var pos : 
  ...
  let new_ctx = Ctx (CF.add_to_estate new_es "matching of view/node") in
    ...
    Debug.devel_pprint ("do_match: "^ "new_conseq after matching:\n"
			^ (Cprinter.string_of_formula new_conseq)) pos;
    let res_es1, prf1 = heap_entail_split_phases prog is_folding is_universal new_ctx new_conseq pos in
      (res_es1,prf1)
...
and heap_entail_non_empty_rhs_heap prog is_folding is_universal ctx0 estate ante conseq lhs_b rhs_b pos : (list_context * proof) =
  ...
  (* let _ = print_string("non_empty_rhs: estate = " ^ (Cprinter.string_of_estate estate) ^ "\n") in *)


  let (ln2, phase2), resth2 = split_linear_node_guided (Util.remove_dups (h_fv lhs_h @ MCP.mfv lhs_p)) rhs_h None in
  let (ln2, phase2), resth2 = if (Cformula.is_complex_heap ln2) then ((ln2, phase2), resth2)
  else split_linear_node rhs_h None in
    ...
    match ln2 with
	  ...
	  *)
	  ...
	  let fnode_results = find_node_one prog lhs_h lhs_p p2 imm2 phase2 (Some (rhs_p,posib_r_alias)) pos in

	    (************************* match_all_nodes ******************)
	    match fnode_results with 
	      | Failed -> 
		  let _ = print_string("failed\n") in
		  (CF.mkFailCtx_in (Basic_Reason ( {
		  ...
		  (*******************************************************************************************************************************************************************************************)
		    ...
		    Util.ho_debug_2 "check_aliased_node"
		      ...
		      (a,r) resth1
		   (*in		      
		  let*) and check_aliased_node (anode, r_flag) resth1 =
		  (* *)
		    match anode with 
		      | ViewNode ({ h_formula_view_node = p1;
				    h_formula_view_name = c1;
			  ...
			  if r_flag = Context.Root then begin (* matching occurs at root *)
				...
				if not subsumes then 
				  ...
				  (    let ans = do_base_case_unfold prog ante conseq estate c1 c2 v1 v2 p1 p2 ln2 is_folding 
					 ...
					 match ans with 
					   | Some x, _ -> let _ = print_string("some of base case unfolding\n") in x
					   | None, y->  
					       let new_estate = 
						 match y with 
						   | Some x-> let _ = print_string("base case unfolding\n") in x 
						 ...
						 if (!Globals.exhaust_match) then 
						   let n_rhs = return_base_cases prog c2 v2 p2 ln2 (mkBase resth2 rhs_p rhs_t rhs_fl rhs_br pos) pos in
						     match n_rhs with
							   ...
							   let res_es2, prf2 = heap_entail_split_phases prog is_folding is_universal (Ctx new_estate2) s pos in
		    ...
		    (* check_node_helper *)
 		    (*****************************************************************************************************************************************)
		    (* check one match *)
		  let rec check_node_helper (all_nodes : (*before: Context.context*) (h_formula * h_formula * Context.match_type * Context.phase_type option * h_formula * int) list) : (list_context * proof list) =
		    (* let _ = print_string("check_node_helper\n") in *)
		    match all_nodes with
		      | (rest_heap, anode, r_flag, phase, frame, hole_id) :: rest ->
...
| HFalse | HTrue | Star _ | Conj _ | Phase _ | Hole _ -> failwith ("heap_entail_conjunct: "
								   ^ "something bad has happened to split_linear_node")


and string_of_phase phase = 
  match phase with 
    | Some Context.Spatial -> "spatial"
    | Some Context.Classic -> "conj"
    | None -> "none"
	...
	ViewNode ({ h_formula_view_node = p2;
	    ...
	    *)
		...
		begin
		  ...
		  let coer_rhs_new = add_origins coer_rhs_new1 (coer.coercion_head_view :: origs) in
		  let _ = reset_int2 () in
		  let xpure_lhs, xpure_lhs_b, memset = xpure prog f in
...
let heap_entail_struc_list_failesc_context_init (prog : prog_decl) (is_folding : bool) (is_universal : bool) (has_post: bool)
  ...
  Debug.devel_pprint ("heap_entail_struc_list_failesc_context_init:"
         ^ "\nctx:\n" ^ (Cprinter.string_of_list_failesc_context cl)
          ^ "\nconseq:"^ (Cprinter.string_of_struc_formula conseq) ^"\n") pos; 
  heap_entail_failesc_prefix_init prog is_folding is_universal has_post cl conseq pos pid (rename_labels_struc,Cprinter.string_of_struc_formula,heap_entail_one_context_struc)
