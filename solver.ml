...
let helper prog h_node = match h_node with	
  ...
  let f_ctx e = 
    ...
    let _ = transform_formula (f_e_f,f_f,f_h_f,(f_memo, f_aset, f_formula, f_b_formula, f_exp), f_perm) f in
    Ctx e in
...
let prune_branches_subsume prog univ_vars lhs_node rhs_node = match lhs_node,rhs_node with
  | DataNode dn1, DataNode dn2-> 
    (match (dn1.h_formula_data_remaining_branches,dn2.h_formula_data_remaining_branches) with
      | None,None -> (true, None)
      | Some l1, _ -> (true, None) (*(Util.subset l1 l2, None)*)
      | None, Some _ -> 
...
let rec xpure (prog : prog_decl) (f0 : formula) : (MCP.mix_formula * (branch_label * CP.formula) list * CPr.perm_formula) = match f0 with
  | Or ({ formula_or_f1 = f1;
    formula_or_f2 = f2;
    formula_or_pos = pos}) ->
        let pf1, pf1b,pf1p = xpure prog f1 in
        let pf2, pf2b,pf2p = xpure prog f2 in
	    ...
	    (MCP.mkOr_mems pf1 pf2 ), (List.map map_fun branches), (CPr.mkOr pf1p pf2p pos)
  | Base ({ formula_base_heap = h;
    formula_base_pure = p;
    formula_base_branches = br;
    formula_base_perm = prm;
    formula_base_pos = pos}) ->
        let (ph, phb,php) = xpure_heap prog h 1 in
        ...
        ((MCP.merge_mems p ph true), rb, CPr.mkAnd prm php pos)
  | Exists ({ formula_exists_qvars = qvars;
    ...
    formula_exists_pos = pos}) ->
        let (pqh, pqhb, pqp) = xpure_heap prog qh 1 in
        ...
        let pr = CPr.mkExists qvars (CPr.mkAnd pqp prm pos) pos in
	    (r_f, rb, pr)

and xpure_heap_d (prog : prog_decl) (h0 : h_formula) (use_xpure0 :int) : (MCP.mix_formula * (branch_label * CP.formula) list * CPr.perm_formula) = match h0 with
  | DataNode ({h_formula_data_node = p;
               h_formula_data_pos = pos}) ->
        let i = fresh_int2 () in
        let non_null = CP.mkEqVarInt p i pos in
	    (MCP.memoise_add_pure_N (MCP.mkMTrue pos) non_null , [], (CPr.mkTrue pos))
  | ViewNode ({ h_formula_view_node = p;
                h_formula_view_name = c;
                h_formula_view_arguments = vs;
                h_formula_view_remaining_branches = rm_br;
                h_formula_view_perm = prm; 
        ...
        let from_svs = vdef.view_perm_var :: sv :: vdef.view_vars in
        let to_svs = prm::p :: vs in
        let subst_m_fun = MCP.subst_avoid_capture_memo(*_debug*) from_svs to_svs in
        let subst_perm_fun = CPr.subst_avoid_capture from_svs to_svs in
        ...
        (match rm_br with
          | Some l -> 
            ...
            ((*subst_m_fun nf*)MCP.mkMTrue no_pos, [], perm_of_view prog c prm p vs pos)
          | None -> 
                let vinv = match use_xpure0 with
                  | -1 -> (MCP.mkMTrue no_pos, [], CPr.mkTrue pos)
                ...
                let (f, b, pr) = vinv in
                (*let f1 = MCP.memoise_add_pure_N f (helper (sv::vdef.view_materialized_vars)) in*)
                let subst_fun = CP.subst_avoid_capture from_svs to_svs in
                let tmp1 = (subst_m_fun f), List.map (fun (x,y) -> x, subst_fun y) b, subst_perm_fun pr in
                tmp1)
            
  | Star ({h_formula_star_h1 = h1;
    h_formula_star_h2 = h2;
    h_formula_star_pos = pos}) ->
        let (ph1, ph1b, ph1p) = xpure_heap_d prog h1 use_xpure0 in
        let (ph2, ph2b, ph2p) = xpure_heap_d prog h2 use_xpure0 in
        let res_form = (MCP.merge_mems ph1 ph2 true, CP.merge_branches ph1b ph2b, CPr.mkAnd ph1p ph2p pos) in
	    res_form
  | HTrue  -> (MCP.mkMTrue no_pos, [], CPr.mkTrue no_pos)
  | HFalse -> (MCP.mkMFalse no_pos, [], CPr.mkFalse no_pos)

and xpure_heap prog h use_xpure0 = xpure_heap_d prog h use_xpure0
(*Util.ho_debug_1 "xpure_heap " Cprinter.string_of_h_formula (fun (a,b,c)->
  (Cprinter.string_of_mix_formula_branches (a,b)) ^ " &p&" ^
  (Cprinter.string_of_perm_formula c))(fun c-> xpure_heap_d prog c use_xpure0) h*)
  
and xpure_symbolic (prog : prog_decl) (f0 : formula) branch : (MCP.mix_formula * CP.spec_var list * CPr.perm_formula) = match f0 with
  | Or ({formula_or_f1 = f1;
    formula_or_f2 = f2;
    formula_or_pos = pos}) ->
        let ipf1, avars1, prm1 = xpure_symbolic prog f1 branch in
        let ipf2, avars2, prm2 = xpure_symbolic prog f2 branch in
        let res_f = MCP.mkOr_mems ipf1 ipf2 in
	    (res_f, avars1 @ avars2, CPr.mkOr prm1 prm2 no_pos)
  | Base ({formula_base_heap = h;
    formula_base_pure = p;
    formula_base_perm = prm;
    formula_base_pos = pos}) ->
        let ph, addrs, xprm = xpure_heap_symbolic prog h branch in
        let res_form = MCP.memoise_add_pure_N p ph  in
        let rprm = CPr.mkAnd xprm prm pos in
        (res_form, addrs, rprm)
  | Exists ({formula_exists_qvars = qvars;
    formula_exists_heap = qh;
    formula_exists_pure = qp;
    formula_exists_perm = prm;
    formula_exists_pos = pos}) ->
        let pqh, addrs, xprm = xpure_heap_symbolic prog qh branch in
        let sqvars = (* List.map CP.to_int_var *) qvars in
        let tmp1 = MCP.memoise_add_pure_N qp pqh in
        let res_form = MCP.memo_pure_push_exists sqvars tmp1 in
        let rprm = CPr.mkAnd xprm prm pos in
	    (res_form, addrs, rprm)

and xpure_heap_symbolic (prog : prog_decl) (h0 : h_formula) branch : (CP.formula * CP.spec_var list * CPr.perm_formula) = match h0 with
  | DataNode ({ h_formula_data_node = p;
  ...
  | ViewNode ({h_formula_view_node = p;
        ...
        (match lbl_lst with
          | None -> 
                let vdef = look_up_view_def pos prog.prog_view_decls c in
                let vinvp, vinvb, vinvperm = vdef.view_x_formula in 
                let vxf = (MCP.fold_mem_lst (CP.mkTrue no_pos) false true vinvp, vinvb) in
                let vinv = CP.combine_branch branch vxf in
                let from_svs = vdef.view_perm_var :: CP.SpecVar (CP.OType vdef.view_data_name, self, Unprimed) :: vdef.view_vars in
                let to_svs = prm :: p :: vs in
                let tmp1 = CP.subst_avoid_capture from_svs to_svs vinv in
                let nperm =  CPr.subst_avoid_capture from_svs to_svs vinvperm in
                ...
                (tmp2, to_addrs, nperm)
          | Some _ ->  (P.mkTrue no_pos, [], perm_of_view prog c prm p vs pos))
  | Star ({h_formula_star_h1 = h1;
    h_formula_star_h2 = h2;
    h_formula_star_pos = pos}) ->
        let ph1, addrs1, pr1 = xpure_heap_symbolic prog h1 branch in
        let ph2, addrs2, pr2 = xpure_heap_symbolic prog h2 branch in
        let all_diff =
	    ...
	    (res_form, addrs1 @ addrs2, CPr.mkAnd pr1 pr2 pos)
  | HTrue -> (P.mkTrue no_pos, [], CPr.mkTrue no_pos)
  | HFalse -> (P.mkFalse no_pos, [], CPr.mkFalse no_pos)

and xpure_symbolic_no_exists (prog : prog_decl) (f0 : formula) : (MCP.mix_formula * (branch_label * CP.formula) list * CP.spec_var list * CPr.perm_formula) =
 match f0 with
  | Or ({ formula_or_f1 = f1;
    formula_or_f2 = f2;
    formula_or_pos = pos}) ->
        let ipf1, pf1b, avars1, pr1 = xpure_symbolic_no_exists prog f1 in
        let ipf2, pf2b, avars2, pr2 = xpure_symbolic_no_exists prog f2 in
        let branches = Util.remove_dups (fst (List.split pf1b) @ (fst (List.split pf2b))) in
	    ...
	    (res_form, br, (avars1 @ avars2), CPr.mkOr pr1 pr2 pos)
  | Base ({ formula_base_heap = h;
    formula_base_pure = p;
    formula_base_branches = fbr;
    formula_base_perm = prm;
    formula_base_pos = pos}) ->
        let ph, br, xprm, addrs = xpure_heap_symbolic_no_exists prog h in
	    ...
	    (n_p, r_br, addrs, CPr.mkAnd xprm prm pos)
  | Exists ({ formula_exists_qvars = qvars;
    formula_exists_heap = qh;
    formula_exists_pure = qp;
    ...
    formula_exists_perm = prm;
    formula_exists_pos = pos}) ->
        let pqh, br, xprm, addrs' = xpure_heap_symbolic_no_exists prog qh in
        let sqvars = (* List.map CP.to_int_var *) qvars in
	    ...
	    (res_form, rb, addrs, CPr.mkExists qvars (CPr.mkAnd xprm prm pos) pos)


and xpure_heap_symbolic_no_exists (prog : prog_decl) (h0 : h_formula) : (MCP.mix_formula * (branch_label * CP.formula) list * CPr.perm_formula * CP.spec_var list) = 
  xpure_heap_symbolic_no_exists_i prog h0 1
      
and xpure_heap_symbolic_no_exists_i (prog : prog_decl) (h0 : h_formula) i: (MCP.mix_formula * (branch_label * CP.formula) list * CPr.perm_formula * CP.spec_var list) = match h0 with
  | DataNode ({ h_formula_data_node = p;
    h_formula_data_label = lbl;
    h_formula_data_pos = pos}) ->
        let non_zero = CP.BForm (CP.Neq (CP.Var (p, pos), CP.Null pos, pos),lbl) in
	    (MCP.memoise_add_pure_N (MCP.mkMTrue pos) non_zero , [], CPr.mkTrue pos, [p])
  | ViewNode ({ h_formula_view_node = p;
    h_formula_view_name = c;
    h_formula_view_arguments = vs;
    h_formula_view_remaining_branches = lbl_lst;
    ...
    h_formula_view_pos = pos}) ->
        (match lbl_lst with
          | None ->
                let vdef = look_up_view_def pos prog.prog_view_decls c in
                let vinv, vinv_b, vinv_perm = if (i=1) then vdef.view_x_formula else vdef.view_user_inv in       
                let from_svs = vdef.view_perm_var ::CP.SpecVar (CP.OType vdef.view_data_name, self, Unprimed) :: vdef.view_vars in
                let to_svs = prm :: p :: vs in
                let from_addrs = vdef.view_addr_vars in
                let to_addrs = CP.fresh_spec_vars from_addrs in
                let subst_m_fun f =
                  let tmp1 = MCP.subst_avoid_capture_memo(*_debug*) from_svs to_svs f in
                ...
                let subst_perm_fun f = CPr.subst_avoid_capture from_svs to_svs f in
                (subst_m_fun vinv, List.map (fun (l,x) -> (l, subst_fun x)) vinv_b, subst_perm_fun vinv_perm, [] (*to_addrs*)) 
          | Some _ -> (MCP.mkMTrue no_pos, [], perm_of_view prog c prm p vs pos, []))
  | Star ({ h_formula_star_h1 = h1;
    h_formula_star_h2 = h2;
    h_formula_star_pos = pos}) ->
        let ph1, b1, xpr1, addrs1 = xpure_heap_symbolic_no_exists_i prog h1 i in
        let ph2, b2, xpr2, addrs2 = xpure_heap_symbolic_no_exists_i prog h2 i in
        let all_diff =
	      if !no_diff then P.mkTrue no_pos
	      else pairwise_diff addrs1 addrs2 pos in
        let tmp1 = MCP.merge_mems ph1 ph2 true in
        let res_form = MCP.memoise_add_pure_N tmp1 all_diff in
	    (res_form, CP.merge_branches b1 b2, CPr.mkAnd xpr1 xpr2 pos, addrs1 @ addrs2)
  | HTrue -> (MCP.mkMTrue no_pos, [], CPr.mkTrue no_pos, [])
  | HFalse -> (MCP.mkMFalse no_pos, [], CPr.mkFalse no_pos, [])


(* xpure of consumed precondition *)
and xpure_consumed_pre (prog : prog_decl) (f0 : formula) : (CP.formula * (branch_label * CP.formula) list * CPr.perm_formula) = match f0 with
  | Or ({ formula_or_f1 = f1;
    formula_or_f2 = f2;
    formula_or_pos = pos}) ->
        let ipf1, pf1b, pf1p = xpure_consumed_pre prog f1 in
        let ipf2, pf2b, pf2p = xpure_consumed_pre prog f2 in
        let branches = Util.remove_dups (fst (List.split pf1b) @ (fst (List.split pf2b))) in
        let map_fun branch =
          try 
            let l1 = List.assoc branch pf1b in
        ...
        in
        let map_fun b = (b, map_fun b) in
	    (CP.mkOr ipf1 ipf2 None pos), (List.map map_fun branches), (CPr.mkOr pf1p pf2p pos)
  | Base ({formula_base_heap = h; formula_base_pos = pos;}) ->
        let (ph, phb, php) = xpure_consumed_pre_heap prog h in
	    (ph, (List.map (fun (l, x) -> (l, CP.mkAnd x ph pos)) phb),php)
  | Exists ({formula_exists_qvars = qvars;
    formula_exists_heap = qh;
    formula_exists_pure = qp;
    formula_exists_pos = pos}) ->
        let (pqh, pqhb, pqhp) = xpure_consumed_pre_heap prog qh in
        let res_form = CP.mkExists qvars pqh None pos in
	    (pqh, (List.map (fun (l, x) -> (l, CP.mkAnd x res_form pos)) pqhb), pqhp)

and xpure_consumed_pre_heap (prog : prog_decl) (h0 : h_formula) : (CP.formula * (branch_label * CP.formula) list * CPr.perm_formula) = match h0 with
  | DataNode ({h_formula_data_node = p;
    h_formula_data_pos = pos}) -> (CP.mkTrue pos, [], CPr.mkTrue pos)
  | ViewNode ({ h_formula_view_node = p;
                h_formula_view_name = c;
                h_formula_view_arguments = vs;
                h_formula_view_perm = prm;
                h_formula_view_pos = pos}) ->
        let vdef = look_up_view_def pos prog.prog_view_decls c in
        let vinv, vinv_b, vinv_p = vdef.view_user_inv in (* views have been ordered such that this dependency is respected *)
        let vinv = MCP.fold_mem_lst (CP.mkTrue no_pos) false true vinv in
        let from_svs = vdef.view_perm_var :: CP.SpecVar (CP.OType vdef.view_data_name, self, Unprimed) :: vdef.view_vars in
        let to_svs = prm :: p :: vs in
        let tmp1 = CP.subst_avoid_capture from_svs to_svs vinv in
        let tmp1p = CPr.subst_avoid_capture from_svs to_svs vinv_p in
        let tmp1b = List.map (fun (l,f) -> (l, CP.subst_avoid_capture from_svs to_svs f)) vinv_b in
        tmp1, tmp1b, tmp1p
  | Star ({ h_formula_star_h1 = h1;
    h_formula_star_h2 = h2;
    h_formula_star_pos = pos}) ->
        let (ph1, ph1b, ph1p) = xpure_consumed_pre_heap prog h1 in
        let (ph2, ph2b, ph2p) = xpure_consumed_pre_heap prog h2 in
        let tmp1 = (CP.mkAnd ph1 ph2 pos, CP.merge_branches ph1b ph2b, CPr.mkAnd ph1p ph2p pos) in
	    tmp1
  | HTrue -> (P.mkTrue no_pos, [], CPr.mkTrue no_pos)
  | HFalse -> (P.mkFalse no_pos, [], CPr.mkFalse no_pos)
  ...
  let rec helper_formulas f = match f with
    ...
    | Exists e ->    
          ...
          mkExists_w_lbl e.formula_exists_qvars rh rp 
                   e.formula_exists_type e.formula_exists_perm e.formula_exists_flow 
    ...
    | Base b ->
          ...
          let rp = f_p_simp rp in
          mkBase_w_lbl rh rp b.formula_base_type  b.formula_base_perm b.formula_base_flow b.formula_base_branches b.formula_base_pos b.formula_base_label
...
and heap_prune_preds prog (hp:h_formula) (old_mem:MCP.memo_pure): (h_formula*MCP.memo_pure*bool)= 
  match hp with
    ...
    | ViewNode v ->   
          ...
          let fr_vars = v_def.view_perm_var :: ((CP.SpecVar (CP.OType v_def.view_data_name, self, Unprimed)):: v_def.view_vars) in
          let to_vars = v.h_formula_view_perm :: (v.h_formula_view_node :: v.h_formula_view_arguments) in
...
and find_node prog lhs_h (lhs_p : MCP.mix_formula) (ps : CP.spec_var list) pos : find_node_result =
  ...
  let tmp1 = List.map (fun p -> find_node_one prog lhs_h lhs_p p None pos) ps in
...
and find_node_one prog lhs_h lhs_p (p : CP.spec_var) rhs_info pos : find_node_result =
  let matches = Context.context_old prog lhs_h lhs_p p rhs_info pos in
...
and unfold_baref prog (h : h_formula) (p : MCP.mix_formula) (pr: Pr.perm_formula) (fl:flow_formula) (v : CP.spec_var) pos b qvars do_perm_norm do_unsat : formula =
  ...
  let unfolded_h = unfold_heap prog h aset v fl do_perm_norm pos in
  let pure_f = mkBase HTrue p TypeTrue pr (mkTrueFlow ()) b pos in
  let tmp_form_norm = normalize_combine unfolded_h pure_f true pos in
  ...
  if do_unsat then match (snd prog) with 
    | None -> 
          ...
          Util.pop_time "unfold_unsat";r)    
    | _ -> resform
  else resform
    

and unfold_heap prog (f : h_formula) (aset : CP.spec_var list) (v : CP.spec_var) fl do_perm_norm pos : formula = match f with
  | ViewNode ({h_formula_view_node = p;
    h_formula_view_name = c;
        ...
        if CP.mem p aset then
	      match (snd prog) with
	        | None ->
	              ...
	              let fr_vars = vdef.view_perm_var:: ((CP.SpecVar (CP.OType vdef.view_data_name, self, Unprimed)) :: vdef.view_vars) in
	              let to_vars = pr :: (v :: vs) in
	        ...
	        | Some (base , br, to_vars, pr) -> 
                  (* let _ = print_string "\n x\n" in*)
	              if (List.fold_left2 (fun a c1 c2-> a&&(c1=c2)) true to_vars vs) 
	              then  CF.replace_formula_label v_lbl  (CF.formula_of_mix_formula_with_branches_fl base pr br fl no_pos)
  ...
  | ViewNode ({ h_formula_view_node = p;
    ...
    h_formula_view_arguments = vs}) -> begin
      try
	    ...
	    let fr_vars = vdef.view_perm_var :: (CP.SpecVar (CP.OType vdef.Cast.view_data_name, self, Unprimed)):: vdef.view_vars in
      let to_vars = pr :: (p :: vs) in
	    ...
	    let rec process_one (ss:CF.steps) rs1  =
	      ...
	      match rs1 with
	        ...
	        | Ctx es ->
				  ...
				  ^ (Cprinter.string_of_mix_formula_branches (fst es.es_pure))) pos;
              Debug.devel_pprint ("fold: es.es_perm: "
				  ^ (Cprinter.string_of_perm_formula (snd es.es_pure))) pos;
...
and process_fold_result prog is_folding estate (fold_rs0:list_context) p2 vs2 base2 pos : (list_context * proof list) =
  ...
  let pr2 = base2.formula_base_perm in
...
and elim_exists_pure w ((f, b),pr) lump pos =
  ((elim_exists_mix_formula w f pos, List.map (fun (l, f) -> (l, elim_exists_pure_branch w f pos)) b),
    CPr.elim_exists_perm w pr pos)
      
and elim_exists_mix_formula w f pos = match f with
  | MCP.MemoF f -> MCP.MemoF (elim_exists_memo_pure w f pos)
  | MCP.OnePF f -> MCP.OnePF (elim_exists_pure_branch w f pos)
...
and elim_exists_memo_pure_debug w f0 pos = 
  ...
  (fun w f0 -> elim_exists_memo_pure w f0 pos) w f0
...
and entail_state_elim_exists es = 
  ...
  let f = CPr.elim_exists_exp_perm f in
  let qvar, base = CF.split_quantifiers f in
  let h, p, pr, fl,b, t = CF.split_components base in
  let simpl_p =	
  ...
  let simpl_pr = Pr.simpl_perm_formula pr in
  let simpl_f = CF.mkExists qvar h simpl_p t simpl_pr simpl_fl b no_pos in
...
and find_unsat (prog : prog_decl) (f : formula):formula list*formula list =  
  ...
  match f with
    | Base _ | Exists _ ->
	      let _ = reset_int2 () in
	      let pf, pfb,pfprm = xpure prog f in
	      let is_ok =        
	        if pfb = [] then 
	      ...
	      if is_ok && (CPr.is_sat pfprm) then ([f],[]) else ([],[f])
    | Or ({formula_or_f1 = f1;
      formula_or_f2 = f2;
      formula_or_pos = pos}) ->
...
and is_unsat_with_branches xpure_f qvars hf mix br prm pos sat_subno=
  (* let wrap_exists f =  List.fold_left (fun a qv -> CP.Exists (qv, a, None, pos)) f qvars in*)
  let (ph, phb,php) = xpure_f hf in
  let phb = CP.merge_branches phb br in 
  if  (not (CPr.is_sat (CPr.mkAnd php prm pos))) then true
  else
  if phb = [] then
...
and unsat_base_x prog (sat_subno:  int ref) f  : bool= 
  match f with
    ...
    | Base ({ formula_base_heap = h;
      ...
      formula_base_perm = prm;
      formula_base_pos = pos}) ->
          is_unsat_with_branches (fun f-> xpure_heap prog f 1) [] h p br prm pos sat_subno
    | Exists ({ formula_exists_qvars = qvars;
      formula_exists_heap = qh;
      formula_exists_pure = qp;
      formula_exists_branches = br;
      formula_exists_perm = prm;
      formula_exists_pos = pos}) ->
          is_unsat_with_branches (fun f-> xpure_heap prog f 1) qvars qh qp br prm pos sat_subno
...
and elim_unsat_all_debug prog (f : formula): formula = 
  Util.ho_debug_2 "elim_unsat " (fun c-> "?") (Cprinter.string_of_formula) (Cprinter.string_of_formula) elim_unsat_all prog f


(* extracts those involve free vars from a set of equations  - here free means that it is not existential and it is not meant for explicit 
instantiation *)
...
and elim_exists (f0 : formula) : formula = match f0 with
  ...
  | Exists ({ formula_exists_qvars = qvar :: rest_qvars;
    ...
    formula_exists_pos = pos}) ->
        ...
        let spr, pr1 = CPr.get_subst_equation_perm_formula pr qvar in
        let st = st@spr in
	    let r = if List.length st = 1 then
          let tmp = mkBase h pp1 t pr1 fl b pos in
          ...
          let tmp1 = mkExists rest_qvars h p t pr fl b pos in
...
and heap_entail_conjunct_lhs_struc 
  ...
  (*let _ = print_string "heap_entail_conjunct_lhs_struc\n" in*)
  ...
  (*and inner_entailer_debug ctx conseq =
	Util.ho_debug_2 "inner_entailer" (Cprinter.string_of_context) (Cprinter.string_of_struc_formula) (fun (l,p) -> (Cprinter.string_of_list_context l)^"\nProof:"^(Prooftracer.string_of_proof p)) inner_entailer_a ctx conseq*)
...
and move_expl_inst_ctx_list (ctx:list_context)(f:MCP.mix_formula) fpr:list_context = 
  let fct es = 
    let nf = 
      let v_l = es.es_gen_impl_vars@es.es_evars in
      if (v_l = []) then es.es_formula
      else 
        let f = MCP.find_rel_constraints f v_l in
        let fpr = CPr.find_rel_constraints fpr v_l in
        let f1 = formula_of_perm_mix_formula (CPr.elim_exists_perm v_l fpr no_pos) (elim_exists_mix_formula(*_debug*) v_l f no_pos) no_pos in
...
and coer_target prog (coer : coercion_decl) node (rhs : CF.formula) (lhs : CF.formula) : bool =
  ...
  let coer_lhs_heap, coer_lhs_guard, coer_lhs_perm, coer_lhs_flow, coer_lhs_branches, _ = split_components coer_lhs in
  let rhs_heap, rhs_pure, rhs_perm, rhs_flow, rhs_branches, _ = split_components rhs in
  let lhs_heap, lhs_pure, lhs_perm, lhs_flow, lhs_branches, _ = split_components lhs in
  ...
  match node, coer_lhs_heap with
    | ViewNode ({ h_formula_view_node = p1;
        ...
        h_formula_view_origins = origs;
        h_formula_view_perm = vp1;
        h_formula_view_arguments = ps1}),
	  ViewNode ({h_formula_view_node = p2;
      h_formula_view_name = c2;
      h_formula_view_perm = vp2;
      h_formula_view_arguments = ps2}) when c1=c2 ->
	      begin
	        (* apply the substitution *)
	        let coer_rhs_new = subst_avoid_capture (vp2:: (p2 :: ps2)) (vp1::(p1 :: ps1)) coer_rhs in
	        let coer_lhs_new = subst_avoid_capture (vp2:: (p2 :: ps2)) (vp1::(p1 :: ps1)) coer_lhs in
...
and check_one_target prog (target : CP.spec_var) (lhs_pure : MCP.mix_formula) (rhs_p : MCP.mix_formula)
  (rhs_h : CF.h_formula) (coer_rhs_h : CF.h_formula)  : bool =
	      ...
	      match ante with
	        | Exists ({ formula_exists_qvars = qvars;
              ...
              formula_exists_pos = pos}) ->
		          ...
		          let baref = mkBase qh qp qt qpr qfl qb pos in
                    ...
                    formula_exists_pos = pos}) ->
			            ...
			            (match tmp_rs with
			              ...
			              | SuccCtx sl ->
				                ...
				                (SuccCtx res_ctx, prf))
		          | _ ->
		                let h1, p1, pr1, fl1, br1, t1 = split_components ante in
                        (*print_string ("heap_entail_conjunct base: mem lst: "^(Cprinter.string_of_memoised_list mem1 )^"\n") ;*)
		                let h2, p2, pr2, fl2, br2, t2 = split_components conseq in
			              ...
			              if (not(CF.is_false_flow fl2.formula_flow_interval)) && not(CF.subsume_flow_ff fl2 fl1) then begin
			                ...
			                match h2 with
			                  ...
			                  | HTrue -> begin
				                  ...
				                  (*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*)
				                  let ctx, proof = heap_entail_empty_rhs_heap prog is_folding is_universal estate b1 p2 pr2 br2 pos in
				                  
				                  let new_ctx =
				                    if is_universal then ((*print_string ("YES Expl inst!!\n");*) move_lemma_expl_inst_ctx_list ctx conseq)
				                    else ((*print_string ("NO Expl inst!!\n");*) ctx )
				                  in
				                  let new_ctx = move_expl_inst_ctx_list new_ctx p2 pr2 in
				                  (new_ctx, proof)
				                end
			                  | _ -> begin
				                  Debug.devel_pprint ("heap_entail_conjunct: "
                                  ...
                                  formula_base_pos = pos } in
                                  let r1,p1 =  heap_entail_non_empty_rhs_heap prog is_folding is_universal ctx0 estate ante conseq b1 b2 pos in
                                  (r1,p1)
				                end
	          end
        end
...
and xpure_imply (prog : prog_decl) (is_folding : bool) (is_universal : bool)  lhs rhs_p (*rhs_perm*) timeout : bool = 
  ...
  (*let lhs_perm = r.formula_base_perm in*)
  let _ = reset_int2 () in
  let xpure_lhs_h, xpure_lhs_h_b, xpure_lhs_perm = xpure_heap prog (mkStarH lhs_h estate.es_heap pos) 1 in
  ...
  (*let tmp_perm = CPr.mkAnd xpure_lhs_perm lhs_perm pos in
  let new_ante_prm, new_conseq_prm = heap_entail_build_perm_formula_check (estate.es_evars@estate.es_gen_expl_vars) tmp_perm rhs_perm
  if not (CPr.imply new_ante_prm new_conseq_prm) then false
  else  *)
...
and heap_entail_empty_rhs_heap p i1 i2 es l rp r_pr rpb pos=
  Util.prof_2 "heap_entail_empty_rhs_heap" (heap_entail_empty_rhs_heap_x p i1 i2 es l rp r_pr) rpb pos

and heap_entail_empty_rhs_heap_x (prog : prog_decl) (is_folding : bool) (is_universal : bool) estate lhs (rhs_p:MCP.mix_formula) rhs_pr rhs_p_br pos : (list_context * proof) =
  ...
  let _ = reset_int2 () in
  let xpure_lhs_h0, xpure_lhs_h0_b,xpure_lhs_h0_p = xpure_heap prog (mkStarH lhs_h estate.es_heap pos) 0 in
  let xpure_lhs_h1, xpure_lhs_h1_b,xpure_lhs_h1_p = xpure_heap prog (mkStarH lhs_h estate.es_heap pos) 1 in
  let fold_fun (is_ok,succs,fails) ((branch_id, rhs_p):string*MCP.mix_formula) =
    if (is_ok = false) then (is_ok,succs,fails) else 
	  ...
	  (*let _ = print_string ("\nheap_entail_empty_rhs_heap: rhs_p: "^(Cprinter.string_of_mix_formula rhs_p)) in*)
	  (*let _ = print_string ("\nheap_entail_empty_rhs_heap: new_conseq0 after heap_entail_build_mix_formula_check: "^(Cprinter.string_of_mix_formula new_conseq0)) in*)
      let new_ante0, new_conseq0 = heap_entail_build_mix_formula_check(*_debug*) (estate.es_evars@estate.es_gen_expl_vars) tmp2 rhs_p pos in
      let new_ante1, new_conseq1 = heap_entail_build_mix_formula_check(*_debug*) (estate.es_evars@estate.es_gen_expl_vars) tmp3 rhs_p pos in
	  ...
	  let res1,res2,res3 = if (MCP.isConstMTrue rhs_p) then (true,[],None) 
		...
		(imply_mix_formula split_ante0 split_ante1 split_conseq imp_no) in	
  ...
  let tmp_perm0 = CPr.mkAnd xpure_lhs_h0_p lhs_pr pos in
  let tmp_perm1 = CPr.mkAnd xpure_lhs_h1_p lhs_pr pos in
  let new_ante_prm0, new_conseq_prm0 = heap_entail_build_perm_formula_check (estate.es_evars@estate.es_gen_expl_vars) tmp_perm0 rhs_pr pos in
  let new_ante_prm1, new_conseq_prm1 = heap_entail_build_perm_formula_check (estate.es_evars@estate.es_gen_expl_vars) tmp_perm1 rhs_pr pos in
  let pr_rez = (CPr.imply new_ante_prm0 new_conseq_prm0) || (CPr.imply new_ante_prm1 new_conseq_prm1) in
  if not pr_rez then begin
    Debug.devel_pprint ("heap_entail_empty_rhs_heap: perm formula is not valid\n") pos;
    (CF.mkFailCtx_in (Basic_Reason ({
        fc_message = "failed in entailing perm formula(s) in conseq";
        fc_current_lhs  = estate;
        fc_prior_steps = estate.es_prior_steps;
        fc_orig_conseq  = struc_formula_of_formula (formula_of_mix_formula_with_branches rhs_p rhs_pr rhs_p_br pos) pos;
        fc_failure_pts = [];})), prf)
  end
  else if r_rez then begin
    let res_delta = mkBase lhs_h lhs_p lhs_t lhs_pr lhs_fl lhs_b no_pos in
    if is_folding then begin
      let res_es = {estate with es_formula = res_delta; 
          es_pure = (
            ((MCP.merge_mems rhs_p (fst (fst estate.es_pure)) true),
            (Cpure.merge_branches (snd (fst estate.es_pure)) rhs_p_br)),
              Pr.mkAnd rhs_pr (snd estate.es_pure) pos );
...
and return_base_cases prog c2 v2 p2 prv2 ln2 rhs pos = 
  ...
  if (is_data ln2) then None
    ...
    match vd.view_raw_base_case with 
      | None  -> None 
      | Some s ->
            let fr_vars = vd.view_perm_var :: (CP.SpecVar (CP.OType vd.Cast.view_data_name, self, Unprimed)) :: vd.view_vars in			
            let to_vars = prv2 :: p2 :: v2 in
            let to_rhs = subst_avoid_capture fr_vars to_vars s in
            let rhs = normalize_combine to_rhs rhs true pos in
            Some rhs
                
                
and do_base_case_unfold prog ante conseq estate c1 c2 v1 v2 prv1 p1 p2 ln2 is_folding is_universal pid pos fold_f =
  ...
  if (is_data ln2) then (None,None)
    ...
    let na,prf = match vd.view_base_case with
      ...
      | Some (bc1,(base1,branches1, basep1)) -> 
		    begin
              (*let _ = print_string ("ante: "^(Cprinter.string_of_formula ante)^"\n conseq "^(Cprinter.string_of_formula conseq)^"\n") in*)
              let fr_vars = vd.Cast.view_perm_var :: ((CP.SpecVar (CP.OType vd.Cast.view_data_name, self, Unprimed)) :: vd.view_vars) in			
              let to_vars = prv1:: (p1 :: v1) in
              ...
              let basep1 = CPr.subst_avoid_capture fr_vars to_vars basep1 in
              ...
              if b then 
				...
				let ctx = unfold_context (prog, Some (base,branches, v1, basep1)) (SuccCtx[nctx]) p1 true pos in
                (ctx,TrueConseq)
 ...
 and do_base_case_unfold_debug prog ante conseq estate c1 c2 v1 v2 prv1 p1 p2 ln2 is_folding is_universal pid pos fold_f = 
   Util.ho_debug_4 "base_case_unfold" 
     ...
     (fun ante conseq p1 p2->  
       do_base_case_unfold prog ante conseq estate c1 c2 v1 v2 prv1 p1 p2 ln2 is_folding is_universal pid pos fold_f ) 
      ante conseq p1 p2
        
and do_match prog estate l_args r_args l_node_name r_node_name perm_imply l_node r_node rhs is_folding is_universal r_var pos : 
      list_context *proof =
  Debug.devel_pprint ("do_match: using " ^
      (Cprinter.string_of_h_formula l_node)	^ " to prove " ^
      (Cprinter.string_of_h_formula r_node)) pos;
    let l_h,l_p,l_pr,l_fl,l_b,l_t = split_components estate.es_formula in
    let l_h, l_pr, qvars, v_pair = CPr.heap_partial_imply l_h l_pr perm_imply mkStarH h_apply_one pos in
    let r_h,r_p,r_pr,r_fl,r_b,r_t = split_components rhs in
    let label_list = try 
    ...
    let (expl_inst, ivars', expl_vars') = (get_eqns_expl_inst (v_pair::rho) estate.es_ivars pos) in
    let new_expl_vars = (estate.es_expl_vars@expl_vars') in
    ...
    let p_to_lhs, p_to_rhs,p_ext_subst = CPr.get_eqns_free v_pair estate.es_evars new_expl_vars estate.es_gen_expl_vars pos in
    let (to_lhs, to_lhs_br),(to_rhs,to_rhs_br),ext_subst = get_eqns_free rho estate.es_evars new_expl_vars estate.es_gen_expl_vars pos in
    let ext_subst = ext_subst@ p_ext_subst in
    (*********************************************************************)
    ...
    let new_ante = (*mkBase*) mkExists qvars l_h new_ante_p l_t (CPr.mkAnd l_pr p_to_lhs pos) l_fl (CP.merge_branches l_b to_lhs_br) pos in
    let tmp_conseq = mkBase r_h new_conseq_p r_t (CPr.mkAnd r_pr p_to_rhs pos) r_fl (CP.merge_branches r_b to_rhs_br) pos  in

    let lhs_vars = ((CP.fv to_lhs) @(CPr.fv p_to_lhs)@(List.concat (List.map (fun (_,c)-> CP.fv c) to_lhs_br))) in
    ...
    let tmp_h2, tmp_p2, tmp_pr2, tmp_fl2, tmp_b2, _ = split_components tmp_conseq' in
    let new_conseq = mkBase tmp_h2 tmp_p2 r_t tmp_pr2 r_fl tmp_b2 pos in
    let new_consumed = if CPr.needs_split perm_imply then estate.es_heap else mkStarH l_node estate.es_heap pos in
...
and heap_entail_non_empty_rhs_heap prog is_folding is_universal ctx0 estate ante conseq lhs_b rhs_b pos : (list_context * proof) =
  ...
  let lhs_pr = lhs_b.formula_base_perm in
  ...
  let rhs_pr = rhs_b.formula_base_perm in
  ...
  match ln2 with
    ...
    | ViewNode ({ h_formula_view_node = p2;
	    ...
	    let do_fold_w_ctx fold_ctx var_to_fold = 
	      ...
	      let existential_eliminator_helper = 
		    ...
		    try
		      ...
		      let subs_vars = (vdef.view_perm_var,r_pr)::(sf,var_to_fold)::subs_vars in
		      ((List.map (fun (c1,c2)-> 
	      ...
	      let view_to_fold = ViewNode ({  
              h_formula_view_node = List.hd(List.tl new_v2) (*var_to_fold*);
              h_formula_view_name = c2;
              ...
              h_formula_view_remaining_branches = r_rem_brs;
              h_formula_view_pruning_conditions = r_p_cond;
              h_formula_view_perm = List.hd new_v2;
	    ...
	    *)
	    ...
	    let fnode_results = find_node_one prog lhs_h lhs_p p2 (Some (rhs_p,posib_r_alias)) pos in
	    (************************* match_all_nodes ******************)
	    match fnode_results with 
                    ...
                    h_formula_data_pos = pos1}) ->
			            if r_flag = Context.Root then begin (* matching occurs at root *)
			              if c1 = c2 then 
                            ...
                            if (not subsumes || not (CPr.does_match perm_imply)) then 
                              ...
                              (*match to_be_proven with
                                ...
                                | None ->*)
                              (    let ans = do_base_case_unfold prog ante conseq estate c1 c2 v1 v2 prv1 p1 p2 ln2 is_folding 
                              ...
                              match ans with 
                                ...
                                | None, y->  
                                      let new_estate = 
                                        match y with 
                                          | Some x-> x 
                                          | None -> {estate with es_formula = (mkBase resth1 lhs_p lhs_t lhs_pr lhs_fl lhs_br pos)} in
                                      let res_es0, prf0 = do_match prog new_estate v1 v2 c1 c2 perm_imply anode ln2 
                                        (mkBase resth2 rhs_p rhs_t rhs_pr rhs_fl rhs_br pos) is_folding is_universal p2 pos in
                                      let res_es1, prf1 = 
                                        if (!Globals.exhaust_match) then 
                                          let n_rhs = return_base_cases prog c2 v2 p2 r_pr ln2 (mkBase resth2 rhs_p rhs_t rhs_pr rhs_fl rhs_br pos) pos in
                                          match n_rhs with
                                            | None -> (res_es0,prf0)
                                            | Some s ->
                                                  (* let _ = print_string ("\n now entailing \n") in *)
                                      ...
                                      let ans =	
                                        if is_view anode then
                                          ...
                                          Some (do_coercion c1 c2 prog estate conseq ctx0 resth1 resth2 anode lhs_p lhs_t lhs_pr lhs_fl lhs_br rhs_p rhs_t rhs_pr rhs_fl lhs_b rhs_b ln2 is_folding pos pid))
				              ...
				              if is_view ln2 && is_data anode then 
				                begin (* fold *)
				                  ...
				                  begin (* unfold *)
                                    ...
                                    let ans = do_base_case_unfold prog ante conseq estate c1 c2 v1 v2 prv1 p1 p2 ln2 is_folding is_universal pid pos do_fold_w_ctx(*_debug*) in
                                    match ans with 
                                      | Some x, _ -> x
                                      | None, _->                          
                                            if !Globals.use_coercion then 
                                              begin
                                                ...
                                                let res, prfs = do_coercion c1 c2 prog estate conseq ctx0 resth1 resth2 anode lhs_p lhs_t lhs_pr lhs_fl lhs_br rhs_p rhs_t rhs_pr rhs_fl lhs_b rhs_b ln2 is_folding pos pid in
			              ...
			              (CF.mkFailCtx_in (Basic_Reason ({
                              ...
                              fc_failure_pts =match pid with | Some s-> [s] | _ -> [];})), NoAlias)
		          | _ -> report_error pos
			            ("heap_entail_non_empty_rhs_heap: something wrong has happened with the context") in
		      (*******************************************************************************************************************************************************************************************)
    ...
    | HFalse | HTrue | Star _ -> failwith ("heap_entail_non_empty_rhs_heap: "
...
and do_universal prog estate node f coer anode lhs_b rhs_b conseq is_folding pos pid: (list_context * proof) =
  begin
    ...
    *)
    ...
    let lhs_heap, lhs_guard, lhs_pr, lhs_fl, lhs_branches, _  = split_components coer_lhs in
	      ...
	      *)
	      ...
	      then
		    ...
		    begin
		      ...
		      let lhs_guard_new = CP.subst_avoid_capture (vp2:: (p2 :: ps2)) (vp1 ::(p1 :: ps1)) lhs_guard in
		      let lhs_branches_new = List.map (fun (s, f) -> (s, (CP.subst_avoid_capture (vp2:: (p2 :: ps2)) (vp1 ::(p1 :: ps1)) f))) lhs_branches in
		      let coer_rhs_new1 = subst_avoid_capture (vp2:: (p2 :: ps2)) (vp1 ::(p1 :: ps1)) coer_rhs in
...
and rewrite_coercion prog estate node f coer lhs_b rhs_b weaken pos : (bool * formula) =
  ...
  let coer_lhs = coer.coercion_head in
  let coer_rhs = coer.coercion_body in
  let lhs_heap, lhs_guard, lhs_perm, lhs_flow, lhs_branches, _ = split_components coer_lhs in
  let lhs_guard = MCP.fold_mem_lst (CP.mkTrue no_pos) true true lhs_guard in
  let br_match br1 br2 = match br1,br2 with
    | None,None -> true
    | Some br1,Some br2 -> (Util.list_equal br1 br2)
	        ...
	        *)
	        ...
	        then
	          ...
	          begin
		        ...
		        let lhs_guard_new = CP.subst_avoid_capture (vp2:: (p2 :: ps2)) (vp1 ::(p1 :: ps1)) lhs_guard in
            let lhs_perm_new = CPr.subst_avoid_capture (vp2:: (p2 :: ps2)) (vp1 ::(p1 :: ps1)) lhs_perm in
		        (*let lhs_branches_new = List.map (fun (s, f) -> (s, (CP.subst_avoid_capture (p2 :: ps2) (p1 :: ps1) f))) lhs_branches in*)
		        let coer_rhs_new1 = subst_avoid_capture (vp2:: (p2 :: ps2)) (vp1 ::(p1 :: ps1)) coer_rhs in
		        let coer_rhs_new = add_origins coer_rhs_new1 (coer.coercion_head_view :: origs) in
		        let _ = reset_int2 () in
		        let xpure_lhs, xpure_lhs_b,xpure_pr = xpure prog f in
            let xpure_lhs = MCP.fold_mem_lst (CP.mkTrue no_pos) true true xpure_lhs in 
		        ...
		        if ((fun (c1,_,_)-> c1) (TP.imply xpure_lhs lhs_guard_new (string_of_int !imp_no) false None))&&
              (CPr.imply xpure_pr lhs_perm_new) then
		          let new_f = normalize coer_rhs_new f pos in
		          (if (not(!Globals.lemma_heuristic) && get_estate_must_match estate) then
			        ((*print_string("disable distribution\n"); *)enable_distribution := false);
		          (true, new_f))
...
(*******************************************************************************************************************************************************************************************)
and apply_universal prog estate coer resth1 anode lhs_p lhs_t lhs_pr lhs_fl lhs_br lhs_b rhs_b c1 c2 conseq is_folding pos pid =
  (*******************************************************************************************************************************************************************************************)
  flush stdout;
    if Util.empty coer.coercion_univ_vars then (CF.mkFailCtx_in ( Basic_Reason (  {
      ...
      let f = mkBase resth1 lhs_p lhs_t lhs_pr lhs_fl lhs_br pos in(* Assume coercions have no branches *)
...
(* do_coercion *)
(*******************************************************************************************************************************************************************************************)
and do_coercion c1 c2 prog estate conseq ctx0 resth1 resth2 anode lhs_p lhs_t lhs_pr lhs_fl lhs_br rhs_p rhs_t rhs_pr rhs_fl lhs_b rhs_b ln2 is_folding pos pid : (CF.list_context * proof list) =
  Debug.devel_pprint ("heap_entail_non_empty_rhs_heap: do_coercion: " ^ "c1 = " ^ c1 ^ ", c2 = " ^ c2 ^ "\n") pos;
    ...
    let univ_r = if (List.length univ_coers)>0 then
      let univ_res_tmp = List.map (fun coer -> apply_universal prog estate coer resth1 anode lhs_p lhs_t lhs_pr lhs_fl lhs_br lhs_b rhs_b c1 c2 conseq is_folding pos pid) univ_coers in
    ...
    let left_r = if (List.length coers1)>0 then
      let tmp1 = List.map  (fun coer -> apply_left_coercion estate coer prog conseq ctx0 resth1 anode lhs_p lhs_t lhs_pr lhs_fl lhs_br lhs_b rhs_b c1 is_folding pos pid) coers1 in
    ...
    let right_r = if (List.length coers2)>0 then
      let tmp2 = List.map (fun coer -> apply_right_coercion estate coer prog conseq ctx0 resth2 ln2 rhs_p rhs_t rhs_pr rhs_fl lhs_b rhs_b c2 is_folding pos pid) coers2 in
    ...
    match univ_r,left_r,right_r with
      ...
      | Some (c1,c2),Some(d1,d2),Some (e1,e2) ->
            ...
            (r,prf)
	            ...
	            (* apply_left_coercion *)
	            (*******************************************************************************************************************************************************************************************)
and apply_left_coercion estate coer prog conseq ctx0 resth1 anode lhs_p lhs_t lhs_pr lhs_fl lhs_br lhs_b rhs_b c1 is_folding pos pid=
  (*let _ = print_string("left coercion\n") in*)
  let f = mkBase resth1 lhs_p lhs_t lhs_pr lhs_fl lhs_br pos in
  ...
  if ok then begin
    ...
    (*******************************************************************************************************************************************************************************************)
and apply_right_coercion estate coer prog conseq ctx0 resth2 ln2 rhs_p rhs_t rhs_pr rhs_fl lhs_b rhs_b c2 is_folding pos pid =
  (*let _ = print_string("right coercion\n") in*)
  let f = mkBase resth2 rhs_p rhs_t rhs_pr rhs_fl [] pos in
  let _ = Debug.devel_pprint ("heap_entail_non_empty_rhs_heap: "
  ^ "right_coercion: c2 = "
  ^ c2 ^ "\n") pos in
...
and elim_exists_exp_loop (f0 : formula) : (formula * bool) = match f0 with
  ...
  | Exists ({ formula_exists_qvars = qvar :: rest_qvars;
        ...
        let st1, pr1 = CPr.get_subst_equation_perm_formula pr qvar in
        let st = st1@st in
...
and combine_struc (f1:struc_formula)(f2:struc_formula) :struc_formula = 
  ...
  let rec combine_ext_struc (f1:ext_formula)(f2:ext_formula):ext_formula = match f1 with
	    ...
	    | EBase d -> EBase 
	          {
	              ...
	              formula_ext_base = normalize_combine b.formula_ext_base d.formula_ext_base true b.formula_ext_pos ;
    ...
    | EAssume (x1,b, (y1',y2') )-> let r = match f2 with
	    ...
	    | EAssume (x2,d,(y1,y2)) -> EAssume ((x1@x2),(normalize_combine b d true (Cformula.pos_of_formula d)),(y1,(y2^y2')))
...
let normalize_perm_d prog (c:formula):formula = 
  ...
  let rec soundness_check f = match f with
    | Or f -> Error.report_error {Error.error_loc = no_pos; Error.error_text ="merge malfunction1"} (*(soundness_check f.formula_or_f1;soundness_check f.formula_or_f2)*)
    | Base _
    | Exists _ ->
      ...
      if List.exists (fun c-> List.length c >1) r then Error.report_error {Error.error_loc = no_pos; Error.error_text ="merge malfunction2"} else () in
