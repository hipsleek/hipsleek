...
and fold_op_x1 prog (ctx : context) (view : h_formula) vd (rhs_p : MCP.mix_formula)
  ...
  | ViewNode ({ h_formula_view_node = p;
      ...
      try (
        ...
        let () = x_tinfo_hp (add_str "Rel Inferred (simplified)" (pr_list Cprinter.string_of_lhs_rhs)) rels no_pos in
  ...
  let fold_fun_impt (is_ok,succs,fails, (fc_kind,(contra_list, must_list, may_list))) (rhs_p:MCP.mix_formula) =
    begin
      ...
      if not (contra) then
        ...
        let (split_ante0_sym, _) as xx = x_add heap_entail_build_mix_formula_check 2 exist_vars tmp0_sym rhs_p pos in
...
and do_match_x prog estate l_node r_node rhs (rhs_matched_set:CP.spec_var list) is_folding pos : 
  ...
  x_binfo_hp (add_str "view_param_ann_lhs" (pr_opt Cprinter.string_of_pure_formula)) view_param_ann_lhs pos;
  x_binfo_hp (add_str "view_param_ann_rhs" (pr_opt Cprinter.string_of_pure_formula)) view_param_ann_rhs pos;
  x_binfo_hp (add_str "Imm annotation mismatch (view param)" (string_of_bool)) (not(rvl)) pos;
  x_binfo_hp (add_str "r,rvl" (pr_pair string_of_bool string_of_bool)) (r,rvl) no_pos;
  let (r, ann_lhs, ann_rhs, ann_rhs_ex) = (
    r && rvl,
