  ...
  let rec helper_formulas f = match f with
    ...
    | Exists e ->    
          ...
          mkExists_w_lbl e.formula_exists_qvars rh rp 
              ...
              e.formula_exists_branches e.formula_exists_and e.formula_exists_pos e.formula_exists_label
  ...
  let rec process_one_x (ss:CF.steps) fold_rs1 = match fold_rs1 with
    ...
    | Ctx fold_es ->
	      ...
	      let new_es = {estate with 
              ...
              es_aux_conseq = fold_es.es_aux_conseq
              (* es_aux_conseq = CP.mkAnd estate.es_aux_conseq to_conseq pos *)
          } 
          in
	      let new_ctx = (Ctx new_es) in
...
and elim_exists_mix_formula w f pos = 
  ...
  Gen.Debug.no_1 "elim_exists_mix_formula" pr pr
...
and entail_state_elim_exists es = 
  ...
  let h, p, fl, b, t, a = CF.split_components base in
  let simpl_p =	
  ...
  let simpl_f = CF.mkExists qvar h simpl_p t simpl_fl b (CF.formula_and_of_formula base) (CF.pos_of_formula base) in (*TO CHECK*)
  ...
  | Exists ({ formula_exists_qvars = qvar :: rest_qvars;
    ...
    formula_exists_pos = pos}) ->
        ...
        let r = if List.length st = 1 then
          let tmp = mkBase h pp1 t fl b a pos in (*TO CHECK*)
          ...
          let tmp1 = mkExists rest_qvars h p t fl b a pos in (*TO CHECK*)
...
and heap_entail_struc_fork_x (prog : prog_decl) (is_folding : bool)  (has_post: bool)(cl : list_context) (conseq : struc_formula) (tid:CP.spec_var) pos pid: (list_context * proof) =
  match cl with 
    | FailCtx _ -> (cl,Failure)
    | SuccCtx cl ->
	      if !Globals.use_set || Gen.is_empty cl then
	        let tmp1 = List.map (fun c -> heap_entail_one_context_struc_fork "4" tid prog is_folding  has_post c conseq pos pid) cl in
	        let tmp2, tmp_prfs = List.split tmp1 in
	        let prf = mkContextList cl conseq tmp_prfs in
            ((fold_context_left tmp2), prf)
	      else
	        (heap_entail_one_context_struc_fork "5" tid prog is_folding  has_post (List.hd cl) conseq pos pid)

and heap_entail_struc_fork (prog : prog_decl) (is_folding : bool)  (has_post: bool)(cl : list_context) (conseq : struc_formula) (tid:CP.spec_var) pos (pid:control_path_id): (list_context * proof) =
  Gen.Debug.no_3 "heap_entail_struc_fork" Cprinter.string_of_list_context Cprinter.string_of_struc_formula Cprinter.string_of_spec_var
      (fun (ls,_) -> Cprinter.string_of_list_context ls) (fun a c tid-> heap_entail_struc_fork_x prog is_folding has_post a c tid pos pid) cl conseq tid
...
(*Handle pre/post condition when a method forked*)
(*Entail pre, add post to thread with tid*)
(*tid is the thread id*)
and heap_entail_one_context_struc_fork n tid p i1 hp cl cs pos pid =
  Gen.Debug.no_1 "heap_entail_one_context_struc_fork" 
      ...
      (fun _ -> heap_entail_one_context_struc_fork_x p i1 hp cl cs tid pos pid) cl

(*Handle pre/post condition when a method forked*)
(*Entail pre, add post to thread with tid*)
and heap_entail_one_context_struc_fork_x (prog : prog_decl) (is_folding : bool)  has_post (ctx : context) (conseq : struc_formula) (tid:CP.spec_var) pos pid : (list_context * proof) =
  Debug.devel_pprint ("heap_entail_one_context_struc_fork:"
  ...
  ^ "\nconseq:\n" ^ (Cprinter.string_of_struc_formula conseq)) pos;
    if isAnyFalseCtx ctx then
      ...
      if isConstETrue conseq then
        ...
        let result, prf = heap_entail_after_sat_struc_fork prog is_folding  has_post ctx conseq tid pos pid []  in
        (result, prf)

(*Handle pre/post condition when a method forked*)
(*Entail pre, add post to thread with tid*)
and heap_entail_after_sat_struc_fork prog is_folding  has_post
      ctx conseq tid pos pid (ss:steps) : (list_context * proof) =
  Gen.Debug.no_2 "heap_entail_after_sat_struc_fork" Cprinter.string_of_context
	  Cprinter.string_of_struc_formula
	  (fun (lctx, _) -> Cprinter.string_of_list_context lctx)
	  (fun _ _ -> heap_entail_after_sat_struc_fork_x prog is_folding has_post ctx conseq tid pos pid ss) ctx conseq

(*Handle pre/post condition when a method forked*)
(*Entail pre, add post to thread with tid*)
and heap_entail_after_sat_struc_fork_x prog is_folding has_post
      ctx conseq tid pos pid (ss:steps) : (list_context * proof) =
  match ctx with
    | OCtx (c1, c2) ->
          Debug.devel_pprint ("heap_entail_after_sat_struc_fork:"
          ^ "\ntid:\n" ^ (Cprinter.string_of_spec_var tid)
		  ^ "\nctx:\n" ^ (Cprinter.string_of_context ctx)
		  ^ "\nconseq:\n" ^ (Cprinter.string_of_struc_formula conseq)) pos;
          let rs1, prf1 = heap_entail_after_sat_struc_fork prog is_folding
            has_post c1 conseq tid pos pid (CF.add_to_steps ss "left OR 5 on ante") in
          let rs2, prf2 = heap_entail_after_sat_struc_fork prog is_folding
    ...
    | Ctx es -> begin
        Debug.devel_pprint ("heap_entail_after_sat_struc_fork: invoking heap_entail_conjunct_lhs_struc_fork"
        ^ "\ntid:\n" ^ (Cprinter.string_of_spec_var tid)
		^ "\ncontext:\n" ^ (Cprinter.string_of_context ctx)
		^ "\nconseq:\n" ^ (Cprinter.string_of_struc_formula conseq)) pos;
        (*let es = {es with es_formula = prune_preds prog es.es_formula } in*)
        let es = (CF.add_to_estate_with_steps es ss) in
        let tmp, prf = heap_entail_conjunct_lhs_struc_fork prog is_folding has_post (Ctx es) conseq tid pos pid in
	    (filter_set tmp, prf)
      end

(*Handle pre/post condition when a method forked*)
(*Entail pre, add post to thread with tid*)
(*Add post to thread only when seeing EAssume*)
and heap_entail_conjunct_lhs_struc_fork
      p is_folding  has_post ctx conseq tid
      pos pid : (list_context * proof) = 
  let pr x = match x with Ctx _ -> "Ctx " | OCtx _ -> ("OCtx "^(Cprinter.string_of_context_short x)) in
  Gen.Debug.no_3 "heap_entail_conjunct_lhs_struc_fork"
      pr (Cprinter.string_of_struc_formula) Cprinter.string_of_spec_var
      (fun (a,b) -> Cprinter.string_of_list_context a)
      (fun ctx conseq tid -> heap_entail_conjunct_lhs_struc_fork_x p is_folding  has_post ctx conseq tid pos pid) ctx conseq tid

and heap_entail_conjunct_lhs_struc_fork_x
      (prog : prog_decl) 
      (is_folding : bool) 
      
      (has_post:bool)
      (ctx_00 : context) 
      (conseq : struc_formula) (tid: CP. spec_var) pos pid : (list_context * proof) =
    ...
    begin
      match f with
			    ...
			    (match n_ctx_list with
	              ...
	              | SuccCtx sc ->
		                if (List.length formula_cont)>0 then
                          let res, n_rpf = heap_entail_struc_fork prog is_folding has_post n_ctx_list formula_cont tid pos pid in
        ...
        | EAssume (ref_vars, post, (i,y)) ->
		      if not has_post then report_error pos ("malfunction: this formula "^ y ^" can not have a post condition when forked!")
	          else
	    ...
	    | EVariance e ->
              let es = match ctx11 with
                | OCtx _ -> report_error no_pos ("heap_entail_conjunct_lhs_struc_fork : OCtx encountered \n")
                | Ctx es -> es in 
...
and coer_target_a prog (coer : coercion_decl) (node:CF.h_formula) (target_rhs : CF.formula) (lhs : CF.formula) : bool =
  ...
  let coer_lhs_heap, coer_lhs_guard,coer_lhs_flow, coer_lhs_branches, _, a = split_components coer_lhs in
  let rhs_heap, rhs_pure, rhs_flow, rhs_branches, _, a = split_components target_rhs in
  let lhs_heap, lhs_pure, lhs_flow, lhs_branches, _, a = split_components lhs in
    ...
    | DataNode ({ h_formula_data_node = p1;
	  ...
	  h_formula_data_arguments = ps2}) when c1=c2 ->
	      begin
	        ...
	        let coer_rhs_h, _,_, _, _, a = split_components coer_rhs_new in
...
and heap_entail_split_rhs_phases_x
      ...
      pos : (list_context * proof) =
  let ctx_with_rhs =  
	let (h, p, fl, b, t, a) = CF.split_components conseq in
        ...
        match ante with
	      ...
	      | _ -> begin
	          match conseq with  
	            | Base(bf) -> 
	                  let (h, p, fl, b, t, a) = CF.split_components conseq in
                      let a = formula_and_of_formula conseq in
	                  helper ctx_with_rhs (* ctx_0 *) h p (fun xh xp -> CF.mkBase xh xp t fl b a pos)
	            | Exists ({formula_exists_qvars = qvars;
		          formula_exists_heap = qh;
		          formula_exists_pure = qp;
		          formula_exists_type = qt;
...
and eliminate_exist_from_LHS qvars qh qp qt qfl qb pos estate =  
  ...
  let baref = mkBase qh qp qt qfl qb a pos in (*TO CHECK ???*)
  let new_baref = subst st baref in
  (* new ctx is the new context after substituting the fresh vars for the exist quantified vars *)
...
and insert_ho_frame_in2_formula f ho_frame = 
  match f with
    | Base({formula_base_heap = h;
	  ...
	  formula_base_pos = pos}) ->
	      mkBase (ho_frame h) p  t fl  b a pos
    | Exists({formula_exists_qvars = qvars;
	  formula_exists_heap = h;
	  formula_exists_pure = p;
	  formula_exists_type = t;
	  ...
	  formula_exists_pos = pos}) ->
	      mkExists qvars (ho_frame h) p t fl b a pos
    | Or({formula_or_f1 = f1;
	  formula_or_f2 = f2;
	  formula_or_pos = pos;}) ->
...
and swap_heap (f : formula) (new_h : h_formula) pos : (formula * h_formula) = 
  match f with
    | Base (bf) ->
	      let (h, p, fl, b, t, a) = CF.split_components f in
          let a = formula_and_of_formula f in
	      (CF.mkBase new_h p t fl b a pos, h)
    | Exists({formula_exists_qvars = qvars;
	  formula_exists_heap = h;
	  formula_exists_pure = p;
	  formula_exists_type = t;
	  ...
	  formula_exists_label = l;
	  formula_exists_pos = pos }) -> 
	      (CF.mkExists qvars new_h p t fl b a pos, h)
...
and heap_entail_split_lhs_phases_x
  ...
  ^ (Cprinter.string_of_formula conseq)) pos;
    ...
    match lhs with 
      | Base(bf) -> 
	        let (h, p, fl, b, t, a) = CF.split_components lhs in
            let a = formula_and_of_formula lhs in
	        helper_lhs h (fun xh -> CF.mkBase xh p t fl b a pos)
      ...
      | Exists({formula_exists_qvars = qvars;
        ...
        formula_exists_label = l;
        formula_exists_pos = pos }) -> 
	        helper_lhs h (fun xh -> CF.mkExists qvars xh p t fl b a pos)
  ...
  let compute_thread_matches (a1 : one_formula list) (a2 : one_formula list) (alla:MCP.mix_formula) (allc: MCP.mix_formula) =
    ...
    let pr_out (a,b,c) =
      ...
      ("\n ### matches = " ^ str1
       ...
       ^ "\n ### rest_a = " ^ str3)
    in
    Gen.Debug.no_4 "compute_thread_matches"
              ...
              *)
              if not !disable_failure_explaining then
                ...
                let fc_template = {
		            ...
		            fc_prior_steps = estate.es_prior_steps;
		  fc_orig_conseq  = struc_formula_of_formula (formula_of_mix_formula_with_branches rhs_p rhs_p_br [] pos) pos; (*TO CHECK: ???*)
		            fc_current_conseq = CF.formula_of_heap HFalse pos;
    ...
    then 
	  ...
	  match rem_r_node with (* Fail whenever the l_node cannot entail r_node *)
	    ...
	    | _ -> 
            ...
            let new_ante = mkBase l_h new_ante_p l_t l_fl (CP.merge_branches l_b to_lhs_br) l_a pos in (*TO CHECK*)
	          (* An Hoa : fix new_ante *)
            let tmp_conseq = mkBase r_h new_conseq_p r_t r_fl (CP.merge_branches r_b to_rhs_br) r_a pos  in (*TO CHECK*)
              ...
              let tmp_h2, tmp_p2, tmp_fl2, tmp_b2, _, tmp_a2 = split_components tmp_conseq' in
            let new_conseq = mkBase tmp_h2 tmp_p2 r_t r_fl tmp_b2 tmp_a2 pos in
  ...
  if Inf.no_infer estate then
    ...
    begin
      match r with
        ...
        | None ->
              (CF.mkFailCtx_in (Basic_Reason (mkFailContext "infer_heap_node" estate (Base rhs_b) None pos,
              CF.mk_failure_bot ("Cannot infer heap and pure 2"))), NoAlias) 
    end
...
and process_action_x caller prog estate conseq lhs_b rhs_b a (rhs_h_matched_set:CP.spec_var list) is_folding pos = 
  if not(Context.is_complex_action a) then
    begin
      Debug.devel_pprint ("process_action :"
    ...
    let r1,r2 = match a with
      ...
      | Context.M_unmatched_rhs_data_node (rhs,rhs_rest) ->
            do_unmatched_rhs rhs rhs_rest caller prog estate conseq lhs_b rhs_b a (rhs_h_matched_set:CP.spec_var list) is_folding pos 
                (*      let _,lhs_p,_,_,_,_ = CF.split_components (estate.es_formula) in              *)
...
and do_universal_x prog estate (node:CF.h_formula) rest_of_lhs coer anode lhs_b rhs_b conseq is_folding pos: (list_context * proof) =
  begin
    ...
    *)
    ...
    let lhs_heap, lhs_guard,lhs_fl, lhs_branches, _, lhs_a  = split_components coer_lhs in
	        ...
	        then
		      ...
		      begin
		        ...
		        let formula,to_aux_conseq = 
                  if !allow_imm then (replace_branches lhs_branches_new (mkTrue (mkTrueFlow ()) pos),lhs_guard_new) (*Why imm delays the checking for guard condition*)
...
and rewrite_coercion_x prog estate node f coer lhs_b rhs_b target_b weaken pos : (int * formula) =
  ...
  let coer_lhs = coer.coercion_head in
  let coer_rhs = coer.coercion_body in
  let lhs_heap, lhs_guard, lhs_flow, lhs_branches, _, lhs_a = split_components coer_lhs in
...
and apply_universal_a prog estate coer resth1 anode (*lhs_p lhs_t lhs_fl lhs_br*) lhs_b rhs_b c1 c2 conseq is_folding pos =
  (*******************************************************************************************************************************************************************************************)
  let (lhs_h,lhs_p,lhs_t,lhs_fl,lhs_br) = CF.extr_formula_base lhs_b in
  let lhs_a = formula_and_of_formula (CF.Base lhs_b) in
  flush stdout;
  if Gen.is_empty coer.coercion_univ_vars then (CF.mkFailCtx_in ( Basic_Reason (  {
    ...
    let f = mkBase resth1 lhs_p lhs_t lhs_fl lhs_br lhs_a pos in(* Assume coercions have no branches *)
...
and apply_left_coercion_a estate coer prog conseq ctx0 resth1 anode (*lhs_p lhs_t lhs_fl lhs_br*) lhs_b rhs_b c1 is_folding pos=
  ...
  if (coer.coercion_case = Cast.Simple) then
    ...
    let f = mkBase resth1 lhs_p lhs_t lhs_fl lhs_br lhs_a pos in
...
and apply_left_coercion_complex_x estate coer prog conseq ctx0 resth1 anode lhs_b rhs_b c1 is_folding pos =
  ...
  let lhs_a = formula_and_of_formula (CF.Base lhs_b) in
  (*let _ = print_string("left coercion\n") in*)
  let f = CF.mkBase resth1 lhs_p lhs_t lhs_fl lhs_br lhs_a pos in
  let coer_lhs = coer.coercion_head in
  let coer_rhs = coer.coercion_body in
	      ...
	      then
            ...
            let rec process_one_x (ss:CF.steps) res = match res with
              ...
              | Ctx es ->
                    ...
                    (* let es_formula_heap,_,es_flow,es_branches, _ , _= split_components es_formula in *)
...
and normalize_w_coers prog (estate:CF.entail_state) (coers:coercion_decl list) (h:h_formula) (p:MCP.mix_formula) : (h_formula * MCP.mix_formula) =
  let rec helper (estate:CF.entail_state) (h:h_formula) (p:MCP.mix_formula) : (h_formula*MCP.mix_formula) =
    ...
    let process_one_x estate anode rest coer h p =
      let f = mkBase rest p CF.TypeTrue (CF.mkTrueFlow ()) [] [] no_pos in (*TO CHECK*)
      let coer_lhs = coer.coercion_head in
      let coer_rhs = coer.coercion_body in

      (*compute free vars in extra heap and guard*)
      let compute_extra_vars () =
        let lhs_heap, lhs_guard, _, _, _, lhs_a = split_components coer_lhs in
        let lhs_hs = CF.split_star_conjunctions lhs_heap in (*|lhs_hs|>1*)
        let head_node, rest = pick_up_node lhs_hs Globals.self in
                ...
                | SuccCtx res -> 
                      ...
                      match ctx with
                        ...
                        | Ctx es ->
                              ...
                              let h1,p1,_,_,_,_ = split_components new_ante in
                              let new_es = {new_estate with es_formula=new_ante; es_trace=old_trace} in
                              (true,new_es,h1,p1)
    in
...
and apply_right_coercion_a estate coer prog (conseq:CF.formula) ctx0 resth2 ln2 (*rhs_p rhs_t rhs_fl*) lhs_b rhs_b (c2:ident) is_folding pos =
  ...
  let rhs_a = formula_and_of_formula (CF.Base rhs_b) in
	...
	formula_exists_pos = pos}) ->
	    ...
	    if  not(List.exists (fun sv -> CP.eq_spec_var sv qvar) fvh) then
	      ...
	      if List.length st > 0 then (* if there exists one substitution  - actually we only take the first one -> therefore, the list should only have one elem *)
	        ...
	        let tmp1 = mkExists rest_qvars h p t fl b a pos in
...
(*Handle pre/post condition when a method forked*)
(*Entail pre, add post to thread with tid*)
let heap_entail_struc_list_failesc_context_fork_x (prog : prog_decl) (is_folding : bool)  (has_post: bool)
	(cl : list_failesc_context) (conseq:struc_formula) (tid: CP.spec_var) pos (pid:control_path_id) : (list_failesc_context * proof) = 
  let _ = set_entail_pos pos in
  Debug.devel_pprint ("heap_entail_struc_list_failesc_context_fork:"
          ^ "\ntid : "^ (Cprinter.string_of_spec_var tid) 
          ^ "\nconseq:"^ (Cprinter.string_of_struc_formula conseq) 
         ^ "\nctx:\n" ^ (Cprinter.string_of_list_failesc_context cl)
  ^"\n") pos; 
  let res,prf = heap_entail_failesc_prefix_init prog is_folding  has_post cl conseq pos pid (rename_labels_struc,Cprinter.string_of_struc_formula,(heap_entail_one_context_struc_fork "2" tid)) in
  (CF.list_failesc_context_simplify res,prf)

(*Handle pre/post condition when a method forked*)
(*Entail pre, add post to thread with tid*)
let heap_entail_struc_list_failesc_context_fork (prog : prog_decl) (is_folding : bool)  (has_post: bool)
	(cl : list_failesc_context) (conseq:struc_formula) (tid: CP.spec_var) pos (pid:control_path_id) : (list_failesc_context * proof) =
  Gen.Debug.no_3 "heap_entail_struc_list_failesc_context_fork"
	Cprinter.string_of_list_failesc_context
	Cprinter.string_of_struc_formula
    Cprinter.string_of_spec_var
	(fun (cl, _) -> Cprinter.string_of_list_failesc_context cl)
	(fun _ _ _ -> heap_entail_struc_list_failesc_context_fork_x prog is_folding has_post cl conseq tid pos pid) cl conseq tid
