...
let crt_ctx = ref (Context.mk_empty_ctx !crt_hole);;
...
and xpure_heap (prog : prog_decl) (h0 : h_formula) (use_xpure0 :int) : (CP.formula * (branch_label * CP.formula) list * CF.mem_formula) = 
  ...
  | Star ({h_formula_star_h1 = h1;
	   h_formula_star_h2 = h2;
	   h_formula_star_pos = pos}) ->
      let (ph1, ph1b) = xpure_heap_helper prog h1 use_xpure0 in
      let (ph2, ph2b) = xpure_heap_helper prog h2 use_xpure0 in
...
(*
and xpure_symbolic (prog : prog_decl) (f0 : formula) : (CP.formula * (branch_label * CP.formula) list) = 
match f0 with
  | Or ({formula_or_f1 = f1;
	formula_or_f2 = f2;
	formula_or_pos = pos}) ->
      let pf1, pf1b = xpure_symbolic prog f1 in
      let pf2, pf2b = xpure_symbolic prog f2 in
      let branches = Util.remove_dups (fst (List.split pf1b) @ (fst (List.split pf2b))) in
      let map_fun branch =
        try 
          let l1 = List.assoc branch pf1b in
  ...
  | Base ({formula_base_heap = h;
	...
	formula_base_branches = br;
	formula_base_pos = pos}) ->
      let (ph, phb) = xpure_heap_symbolic prog h 1 in
      let phb = CP.merge_branches phb br in
      let res_form = CP.mkAnd ph p pos in
	  (res_form, (List.map (fun (l, x) -> (l, CP.mkAnd x res_form pos)) phb))
  | Exists ({formula_exists_qvars = qvars;
	...
	formula_exists_branches = br;
	formula_exists_pos = pos}) ->
      let (pqh, pqhb) = xpure_heap_symbolic prog qh 1 in
      let pqhb = CP.merge_branches pqhb br in
      let sqvars = (* List.map CP.to_int_var *) qvars in
      let tmp1 = CP.mkAnd pqh qp pos in
      let wrap_exists f = List.fold_left
...
*)
(*
and xpure_heap_symbolic (prog : prog_decl) (h0 : h_formula) : (CP.formula * (branch_label * CP.formula) list) = match h0 with
  | DataNode ({h_formula_data_node = p;
  ...
  | ViewNode ({h_formula_view_node = p;
	...
	h_formula_view_pos = pos}) ->
      ...
      in
	  (subst_fun vinv, List.map (fun (l,x) -> (l, subst_fun x)) vinv_b)
  | Star ({h_formula_star_h1 = h1;
	h_formula_star_h2 = h2;
	h_formula_star_pos = pos}) 
(* for imm - modify *)
...
and find_node prog lhs_h lhs_p (ps : CP.spec_var list) pos : find_node_result =
  ...
  let tmp1 = List.map (fun p -> find_node_one prog lhs_h lhs_p p true (* todo: change the true!! *) None(*--(Some Context.Classic)*) pos) ps in
...
(* before:	      | (anode, r_flag, _, resth1, new_ctx) :: rest ->*)
		      | (resth1, anode, r_flag, phase, ctx, hole_id) :: rest ->	  
				...
				Context.ctx_heap_input tmp_ctx !crt_hole resth1)
			    else resth1 in
			    ...
			    (* update the current phase *)
			      crt_phase := phase;
			    (* for each write -> drop the reading phases before *)
			  (*let resth1 = 
			    if !(get_imm anode) then
			      drop_rp resth1
			    else resth1	
			  *)
...
and xpure_imply (prog : prog_decl) (is_folding : bool) (is_universal : bool)  lhs rhs_p timeout : bool = 
  ...
  let fold_fun is_ok (branch_id, rhs_p) =
    if (is_ok = false) then false else begin
      ...
      let res,_,_ = TP.imply_timeout new_ante new_conseq memset ((string_of_int !sat_no) ^ "." ^ (string_of_int !sat_subno)) timeout in
	  Debug.devel_pprint ("SAT #" ^ (string_of_int !sat_no) ^ "." ^ (string_of_int !sat_subno)) no_pos;				
	  sat_subno := !sat_subno+1;  
	  let res = 
...
and heap_entail_empty_rhs_heap (prog : prog_decl) (is_folding : bool) (is_universal : bool) estate lhs rhs_p rhs_p_br pos : (list_context * proof) =
  ...
  if r_rez then begin
    ...
    (****************************************************************)  
and imply_conj ante_disj0 ante_disj1 conseq_conj memset = 
  match conseq_conj with
    | h :: rest -> 
	    let (r1,r2,r3)=(imply_one_conj ante_disj0 ante_disj1 h memset) in
	    if r1 then 
	      let r1,r22,r23 = (imply_conj ante_disj0 ante_disj1 rest memset) in
	      (r1,r2@r22,r23)
	    else (r1,r2,r3)
    | [] -> (true,[],None)

and imply_one_conj ante_disj0 ante_disj1 conseq memset = 
  (*let _ = print_string ("\nSplitting the antecedent for xpure0:\n") in*)
  let xp01,xp02,xp03 = imply_disj ante_disj0 conseq memset ("IMP #" ^ (string_of_int !imp_no) ^ "." ^ (string_of_int 1(*!imp_subno*)) ^ " with XPure0") in  
  (*let _ = print_string ("\nDone splitting the antecedent for xpure0:\n") in*)
  if (not(xp01) (*&& (ante_disj0 <> ante_disj1)*)) then
    let _ = Debug.devel_pprint ("\nSplitting the antecedent for xpure1:\n") in
    let xp1 = imply_disj ante_disj1 conseq memset ("IMP #" ^ (string_of_int !imp_no) ^ "." ^ (string_of_int 2(*!imp_subno*)) ^ " with XPure1") in
    let _ = Debug.devel_pprint ("\nDone splitting the antecedent for xpure1:\n") in
	xp1
  else (xp01,xp02,xp03)	
    
and imply_disj ante_disj conseq memset str =
  match ante_disj with
    | h :: rest -> 
	    let _ = Debug.devel_pprint str no_pos in
	    let r1,r2,r3 = (TP.imply h conseq memset (string_of_int !imp_no)) in
	    if r1 then 
	      let r1,r22,r23 = (imply_disj rest conseq memset str) in
	      (r1,r2@r22,r23)
	    else (r1,r2,r3)
    | [] -> (true,[],None)
...
and heap_entail_non_empty_rhs_heap prog is_folding is_universal ctx0 estate ante conseq lhs_b rhs_b pos : (list_context * proof) =
  ...
  let (ln2, phase2), resth2 = split_linear_node_guided ( Util.remove_dups (h_fv lhs_h @ CP.fv lhs_p)) rhs_h None in
				              ...
				              begin (* fold *)
				                ...
				                begin (* unfold *)
				                  ...
				                  begin
					                ...
					                begin
					                  Debug.devel_pprint ("heap_enta
il_conjunct: " ^ "can't reduce, fold, unfold") pos;
					                  (CF.mkFailCtx_in (Basic_Reason ( {
			...
			let rec check_node_helper (all_nodes : (*before: Context.context*) (h_formula * h_formula * Context.match_type * Context.phase_type option * h_formula * int) list) : (list_context * proof list) =
			  match all_nodes with
				...
				(* if the current phase is Conj -> we need to put the crt_ctx back into the current program state *)
				let rest_heap = 
				  if (*!crt_phase*) (phase = Some Context.Classic) or (phase = None) then 
				    let tmp_ctx = !crt_ctx in
				      crt_ctx := Context.mk_empty_ctx !crt_hole;
	      ...
	      | [] -> (CF.mkFailCtx_in(Trivial_Reason "impossible here : end of check_node_helper"),[]) in
			    ...
			    in
			      ...
			      (* todo:
			      let new_rs = List.map (fun c -> match c with 
						     | Ctx(es) -> crt_ctx := Hole !crt_hole; 
				 Ctx(
				 es with {es_formula = 
				 
				 Context.ctx_input !crt_ctx !crt_hole es)
						     | _ -> c	 
						  )*)
	    ...
	    *)
	    ...
	    then
	      ...
	      begin
		    ...
		    imp_no := !imp_no+1;
		    if ((fun (c1,_,_)-> c1) (TP.imply xpure_lhs lhs_guard_new memset (string_of_int !imp_no))) then
		      let new_f = normalize coer_rhs_new f pos in
		      (if (not(!Globals.lemma_heuristic) && get_estate_must_match estate) then
			    ((*print_string("disable distribution\n"); *)enable_distribution := false);
		      (true, new_f))
