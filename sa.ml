    ...
    (* else *)
      begin
          ...
          if res = [] then ([(hp_name,[-1])],[]) (*renturn -1 to indicate that none is allowed to drop*)
            ...
            (*check full unk hp*)
            if List.length res = List.length args then
            ([(hp_name, res)],[(hp_name, args,res)])
            else ([(hp_name, res)],[])
      end
  in
...
let rec collect_par_defs_one_side_one_hp_rhs_x prog lhs rhs (hrel, args) def_ptrs
  ...
  begin
      ...
      let _ = Debug.info_pprint ("    undef_args:" ^ (!CP.print_svl undef_args)) no_pos in
          ...
          begin
              if CP.mem_svl hp hpdefs then (*defined already drop constraint, no new definition*)
                ...
                if (not (SAU.is_empty_f r)) then
                  ...
                  let _ = DD.info_pprint ((!CP.print_sv hp)^"(" ^(!CP.print_svl args) ^ ")=" ^
