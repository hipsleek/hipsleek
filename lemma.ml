...
let subst_cont vn cont_args f ihf chf self_hns self_null pos=
  ...
  if self_null <> [] then
    ...
    (subst_helper ss f, IF.mkBase ihf ip IvpermUtils.empty_vperm_sets IF.top_flow [] pos,
     CF.mkBase chf (MCP.mix_of_pure cp) emp_vps CF.TypeTrue (CF.mkNormalFlow()) [] (CF.get_path_trace f) pos)
...
let check_view_subsume iprog cprog view1 view2 need_cont_ana=
  let v_f1 = CF.formula_of_disjuncts (List.map (* Gen.fst3 *) fst view1.C.view_un_struc_formula) in
  let v_f2 = CF.formula_of_disjuncts (List.map (* Gen.fst3 *) fst view2.C.view_un_struc_formula) in
