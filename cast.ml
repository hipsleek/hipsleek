...
let is_touching_view_x (vdecl: view_decl) : bool =
  ...
  let branches, _ =  List.split (* Gen.BList.split3 *) vdecl.view_un_struc_formula in
...
let is_tail_recursive_view_x (vd: view_decl) : bool =
  ...
  let branches, _ = List.split (* Gen.BList.split3 *) vd.view_un_struc_formula in
...
let unfold_base_case_formula (f: F.formula) (vd: view_decl) (base_f: F.formula) =
  ...
  let replace_hf hf = (match hf with
      | F.ViewNode vn ->
        if (String.compare vn.F.h_formula_view_name vname = 0) then
          ...
          let () = Debug.binfo_hprint (add_str "base_f" !F.print_formula) base_f no_pos in
          ...
          let () = Debug.binfo_hprint (add_str "subs" (pr_list (pr_pair !P.print_sv !P.print_sv))) subs no_pos in
          let replacing_f = F.subst_one_by_one subs base_f in
          let () = Debug.binfo_hprint (add_str "replacing_f" !F.print_formula) replacing_f no_pos in
          let (replacing_hf,extra_pf,_,_,_,_) = F.split_components replacing_f in
          let extra_qvars = F.get_exists replacing_f in
          let () = Debug.binfo_hprint (add_str "extra_qvars" (pr_list !P.print_sv)) extra_qvars no_pos in
  ...
  let f_e e = Some e in
  let () = Debug.binfo_hprint (add_str "f" !F.print_formula) f no_pos in
  let new_f = F.transform_formula (f_ef, f_f, replace_hf, (f_m, f_a, f_pf, f_b, f_e)) f in
  let () = Debug.binfo_hprint (add_str "new_f" !F.print_formula) new_f no_pos in
  ...
  let () = Debug.binfo_hprint (add_str "new_f_final" !F.print_formula) new_f no_pos in
  new_f
