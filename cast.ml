...
let rec look_up_proc_def pos (procs : (ident, proc_decl) Hashtbl.t) (name : string) =
  ...
  with Not_found ->
       let proc = Hashtbl.fold (fun ident proc_decl proc ->
                            match proc with
                             | Some proc -> Some proc
                             | None ->
            if ((String.contains ident '`') &&
                (Globals.eq_mingled_name ident name)) then Some proc_decl
            else None ) procs None in
       match proc with
        | Some proc -> proc
        | None ->
           Error.report_error {
...
let look_up_proc_def pos (procs : (ident, proc_decl) Hashtbl.t) (name : string) =
  Debug.no_1 "look_up_proc_def" pr_id pr_none (look_up_proc_def pos (procs : (ident, proc_decl) Hashtbl.t)) (name : string)
...
let compute_view_forward_backward_info_x (vdecl: view_decl) (prog: prog_decl)
  ...
  if not (is_view_NORM vdecl.view_kind) then None
  else
    let pos = vdecl.view_pos in
    let vname = vdecl.view_name in
