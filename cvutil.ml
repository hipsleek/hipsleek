    ...
    let rec fct heaps p =
      match heaps with
        ...
        | hx::hxs ->
              ...
              let _ = Debug.tinfo_hprint (add_str "hx_here" Cprinter.string_of_h_formula) hx no_pos in
			  ...
			  b
                    ...
                    if(res) then
                      ...
                      if (p_vars=[] || p1_vars=[]) then
                        let _ = print_endline_quiet ("xpure_perm: check: something wrong happened with heap nodes") in
                        MCP.mkMTrue no_pos
                      else
                        let np = CP.mkNeqVar (List.hd p_vars) (List.hd p1_vars) no_pos in
                        (mix_of_pure np)
