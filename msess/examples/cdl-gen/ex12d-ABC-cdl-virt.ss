...
pred_sess_prot Gd<A,B,C,c> == A->B:1;;[A,B]:downd<c>;;[C,B]:await<c>;;B->C:1;
pred_sess_prot Gv<A,B,C,c> == A->B:1;;[A,B]:downv<c,1>;;[C,B]:await<c>;;B->C:1;
/************* LOCAL VIEW: **************/
/*  A@B: !1   */
/*  B@A: ?1;down(id)   */
/*  B@C: await(id);!1  */
...
void B1(Channel a, Channel b)
  ...
  /* ensures  a::Chan{emp}<> * b::Chan{emp}<> * c::CNT<(-1)>; */
  requires a::Chan{@S Gv<A@sec,B@prim,C,c>}<> * b::Chan{@S Gv<A,B@prim,C@sec,c>}<>  //* c::CNT<1>
  ensures  a::Chan{emp}<> * b::Chan{emp}<> * c::CNT<(-1)>;
{
  int x = receive(a);
  send(b,1);
