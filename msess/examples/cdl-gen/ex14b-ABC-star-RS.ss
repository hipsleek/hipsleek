...
pred_sess_prot Gd<A,B,C,c> == ((A->B:1;;[A,B]:downv<c,1,20>) * (A->C:1;;[A,C]:downd<c,10>))
  ;;[C,B]:await<c,10>;;[C,B]:await<c,20>;;B->C:2;



void B(Channel a, Channel b, CDL c)
  requires a::Chan{@S Gd<A@sec,B@prim,C,c>}<> * b::Chan{@S Gd<A,B@prim,C@sec,c>}<> * c::CNT<0,10>
  ensures  a::Chan{emp}<> * b::Chan{emp}<> * c::CNT<(-1),10> * c::CNT<(-1),20>;
{
  int x = receive(a);
  dprint;
  AWAIT(c);
...
void C(Channel a, Channel b, CDL c)
  requires a::Chan{@S Gd<A@sec,B@prim,C,c>}<> * b::Chan{@S Gd<A,B@prim,C@sec,c>}<> * c::CNT<0,10>
