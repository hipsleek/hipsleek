...
/*   (where the server sends all the info to the helper, */
/*         before waiting for the resulted media)        */
/*********************************************************/
...
pred_sess_prot FILTER_HELPER<S:role,H1:role,H2:role,c1:chan,c2:chan,vid,aud> ==
       (S->H1:c1(v#v::media<> & v=vid) ;; H1->S:c1(v#v::effv<vid>))
       *
       (S->H2:c2(v#v::media<> & v=aud) ;; H2->S:c2(v#v::effa<aud>)) ;
