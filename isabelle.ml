...
let rec isabelle_of_typ = function
  ...
  | NUM | TVar _ | Named _ | Array _ |RelT |AnnT | HpT | Symbol->
        Error.report_error {Error.error_loc = no_pos; 
...
let rec isabelle_of_exp e0 = match e0 with
  ...
  | CP.Sequence _ -> failwith ("isabelle.isabelle_of_exp: Sequence should not appear here")
  | CP.Bag (elist, _) ->
...
let is_sat (f : CP.formula) (sat_no : string) : bool = begin
	if !log_all_flag == true then
				output_string log_all ("\n\n#is_sat " ^ sat_no ^ "\n");
	let answ = (imply_sat f (CP.BForm((CP.BConst(false, no_pos), None), None, None)) !Globals.sat_timeout_limit sat_no) in
    if !log_all_flag == true then
