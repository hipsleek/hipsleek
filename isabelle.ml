...
and isabelle_of_b_formula b = match b with
  ...
  | CP.Lte (a1, a2, _) -> " ( " ^ (isabelle_of_exp a1) ^ " \\<le> " ^ (isabelle_of_exp a2) ^ ")"
  | CP.Gt (a1, a2, _) -> " ( " ^ (isabelle_of_exp a1) ^ " > " ^ (isabelle_of_exp a2) ^ ")"
  | CP.Gte (a1, a2, _) -> "(" ^ (isabelle_of_exp a1) ^ " \\<ge> " ^ (isabelle_of_exp a2) ^ ")"
  ...
  begin
 	if CP.is_null a2 then 
	  ...
	  if CP.is_null a1 then 
	     ...
	     "( " ^ (isabelle_of_exp a1) ^ " \\<noteq> " ^ (isabelle_of_exp a2) ^ ")"
  end 
  | CP.EqMax (a1, a2, a3, _) ->
  ...
  | CP.BagNotIn (v, e, l) -> 
      if !bag_flag then 
	"\\<not>(" ^  (isabelle_of_spec_var v) ^ ":#" ^ (isabelle_of_exp e) ^ ")"
	(*"(count " ^ (isabelle_of_exp e) ^ " " ^ (isabelle_of_spec_var v) ^ ") = 0"*)
	(*"\\<not>(" ^ (isabelle_of_spec_var v) ^ " \\<in> set_of(" ^ (isabelle_of_exp e) ^"))"*)
      else "\\<not>(" ^ (isabelle_of_spec_var v) ^ " \\<in> " ^ (isabelle_of_exp e) ^")"
  | CP.BagSub (e1, e2, l) -> 
...
let rec check fd isabelle_file_name : bool= 
	begin
		seek_in fd ((in_channel_length fd) - 26);
...
let write (pe : CP.formula) : bool =
  begin
		...
		let fstr = isabelle_of_formula pe in
    		begin
    		(* creating the theory file *)
    		
		if !bag_flag then 
		  begin
		    output_string isabelle_file ("theory " ^ "test" ^ string_of_int !isabelle_file_number ^ " = Multiset + Main:" ^ Util.new_line_str);
    		    output_string isabelle_file ("declare union_ac [simp]\n");
		  end  
		else  
		  output_string isabelle_file ("theory " ^ "test" ^ string_of_int !isabelle_file_number ^ " = Main:" ^ Util.new_line_str);
		...
		begin
		   if !bag_flag then 
		   begin
		     output_string log_file ("theory " ^ "test" ^ string_of_int !isabelle_file_number ^ " = Multiset + Main:" ^ Util.new_line_str);
		     output_string log_file ("declare union_ac [simp]\n");
		   end  
		   else
		     output_string log_file ("theory " ^ "test" ^ string_of_int !isabelle_file_number ^ " = Main:" ^ Util.new_line_str);
		...
		ignore(Sys.command "isabelle -u -q -r MyImage > res");
...
let building_image flag = begin
	if flag = "true" & !bag_flag then begin
	  ...
	  begin 
		output_string multiset_file ("theory multiset = Multiset+Main:\n end");
		flush multiset_file;
