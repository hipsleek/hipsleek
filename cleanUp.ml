...
let cleanUpFormulas_x (ante:CF.formula) (conseq:CF.formula) : (CF.formula*CF.formula) = 
  ...
  if (CF.is_or_formula ante || CF.is_or_formula conseq) then (ante,conseq)
    ...
    let aev, ah, ap, avp, _, _, _, pt,_, pos = CF.all_components ante in
    let cev, ch, cp, cvp, _, _, _, _,_, _ = CF.all_components conseq in
    ...
    let ante = CF.mkBase ah ap avp CF.TypeTrue (CF.mkTrueFlow ()) [] pt pos in
