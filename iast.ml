...
let def_exp_call_nrecv_poly_args = []
let def_proc_ho_arg = []
...
and exp_call_nrecv = {
  ...
  exp_call_nrecv_poly_args : typ list;
...
and exp_call_recv = { exp_call_recv_receiver : exp;
                      ...
                      exp_call_recv_poly_args : typ list;
...
and look_up_proc_def_mingled_name (procs : proc_decl list) (name : string) =
  match procs with
  | p :: rest ->
    if (p.proc_mingled_name = name) or
        ((String.contains p.proc_mingled_name '`') &&
         (Globals.eq_mingled_name p.proc_mingled_name name)) then
      p
    else
...
and collect_data_view_from_pure_bformula_x (bf : P.b_formula) (data_decls: data_decl list)
  ...
  (* | P.VarPerm _ *) | P.RelForm _ | P.ImmRel _ | P.TVar _ -> ([], [], henv)
...
let mk_lemma lemma_name kind orig coer_type ihps ?prio:(prio=def_coerc_prio) ihead ibody =
