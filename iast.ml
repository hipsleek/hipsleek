...
and mkCharLit s pos =
  CharLit { exp_char_lit_val = s;
           exp_char_lit_pos = pos }
...
let trans_to_exp_form exp0 =
  let rec helper exp =
    ...
    | StringLit s -> P.SConst (s.exp_string_lit_val, s.exp_string_lit_pos)
    | CharLit c -> P.CConst (c.exp_char_lit_val, c.exp_char_lit_pos)
    | Binary b ->
      (begin
