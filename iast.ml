...
let trans_exp (e:exp) (init_arg:'b) (f:'b->exp->(exp* 'a) option)
  ...
  let rec helper (in_arg:'b) (e:exp) :(exp* 'a) =
    ...
    | None  ->
      ...
      | Free d ->
        let e1, r1 = helper n_arg d.exp_free_exp in
        (Free {d with exp_free_exp = e1}, r1)
  ...
  let rec helper (in_arg:'b) (e:exp) : 'a =
    ...
    | None  ->
      ...
      | New b ->
        ...
        comb_f rl
      | Free d -> helper n_arg d.exp_free_exp
      | Raise b -> (match b.exp_raise_val with
          | None -> zero
...
let genESpec_wNI body_header body_opt args ret pos=
  let _print_gen_spec ss unk_hps=
...
and collect_data_view_from_h_formula_x (h0 : F.h_formula) (data_decls: data_decl list)
  ...
  | F.Star h ->
    ...
    let _d1 = List.length (dl1 @ vl1) in
    let _d2 = List.length (dl2 @ vl2) in
...
and find_data_view_x (vdecl: view_decl) (data_decls: data_decl list) pos
  ...
  | [] -> (
      try
        ...
        let _todo_unknown = x_add look_up_data_def_raw data_decls tname in
        ([tname], el)
      with Not_found ->
        ([], el)
    )
...
and set_check_fixpt (iprog:prog_decl) (data_decls : data_decl list) (view_decls: view_decl list)  =
  let _pr = pr_list_ln !print_data_decl in
...
let exists_path (c1 : ident) (c2 : ident) :bool =
  if c2="null" then true
    ...
    try
      let _todo_unknown = PathCH.shortest_path class_hierarchy
...
let build_exc_hierarchy (clean:bool)(prog : prog_decl) =
  (* build the class hierarchy *)
  let _todo_unknown = List.map (fun c-> (exlist # add_edge c.data_name c.data_parent_name)) (prog.prog_data_decls) in
...
let rec label_e e =
  ...
  and helper2 e = match e with
    ...
    | Free e ->
      let nl = fresh_branch_point_id "" in
      iast_label_table:=(nl, "malloc", [], e.exp_free_pos)::!iast_label_table;
      Free {e with exp_free_exp = label_e e.exp_free_exp}
    | Try e ->
      let nl = fresh_branch_point_id "" in
...
let find_all_num_trailer iprog =
  ...
  let _proc_list = List.filter (fun proc ->
      proc.proc_is_main
...
let gen_name_pairs_struc view_decls0 vname (f:F.struc_formula): (ident * ident) list =
  let rec gen_name_pairs_heap vname h =
    ...
    | F.HeapNode { F.h_formula_heap_name = c } ->
      ...
      (* else *)
      (try
         let _todo_unk = look_up_view_def_raw x_loc view_decls0 c in [ (vname, c) ]
       with | Not_found ->
...
let neg_bin_op op = match op with
  | OpEq -> Some OpNeq
  | OpLt -> Some OpGte
  | OpLte -> Some OpGt
  | OpGt -> Some OpLte
...
let neg_exp exp = match exp with
  | Binary bin -> (match neg_bin_op bin.exp_binary_op with
    | Some n_op -> Some (Binary {bin with exp_binary_op = n_op})
    | None -> None)
  | _ -> None
...
let rec repair_exp exp exp_decls =
  ...
  | ArrayAlloc _ -> exp
  | Assert _ -> exp
  | Free _ -> report_error no_pos "repair_exp Free: to inspect"
  | Assign e ->
...
let collect_vars_exp exp =
    let rec aux exp list = match exp with
      ...
      | Member e ->
        let _n_list = aux e.exp_member_base list in
...
let is_zero_exp exp = match exp with
