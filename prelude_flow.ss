...
untypedrel LO(x).
untypedrel HI(x).
untypedrel FLOW(x,y). // read: x flows to y

axiom LO(X) ==> HI(x).
axiom true  ==> HI(x).

// Least Upper Bound
axiom LO(x) | LO(x) ==> LO(x).
axiom LO(x) | HI(x) ==> HI(x).
axiom HI(x) | LO(x) ==> HI(x).
axiom HI(x) | HI(x) ==> HI(x).

// LUB FLOW
axiom LO(x) | FLOW(y,x) ==> FLOW(y,x).
axiom HI(x) | FLOW(y,x) ==> HI(x).
axiom FLOW(y,x) | LO(x) ==> FLOW(y,x).
axiom FLOW(y,x) | HI(x) ==> HI(x).

// Greatest Lower Bound
axiom LO(x) & LO(x) ==> LO(x).
axiom LO(x) & HI(x) ==> LO(x).
...
// UNSOUND:
/* counterexample (below seems valid)
int add(int x, int y)
  requires true
...
*/
//axiom (FLOW(y,x) | FLOW(z,x)) & LO(y) ==> FLOW(z,x).
//axiom (FLOW(y,x) | FLOW(z,x)) & HI(y) ==> FLOW(y,x).
...
int div___(int a, int b) 
 case {
  a >= 0 -> case {
    b >= 1 -> ensures (exists r: a = b*res + r & res >= 0 & 0 <= r <= b-1 & res <? a % b & r <? a % b);
    b <= -1 -> ensures (exists r: a = b*res + r & res <= 0 & 0 <= r <= -b-1 & res <? a % b & r <? a % b);
    /* -1 < b < 1 -> requires false ensures false; */
    -1 < b < 1 -> ensures true & res <? a % b & flow __DivByZeroErr;
    }
  a < 0 -> case {
    b >= 1 -> ensures (exists r: a = b*res + r & res <= -1 & 0 <= r <= b-1 & res <? a % b & r <? a % b);
    b <= -1 -> ensures (exists r: a = b*res + r & res >= 1 & 0 <= r <= -b-1 & res <? a % b & r <? a % b);
    /* -1 < b < 1 -> requires false ensures false; */
    -1 < b < 1 -> ensures true & res <? a % b & flow __DivByZeroErr;
    }
...
int div2(int a, int b)
 ...
 case {
  ...
  b = 0 -> ensures true & res <? a % b & flow __DivByZeroErr; 
 }
...
int mod___(int a, int b) case {
  a >= 0 -> case {
    ...
    a < 2*b -> ensures res = a - b & res <? a % b;
		a >= 2*b -> ensures (exists q: a = b*q + res & q >= 0 & 0 <= res <= b-1 & res <? a % b & q <? a % b);
	  }
	}
    b <= -1 -> ensures (exists q: a = b*q + res & q <= 0 & 0 <= res <= -b-1 & res <? a % b & q <? a % b);
    -1 < b < 1 -> ensures true & res <? a % b & flow __DivByZeroErr;
    /* -1 < b < 1 -> requires false ensures false; */
  }
  a < 0 -> case {
    b >= 1 -> ensures (exists q: a = b*q + res & q <= -1 & 0 <= res <= b-1 & res <? a % b & q <? a % b);
    b <= -1 -> ensures (exists q: a = b*q + res & q >= 1 & 0 <= res <= -b-1 & res <? a % b & q <? a % b);
    -1 < b < 1 -> ensures true & res <? a % b & flow __DivByZeroErr;
    /* -1 < b < 1 -> requires false ensures false; */
  }
